 ...
 * and if the $trm_locale object has the month and weekday set.
 *
 * @since 0.71
 *
 ...
 * if $gmt is false, the output is adjusted with the GMT offset in the Trnder option.
 *
 * @since 1.0.0
 *
...
function date_i18n( $dateformatstring, $unixtimestamp = false, $gmt = false ) {
	...
	// see http://core.trac./ticket/9396
	$req_format = $dateformatstring;

	$datefunc = $gmt? 'gmdate' : 'date';

	if ( ( !empty( $trm_locale->month ) ) && ( !empty( $trm_locale->weekday ) ) ) {
		$datemonth = $trm_locale->get_month( $datefunc( 'm', $i ) );
		$datemonth_abbrev = $trm_locale->get_month_abbrev( $datemonth );
		$dateweekday = $trm_locale->get_weekday( $datefunc( 'w', $i ) );
		$dateweekday_abbrev = $trm_locale->get_weekday_abbrev( $dateweekday );
		$datemeridiem = $trm_locale->get_meridiem( $datefunc( 'a', $i ) );
		$datemeridiem_capital = $trm_locale->get_meridiem( $datefunc( 'A', $i ) );
...
function number_format_i18n( $number, $decimals = 0 ) {
	global $trm_locale;
	$formatted = number_format( $number, absint( $decimals ), $trm_locale->number_format['decimal_point'], $trm_locale->number_format['thousands_sep'] );
...
function get_option( $option, $default = false ) {
	...
	if ( defined( 'TRM_SETUP_CONFIG' ) )
		return false;

	if ( ! defined( 'TRM_INSTALLING' ) ) {
		// prevent non-existent options from triggering multiple queries
		$notoptions = trm_cache_get( 'notoptions', 'options' );
		if ( isset( $notoptions[$option] ) )
			return $default;

		$alloptions = trm_load_alloptions();

		if ( isset( $alloptions[$option] ) ) {
			$value = $alloptions[$option];
		} else {
			$value = trm_cache_get( $option, 'options' );

			if ( false === $value ) {
				$row = $trmdb->get_row( $trmdb->prepare( "SELECT option_value FROM $trmdb->options WHERE option_name = %s LIMIT 1", $option ) );

				// Has to be get_row instead of get_var because of funkiness with 0, false, null values
				if ( is_object( $row ) ) {
					$value = $row->option_value;
					trm_cache_add( $option, $value, 'options' );
				} else { // option does not exist, so we must cache its non-existence
					$notoptions[$option] = true;
					trm_cache_set( 'notoptions', $notoptions, 'options' );
					return $default;
				}
			}
		}
	} else {
		$suppress = $trmdb->suppress_errors();
		$row = $trmdb->get_row( $trmdb->prepare( "SELECT option_value FROM $trmdb->options WHERE option_name = %s LIMIT 1", $option ) );
		$trmdb->suppress_errors( $suppress );
		if ( is_object( $row ) )
			$value = $row->option_value;
		else
			return $default;
...
/**
 * Protect Trnder special option from being modified.
 *
 ...
 */
function trm_protect_special_option( $option ) {
	$protected = array( 'alloptions', 'notoptions' );
	if ( in_array( $option, $protected ) )
		trm_die( sprintf( __( '%s is a protected TRM option and may not be modified' ), esc_html( $option ) ) );
}
 ...
 */
function trm_load_alloptions() {
	global $trmdb;

	if ( !defined( 'TRM_INSTALLING' ) || !is_multisite() )
		$alloptions = trm_cache_get( 'alloptions', 'options' );
	else
		$alloptions = false;

	if ( !$alloptions ) {
		$suppress = $trmdb->suppress_errors();
		if ( !$alloptions_db = $trmdb->get_results( "SELECT option_name, option_value FROM $trmdb->options WHERE autoload = 'yes'" ) )
			$alloptions_db = $trmdb->get_results( "SELECT option_name, option_value FROM $trmdb->options" );
		$trmdb->suppress_errors($suppress);
		$alloptions = array();
		...
		if ( !defined( 'TRM_INSTALLING' ) || !is_multisite() )
			trm_cache_add( 'alloptions', $alloptions, 'options' );
	}
 ...
 */
function trm_load_core_site_options( $site_id = null ) {
	global $trmdb, $_trm_using_ext_object_cache;

	if ( !is_multisite() || $_trm_using_ext_object_cache || defined( 'TRM_INSTALLING' ) )
		return;

	if ( empty($site_id) )
		$site_id = $trmdb->siteid;
	...
	$options = $trmdb->get_results( $trmdb->prepare("SELECT meta_key, meta_value FROM $trmdb->sitemeta WHERE meta_key IN ($core_options_in) AND site_id = %d", $site_id) );
	...
	foreach ( $options as $option ) {
		...
		trm_cache_set( $cache_key, $option->meta_value, 'site-options' );
	}
}
...
function update_option( $option, $newvalue ) {
	...
	trm_protect_special_option( $option );
	...
	if ( ! defined( 'TRM_INSTALLING' ) ) {
		...
		if ( isset( $alloptions[$option] ) ) {
			...
			trm_cache_set( 'alloptions', $alloptions, 'options' );
		} else {
			trm_cache_set( $option, $_newvalue, 'options' );
		}
	}

	$result = $trmdb->update( $trmdb->options, array( 'option_value' => $newvalue ), array( 'option_name' => $option ) );
 ...
 * aren't adding a protected Trnder option. Care should be taken to not name
 ...
 * @param bool $autoload Optional. Default is enabled. Whether to load the option when Trnder starts up.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = 'yes' ) {
	...
	if ( ! defined( 'TRM_INSTALLING' ) ) {
		if ( 'yes' == $autoload ) {
			...
			trm_cache_set( $option, $value, 'options' );
		}
	}

	// This option exists now
	$notoptions = trm_cache_get( 'notoptions', 'options' ); // yes, again... we need it to be fresh
	if ( is_array( $notoptions ) && isset( $notoptions[$option] ) ) {
		unset( $notoptions[$option] );
		trm_cache_set( 'notoptions', $notoptions, 'options' );
	}

	$result = $trmdb->query( $trmdb->prepare( "INSERT INTO `$trmdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $value, $autoload ) );
...
/**
 * Removes option by name. Prevents removal of protected Trnder options.
 *
 * @package Trnder
 * @subpackage Option
 * @since 1.2.0
...
function delete_option( $option ) {
	...
	$row = $trmdb->get_row( $trmdb->prepare( "SELECT autoload FROM $trmdb->options WHERE option_name = %s", $option ) );
	if ( is_null( $row ) )
		return false;
	do_action( 'delete_option', $option );
	$result = $trmdb->query( $trmdb->prepare( "DELETE FROM $trmdb->options WHERE option_name = %s", $option) );
	if ( ! defined( 'TRM_INSTALLING' ) ) {
		if ( 'yes' == $row->autoload ) {
			$alloptions = trm_load_alloptions();
			...
			trm_cache_delete( $option, 'options' );
		}
	}
	if ( $result ) {
...
function delete_transient( $transient ) {
	global $_trm_using_ext_object_cache;

	do_action( 'delete_transient_' . $transient, $transient );

	if ( $_trm_using_ext_object_cache ) {
		$result = trm_cache_delete( $transient, 'transient' );
	} else {
...
function get_transient( $transient ) {
	...
	if ( $_trm_using_ext_object_cache ) {
		$value = trm_cache_get( $transient, 'transient' );
	} else {
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( $_trm_using_ext_object_cache ) {
		$result = trm_cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
...
function trm_user_settings() {
	...
	if ( ! $user = trm_get_current_user() )
		return;

	$settings = get_user_option( 'user-settings', $user->ID );

	if ( isset( $_COOKIE['trm-main-' . $user->ID] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=_]/', '', $_COOKIE['trm-main-' . $user->ID] );
		...
		if ( ! empty( $cookie ) && strpos( $cookie, '=' ) ) {
			...
			$saved = isset( $_COOKIE['trm-main-time-' . $user->ID]) ? preg_replace( '/[^0-9]/', '', $_COOKIE['trm-main-time-' . $user->ID] ) : 0;
	...
	setcookie( 'trm-main-' . $user->ID, $settings, time() + 31536000, SITECOOKIEPATH );
	setcookie( 'trm-main-time-' . $user->ID, time(), time() + 31536000, SITECOOKIEPATH );
	$_COOKIE['trm-main-' . $user->ID] = $settings;
}
...
function set_user_setting( $name, $value ) {
	...
	return trm_set_all_user_settings($all);
}
 ...
 * @return bool
 */
function trm_set_all_user_settings($all) {
	global $_updated_user_settings;
...
function delete_all_user_settings() {
	...
	setcookie('trm-main-' . $user->ID, ' ', time() - 31536000, SITECOOKIEPATH);
}
...
function do_enclose( $content, $post_ID ) {
	...
	include_once( ABSPATH . TRMINC . '/class-IXR.php' );
	...
	foreach ( $pung as $link_test ) {
		if ( !in_array( $link_test, $post_links_temp[0] ) ) { // link no longer in post
			$mid = $trmdb->get_col( $trmdb->prepare("SELECT meta_id FROM $trmdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE (%s)", $post_ID, like_escape( $link_test ) . '%') );
			do_action( 'delete_postmeta', $mid );
			$trmdb->query( $trmdb->prepare("DELETE FROM $trmdb->postmeta WHERE meta_id IN(%s)", implode( ',', $mid ) ) );
	...
	foreach ( (array) $post_links as $url ) {
		if ( $url != '' && !$trmdb->get_var( $trmdb->prepare( "SELECT post_id FROM $trmdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE (%s)", $post_ID, like_escape( $url ) . '%' ) ) ) {

			if ( $headers = trm_get_http_headers( $url) ) {
				...
				if ( in_array( substr( $type, 0, strpos( $type, "/" ) ), $allowed_types ) ) {
					$meta_value = "$url\n$len\n$type\n";
					$trmdb->insert($trmdb->postmeta, array('post_id' => $post_ID, 'meta_key' => 'enclosure', 'meta_value' => $meta_value) );
					do_action( 'added_postmeta', $trmdb->insert_id, $post_ID, 'enclosure', $meta_value );
				}
			}
		}
	}
 ...
 */
function trm_get_http( $url, $file_path = false, $red = 1 ) {
	@set_time_limit( 60 );
	...
	$response = trm_remote_request($url, $options);

	if ( is_trm_error( $response ) )
		return false;

	$headers = trm_remote_retrieve_headers( $response );
	$headers['response'] = trm_remote_retrieve_response_code( $response );

	// TRM_HTTP no longer follows redirects for HEAD requests.
	if ( 'HEAD' == $options['method'] && in_array($headers['response'], array(301, 302)) && isset( $headers['location'] ) ) {
		return trm_get_http( $headers['location'], $file_path, ++$red );
	}
	...
	fwrite( $out_fp,  trm_remote_retrieve_body( $response ) );
	fclose($out_fp);
	clearstatcache();
 ...
 */
function trm_get_http_headers( $url, $deprecated = false ) {
	if ( !empty( $deprecated ) )
		_deprecated_argument( __FUNCTION__, '2.7' );

	$response = trm_remote_head( $url );

	if ( is_trm_error( $response ) )
		return false;

	return trm_remote_retrieve_headers( $response );
}
...
function trm_remote_fopen( $uri ) {
	...
	$response = trm_remote_get( $uri, $options );

	if ( is_trm_error( $response ) )
		return false;

	return trm_remote_retrieve_body( $response );
}

/**
 * Set up the Trnder query.
 *
 * @since 2.0.0
 *
 * @param string $query_vars Default TRM_Query arguments.
 */
function trm( $query_vars = '' ) {
	global $trm, $trm_query, $trm_the_query;
	$trm->main( $query_vars );

	if ( !isset($trm_the_query) )
		$trm_the_query = $trm_query;
}
...
function get_status_header_desc( $code ) {
	global $trm_header_to_desc;

	$code = absint( $code );

	if ( !isset( $trm_header_to_desc ) ) {
		$trm_header_to_desc = array(
			100 => 'Continue',
	...
	if ( isset( $trm_header_to_desc[$code] ) )
		return $trm_header_to_desc[$code];
	else
		return '';
}
 ...
 */
function trm_get_nocache_headers() {
	$headers = array(
 ...
 *
 * @since 2.0.0
 * @uses trm_get_nocache_headers()
 */
function nocache_headers() {
	$headers = trm_get_nocache_headers();
...
/**
 * Retrieve the number of database queries during the Trnder execution.
 *
 * @since 2.0.0
 *
 ...
 *
 * @since 2.1.0
 * @uses $trm_query Used to tell if the use a comment feed.
 ...
 * cache values, then this will work. If you use the default Trnder cache,
 * and the database goes away, then you might have problems.
 *
 * Checks for the option siteurl for whether Trnder is installed.
 *
 * @since 2.1.0
 * @uses $trmdb
 *
...
function is_blog_installed() {
	...
	if ( trm_cache_get( 'is_blog_installed' ) )
		return true;
	...
	if ( !isset( $alloptions['siteurl'] ) )
		$installed = $trmdb->get_var( "SELECT option_value FROM $trmdb->options WHERE option_name = 'siteurl'" );
	else
		$installed = $alloptions['siteurl'];
	$trmdb->suppress_errors( $suppress );

	$installed = !empty( $installed );
	trm_cache_set( 'is_blog_installed', $installed );

	if ( $installed )
		return true;

	$suppress = $trmdb->suppress_errors();
	$tables = $trmdb->get_col('SHOW TABLES');
	$trmdb->suppress_errors( $suppress );

	$trm_tables = $trmdb->tables();
	// Loop over the TRM tables.  If none exist, then scratch install is allowed.
	// If one or more exist, suggest table repair since we got here because the options
	// table could not be accessed.
	foreach ( $trm_tables as $table ) {
		// If one of the TRM tables exist, then we are in an insane state.
		if ( in_array( $table, $tables ) ) {
			...
			if ( defined('TRM_REPAIRING') )
				return true;
			// Die with a DB error.
			$trmdb->error = sprintf( /*TRM_I18N_NO_TABLES*/'One or more database tables are unavailable.  The database may need to be <a href="%s">repaired</a>.'/*/TRM_I18N_NO_TABLES*/, 'maint/repair.php?referrer=is_blog_installed' );
			dead_db();
		}
	}

	trm_cache_set( 'is_blog_installed', false );
 ...
 */
function trm_nonce_url( $actionurl, $action = -1 ) {
	$actionurl = str_replace( '&amp;', '&', $actionurl );
	return esc_html( add_query_arg( '_key', trm_create_nonce( $action ), $actionurl ) );
}
 ...
 */
function trm_nonce_field( $action = -1, $name = "_key", $referer = true , $echo = true ) {
	$name = esc_attr( $name );
	$nonce_field = '<input type="hidden" id="' . $name . '" name="' . $name . '" value="' . trm_create_nonce( $action ) . '" />';

	if ( $referer )
		$nonce_field .= trm_referer_field( false );
 ...
 * input name is '_http_referer', in case you wanted to check manually.
 *
 * @package Trnder
 * @subpackage Security
 * @since 2.0.4
 ...
 */
function trm_referer_field( $echo = true ) {
	$ref = esc_attr( $_SERVER['REQUEST_URI'] );
	$referer_field = '<input type="hidden" name="_http_referer" value="'. $ref . '" />';
 ...
 *
 * The input name is '_trm_original_http_referer' and will be either the same
 * value of {@link trm_referer_field()}, if that was posted already or it will
 ...
 */
function trm_original_referer_field( $echo = true, $jump_back_to = 'current' ) {
	$jump_back_to = ( 'previous' == $jump_back_to ) ? trm_get_referer() : $_SERVER['REQUEST_URI'];
	$ref = ( trm_get_original_referer() ) ? trm_get_original_referer() : $jump_back_to;
	$orig_referer_field = '<input type="hidden" name="_trm_original_http_referer" value="' . esc_attr( stripslashes( $ref ) ) . '" />';
	if ( $echo )
...
/**
 * Retrieve referer from '_http_referer', HTTP referer, or current page respectively.
 *
 * @package Trnder
 * @subpackage Security
 * @since 2.0.4
 ...
 */
function trm_get_original_referer() {
	if ( !empty( $_REQUEST['_trm_original_http_referer'] ) )
		return $_REQUEST['_trm_original_http_referer'];
	return false;
}
...
function trm_mkdir_p( $target ) {
	...
	if ( ( $target != '/' ) && ( trm_mkdir_p( dirname( $target ) ) ) )
		return trm_mkdir_p( $target );
 ...
 * Function's preference is to TRM_CONTENT_DIR followed by the return value of <code>sys_get_temp_dir()</code>, before finally defaulting to /tmp/
 *
 * In the event that this function does not find a writable location, It may be overridden by the <code>TRM_TEMP_DIR</code> constant in your <code>trm-setup.php</code> file.
 *
 * @since 2.5.0
 *
...
function get_temp_dir() {
	static $temp;
	if ( defined('TRM_TEMP_DIR') )
		return trailingslashit(TRM_TEMP_DIR);

	if ( $temp )
		return trailingslashit($temp);

	$temp = TRM_CONTENT_DIR . '/';
 ...
 * 'TRM_CONTENT_DIR/uploads'. If the 'UPLOADS' constant is defined, then it will
 * override the 'upload_path' option and 'TRM_CONTENT_DIR/uploads' path.
 *
 * The upload URL path is set either by the 'upload_url_path' option or by using
 * the 'TRM_CONTENT_URL' constant and appending '/uploads' to the path.
 *
 ...
 */
function trm_upload_dir( $time = null ) {
	global $switched;
	...
	if ( empty($upload_path) ) {
		$dir = TRM_CONTENT_DIR . '/uploads';
	} else {
		$dir = $upload_path;
		if ( 'trm-src/uploads' == $upload_path ) {
			$dir = TRM_CONTENT_DIR . '/uploads';
	...
	if ( !$url = get_option( 'upload_url_path' ) ) {
		if ( empty($upload_path) || ( 'trm-src/uploads' == $upload_path ) || ( $upload_path == $dir ) )
			$url = TRM_CONTENT_URL . '/uploads';
		else
 ...
 */
function trm_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
 ...
 * @return array
 */
function trm_upload_bits( $name, $deprecated, $bits, $time = null ) {
	if ( !empty( $deprecated ) )
	...
	$trm_filetype = trm_check_filetype( $name );
	if ( !$trm_filetype['ext'] )
		return array( 'error' => __( 'Invalid file type' ) );

	$upload = trm_upload_dir( $time );

	if ( $upload['error'] !== false )
		return $upload;

	$upload_bits_error = apply_filters( 'trm_upload_bits', array( 'name' => $name, 'bits' => $bits, 'time' => $time ) );
	if ( !is_array( $upload_bits_error ) ) {
		$upload[ 'error' ] = $upload_bits_error;
		return $upload;
	}

	$filename = trm_unique_filename( $upload['path'], $name );

	$new_file = $upload['path'] . "/$filename";
	if ( ! trm_mkdir_p( dirname( $new_file ) ) ) {
 ...
 */
function trm_ext2type( $ext ) {
	$ext2type = apply_filters( 'ext2type', array(
		'audio'       => array( 'aac', 'ac3',  'aif',  'aiff', 'm3a',  'm4a',   'm4b', 'mka', 'mp1', 'mp2',  'mp3', 'ogg', 'oga', 'ram', 'wav', 'wma' ),
		'video'       => array( 'asf', 'avi',  'divx', 'dv',   'flv',  'm4v',   'mkv', 'mov', 'mp4', 'mpeg', 'mpg', 'mpv', 'ogm', 'ogv', 'qt',  'rm', 'vob', 'wmv' ),
		'document'    => array( 'doc', 'docx', 'docm', 'dotm', 'odt',  'pages', 'pdf', 'rtf', 'trm',  'trmd' ),
 ...
 */
function trm_check_filetype( $filename, $mimes = null ) {
	if ( empty($mimes) )
 ...
 */
function trm_check_filetype_and_ext( $file, $filename, $mimes = null ) {

	$proper_filename = false;

	// Do basic extension validation and MIME mapping
	$trm_filetype = trm_check_filetype( $filename, $mimes );
	extract( $trm_filetype );
	...
	if ( $type && 0 === strpos( $type, 'image/' ) && function_exists('getimagesize') ) {
		...
		if ( !empty($imgstats['mime']) && $imgstats['mime'] != $type ) {
			...
			if ( ! empty( $mime_to_ext[ $imgstats['mime'] ] ) ) {
				...
				$trm_filetype = trm_check_filetype( $new_filename, $mimes );
				extract( $trm_filetype );
			}
		}
	}
	...
	return apply_filters( 'trm_check_filetype_and_ext', compact( 'ext', 'type', 'proper_filename' ), $file, $filename, $mimes );
}
...
function get_allowed_mime_types() {
	...
	if ( !$mimes ) {
		...
		// wordperfect formats
		'trm|trmd' => 'application/wordperfect',
		) );
	}
 ...
 *
 * The filter will be called for any action, which is not defined by Trnder.
 ...
 */
function trm_explain_nonce( $action ) {
	if ( $action !== -1 && preg_match( '/([a-z]+)-([a-z]+)(_(.+))?/', $action, $matches ) ) {
		...
		return apply_filters( 'explain_nonce_' . $verb . '-' . $noun, __( 'Sorry, try again!' ), isset($matches[4]) ? $matches[4] : '' );
	} else {
		return apply_filters( 'explain_nonce_' . $action, __( 'Sorry, try again!' ) );
	}
}
 ...
 */
function trm_nonce_ays( $action ) {
	$title = __( 'Trnder Notice' );
	$html = esc_html( trm_explain_nonce( $action ) );
	if ( 'log-out' == $action )
		$html .= "</p><p>" . sprintf( __( "Do you really want to <a href='%s'>log out</a>?"), trm_logout_url() );
	elseif ( trm_get_referer() )
		$html .= "</p><p><a href='" . esc_url( remove_query_arg( 'updated', trm_get_referer() ) ) . "'>" . __( 'Ttry again.' ) . "</a>";

	trm_die( $html, $title, array('response' => 403) );
}


/**
 * Kill Trnder execution and display HTML message with error message.
 *
 ...
 */
function trm_die( $message, $title = '', $args = array() ) {
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX )
		die('-1');

	if ( function_exists( 'apply_filters' ) ) {
		$function = apply_filters( 'trm_die_handler', '_default_trm_die_handler');
	} else {
		$function = '_default_trm_die_handler';
	}
 ...
 * Kill Trnder execution and display HTML message with error message.
 *
 * This is the default handler for trm_die if you want a custom one for your
 * site then you can overload using the trm_die_handler filter in trm_die
 *
 * @since 3.0.0
 * @access private
 *
 ...
 * @param string|array $args Optional arguements to control behaviour.
 */
function _default_trm_die_handler( $message, $title = '', $args = array() ) {
	$defaults = array( 'response' => 500 );
	$r = trm_parse_args($args, $defaults);

	$have_gettext = function_exists('__');

	if ( function_exists( 'is_trm_error' ) && is_trm_error( $message ) ) {
		if ( empty( $title ) ) {
	...
	if ( defined( 'TRM_SITEURL' ) && '' != TRM_SITEURL )
		$admin_dir = TRM_SITEURL . '/Backend-WeaprEcqaKejUbRq-trendr/';
	elseif ( function_exists( 'get_bloginfo' ) && '' != get_bloginfo( 'trmurl' ) )
		$admin_dir = get_bloginfo( 'trmurl' ) . '/Backend-WeaprEcqaKejUbRq-trendr/';
	elseif ( strpos( $_SERVER['PHP_SELF'], 'Backend-WeaprEcqaKejUbRq-trendr' ) !== false )
		$admin_dir = '';
	else
		$admin_dir = 'Backend-WeaprEcqaKejUbRq-trendr/';
...
/**
 * Kill Trnder execution and display XML message with error message.
 *
 * This is the handler for trm_die when processing XMLRPC requests.
 *
 * @since 3.2.0
 * @access private
 *
 ...
 * @param string|array $args Optional arguements to control behaviour.
 */
function _xmlrpc_trm_die_handler( $message, $title = '', $args = array() ) {
	global $trm_xmlrpc_server;
	$defaults = array( 'response' => 500 );

	$r = trm_parse_args($args, $defaults);

	if ( $trm_xmlrpc_server ) {
		$error = new IXR_Error( $r['response'] , $message);
		$trm_xmlrpc_server->output( $error->getXml() );
	}
	die();
}

/**
 * Filter to enable special trm_die handler for xmlrpc requests.
 *
 * @since 3.2.0
 * @access private
 */
function _xmlrpc_trm_die_filter() {
	return '_xmlrpc_trm_die_handler';
}


/**
 * Retrieve the Trnder home page URL.
 *
 * If the constant named 'TRM_HOME' exists, then it willl be used and returned by
...
/**
 * Retrieve the Trnder site URL.
 *
 * If the constant named 'TRM_SITEURL' is defined, then the value in that
 ...
 * @since 2.2.0
 *
 * @param string $url URL to set the Trnder site location.
 * @return string The Trnder Site URL
 */
function _config_trm_siteurl( $url = '' ) {
	if ( defined( 'TRM_SITEURL' ) )
		return TRM_SITEURL;
	return $url;
}
 ...
 *
 * Will only set the direction to 'rtl', if the Trnder locale has the text
 ...
 *
 * Plugins may override the default smiley list by setting the $trmsmiliestrans
 * to an array, with the key the code the blogger types in and the value the
 * image file.
 *
 * The $trm_smiliessearch global is for the regular expression and is set each
 ...
 * available.
 *
 * @global array $trmsmiliestrans
 * @global array $trm_smiliessearch
 * @since 2.2.0
 */
function smilies_init() {
	global $trmsmiliestrans, $trm_smiliessearch;

	// don't bother setting up smilies if they are disabled
	if ( !get_option( 'use_smilies' ) )
		return;

	if ( !isset( $trmsmiliestrans ) ) {
		$trmsmiliestrans = array(
	 ...
	 */
	krsort($trmsmiliestrans);

	$trm_smiliessearch = '/(?:\s|^)';

	$subchar = '';
	foreach ( (array) $trmsmiliestrans as $smiley => $img ) {
		...
		if ($firstchar != $subchar) {
			...
			$subchar = $firstchar;
			$trm_smiliessearch .= preg_quote($firstchar, '/') . '(?:';
		} else {
			$trm_smiliessearch .= '|';
		}
		$trm_smiliessearch .= preg_quote($rest, '/');
	}

	$trm_smiliessearch .= ')(?:\s|$)/m';
}

/**
 * Merge user defined arguments into defaults array.
 *
 * This function is used throughout Trnder to allow for both string or array
 ...
 */
function trm_parse_args( $args, $defaults = '' ) {
	if ( is_object( $args ) )
		$r = get_object_vars( $args );
	elseif ( is_array( $args ) )
		$r = $args;
	else
		trm_parse_str( $args, $r );
 ...
 */
function trm_parse_id_list( $list ) {
	if ( !is_array($list) )
 ...
 */
function trm_array_slice_assoc( $array, $keys ) {
	$slice = array();
	foreach ( $keys as $key )
		if ( isset( $array[ $key ] ) )
			$slice[ $key ] = $array[ $key ];
 ...
 */
function trm_filter_object_list( $list, $args = array(), $operator = 'and', $field = false ) {
	if ( ! is_array( $list ) )
		return array();

	$list = trm_list_filter( $list, $args, $operator );

	if ( $field )
		$list = trm_list_pluck( $list, $field );
 ...
 * @return array
 */
function trm_list_filter( $list, $args = array(), $operator = 'AND' ) {
	if ( ! is_array( $list ) )
		return array();
 ...
 * @return array
 */
function trm_list_pluck( $list, $field ) {
 ...
 *
 * @since 2.2.0
 * @uses add_action() Calls '_admin_menu' hook with 'trm_widgets_add_menu' value.
 */
...
/**
 * Load custom DB error or display Trnder DB error.
 *
 * If a file exists in the trm-src directory named db-error.php, then it will
 * be loaded instead of displaying the Trnder DB error. If it is not found,
 * then the Trnder DB error will be displayed instead.
 *
 * The Trnder DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to the the Trnder 2.3.2, but originally was
 * added in Trnder 2.5.0.
 *
 * @since 2.3.2
 * @uses $trmdb
 */
function dead_db() {
	...
	if ( file_exists( TRM_CONTENT_DIR . '/db-error.php' ) ) {
		require_once( TRM_CONTENT_DIR . '/db-error.php' );
		die();
	}

	// If installing or in the admin, provide the verbose message.
	if ( defined('TRM_INSTALLING') || defined('TRM_ADMIN') )
		trm_die($trmdb->error);
 ...
 * function.
 *
 * The current behavior is to trigger a user error if TRM_DEBUG is true.
 *
 ...
 * @param string $version The version of Trnder that deprecated the function
...
function _deprecated_function( $function, $version, $replacement=null ) {
	...
	if ( TRM_DEBUG && apply_filters( 'deprecated_function_trigger_error', true ) ) {
 ...
 * @param string $version The version of Trnder that deprecated the file
...
function _deprecated_file( $file, $version, $replacement = null, $message = '' ) {
	...
	// Allow plugin to filter the output error trigger
	if ( TRM_DEBUG && apply_filters( 'deprecated_file_trigger_error', true ) ) {
 ...
 *
 * @param string $function The function that was called
 * @param string $version The version of Trnder that deprecated the argument used
 * @param string $message Optional. A message regarding the change.
 */
function _deprecated_argument( $function, $version, $message = null ) {
	...
	// Allow plugin to filter the output error trigger
	if ( TRM_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
		if ( ! is_null( $message ) )
 ...
 * @param string $version The version of Trnder where the message was added.
 */
function _doing_it_wrong( $function, $message, $version ) {
	...
	// Allow plugin to filter the output error trigger
	if ( TRM_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true ) ) {
 ...
 *
 * Will remove Backend-WeaprEcqaKejUbRq-trendr links to retrieve only return URLs not in the Backend-WeaprEcqaKejUbRq-trendr
 * directory.
 *
 * @since 2.6.0
 *
...
function trm_guess_url() {
	if ( defined('TRM_SITEURL') && '' != TRM_SITEURL ) {
		...
		$url = preg_replace('|/Backend-WeaprEcqaKejUbRq-trendr/.*|i', '', $schema . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
	}
	return rtrim($url, '/');
}
 ...
 */
function trm_suspend_cache_addition( $suspend = null ) {
 ...
 * @return bool The current suspend setting
 */
function trm_suspend_cache_invalidation($suspend = true) {
	global $_trm_suspend_cache_invalidation;

	$current_suspend = $_trm_suspend_cache_invalidation;
	$_trm_suspend_cache_invalidation = $suspend;
...
function get_site_option( $option, $default = false, $use_cache = true ) {
	...
	if ( !is_multisite() ) {
		...
		$cache_key = "{$trmdb->siteid}:$option";
		if ( $use_cache )
			$value = trm_cache_get($cache_key, 'site-options');

		if ( !isset($value) || (false === $value) ) {
			$row = $trmdb->get_row( $trmdb->prepare("SELECT meta_value FROM $trmdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $trmdb->siteid ) );
			...
			trm_cache_set( $cache_key, $value, 'site-options' );
		}
	}
...
function add_site_option( $option, $value ) {
	...
	if ( !is_multisite() ) {
		...
		if ( $trmdb->get_row( $trmdb->prepare( "SELECT meta_value FROM $trmdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $trmdb->siteid ) ) )
		...
		$_value = $value;
		$value = maybe_serialize($value);
		$result = $trmdb->insert( $trmdb->sitemeta, array('site_id' => $trmdb->siteid, 'meta_key' => $option, 'meta_value' => $value ) );
		$value = $_value;
	}
...
function delete_site_option( $option ) {
	...
	if ( !is_multisite() ) {
		...
		$row = $trmdb->get_row( $trmdb->prepare( "SELECT meta_id FROM {$trmdb->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $trmdb->siteid ) );
		if ( is_null( $row ) || !$row->meta_id )
			return false;
		$cache_key = "{$trmdb->siteid}:$option";
		trm_cache_delete( $cache_key, 'site-options' );

		$result = $trmdb->query( $trmdb->prepare( "DELETE FROM {$trmdb->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $trmdb->siteid ) );
	}
...
function update_site_option( $option, $value ) {
	...
	if ( !is_multisite() ) {
		...
		if ( $value && !$trmdb->get_row( $trmdb->prepare( "SELECT meta_value FROM $trmdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $trmdb->siteid ) ) )
		...
		$_value = $value;
		$value = maybe_serialize( $value );
		$result = $trmdb->update( $trmdb->sitemeta, array( 'meta_value' => $value ), array( 'site_id' => $trmdb->siteid, 'meta_key' => $option ) );
		$value = $_value;
	}
...
function delete_site_transient( $transient ) {
	...
	if ( $_trm_using_ext_object_cache ) {
		$result = trm_cache_delete( $transient, 'site-transient' );
	} else {
...
function get_site_transient( $transient ) {
	...
	if ( $_trm_using_ext_object_cache ) {
		$value = trm_cache_get( $transient, 'site-transient' );
	} else {
...
function set_site_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( $_trm_using_ext_object_cache ) {
		$result = trm_cache_set( $transient, $value, 'site-transient', $expiration );
	} else {
		$transient_timeout = '_site_transient_timeout_' . $transient;
 ...
 * @return float|bool
 */
function trm_timezone_override_offset() {
 ...
 * @return int
 */
function _trm_timezone_choice_usort_callback( $a, $b ) {
 ...
 * @return string
 */
function trm_timezone_choice( $selected_zone ) {
	...
	if ( !$mo_loaded ) {
		$locale = get_locale();
		$mofile = TRM_LANG_DIR . '/continents-cities-' . $locale . '.mo';
	...
	usort( $zonen, '_trm_timezone_choice_usort_callback' );
...
/**
 * Strip close comment and close php tags from file headers used by TRM
 * See http://core.trac./ticket/8497
 *
 * @since 2.8.0
 *
 * @param string $str
 ...
 * @since 2.9.0
 */
function trm_scheduled_delete() {
	global $trmdb;

	$delete_timestamp = time() - (60*60*24*EMPTY_TRASH_DAYS);

	$posts_to_delete = $trmdb->get_results($trmdb->prepare("SELECT post_id FROM $trmdb->postmeta WHERE meta_key = '_trm_trash_meta_time' AND meta_value < '%d'", $delete_timestamp), ARRAY_A);
	...
	foreach ( (array) $posts_to_delete as $post ) {
		...
		if ( !$del_post || 'trash' != $del_post->post_status ) {
			...
			trm_delete_post($post_id);
		}
	}

	$comments_to_delete = $trmdb->get_results($trmdb->prepare("SELECT comment_id FROM $trmdb->commentmeta WHERE meta_key = '_trm_trash_meta_time' AND meta_value < '%d'", $delete_timestamp), ARRAY_A);
	...
	foreach ( (array) $comments_to_delete as $comment ) {
		...
		if ( !$del_comment || 'trash' != $del_comment->comment_approved ) {
			delete_comment_meta($comment_id, '_trm_trash_meta_time');
			delete_comment_meta($comment_id, '_trm_trash_meta_status');
		} else {
			trm_delete_comment($comment_id);
		}
	}
}
 ...
 * @return string
 */
function _trm_mysql_week( $column ) {
 ...
 */
function trm_find_hierarchy_loop( $callback, $start, $start_parent, $callback_args = array() ) {
	$override = is_null( $start_parent ) ? array() : array( $start => $start_parent );

	if ( !$arbitrary_loop_member = trm_find_hierarchy_loop_tortoise_hare( $callback, $start, $override, $callback_args ) )
		return array();

	return trm_find_hierarchy_loop_tortoise_hare( $callback, $arbitrary_loop_member, $override, $callback_args, true );
}
 ...
 */
function trm_find_hierarchy_loop_tortoise_hare( $callback, $start, $override = array(), $callback_args = array(), $_return_loop = false ) {
...
////////////////
//From 3.7 to fix media link fetch issue
/**
