...
(function($){
	...
	$.widget("trm.pointer", {
		options: {
			pointerClass: 'trm-pointer',
			pointerWidth: 320,
			content: function( respond, event, t ) {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var close  = ( trmPointerL10n ) ? trmPointerL10n.dismiss : 'Dismiss',
		...
		_create: function() {
			...
			this.content = $('<div class="trm-pointer-content"></div>');
			this.arrow   = $('<div class="trm-pointer-arrow"><div class="trm-pointer-arrow-inner"></div></div>');
			...
			this.pointer = $('<div />')
				.append( this.content )
				.append( this.arrow )
				.attr('id', 'trm-pointer-' + identifier++)
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="trm-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /trm-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'trm-pointer-' + edge );
		},
