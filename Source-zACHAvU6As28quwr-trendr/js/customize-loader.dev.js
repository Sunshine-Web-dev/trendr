...
(function( exports, $ ){
	...
	Loader = $.extend( {}, api.Events, {
		initialize: function() {
			...
			$('#trmbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function( e ) {
			...
			if ( hash && 0 === hash.indexOf( 'trm_customize=on' ) )
		...
		open: function( src ) {
			...
			if ( $.support.history && window.location.href !== src )
				...
				window.location.hash = 'trm_customize=on&' + hash;
	...
	$( function() {
		Loader.settings = _trmCustomizeLoaderSettings;
		Loader.initialize();
	});
