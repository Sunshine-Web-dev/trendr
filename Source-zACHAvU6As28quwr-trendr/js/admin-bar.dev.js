...
if ( typeof(jQuery) != 'undefined' ) {
	...
	jQuery(document).ready(function($){
		...
		$('#trmadminbar').removeClass('nojq').removeClass('nojs').find('li.menupop').hoverIntent({
			over: function(e){
				$(this).addClass('hover');
			},
			out: function(e){
		...
		$('#trm-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#trm-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});

		$('#trmadminbar li.menupop > .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 13 )
				return;
		...
		$('#trmadminbar .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 27 )
				return;
		...
		$('#trmadminbar').click( function(e) {
			if ( e.target.id != 'trmadminbar' && e.target.id != 'trm-admin-bar-top-secondary' )
				return;
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'trm-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			// Ensure that the #trmadminbar was the target of the click.
			if ( t.id != 'trmadminbar' && t.id != 'trm-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			aB = d.getElementById('trmadminbar');
