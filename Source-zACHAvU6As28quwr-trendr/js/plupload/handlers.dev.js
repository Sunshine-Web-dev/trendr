	...
	if ( max > hundredmb && file.size > hundredmb ) {
		setTimeout(function(){
			...
			if ( file.status < 3 && file.loaded == 0 ) { // not uploading
				trmFileError(file, pluploadL10n.big_upload_failed.replace('%1$s', '<a class="uploader-html" href="#">').replace('%2$s', '</a>'));
	...
	jQuery('a.delete', item).click(function(){
		...
		jQuery.ajax({
			...
			data: {
				...
				action : 'trash-post',
				_ajax_nonce : this.href.replace(/^.*trmnonce=/,'')
			}
		});
		return false;
	});
...
function trmQueueError(message) {
	jQuery('#media-upload-error').show().html( '<div class="error"><p>' + message + '</p></div>' );
}

// file-specific error messages
function trmFileError(fileObj, message) {
...
function uploadError(fileObj, errorCode, message, uploader) {
	...
	switch (errorCode) {
		case plupload.FAILED:
			trmFileError(fileObj, pluploadL10n.upload_failed);
			break;
		case plupload.FILE_EXTENSION_ERROR:
			trmFileError(fileObj, pluploadL10n.invalid_filetype);
			break;
		case plupload.FILE_SIZE_ERROR:
			uploadSizeError(uploader, fileObj);
			break;
		case plupload.IMAGE_FORMAT_ERROR:
			trmFileError(fileObj, pluploadL10n.not_an_image);
			break;
		case plupload.IMAGE_MEMORY_ERROR:
			trmFileError(fileObj, pluploadL10n.image_memory_exceeded);
			break;
		case plupload.IMAGE_DIMENSIONS_ERROR:
			trmFileError(fileObj, pluploadL10n.image_dimensions_exceeded);
			break;
		case plupload.GENERIC_ERROR:
			trmQueueError(pluploadL10n.upload_failed);
			break;
		case plupload.IO_ERROR:
			max = parseInt(uploader.settings.max_file_size, 10);

			if ( max > hundredmb && fileObj.size > hundredmb )
				trmFileError(fileObj, pluploadL10n.big_upload_failed.replace('%1$s', '<a class="uploader-html" href="#">').replace('%2$s', '</a>'));
			else
				trmQueueError(pluploadL10n.io_error);
			break;
		case plupload.HTTP_ERROR:
			trmQueueError(pluploadL10n.http_error);
			break;
		case plupload.INIT_ERROR:
			jQuery('.media-upload-form').addClass('html-uploader');
			break;
		case plupload.SECURITY_ERROR:
			trmQueueError(pluploadL10n.security_error);
			break;
/*		case plupload.UPLOAD_ERROR.UPLOAD_STOPPED:
		case plupload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			break;*/
		default:
			trmFileError(fileObj, pluploadL10n.default_error);
	}
}
...
jQuery(document).ready(function($){
	...
	uploader_init = function() {
		uploader = new plupload.Uploader(trmUploaderInit);
		...
		uploader.bind('Init', function(up) {
			...
			if ( up.features.dragdrop && ! $(document.body).hasClass('mobile') ) {
				...
				$('#drag-drop-area').bind('dragover.trm-uploader', function(){ // dragenter doesn't fire right :(
					uploaddiv.addClass('drag-over');
				}).bind('dragleave.trm-uploader, drop.trm-uploader', function(){
					uploaddiv.removeClass('drag-over');
				});
			} else {
				uploaddiv.removeClass('drag-drop');
				$('#drag-drop-area').unbind('.trm-uploader');
			}
		});
	...
	if ( typeof(trmUploaderInit) == 'object' )
		uploader_init();
