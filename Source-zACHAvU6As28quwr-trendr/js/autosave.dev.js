...
jQuery(document).ready( function($) {
	...
	window.onbeforeunload = function(){
		...
		if ( mce && !mce.isHidden() ) {
			...
			if ( fullscreen && fullscreen.settings.visible ) {
				title = $('#trm-fullscreen-title').val();
	...
	$(window).unload( function(e) {
		...
		$.ajax({
			...
			async: false,
			data: {
				action: 'trm-remove-post-lock',
				_trmnonce: $('#_trmnonce').val(),
				post_ID: $('#post_ID').val(),
	...
	doPreview = function() {
		$('input#trm-preview').val('dopreview');
		$('form#post').attr('target', 'trm-preview').submit().attr('target', '');
...
function autosave_parse_response(response) {
	var res = trmAjax.parseAjaxResponse(response, 'autosave'), message = '', postID, sup;
...
function autosave_update_slug(post_id) {
	...
	if ( 'undefined' != makeSlugeditClickable && jQuery.isFunction(makeSlugeditClickable) && !jQuery('#edit-slug-box > *').size() ) {
		jQuery.post( ajaxurl, {
				...
				post_id: post_id,
				new_title: fullscreen && fullscreen.settings.visible ? jQuery('#trm-fullscreen-title').val() : jQuery('#title').val(),
...
autosave = function() {
	...
	if ( rich && doAutoSave ) {
		...
		if ( ed.plugins.spellchecker && ed.plugins.spellchecker.active ) {
			...
			if ( 'mce_fullscreen' == ed.id || 'trm_mce_fullscreen' == ed.id )
	...
	if ( fullscreen && fullscreen.settings.visible ) {
		post_data["post_title"] = jQuery('#trm-fullscreen-title').val() || '';
		post_data["content"] = jQuery("#trm_mce_fullscreen").val() || '';
	} else {
	...
	if ( doAutoSave ) {
		...
		jQuery(document).triggerHandler('trmcountwords', [ post_data["content"] ]);
	} else {
