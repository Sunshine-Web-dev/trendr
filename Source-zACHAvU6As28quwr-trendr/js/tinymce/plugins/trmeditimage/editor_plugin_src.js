...
(function() {
	tinymce.create('tinymce.plugins.trmEditImage', {
		url: '',
		editor: {},
		...
		init: function(ed, url) {
			...
			ed.addCommand('TRM_EditImage', function() {
				var el = ed.selection.getNode(), vp, H, W, cls = ed.dom.getAttrib(el, 'class');

				if ( cls.indexOf('mceItem') != -1 || cls.indexOf('trmGallery') != -1 || el.nodeName != 'IMG' )
					return;
				...
				if ( mouse.x && (e.clientX != mouse.x || e.clientY != mouse.y) ) {
					...
					if ( 'IMG' == n.nodeName ) {
						window.setTimeout(function(){
							var DL = ed.dom.getParent(n, 'dl.trm-caption'), width;
			...
			ed.onMouseDown.add(function(ed, e) {
				...
				if ( ed.dom.getAttrib(target, 'class').indexOf('mceItem') == -1 ) {
					...
					ed.plugins.trendr._showButtons(target, 'trm_editbtns');
				}
			});
			...
			ed.onBeforeSetContent.add(function(ed, o) {
				o.content = ed.trmSetImgCaption(o.content);
			});

			ed.onPostProcess.add(function(ed, o) {
				if (o.get)
					o.content = ed.trmGetImgCaption(o.content);
			});

			ed.trmSetImgCaption = function(content) {
				return t._do_shcode(content);
			};

			ed.trmGetImgCaption = function(content) {
				return t._get_shcode(content);
			};
		},

		_do_shcode : function(content) {
			return content.replace(/(?:<p>)?\[(?:trm_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:trm_)?caption\](?:<\/p>)?/g, function(a,b,c){
				...
				return '<div class="'+div_cls+'"><dl id="'+id+'" class="trm-caption '+cls+'" style="width: '+( 10 + parseInt(w) )+
				'px"><dt class="trm-caption-dt">'+img+'</dt><dd class="trm-caption-dd">'+cap+'</dd></dl></div>';
			});
		},
		...
		_createButtons : function() {
			...
			editButton = DOM.add('trm_editbtns', 'img', {
				src : t.url+'/img/image.png',
				id : 'trm_editimgbtn',
				width : '24',
				height : '24',
				title : ed.getLang('trmeditimage.edit_img')
			});

			tinymce.dom.Event.add(editButton, 'mousedown', function(e) {
				var ed = tinyMCE.activeEditor;
				ed.windowManager.bookmark = ed.selection.getBookmark('simple');
				ed.execCommand("TRM_EditImage");
			});

			dellButton = DOM.add('trm_editbtns', 'img', {
				src : t.url+'/img/delete.png',
				id : 'trm_delimgbtn',
				width : '24',
				height : '24',
				title : ed.getLang('trmeditimage.del_img')
			});
	...
	tinymce.PluginManager.add('trmeditimage', tinymce.plugins.trmEditImage);
})();
