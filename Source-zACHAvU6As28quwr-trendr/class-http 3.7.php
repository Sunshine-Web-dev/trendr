 ...
 *
 * Standardizes the HTTP requests for trendr. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * trendr HTTP Class for managing HTTP Transports and making HTTP requests.
 *
...
class TRM_Http {
	 ...
	 * replace the default user-agent, which is 'trendr/TRM_Version', where TRM_Version is the
	 * value from $trm_version.
	 *
	 ...
	 * @return array|object Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A TRM_Error instance upon error
	 */
	function request( $url, $args = array() ) {
		...
		$defaults = array(
			...
			'user-agent' => apply_filters( 'http_headers_useragent', 'trendr/' . $trm_version . '; ' . get_bloginfo( 'url' ) ),
			...
			'decompress' => true,
			'sslverify' => false,
			//'sslcertificates' => ABSPATH . TRMINC . '/certificates/ca-bundle.crt',
			'stream' => false,
			'filename' => null,
		...
		if ( function_exists( 'trm_kses_bad_protocol' ) ) {
			if ( $r['reject_unsafe_urls'] )
				$url = trm_http_validate_url( $url );
			$url = trm_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
		}

		$arrURL = @parse_url( $url );

		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new TRM_Error('http_request_failed', __('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new TRM_Error( 'http_request_failed', __( 'User has blocked requests through HTTP.' ) );

		// Determine if this is a https call and pass that on to the transport functions
		// so that we can blacklist the transports that do not support ssl verification
		$r['ssl'] = $arrURL['scheme'] == 'https' || $arrURL['scheme'] == 'ssl';

		// Determine if this request is to OUR install of trendr
		$homeURL = parse_url( get_bloginfo( 'url' ) );
		$r['local'] = $homeURL['host'] == $arrURL['host'] || 'localhost' == $arrURL['host'];
		unset( $homeURL );

		// If we are streaming to a file but no filename was given drop it in the TRM temp dir
		...
		if ( $r['stream'] ) {
			$r['blocking'] = true;
			if ( ! trm_is_writable( dirname( $r['filename'] ) ) )
				return new TRM_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
		...
		if ( ! is_array( $r['headers'] ) ) {
			$processedHeaders = TRM_Http::processHeaders( $r['headers'], $url );
		...
		TRM_Http::buildCookieHeader( $r );

		// Avoid issues where mbstring.func_overload is enabled
		mbstring_binary_safe_encoding();

		if ( ! isset( $r['headers']['Accept-Encoding'] ) ) {
			if ( $encoding = TRM_Http_Encoding::accept_encoding( $url, $r ) )
		...
		if ( ! empty( $r['cookies'] ) ) {
			$cookies_set = trm_list_pluck( $response['cookies'], 'name' );
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( !$class )
			return new TRM_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
	...
	public static function processHeaders( $headers, $url = '' ) {
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' == $key )
				$cookies[] = new TRM_Http_Cookie( $value, $url );
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to TRM_Http_Cookie instances,
	...
	public static function buildCookieHeader( &$r ) {
		if ( ! empty($r['cookies']) ) {
			// Upgrade any name => value cookie pairs to TRM_HTTP_Cookie instances
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) )
					$r['cookies'][ $name ] = new TRM_HTTP_Cookie( array( 'name' => $name, 'value' => $value ) );
			}
	 ...
	 * prevent plugins from working and core functionality, if you don't include
	 * You block external URL requests by defining TRM_HTTP_BLOCK_EXTERNAL as true in your trm-config.php
	 * file and this will only allow localhost and your blog to make requests. The constant
	 * TRM_ACCESSIBLE_HOSTS will allow additional hosts to go through for requests. The format of the
	 * TRM_ACCESSIBLE_HOSTS constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg *. will allow for all subdomains of  to be contacted.
	 *
	 * @since 2.8.0
	...
	function block_request($uri) {
		...
		if ( ! defined( 'TRM_HTTP_BLOCK_EXTERNAL' ) || ! TRM_HTTP_BLOCK_EXTERNAL )
			return false;
		...
		if ( !defined('TRM_ACCESSIBLE_HOSTS') )
			return true;

		static $accessible_hosts;
		static $wildcard_regex = false;
		if ( null == $accessible_hosts ) {
			$accessible_hosts = preg_split('|,\s*|', TRM_ACCESSIBLE_HOSTS);

			if ( false !== strpos(TRM_ACCESSIBLE_HOSTS, '*') ) {
	 ...
	 * @return false|object False if no redirect is present, a TRM_HTTP or TRM_Error result otherwise.
	 */
	static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 )
			return new TRM_Error( 'http_request_failed', __('Too many redirects.') );
		...
		$redirect_location = TRM_HTTP::make_absolute_url( $redirect_location, $url );
		...
		return trm_remote_request( $redirect_location, $args );
	}
 ...
 */
class TRM_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see TRM_Http::request For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	...
	function request($url, $args = array()) {
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str )
				return new TRM_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );

			return new TRM_Error('http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $r['stream'] ) {
			if ( ! TRM_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new TRM_Error( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
			...
			while ( ! feof($handle) && $keep_reading ) {
				...
				if ( ! $bodyStarted ) {
					...
					if ( strpos( $strResponse, "\r\n\r\n" ) ) {
						$process = TRM_Http::processResponse( $strResponse );
						$bodyStarted = true;
						$block = $process['body'];
						unset( $strResponse );
						$process['body'] = '';
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new TRM_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$arrHeaders = TRM_Http::processHeaders( $process['headers'], $url );
		...
		// Handle redirects
		if ( false !== ( $redirect_response = TRM_HTTP::handle_redirects( $url, $r, $response ) ) )
			return $redirect_response;

		// If the body was chunk encoded, then decode it.
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
			$process['body'] = TRM_Http::chunkTransferDecode($process['body']);

		if ( true === $r['decompress'] && true === TRM_Http_Encoding::should_decode($arrHeaders['headers']) )
			$process['body'] = TRM_Http_Encoding::decompress( $process['body'] );
	...
	static function verify_ssl_certificate( $stream, $host ) {
		...
		$host_type = ( TRM_HTTP::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of TRM_HTTP directly through it's API.
 *
 * @see TRM_HTTP::request
 *
 * @package trendr
 * @subpackage HTTP
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use TRM_HTTP::request() directly
 */
class TRM_HTTP_Fsockopen extends TRM_HTTP_Streams {
	...
	function request($url, $args = array()) {
		...
		if ( ! $r['blocking'] ) {
			...
			if ( $curl_error = curl_error( $handle ) ) {
				curl_close( $handle );
				return new TRM_Error( 'http_request_failed', $curl_error );
			}
		...
		$theHeaders = TRM_Http::processHeaders( $this->headers, $url );
		$theBody = $this->body;
		...
		if ( true === $r['decompress'] && true === TRM_Http_Encoding::should_decode($theHeaders['headers']) )
			$theBody = TRM_Http_Encoding::decompress( $theBody );
...
/**
 * Adds Proxy support to the trendr HTTP API.
 *
 * There are caveats to proxy support. It requires that defines be made in the trm-config.php file to
 ...
 * <ol>
 * <li>TRM_PROXY_HOST - Enable proxy support and host for connecting.</li>
 * <li>TRM_PROXY_PORT - Proxy port for connection. No default, must be defined.</li>
 * <li>TRM_PROXY_USERNAME - Proxy username, if it requires authentication.</li>
 * <li>TRM_PROXY_PASSWORD - Proxy password, if it requires authentication.</li>
 * <li>TRM_PROXY_BYPASS_HOSTS - Will prevent the hosts in this list from going through the proxy.
 ...
 * <code>
 * define('TRM_PROXY_HOST', '192.168.84.101');
 * define('TRM_PROXY_PORT', '8080');
...
class TRM_HTTP_Proxy {
	 ...
	 */
	function is_enabled() {
		return defined('TRM_PROXY_HOST') && defined('TRM_PROXY_PORT');
	}
	 ...
	 *
	 * @since 2.8
	 * @use TRM_PROXY_USERNAME
	 * @use TRM_PROXY_PASSWORD
	 *
	 * @return bool
	 */
	function use_authentication() {
		return defined('TRM_PROXY_USERNAME') && defined('TRM_PROXY_PASSWORD');
	}
	 ...
	 */
	function host() {
		if ( defined('TRM_PROXY_HOST') )
			return TRM_PROXY_HOST;
	 ...
	 */
	function port() {
		if ( defined('TRM_PROXY_PORT') )
			return TRM_PROXY_PORT;
	 ...
	 */
	function username() {
		if ( defined('TRM_PROXY_USERNAME') )
			return TRM_PROXY_USERNAME;
	 ...
	 */
	function password() {
		if ( defined('TRM_PROXY_PASSWORD') )
			return TRM_PROXY_PASSWORD;
	...
	function send_through_proxy( $uri ) {
		...
		if ( !defined('TRM_PROXY_BYPASS_HOSTS') )
			return true;

		static $bypass_hosts;
		static $wildcard_regex = false;
		if ( null == $bypass_hosts ) {
			$bypass_hosts = preg_split('|,\s*|', TRM_PROXY_BYPASS_HOSTS);

			if ( false !== strpos(TRM_PROXY_BYPASS_HOSTS, '*') ) {
				$wildcard_regex = array();
 ...
 * already a TRM_Http_Cookie() object, then they are turned into one.
 *
 * @todo The trendr convention is to use underscores instead of camelCase for function and method
...
class TRM_Http_Cookie {
	...
	function getHeaderValue() {
		...
		return $this->name . '=' . apply_filters( 'trm_http_cookie_value', $this->value, $this->name );
	}

	/**
	 * Retrieve cookie header for usage in the rest of the trendr HTTP API.
	 *
	 * @access public
	 * @since 2.8.0
	 *
...
class TRM_Http_Encoding {
	...
	public static function decompress( $compressed, $length = null ) {
		...
		if ( false !== ( $decompressed = TRM_Http_Encoding::compatible_gzinflate( $compressed ) ) )
			return $decompressed;
	...
	public static function accept_encoding( $url, $args ) {
		$type = array();
		$compression_enabled = TRM_Http_Encoding::is_available();
		...
		$type = apply_filters( 'trm_http_accept_encoding', $type, $url, $args );
