<?php
/**
 * Trnder Link Template Functions
 *
 * @package Trnder
 * @subpackage Template
 */
...
function user_trailingslashit($string, $type_of_url = '') {
	global $trm_rewrite;
	if ( $trm_rewrite->use_trailing_slashes )
	...
	if ( '' != $permalink && !in_array($post->post_status, array('draft', 'pending', 'auto-draft')) ) {
		...
		if ( strpos($permalink, '%category%') !== false ) {
			...
			if ( empty($category) ) {
				...
				$category = is_trm_error( $default_category ) ? '' : $default_category->slug;
			}
		}
...
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $trm_rewrite->get_extra_permastruct($post->post_type);
...
function _get_page_link( $id = false, $leavename = false, $sample = false ) {
	...
	$link = $trm_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the Trnder Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @param int $id Optional. Post ID.
...
function get_attachment_link($id = false) {
	...
	$object = get_post($id);
	if ( $trm_rewrite->using_permalinks() && ($object->post_parent > 0) && ($object->post_parent != $id) ) {
...
function get_year_link($year) {
	...
	$yearlink = $trm_rewrite->get_year_permastruct();
	if ( !empty($yearlink) ) {
...
function get_month_link($year, $month) {
	...
	$monthlink = $trm_rewrite->get_month_permastruct();
	if ( !empty($monthlink) ) {
...
function get_day_link($year, $month, $day) {
	...
	$daylink = $trm_rewrite->get_day_permastruct();
	if ( !empty($daylink) ) {
...
function get_feed_link($feed = '') {
	...
	$permalink = $trm_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos($feed, 'comments_') ) {
			$feed = str_replace('comments_', '', $feed);
			$permalink = $trm_rewrite->get_comment_feed_permastruct();
		}
...
function get_term_feed_link( $term_id, $taxonomy = 'category', $feed = '' ) {
	...
	if ( empty( $term ) || is_trm_error( $term ) )
		return false;
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $trm_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] )
			$struct = $trm_rewrite->front . $struct;
		else
			$struct = $trm_rewrite->root . $struct;
 ...
 *
 * Can be used within the Trnder loop or outside of it. Can be used with
 ...
 *
 * Can be used within the Trnder loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int $id Optional. Post ID.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	...
	return apply_filters( 'get_delete_post_link', trm_nonce_url( $delete_link, "$action-{$post->post_type}_{$post->ID}" ), $post->ID, $force_delete );
}
...
function get_adjacent_post($in_same_cat = false, $excluded_categories = '', $previous = true) {
	...
	if ( $in_same_cat || !empty($excluded_categories) ) {
		$join = " INNER JOIN $trmdb->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $trmdb->term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";

		if ( $in_same_cat ) {
			$cat_array = trm_get_object_terms($post->ID, 'category', array('fields' => 'ids'));
	...
	$where = apply_filters( "get_{$adjacent}_post_where", $trmdb->prepare("WHERE p.post_date $op %s AND p.post_type = %s AND p.post_status = 'publish' $posts_in_ex_cats_sql", $current_post_date, $post->post_type), $in_same_cat, $excluded_categories );
	$sort  = apply_filters( "get_{$adjacent}_post_sort", "ORDER BY p.post_date $order LIMIT 1" );

	$query = "SELECT p.* FROM $trmdb->posts AS p $join $where $sort";
	$query_key = 'adjacent_post_' . md5($query);
	$result = trm_cache_get($query_key, 'counts');
	if ( false !== $result )
		return $result;

	$result = $trmdb->get_row("SELECT p.* FROM $trmdb->posts AS p $join $where $sort");
	if ( null === $result )
		$result = '';

	trm_cache_set($query_key, $result, 'counts');
	return $result;
}
 ...
 *
 * This is meant to be attached to actions like 'trm_head'.  Do not call this directly in plugins or theme templates.
 * @since 3.0.0
 *
 */
function adjacent_posts_rel_link_trm_head() {
...
function get_pagenum_link($pagenum = 1) {
	...
	if ( !$trm_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$trm_rewrite->pagination_base/\d+/?$|", '', $request);
		$request = preg_replace( '|^index\.php|', '', $request);
		$request = ltrim($request, '/');

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $trm_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) )
			$base .= 'index.php/';

		if ( $pagenum > 1 ) {
			$request = ( ( !empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $trm_rewrite->pagination_base . "/" . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $trm_query;

	if ( !$max_page )
		$max_page = $trm_query->max_num_pages;
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		...
		$max_num_pages = $trm_query->max_num_pages;
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($max_page) )
		$max_page = $trm_query->max_num_comment_pages;
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url($blog_id, 'Backend-WeaprEcqaKejUbRq-trendr/', $scheme);
...
function plugins_url($path = '', $plugin = '') {
	...
	$mu_plugin_dir = TRMMU_PLUGIN_DIR;
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url('Backend-WeaprEcqaKejUbRq-trendr/network/', $scheme);
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url('Backend-WeaprEcqaKejUbRq-trendr/user/', $scheme);
...
function rel_canonical() {
	...
	if ( !$id = $trm_the_query->get_queried_object_id() )
		return;
 ...
 */
function trm_get_shortlink($id = 0, $context = 'post', $allow_slugs = true) {
 ...
 *
 *  Attached to the trm_head action.
 *
 * @since 3.0.0
 *
 * @uses trm_get_shortlink()
 */
function trm_shortlink_trm_head() {
	$shortlink = trm_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the trm action.
 *
 * @since 3.0.0
 *
 * @uses trm_get_shortlink()
 */
function trm_shortlink_header() {
	if ( headers_sent() )
		return;
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = trm_get_shortlink( $post->ID );
