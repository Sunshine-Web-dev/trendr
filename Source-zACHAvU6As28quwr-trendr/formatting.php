 ...
 *
 * @since 0.71
 * @uses $trm_cockneyreplace Array of formatted entities for certain common phrases
 *
 * @param string $text The text to be formatted
 * @return string The string replaced with html entities
 */
function trmtexturize($text) {
	global $trm_cockneyreplace;
	...
	if ( empty( $opening_quote ) ) {
		...
		if ( isset($trm_cockneyreplace) ) {
			$cockney = array_keys($trm_cockneyreplace);
			$cockneyreplace = array_values($trm_cockneyreplace);
		} else {
	...
	// Transform into regexp sub-expression used in _trmtexturize_pushpop_element
	...
	foreach ( $textarr as &$curl ) {
		...
		// Only call _trmtexturize_pushpop_element if first char is correct tag opening
		$first = $curl[0];
		if ( '<' === $first ) {
			_trmtexturize_pushpop_element($curl, $no_texturize_tags_stack, $no_texturize_tags, '<', '>');
		} elseif ( '[' === $first ) {
			_trmtexturize_pushpop_element($curl, $no_texturize_shortcodes_stack, $no_texturize_shortcodes, '[', ']');
 ...
 * @return object
 */
function _trmtexturize_pushpop_element($text, &$stack, $disabled_elements, $opening = '<', $closing = '>') {
...
/**
 * Accepts matches array from preg_replace_callback in trmautop() or a string.
 *
...
function trmautop($pee, $br = 1) {
	...
	if ($br) {
		...
		$pee = str_replace('<TRMPreserveNewline />', "\n", $pee);
	}
...
/**
 * Newline preservation help function for trmautop
 *
 * @since 3.1.0
 * @access private
 ...
 */
function _trm_specialchars( $string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	...
	// Store the site charset as a static to avoid multiple calls to trm_load_alloptions()
	if ( !$charset ) {
		static $_charset;
		if ( !isset( $_charset ) ) {
	...
	if ( !$double_encode ) {
		$string = trm_specialchars_decode( $string, $_quote_style );

		/* Critical */
		// The previous line decodes &amp;phrase; into &phrase;  We must guarantee that &phrase; is valid before proceeding.
		$string = trm_kses_normalize_entities($string);

		// Now proceed with custom double-encoding silliness
		$string = preg_replace( '/&(#?x?[0-9a-z]+);/i', '|trm_entity|$1|/trm_entity|', $string );
	}
	...
	// Handle double encoding ourselves
	if ( !$double_encode ) {
		$string = str_replace( array( '|trm_entity|', '|/trm_entity|' ), array( '&', ';' ), $string );
	}
 ...
 * @param mixed $quote_style Optional. Converts double quotes if set to ENT_COMPAT, both single and double if set to ENT_QUOTES or none if set to ENT_NOQUOTES. Also compatible with old _trm_specialchars() values; converting single quotes if set to 'single', double if set to 'double' or both if otherwise set. Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function trm_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _trm_specialchars() when the $quote_style is not an accepted value
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
	} elseif ( !in_array( $quote_style, array( 0, 2, 3, 'single', 'double' ), true ) ) {
		$quote_style = ENT_QUOTES;
 ...
 */
function trm_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
...
function sanitize_user( $username, $strict = false ) {
	...
	$username = trm_strip_all_tags( $username );
...
function convert_chars($content, $deprecated = '') {
	...
	// Fix Word pasting
	$content = strtr($content, $trm_htmltranswinuni);
...
function force_balance_tags( $text ) {
	...
	// TRM bug fix for comments - in case you REALLY meant to type '< !--'
	$text = str_replace('< !--', '<    !--', $text);
	// TRM bug fix for LOVE <3 (and other situations with '<' before a number)
	...
	// TRM fix for the bug with HTML comments
 ...
 */
function trm_rel_nofollow( $text ) {
	// This is a pre save filter, so text is already escaped.
	$text = stripslashes($text);
	$text = preg_replace_callback('|<a (.+?)>|i', 'trm_rel_nofollow_callback', $text);
	$text = esc_sql($text);
	return $text;
}
 ...
 */
function trm_rel_nofollow_callback( $matches ) {
	$text = $matches[1];
 ...
 *
 * Looks up one smiley code in the $trmsmiliestrans global array and returns an
...
function translate_smiley($smiley) {
	...
	$img = $trmsmiliestrans[$smiley];
	$smiley_masked = esc_attr($smiley);

	$srcurl = apply_filters('smilies_src', includes_url("images/smilies/$img"), $img, site_url());

	return " <img src='$srcurl' alt='$smiley_masked' class='trm-smiley' /> ";
}
 ...
 *
 * @since 0.71
 * @uses $trm_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies($text) {
	global $trm_smiliessearch;
	$output = '';
	if ( get_option('use_smilies') && !empty($trm_smiliessearch) ) {
		...
		for ($i = 0; $i < $stop; $i++) {
			...
			if ((strlen($content) > 0) && ('<' != $content[0])) { // If it's not a tag
				$content = preg_replace_callback($trm_smiliessearch, 'translate_smiley', $content);
			}
			$output .= $content;
		}
	} else {
 ...
 *
 * @since 1.2.0
 * @usedby trm_mail() handles charsets in email subjects
 *
 * @param string $string Subject line
 * @return string Converted string to ASCII
 */
function trm_iso_descrambler($string) {
	/* this may only work with iso-8859-1, I'm afraid */
	if (!preg_match('#\=\?(.+)\?Q\?(.+)\?\=#i', $string, $matches)) {
		return $string;
	} else {
		$subject = str_replace('_', ' ', $matches[2]);
		$subject = preg_replace_callback('#\=([0-9a-f]{2})#i', '_trm_iso_convert', $subject);
		return $subject;
	}
}
 ...
 */
function _trm_iso_convert( $match ) {
 ...
 *
 * The 55 word limit can be modified by plugins/858483 using the excerpt_length filter
 * The ' [...]' string can be modified by plugins/858483 using the excerpt_more filter
 *
 * @since 1.5.0
 *
 ...
 */
function trm_trim_excerpt($text) {
	$raw_excerpt = $text;
	if ( '' == $text ) {
	...
	return apply_filters('trm_trim_excerpt', $text, $raw_excerpt);
}
 ...
 */
function trm_htmledit_pre($output) {
	if ( !empty($output) )
 ...
 *
 * This is just a handy shortcut for $trmdb->escape(), for completeness' sake
 *
 * @since 2.8.0
 ...
 *
 * @since 2.8.0
 * @uses trm_kses_bad_protocol() To only permit protocols in the URL set
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	if ( 'display' == $_context ) {
		$url = trm_kses_normalize_entities( $url );
		$url = str_replace( '&amp;', '&#038;', $url );
		$url = str_replace( "'", '&#039;', $url );
	}
	...
	if ( trm_kses_bad_protocol( $url, $protocols ) != $url )
		return '';
...
function esc_js( $text ) {
	$safe_text = trm_check_invalid_utf8( $text );
	$safe_text = _trm_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = trm_check_invalid_utf8( $text );
	$safe_text = _trm_specialchars( $safe_text, ENT_QUOTES );
 ...
 */
function trm_make_link_relative( $link ) {
...
function sanitize_option($option, $value) {
	...
	switch ( $option ) {
		...
		case 'blogname':
			...
			$value = trm_filter_post_kses( $value ); // calls stripslashes then addslashes
			$value = stripslashes($value);
		...
		case 'upload_path':
			...
			$value = addslashes($value);
			$value = trm_filter_kses($value); // calls stripslashes then addslashes
			$value = stripslashes($value);
			break;
		...
		case 'siteurl':
			if ( (bool)preg_match( '#http(s?)://(.+)#i', $value) ) {
				...
				$value = get_option( $option ); // Resets option to stored value in the case of failed sanitization
				if ( function_exists('add_settings_error') )
					add_settings_error('siteurl', 'invalid_siteurl', __('The Trnder address you entered did not appear to be a valid URL. Please enter a valid URL.'));
			}
			break;
 ...
 *
 * @since 2.2.1
 * @uses apply_filters() for the 'trm_parse_str' filter.
 *
 * @param string $string The string to be parsed.
 * @param array $array Variables will be stored in this array.
 */
function trm_parse_str( $string, &$array ) {
	parse_str( $string, $array );
	if ( get_magic_quotes_gpc() )
		$array = stripslashes_deep( $array );
	$array = apply_filters( 'trm_parse_str', $array );
}
 ...
 *
 * @uses trm_pre_kses_less_than_callback in the callback function.
 * @since 2.3.0
 *
 * @param string $text Text to be converted.
 * @return string Converted text.
 */
function trm_pre_kses_less_than( $text ) {
	return preg_replace_callback('%<[^>]*?((?=<)|>|$)%', 'trm_pre_kses_less_than_callback', $text);
}
 ...
 */
function trm_pre_kses_less_than_callback( $matches ) {
...
/**
 * Trnder implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
...
function trm_sprintf( $pattern ) {
	...
	while ( $len > $start ) {
		...
		if ( $pattern[$start] == '%' ) {
			...
			$_fragment = apply_filters( 'trm_sprintf', $fragment, $arg );
 ...
 */
function trm_sprintf_l($pattern, $args) {
	// Not a match
	...
	$l = apply_filters('trm_sprintf_l', array(
 ...
 * @return string The excerpt.
 */
function trm_html_excerpt( $str, $count ) {
	$str = trm_strip_all_tags( $str, true );
 ...
 * @return string The processed string.
 */
function trm_strip_all_tags($string, $remove_breaks = false) {
...
function sanitize_text_field($str) {
	$filtered = trm_check_invalid_utf8( $str );

	if ( strpos($filtered, '<') !== false ) {
		$filtered = trm_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = trm_strip_all_tags( $filtered, true );
	} else {
 ...
 * @return string
 */
function trm_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( '%2F', '/', urlencode( $path ) ), $suffix ) );
}

/**
 * Forever eliminate "Trnder" from the planet (or at least the little bit we can influence).
 *
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === current_filter() )
		return str_replace( 'Trnder', 'Trnder', $text );
	// Still here? Use the more judicious replacement
	static $dblq = false;
	if ( false === $dblq )
		$dblq = _x('&#8220;', 'opening curly quote');
	return str_replace(
		array( ' Trnder', '&#8216;Trnder', $dblq . 'Trnder', '>Trnder', '(Trnder' ),
		array( ' Trnder', '&#8216;Trnder', $dblq . 'Trnder', '>Trnder', '(Trnder' ),
	$text );
