...
function get_bookmark($bookmark, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($bookmark) ) {
		...
		trm_cache_add($bookmark->link_id, $bookmark, 'bookmark');
		$_bookmark = $bookmark;
	} else {
		if ( isset($GLOBALS['link']) && ($GLOBALS['link']->link_id == $bookmark) ) {
			$_bookmark = $GLOBALS['link'];
		} elseif ( ! $_bookmark = trm_cache_get($bookmark, 'bookmark') ) {
			$_bookmark = $trmdb->get_row($trmdb->prepare("SELECT * FROM $trmdb->links WHERE link_id = %d LIMIT 1", $bookmark));
			$_bookmark->link_category = array_unique( trm_get_object_terms($_bookmark->link_id, 'link_category', array('fields' => 'ids')) );
			trm_cache_add($_bookmark->link_id, $_bookmark, 'bookmark');
		}
	}
 ...
 * @since 2.1.0
 * @uses $trmdb Database Object
 * @link http://codex./Template_Tags/get_bookmarks
 *
...
function get_bookmarks($args = '') {
	...
	$cache = array();
	$key = md5( serialize( $r ) );
	if ( $cache = trm_cache_get( 'get_bookmarks', 'bookmark' ) ) {
	...
	if ( !empty($category_name) ) {
		if ( $category = get_term_by('name', $category_name, 'link_category') ) {
			...
			$cache[ $key ] = array();
			trm_cache_set( 'get_bookmarks', $cache, 'bookmark' );
	...
	if (!empty($category_query)) {
		...
		$join = " INNER JOIN $trmdb->term_relationships AS tr ON ($trmdb->links.link_id = tr.object_id) INNER JOIN $trmdb->term_taxonomy as tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
	}
	...
	$query = "SELECT * $length $recently_updated_test $get_updated FROM $trmdb->links $join WHERE 1=1 $visible $category_query";
	$query .= " $exclusions $inclusions $search";
	$query .= " ORDER BY $orderby $order";
	if ($limit != -1)
		$query .= " LIMIT $limit";

	$results = $trmdb->get_results($query);
 ...
 *
 * @since 2.7.0
 * @uses trm_cache_delete() Deletes the contents of 'get_bookmarks'
 */
function clean_bookmark_cache($bookmark_id) {
	trm_cache_delete( $bookmark_id, 'bookmark' );
	trm_cache_delete( 'get_bookmarks', 'bookmark' );
}
