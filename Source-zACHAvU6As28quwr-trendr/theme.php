 ...
 * Trnder workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	elseif ( !empty($trm_locale->text_direction) && file_exists("$dir/{$trm_locale->text_direction}.css") )
		$stylesheet_uri = "$stylesheet_dir_uri/{$trm_locale->text_direction}.css";
	else
		$stylesheet_uri = '';
 ...
 * version, status, and finally tags. Some of these are not used by Trnder
...
function get_theme_data( $theme_file ) {
	...
	$theme_data['Name'] = $theme_data['Title'] = trm_kses( $theme_data['Name'], $themes_allowed_tags );

	$theme_data['URI'] = esc_url( $theme_data['URI'] );

	$theme_data['Description'] = trmtexturize( trm_kses( $theme_data['Description'], $themes_allowed_tags ) );

	$theme_data['AuthorURI'] = esc_url( $theme_data['AuthorURI'] );

	$theme_data['Template'] = trm_kses( $theme_data['Template'], $themes_allowed_tags );

	$theme_data['Version'] = trm_kses( $theme_data['Version'], $themes_allowed_tags );

	if ( $theme_data['Status'] == '' )
		$theme_data['Status'] = 'publish';
	else
		$theme_data['Status'] = trm_kses( $theme_data['Status'], $themes_allowed_tags );

	if ( $theme_data['Tags'] == '' )
		$theme_data['Tags'] = array();
	else
		$theme_data['Tags'] = array_map( 'trim', explode( ',', trm_kses( $theme_data['Tags'], array() ) ) );

	if ( $theme_data['Author'] == '' ) {
		$theme_data['Author'] = $theme_data['AuthorName'] = __('Anonymous');
	} else {
		$theme_data['AuthorName'] = trm_kses( $theme_data['Author'], $themes_allowed_tags );
 ...
 * list is saved in the {@link $trm_broken_themes} global, which is displayed on
 * the theme list in the administration panels.
 *
 * @since 1.5.0
 * @global array $trm_broken_themes Stores the broken themes.
 * @global array $trm_themes Stores the working themes.
 *
 * @return array Theme list with theme data.
 */
function get_themes() {
	global $trm_themes, $trm_broken_themes;
	...
	foreach ( (array) $theme_files as $theme_file ) {
		...
		if ( !is_readable("$theme_root/$theme_file") ) {
			$trm_broken_themes[$theme_file] = array('Name' => $theme_file, 'Title' => $theme_file, 'Description' => __('File not readable.'));
			continue;
		}
		...
		$description = trmtexturize($theme_data['Description']);
			...
			if ( file_exists("$theme_root/$parent_dir/$template/index.php") ) {
				...
				if ( isset($theme_files[$template]) && file_exists( $theme_files[$template]['theme_root'] . "/$template/index.php" ) ) {
					...
					if ( empty( $parent_template) )
						$trm_broken_themes[$name] = array('Name' => $name, 'Title' => $title, 'Description' => __('Template is missing.'), 'error' => 'no_template');
					else
						$trm_broken_themes[$name] = array('Name' => $name, 'Title' => $title, 'Description' => sprintf( __('The parent theme is missing. Please install the "%s" parent theme.'),  $parent_template ), 'error' => 'no_parent', 'parent' => $parent_template );
					continue;
				}
		...
		// their pretenders.
		if ( isset($trm_themes[$name]) ) {
			$trump_cards = array(
				'classic'      => 'Trnder Classic',
				'default'      => 'Trnder Default',
			...
			if ( isset( $trump_cards[ $stylesheet ] ) && $name == $trump_cards[ $stylesheet ] ) {
				...
				// them aside.
				$suffix = $trm_themes[$name]['Stylesheet'];
				$new_name = "$name/$suffix";
				$trm_themes[$new_name] = $trm_themes[$name];
				$trm_themes[$new_name]['Name'] = $new_name;
			} else {
				$name = "$name/$stylesheet";
			}
		}

		$theme_roots[$stylesheet] = str_replace( TRM_CONTENT_DIR, '', $theme_root );
		$trm_themes[$name] = array(
			'Name' => $name,
			'Title' => $title,
			'Description' => $description,
			'Author' => $author,
			...
			'Theme Root URI' => str_replace( TRM_CONTENT_DIR, content_url(), $theme_root ),
		);
	}
	...
	$theme_names = array_keys( $trm_themes );
	foreach ( (array) $theme_names as $theme_name ) {
		$trm_themes[$theme_name]['Parent Theme'] = '';
		if ( $trm_themes[$theme_name]['Stylesheet'] != $trm_themes[$theme_name]['Template'] ) {
			foreach ( (array) $theme_names as $parent_theme_name ) {
				if ( ($trm_themes[$parent_theme_name]['Stylesheet'] == $trm_themes[$parent_theme_name]['Template']) && ($trm_themes[$parent_theme_name]['Template'] == $trm_themes[$theme_name]['Template']) ) {
					$trm_themes[$theme_name]['Parent Theme'] = $trm_themes[$parent_theme_name]['Name'];
					break;
				}
			}
		}
...
function get_theme_roots() {
	global $trm_theme_directories;

	if ( count($trm_theme_directories) <= 1 )
		return '/858483';
 ...
 * @since 2.9.0
 *
 * @param string $directory Either the full filesystem path to a theme folder or a folder within TRM_CONTENT_DIR
 * @return bool
 */
function register_theme_directory( $directory) {
	...
	if ( !file_exists( $directory ) )
		...
		$registered_directory = TRM_CONTENT_DIR . '/' . $directory;
	else
	...
	$trm_theme_directories[] = $registered_directory;
...
function search_theme_directories() {
	global $trm_theme_directories, $trm_broken_themes;
	if ( empty( $trm_theme_directories ) )
		return false;

	$theme_files = array();
	$trm_broken_themes = array();

	/* Loop the registered theme directories and extract all themes */
	foreach ( (array) $trm_theme_directories as $theme_root ) {
		$theme_loc = $theme_root;

		/* We don't want to replace all forward slashes, see Trac #4541 */
		if ( '/' != TRM_CONTENT_DIR )
			$theme_loc = str_replace(TRM_CONTENT_DIR, '', $theme_root);
			...
			if ( is_dir($theme_root . '/' . $theme_dir) && is_readable($theme_root . '/' . $theme_dir) ) {
				...
				if ( !$found_stylesheet ) { // look for themes in that dir
					...
					if ( !$found_subdir_themes )
						$trm_broken_themes[$theme_dir] = array('Name' => $theme_dir, 'Title' => $theme_dir, 'Description' => __('Stylesheet is missing.'));
				}
			}
		}
		@closedir( $themes_dir );
...
function get_theme_root( $stylesheet_or_template = false ) {
	if ( $stylesheet_or_template ) {
		if ( $theme_root = get_raw_theme_root($stylesheet_or_template) )
			$theme_root = TRM_CONTENT_DIR . $theme_root;
		else
			$theme_root = TRM_CONTENT_DIR . '/858483';
	} else {
		$theme_root = TRM_CONTENT_DIR . '/858483';
	}
 ...
 *
 * Works by first retrieving the current tag name, for example 'tag-trendr.php' and then
...
function get_page_template() {
	...
	$template = get_post_meta($id, '_trm_page_template', true);
...
function get_comments_popup_template() {
	...
	if ('' == $template)
		$template = ABSPATH . TRMINC . '/theme-compat/comments-popup.php';
...
/**
 * Require the template file with Trnder environment.
 *
 * The globals are set up for the template file to ensure that the Trnder
...
function load_template( $_template_file, $require_once = true ) {
	global $posts, $post, $trm_did_header, $trm_did_template_redirect, $trm_query, $trm_rewrite, $trmdb, $trm_version, $trm, $id, $comment, $user_ID;

	if ( is_array( $trm_query->query_vars ) )
		extract( $trm_query->query_vars, EXTR_SKIP );
...
function preview_theme_ob_filter_callback( $matches ) {
	...
	if (
		( false !== strpos($matches[3], '/Backend-WeaprEcqaKejUbRq-trendr/') )
	||
 ...
 *
 * @since 1.5.0
 * @see TRM_DEFAULT_THEME
 *
 * @return bool
 */
function validate_current_theme() {
	// Don't validate during an install/upgrade.
	if ( defined('TRM_INSTALLING') || !apply_filters( 'validate_current_theme', true ) )
		return true;

	if ( get_template() != TRM_DEFAULT_THEME && !file_exists(get_template_directory() . '/index.php') ) {
		switch_theme( TRM_DEFAULT_THEME, TRM_DEFAULT_THEME );
		return false;
	}

	if ( get_stylesheet() != TRM_DEFAULT_THEME && !file_exists(get_template_directory() . '/style.css') ) {
		switch_theme( TRM_DEFAULT_THEME, TRM_DEFAULT_THEME );
		return false;
	}
...
function get_random_header_image() {
	...
	elseif ( ! empty( $_trm_default_headers ) ) {
		if ( 'random-default-image' == $header_image_mod ) {
			$headers = $_trm_default_headers;
		} else {
...
function get_uploaded_header_images() {
	...
	// @todo caching
	$headers = get_posts( array( 'post_type' => 'attachment', 'meta_key' => '_trm_attachment_is_custom_header', 'meta_value' => get_option('stylesheet'), 'orderby' => 'none', 'nopaging' => true ) );
 ...
 * content for the 'trm_head' action. The parameter $admin_header_callback
 ...
 *
 * @param callback $header_callback Call on 'trm_head' action.
...
function add_custom_image_header( $header_callback, $admin_header_callback, $admin_image_div_callback = '' ) {
	if ( ! empty( $header_callback ) )
		add_action('trm_head', $header_callback);
	...
	require_once( ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/custom-header.php' );
...
function remove_custom_image_header() {
	...
	remove_action( 'trm_head', $callback[0]['callback'] );
...
function register_default_headers( $headers ) {
	...
	$_trm_default_headers = array_merge( (array) $_trm_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_trm_default_headers[ $header ] ) ) {
		unset( $_trm_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function add_custom_background( $header_callback = '', $admin_header_callback = '', $admin_image_div_callback = '' ) {
	...
	if ( ! is_admin() )
		return;
	require_once( ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/custom-background.php' );
...
function add_theme_support( $feature ) {
	global $_trm_theme_features;

	if ( func_num_args() == 1 )
		$_trm_theme_features[$feature] = true;
	else
		$_trm_theme_features[$feature] = array_slice( func_get_args(), 1 );

	if ( $feature == 'post-formats' && is_array( $_trm_theme_features[$feature][0] ) )
		$_trm_theme_features[$feature][0] = array_intersect( $_trm_theme_features[$feature][0], array_keys( get_post_format_slugs() ) );
}
...
function get_theme_support( $feature ) {
	global $_trm_theme_features;
	if ( !isset( $_trm_theme_features[$feature] ) )
		return false;
	else
		return $_trm_theme_features[$feature];
}
...
function _remove_theme_support( $feature ) {
	...
	if ( ! isset( $_trm_theme_features[$feature] ) )
		return false;
	unset( $_trm_theme_features[$feature] );
	return true;
}
...
function current_theme_supports( $feature ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			...
			if ( true === $_trm_theme_features[$feature] )  // Registered for all types
				return true;
			$content_type = $args[0];
			if ( in_array($content_type, $_trm_theme_features[$feature][0]) )
				return true;
			else
				return false;
			break;
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = trm_get_attachment_url( $id );
