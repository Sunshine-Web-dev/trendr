...
function get_the_category_list( $separator = '', $parents='', $post_id = false ) {
	...
	$rel = ( is_object( $trm_rewrite ) && $trm_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of Trnder, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the Trnder Loop.
 ...
 */
function trm_dropdown_categories( $args = '' ) {
	$defaults = array(
	...
	$output = apply_filters( 'trm_dropdown_cats', $output );
 ...
 * @return string HTML content only if 'echo' argument is 0.
 */
function trm_list_categories( $args = '' ) {
	$defaults = array(
	...
	$output = apply_filters( 'trm_list_categories', $output, $args );
...
function trm_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_trm_error( $tags ) )
		return;
	...
	$return = trm_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args

	$return = apply_filters( 'trm_tag_cloud', $return, $args );
 ...
 * @return string
 */
function trm_generate_tag_cloud( $tags, $args = '' ) {
	global $trm_rewrite;
	$defaults = array(
	...
	if ( $filter )
		return apply_filters( 'trm_generate_tag_cloud', $return, $tags, $args );
	else
		return $return;
}
 ...
 * @access private
 */
function _trm_tag_cloud_name_sort_cb( $a, $b ) {
 ...
 * @access private
 */
function _trm_tag_cloud_count_sort_cb( $a, $b ) {
...
function term_description( $term = 0, $taxonomy = 'post_tag' ) {
	...
	return is_trm_error( $description ) ? '' : $description;
}
...
function get_the_terms( $id = 0, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = trm_get_object_terms( $id, $taxonomy );
		trm_cache_add($id, $terms, $taxonomy . '_relationships');
	}
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return null|bool False on Trnder error. Returns null when displaying.
 */
function the_terms( $id = 0, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_trm_error( $term_list ) )
		return false;
 ...
 *
 * Prior to v2.7 of Trnder, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the Trnder Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 2.6.0
