...
function image_constrain_size_for_editor($width, $height, $size = 'medium') {
	global $content_width, $_trm_additional_image_sizes;
	...
	} elseif ( isset( $_trm_additional_image_sizes ) && count( $_trm_additional_image_sizes ) && in_array( $size, array_keys( $_trm_additional_image_sizes ) ) ) {
		$max_width = intval( $_trm_additional_image_sizes[$size]['width'] );
		$max_height = intval( $_trm_additional_image_sizes[$size]['height'] );
	...
	return trm_constrain_dimensions( $width, $height, $max_width, $max_height );
}
...
function image_downsize($id, $size = 'medium') {
	...
	if ( !trm_attachment_is_image($id) )
		return false;

	$img_url = trm_get_attachment_url($id);
	$meta = trm_get_attachment_metadata($id);
	$width = $height = 0;
	$is_intermediate = false;
	$img_url_basename = trm_basename($img_url);
	...
	elseif ( $size == 'thumbnail' ) {
		...
		if ( ($thumb_file = trm_get_attachment_thumb_file($id)) && $info = getimagesize($thumb_file) ) {
			$img_url = str_replace($img_url_basename, trm_basename($thumb_file), $img_url);
			$width = $info[0];
			$height = $info[1];
			$is_intermediate = true;
		}
...
function add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
	global $_trm_additional_image_sizes;
	$_trm_additional_image_sizes[$name] = array( 'width' => absint( $width ), 'height' => absint( $height ), 'crop' => (bool) $crop );
}
 ...
 * parameters are: what Trnder will use for the class, the Attachment ID,
...
function get_image_tag($id, $alt, $title, $align, $size='medium') {
	...
	$class = 'align' . esc_attr($align) .' size-' . esc_attr($size) . ' trm-image-' . $id;
 ...
 */
function trm_constrain_dimensions( $current_width, $current_height, $max_width=0, $max_height=0 ) {
...
function image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = trm_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}
 ...
 * support. This is not the fault of Trnder (where functionality is
...
function image_make_intermediate_size($file, $width, $height, $crop=false) {
	if ( $width || $height ) {
		...
		if ( !is_trm_error($resized_file) && $resized_file && $info = getimagesize($resized_file) ) {
			$resized_file = apply_filters('image_make_intermediate_size', $resized_file);
			return array(
				'file' => trm_basename( $resized_file ),
				'width' => $info[0],
				'height' => $info[1],
			);
		}
...
function image_get_intermediate_size($post_id, $size='thumbnail') {
	if ( !is_array( $imagedata = trm_get_attachment_metadata( $post_id ) ) )
		return false;
	...
	if ( empty($data['path']) && !empty($data['file']) ) {
		$file_url = trm_get_attachment_url($post_id);
...
function get_intermediate_image_sizes() {
	...
	if ( isset( $_trm_additional_image_sizes ) && count( $_trm_additional_image_sizes ) )
		$image_sizes = array_merge( $image_sizes, array_keys( $_trm_additional_image_sizes ) );
 ...
 */
function trm_get_attachment_image_src($attachment_id, $size='thumbnail', $icon = false) {
	...
	if ( $icon && $src = trm_mime_type_icon($attachment_id) ) {
		$icon_dir = apply_filters( 'icon_dir', ABSPATH . TRMINC . '/images/crystal' );
		$src_file = $icon_dir . '/' . trm_basename($src);
 ...
 *
 * @see add_image_size()
 * @uses apply_filters() Calls 'trm_get_attachment_image_attributes' hook on attributes array
 * @uses trm_get_attachment_image_src() Gets attachment file URL and dimensions
 * @since 2.5.0
 *
 * @param int $attachment_id Image attachment ID.
 * @param string $size Optional, default is 'thumbnail'.
 ...
 */
function trm_get_attachment_image($attachment_id, $size = 'thumbnail', $icon = false, $attr = '') {

	$html = '';
	$image = trm_get_attachment_image_src($attachment_id, $size, $icon);
	if ( $image ) {
		...
		$default_attr = array(
			...
			'alt'	=> trim(strip_tags( get_post_meta($attachment_id, '_trm_attachment_image_alt', true) )), // Use Alt field first
		...
		$attr = trm_parse_args($attr, $default_attr);
		$attr = apply_filters( 'trm_get_attachment_image_attributes', $attr, $attachment );
...
/**
 * Adds a 'trm-post-image' class to post thumbnail thumbnails
 ...
 * @return array
 */
function _trm_post_thumbnail_class_filter( $attr ) {
	$attr['class'] .= ' trm-post-image';
	return $attr;
}

/**
 * Adds _trm_post_thumbnail_class_filter to the trm_get_attachment_image_attributes filter
 *
 * @since 2.9.0
 */
function _trm_post_thumbnail_class_filter_add( $attr ) {
	add_filter( 'trm_get_attachment_image_attributes', '_trm_post_thumbnail_class_filter' );
}

/**
 * Removes _trm_post_thumbnail_class_filter from the trm_get_attachment_image_attributes filter
 *
 * @since 2.9.0
 */
function _trm_post_thumbnail_class_filter_remove( $attr ) {
	remove_filter( 'trm_get_attachment_image_attributes', '_trm_post_thumbnail_class_filter' );
}

add_shortcode('trm_caption', 'img_caption_shortcode');
...
function img_caption_shortcode($attr, $content = null) {
	...
	// Allow plugins/858483 to override the default caption template.
	...
	return '<div ' . $id . 'class="trm-caption ' . esc_attr($align) . '" style="width: ' . (10 + (int) $width) . 'px">'
	. do_shortcode( $content ) . '<p class="trm-caption-text">' . $caption . '</p></div>';
}
...
function gallery_shortcode($attr) {
	global $post, $trm_locale;

	static $instance = 0;
	$instance++;

	// Allow plugins/858483 to override the default gallery template.
	...
	if ( is_feed() ) {
		...
		foreach ( $attachments as $att_id => $attachment )
			$output .= trm_get_attachment_link($att_id, $size, true) . "\n";
		return $output;
	}
	...
	if ( apply_filters( 'use_default_gallery_style', true ) )
		...
		<!-- see gallery_shortcode() in Source-zACHAvU6As28quwr-trendr/media.php -->";
	...
	foreach ( $attachments as $id => $attachment ) {
		$link = isset($attr['link']) && 'file' == $attr['link'] ? trm_get_attachment_link($id, $size, false, false) : trm_get_attachment_link($id, $size, true, false);
		...
		if ( $captiontag && trim($attachment->post_excerpt) ) {
			$output .= "
				<{$captiontag} class='trm-caption-text gallery-caption'>
				" . trmtexturize($attachment->post_excerpt) . "
				</{$captiontag}>";
		}
		$output .= "</{$itemtag}>";
...
function adjacent_image_link($prev = true, $size = 'thumbnail', $text = false) {
	...
	if ( isset($attachments[$k]) )
		echo trm_get_attachment_link($attachments[$k]->ID, $size, true, false, $text);
}
