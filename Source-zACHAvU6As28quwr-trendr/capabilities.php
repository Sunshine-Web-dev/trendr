...
class TRM_Roles {
	 ...
	 *
	 * The role key is set to the current prefix for the $trmdb object with
	 * 'user_roles' appended. If the $trm_user_roles global is set, then it will
	 * be used and the role option will not be updated or used.
	 *
	 * @since 2.1.0
	 * @access protected
	 * @uses $trmdb Used to get the database prefix.
	 * @global array $trm_user_roles Used to set the 'roles' property value.
	 */
	function _init () {
		global $trmdb, $trm_user_roles;
		$this->role_key = $trmdb->prefix . 'user_roles';
		if ( ! empty( $trm_user_roles ) ) {
			$this->roles = $trm_user_roles;
			$this->use_db = false;
		} else {
		...
		foreach ( (array) $this->roles as $role => $data ) {
			$this->role_objects[$role] = new TRM_Role( $role, $this->roles[$role]['capabilities'] );
	 ...
	 * @return null|TRM_Role TRM_Role object if role is added, null if already exists.
	 */
	function add_role( $role, $display_name, $capabilities = array() ) {
		...
		$this->role_objects[$role] = new TRM_Role( $role, $capabilities );
	 ...
	 *
	 * @param string $role Role name.
	 * @return object|null Null, if role does not exist. TRM_Role object, if found.
	 */
...
class TRM_Role {
	 ...
	 *
	 * @see TRM_Roles::add_cap() Method uses implementation for role.
	 * @since 2.0.0
	 * @access public
	 *
	 * @param string $cap Capability name.
	...
	function add_cap( $cap, $grant = true ) {
		...
		$trm_roles->add_cap( $this->name, $cap, $grant );
	}

	/**
	 * Remove capability from role.
	 *
	 * This is a container for {@link TRM_Roles::remove_cap()} to remove the
	 * capability from the role. That is to say, that {@link
	 * TRM_Roles::remove_cap()} implements the functionality, but it also makes
	...
	function remove_cap( $cap ) {
		...
		$trm_roles->remove_cap( $this->name, $cap );
	}
...
class TRM_User {
	 ...
	 * properties of the object. Calls {@link TRM_User::_init_caps()} after
	...
	function _init_caps( $cap_key = '' ) {
		global $trmdb;
		if ( empty($cap_key) )
			$this->cap_key = $trmdb->prefix . 'capabilities';
		else
	...
	function get_role_caps() {
		...
		if ( is_array( $this->caps ) )
			$this->roles = array_filter( array_keys( $this->caps ), array( &$trm_roles, 'is_role' ) );

		//Build $allcaps from role caps, overlay user's $caps
		$this->allcaps = array();
		foreach ( (array) $this->roles as $role ) {
			$the_role = $trm_roles->get_role( $role );
	...
	function update_user_level_from_caps() {
		...
		update_user_meta( $this->ID, $trmdb->prefix . 'user_level', $this->user_level );
	}
	...
	function remove_all_caps() {
		...
		delete_user_meta( $this->ID, $trmdb->prefix . 'user_level' );
		$this->get_role_caps();
	}
	...
	function for_blog( $blog_id = '' ) {
		global $trmdb;
		if ( ! empty( $blog_id ) )
			$cap_key = $trmdb->get_blog_prefix( $blog_id ) . 'capabilities';
		else
			$cap_key = '';
	...
	case 'delete_page':
		...
		if ( is_object( $post_author_data ) && $user_id == $post_author_data->ID ) {
			...
			if ( 'publish' == $post->post_status ) {
				...
				if ('publish' == get_post_meta($post->ID, '_trm_trash_meta_status', true) )
					$caps[] = $post_type->cap->delete_published_posts;
			} else {
...
function current_user_can_for_blog( $blog_id, $capability ) {
	...
	$user = new TRM_User( $current_user->id) ;

	// Set the blog id.  @todo add blog id arg to TRM_User constructor?
...
function author_can( $post, $capability ) {
	...
	$author = new TRM_User( $post->post_author );
 ...
 *
 * @see TRM_Roles::get_role() Uses method to retrieve role object.
 * @since 2.0.0
 *
 * @param string $role Role name.
 * @return object
...
function get_role( $role ) {
	...
	return $trm_roles->get_role( $role );
}

/**
 * Add role, if it does not exist.
 *
 * @see TRM_Roles::add_role() Uses method to add role.
 * @since 2.0.0
 *
 * @param string $role Role name.
...
function add_role( $role, $display_name, $capabilities = array() ) {
	...
	return $trm_roles->add_role( $role, $display_name, $capabilities );
}

/**
 * Remove role, if it exists.
 *
 * @see TRM_Roles::remove_role() Uses method to remove role.
 * @since 2.0.0
 *
 * @param string $role Role name.
 * @return null
...
function remove_role( $role ) {
	...
	return $trm_roles->remove_role( $role );
}
