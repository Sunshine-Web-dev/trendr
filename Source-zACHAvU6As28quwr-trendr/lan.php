 ...
 *
 * If the locale is not set already, then the TRMLANG constant is used if it is
...
function get_locale() {
	...
	// TRMLANG is defined in trm-setup.
	if ( defined( 'TRMLANG' ) )
		$locale = TRMLANG;

	// If multisite, check options.
	if ( is_multisite() && !defined('TRM_INSTALLING') ) {
		$ms_locale = get_option('TRMLANG');
		if ( $ms_locale === false )
			$ms_locale = get_site_option('TRMLANG');
 ...
 *
 * If the domain is not set in the $lan list, then a comparison will be made
 ...
 *
 * @since 2.8.0
 * @uses $lan Gets list of domain translated string (gettext_reader) objects
 ...
 *
 * On success, the .mo file will be placed in the $lan global by $domain
 * and will be a MO object.
 *
 * @since 1.5.0
 * @uses $lan Gets list of domain translated string objects
 *
 ...
 *
 * Loads the .mo file in TRM_LANG_DIR constant path from Trnder root. The
...
function load_default_textdomain() {
	...
	load_textdomain( 'default', TRM_LANG_DIR . "/$locale.mo" );

	if ( is_multisite() || ( defined( 'TRM_NETWORK_ADMIN_PAGE' ) && TRM_NETWORK_ADMIN_PAGE ) ) {
		load_textdomain( 'default', TRM_LANG_DIR . "/ms-$locale.mo" );
	}
}
 ...
 * @param string $plugin_rel_path Optional. Relative path to TRM_PLUGIN_DIR. This is the preferred argument to use. It takes precendence over $abs_rel_path
 */
function load_plugin_textdomain( $domain, $abs_rel_path = false, $plugin_rel_path = false ) {
	$locale = apply_filters( 'plugin_locale', get_locale(), $domain );

	if ( false !== $plugin_rel_path	) {
		$path = TRM_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
 ...
 *
 * @param string $domain Unique identifier for retrieving translated strings
 * @param strings $mu_plugin_rel_path Relative to TRMMU_PLUGIN_DIR directory in which
 * the MO file resides. Defaults is empty string.
 */
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	$locale = apply_filters( 'plugin_locale', get_locale(), $domain );
	$path = TRMMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
...
function &get_translations_for_domain( $domain ) {
	global $lan;
	if ( !isset( $lan[$domain] ) ) {
		$lan[$domain] = new NOOP_Translations;
	}
	return $lan[$domain];
}
...
/**
 * Get all available languages based on the presence of *.mo files in a given directory. The default directory is TRM_LANG_DIR.
 *
 * @since 3.0.0
 *
 * @param string $dir A directory in which to search for language files. The default directory is TRM_LANG_DIR.
 * @return array Array of language codes or an empty array if no languages are present.  Language codes are formed by stripping the .mo extension from the language file names.
 */
function get_available_languages( $dir = null ) {
	$languages = array();

	foreach( (array)glob( ( is_null( $dir) ? TRM_LANG_DIR : $dir ) . '/*.mo' ) as $lang_file ) {
