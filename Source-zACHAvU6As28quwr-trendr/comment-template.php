 ...
 *
 * These functions are meant to live inside of the Trnder loop.
 *
 * @package Trnder
 * @subpackage Template
 */
...
function get_comment_link( $comment = null, $args = array() ) {
	global $trm_rewrite, $in_comment_loop;
	...
	if ( $args['per_page'] ) {
		...
		if ( $trm_rewrite->using_permalinks() )
...
function get_trackback_url() {
	if ( '' != get_option('permalink_structure') ) {
		...
		$tb_url = get_option('siteurl') . '/trm-trackback.php?p=' . get_the_ID();
	}
...
function trackback_rdf( $deprecated = '' ) {
	...
	echo '<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
			...
			xmlns:trackback="http://madskills.com/public/xml/rss/858483/trackback/">
	...
	the_permalink();
	echo '"'."\n";
	echo '    dc:title="'.str_replace('--', '&#x2d;&#x2d;', trmtexturize(strip_tags(get_the_title()))).'"'."\n";
 ...
 */
function trm_comment_form_unfiltered_html_nonce() {
	global $post;
	...
	if ( current_user_can('unfiltered_html') )
		trm_nonce_field('unfiltered-html-comment_' . $post_id, '_trm_unfiltered_html_comment', false);
}
 ...
 *
 * Uses the Trnder database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the Trnder process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $trm_query, $withcomments, $post, $trmdb, $id, $comment, $user_login, $user_ID, $user_identity, $overridden_cpage;
	 ...
	 *
	 * @uses trm_get_current_commenter()
	 */
	...
	if ( $user_ID) {
		$comments = $trmdb->get_results($trmdb->prepare("SELECT * FROM $trmdb->comments WHERE comment_post_ID = %d AND (comment_approved = '1' OR ( user_id = %d AND comment_approved = '0' ) )  ORDER BY comment_date_gmt", $post->ID, $user_ID));
	} else if ( empty($comment_author) ) {
		$comments = get_comments( array('post_id' => $post->ID, 'status' => 'approve', 'order' => 'ASC') );
	} else {
		$comments = $trmdb->get_results($trmdb->prepare("SELECT * FROM $trmdb->comments WHERE comment_post_ID = %d AND ( comment_approved = '1' OR ( comment_author = %s AND comment_author_email = %s AND comment_approved = '0' ) ) ORDER BY comment_date_gmt", $post->ID, trm_specialchars_decode($comment_author,ENT_QUOTES), $comment_author_email));
	}

	// keep $comments for legacy's sake
	$trm_query->comments = apply_filters( 'comments_array', $comments, $post->ID );
	$comments =$trm_query->comments;
	$trm_query->comment_count = count($trm_query->comments);
	update_comment_cache($trm_query->comments);

	if ( $separate_comments ) {
		$trm_query->comments_by_type =separate_comments($comments);
		$comments_by_type =$trm_query->comments_by_type;
	}
	...
	elseif ( file_exists( TEMPLATEPATH . $file ) )
		...
		require( ABSPATH . TRMINC . '/theme-compat/comments.php');
}
 ...
 *
 * @since 0.71
 * @global string $trmcommentspopupfile The URL to use for the popup window
 * @global int $trmcommentsjavascript Whether to use JavaScript. Set when function is called
 *
...
function comments_popup_script($width=400, $height=400, $file='') {
	global $trmcommentspopupfile, $trmcommentsjavascript;

	if (empty ($file)) {
		$trmcommentspopupfile = '';  // Use the index.
	} else {
		$trmcommentspopupfile = $file;
	}

	$trmcommentsjavascript = 1;
	$javascript = "<script type='text/javascript'>\nfunction trmopen (macagna) {\n    window.open(macagna, '_blank', 'width=$width,height=$height,scrollbars=yes,status=yes');\n}\n</script>\n";
	echo $javascript;
}
 ...
 *
 * @since 0.71
 * @uses $trmcommentspopupfile
 * @uses $trmcommentsjavascript
 * @uses $post
 *
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo '<a href="';
	if ( $trmcommentsjavascript ) {
		if ( empty( $trmcommentspopupfile ) )
			$home = home_url();
		else
			$home = get_option('siteurl');
		echo $home . '/' . $trmcommentspopupfile . '?comments_popup=' . $id;
		echo '" onclick="trmopen(this.href); return false"';
...
function get_comment_reply_link($args = array(), $comment = null, $post = null) {
	...
	if ( get_option('comment_registration') && !$user_ID )
		$link = '<a rel="nofollow" class="comment-reply-login" href="' . esc_url( trm_login_url( get_permalink() ) ) . '">' . $login_text . '</a>';
	else
...
function get_post_reply_link($args = array(), $post = null) {
	...
	if ( get_option('comment_registration') && !$user_ID ) {
		$link = '<a rel="nofollow" href="' . trm_login_url( get_permalink() ) . '">' . $login_text . '</a>';
	} else {
...
?>
		...
		<?php if ($args['portrait_size'] != 0) echo get_portrait( $comment, $args['portrait_size'] ); ?>
 ...
 */
function trm_list_comments($args = array(), $comments = null ) {
	global $trm_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	...
	$defaults = array('walker' => null, 'max_depth' => '', 'style' => 'ul', 'callback' => null, 'end-callback' => null, 'type' => 'all',
		'page' => '', 'per_page' => '', 'portrait_size' => 32, 'reverse_top_level' => null, 'reverse_children' => '');
	...
	if ( null !== $comments ) {
		...
		if ( empty($trm_query->comments) )
			return;
		if ( 'all' != $r['type'] ) {
			if ( empty($trm_query->comments_by_type) )
				$trm_query->comments_by_type =separate_comments($trm_query->comments);
			if ( empty($trm_query->comments_by_type[$r['type']]) )
				return;
			$_comments = $trm_query->comments_by_type[$r['type']];
		} else {
			$_comments = $trm_query->comments;
		}
	}
	...
	$trm_query->max_num_comment_pages = $walker->max_pages;
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), trm_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), admin_url( 'profile.php' ), $user_identity, trm_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
	...
	$args = trm_parse_args( $args, apply_filters( 'comment_form_defaults', $defaults ) );
	...
	?>
		<?php if ( comments_open() ) : ?>
			...
			<div id="respond">
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/trm-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
