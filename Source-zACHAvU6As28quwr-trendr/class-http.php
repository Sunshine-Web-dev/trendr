 ...
 *
 * Standardizes the HTTP requests for Trnder. Handles cookies, gzip encoding and decoding, chunk
 * decoding, if HTTP 1.1 and various other difficult HTTP protocol implementations.
 *
 * @link http://trac./ticket/4779 HTTP API Proposal
 *
 * @package Trnder
 * @subpackage HTTP
 * @since 2.7.0
...
/**
 * Trnder HTTP Class for managing HTTP Transports and making HTTP requests.
 *
...
class TRM_Http {
	 ...
	 * 'Trnder/TRM_Version', where TRM_Version is the value from $trm_version.
	 *
	...
	function request( $url, $args = array() ) {
		...
		$defaults = array(
			...
			'user-agent' => apply_filters( 'http_headers_useragent', 'Trnder/' . $trm_version . '; ' . get_bloginfo( 'url' )  ),
			'blocking' => true,
			'headers' => array(),
			'cookies' => array(),
			'body' => null,
		...
		// Determine if this request is to OUR install of Trnder
		...
		if ( ! is_array( $r['headers'] ) ) {
			$processedHeaders = TRM_Http::processHeaders( $r['headers'] );
		...
		if ( TRM_Http_Encoding::is_available() )
			$r['headers']['Accept-Encoding'] = TRM_Http_Encoding::accept_encoding();
	...
	function processHeaders($headers) {
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( !empty( $value ) ) {
				...
				if ( 'set-cookie' == $key )
					$cookies[] = new TRM_Http_Cookie( $value );
			}
		}
	 ...
	 *
	 * If it's found, then it's assumed to contain TRM_Http_Cookie objects, which are each parsed
	 ...
	 * prevent plugins from working and core functionality, if you don't include api..
	 *
	 * You block external URL requests by defining TRM_HTTP_BLOCK_EXTERNAL as true in your trm-setup.php
	 ...
	 *
	 * @since 2.8.0
	 * @link http://core.trac./ticket/8927 Allow preventing external requests.
	 * @link http://core.trac./ticket/14636 Allow wildcard domains in TRM_ACCESSIBLE_HOSTS
	 *
	...
	function block_request($uri) {
		 ...
		 * Trnder does not do any checking currently for non-proxy requests, so it is keeps with
...
class TRM_Http_Fsockopen {
	...
	function request($url, $args = array()) {
		...
		if ( false === $handle )
			return new TRM_Error('http_request_failed', $iError . ': ' . $strError);
		...
		$arrHeaders = TRM_Http::processHeaders( $process['headers'] );
...
class TRM_Http_Streams {
	...
	function request($url, $args = array()) {
		...
		if ( false === $arrURL )
			return new TRM_Error('http_request_failed', sprintf(__('Malformed URL: %s'), $url));
		...
		if ( ! $handle )
			return new TRM_Error('http_request_failed', sprintf(__('Could not open handle for fopen() to %s'), $url));
		...
		if ( isset( $meta['wrapper_data']['headers'] ) )
			$processedHeaders = TRM_Http::processHeaders($meta['wrapper_data']['headers']);
		else
			$processedHeaders = TRM_Http::processHeaders($meta['wrapper_data']);
		...
		if ( ! empty( $strResponse ) && isset( $processedHeaders['headers']['transfer-encoding'] ) && 'chunked' == $processedHeaders['headers']['transfer-encoding'] )
			$strResponse = TRM_Http::chunkTransferDecode($strResponse);

		if ( true === $r['decompress'] && true === TRM_Http_Encoding::should_decode($processedHeaders['headers']) )
			$strResponse = TRM_Http_Encoding::decompress( $strResponse );
...
class TRM_Http_Curl {
	...
	function request($url, $args = array()) {
		...
		$theBody = '';
		$theHeaders = TRM_Http::processHeaders( $this->headers );
...
/**
 * Adds Proxy support to the Trnder HTTP API.
 *
 * There are caveats to proxy support. It requires that defines be made in the trm-setup.php file to
 ...
 * through the proxy. The list should be presented in a comma separated list, wildcards using * are supported, eg. *.</li>
 * </ol>
 *
 ...
 * define('TRM_PROXY_BYPASS_HOSTS', 'localhost, www.example.com, *.');
 * </code>
 *
 * @link http://core.trac./ticket/4011 Proxy support ticket in Trnder.
 * @link http://core.trac./ticket/14636 Allow wildcard domains in TRM_PROXY_BYPASS_HOSTS
 * @since 2.8
 */
 ...
 *
 * @todo The Trnder convention is to use underscores instead of camelCase for function and method
...
class TRM_Http_Cookie {
	...
	/**
	 * Retrieve cookie header for usage in the rest of the Trnder HTTP API.
	 *
	 * @access public
	 * @since 2.8.0
	 *
