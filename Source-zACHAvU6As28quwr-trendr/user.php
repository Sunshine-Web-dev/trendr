 ...
 * @return object Either TRM_Error on failure, or TRM_User on success.
 */
function trm_signon( $credentials = '', $secure_cookie = '' ) {
	...
	// TODO do we deprecate the trm_authentication action?
	do_action_ref_array('trm_authenticate', array(&$credentials['user_login'], &$credentials['user_password']));
	...
	global $auth_secure_cookie; // XXX ugly hack to pass this to trm_authenticate_cookie
	$auth_secure_cookie = $secure_cookie;

	add_filter('authenticate', 'trm_authenticate_cookie', 30, 3);

	$user = trm_authenticate($credentials['user_login'], $credentials['user_password']);
	...
	trm_set_auth_cookie($user->ID, $credentials['remember'], $secure_cookie);
	do_action('trm_login', $credentials['user_login']);
	return $user;
}
 ...
 */
add_filter('authenticate', 'trm_authenticate_username_password', 20, 3);
function trm_authenticate_username_password($user, $username, $password) {
	if ( is_a($user, 'TRM_User') ) { return $user; }
	...
	if ( !$userdata )
		return new TRM_Error('invalid_username', sprintf(__('Incorrect username or password combination!'), site_url('enter.php?action=lostpassword', 'login')));

	if ( is_multisite() ) {
		// Is user marked as spam?
		if ( 1 == $userdata->spam)
			return new TRM_Error('invalid_username', __(' Your account has been marked as a spammer.'));

		// Is a user's blog marked as spam?
		if ( !is_super_admin( $userdata->ID ) && isset($userdata->primary_blog) ) {
			$details = get_blog_details( $userdata->primary_blog );
			if ( is_object( $details ) && $details->spam == 1 )
				return new TRM_Error('blog_suspended', __('Site Suspended.'));
		}
	}

	$userdata = apply_filters('trm_authenticate_user', $userdata, $password);
	if ( is_trm_error($userdata) )
		return $userdata;

	if ( !trm_check_password($password, $userdata->user_pass, $userdata->ID) )
		return new TRM_Error( 'incorrect_password', sprintf( __( ' Incorrect username or password combination!' ),
		$username, site_url( 'enter.php?action=lostpassword', 'login' ) ) );

	$user =  new TRM_User($userdata->ID);
	return $user;
}

/**
 * Authenticate the user using the Trnder auth cookie.
 */
function trm_authenticate_cookie($user, $username, $password) {
	if ( is_a($user, 'TRM_User') ) { return $user; }

	if ( empty($username) && empty($password) ) {
		$user_id = trm_validate_auth_cookie();
		if ( $user_id )
			return new TRM_User($user_id);
		...
		if ( !empty($_COOKIE[$auth_cookie]) )
			return new TRM_Error('expired_session', __('Please log in again.'));
...
function count_user_posts($userid) {
	...
	$count = $trmdb->get_var( "SELECT COUNT(*) FROM $trmdb->posts $where" );
...
function count_many_users_posts($users, $post_type = 'post' ) {
	...
	$result = $trmdb->get_results( "SELECT post_author, COUNT(*) FROM $trmdb->posts $where AND post_author IN ($userlist) GROUP BY post_author", ARRAY_N );
 ...
 *
 * @since 0.71
 * @todo xmlrpc only. Maybe move to trm-xm-lrpc.php.
 *
...
function user_pass_ok($user_login, $user_pass) {
	$user = trm_authenticate($user_login, $user_pass);
	if ( is_trm_error($user) )
		return false;
...
function get_user_option( $option, $user = 0, $deprecated = '' ) {
	...
	if ( isset( $user->{$trmdb->prefix . $key} ) ) // Blog specific
		$result = $user->{$trmdb->prefix . $key};
 ...
 * it will prepend the Trnder table prefix to the option name.
 *
...
/**
 * Trnder User Query class.
 *
 * @since 3.1.0
 */
class TRM_User_Query {
	 ...
	 * @return TRM_User_Query
	 */
	function __construct( $query = null ) {
		if ( !empty( $query ) ) {
			$this->query_vars = trm_parse_args( $query, array(
	...
	function prepare_query() {
		...
		if ( is_array( $qv['fields'] ) ) {
			...
			foreach ( $qv['fields'] as $field )
				$this->query_fields[] = $trmdb->users . '.' . esc_sql( $field );
			$this->query_fields = implode( ',', $this->query_fields );
		} elseif ( 'all' == $qv['fields'] ) {
			$this->query_fields = "$trmdb->users.*";
		} else {
			$this->query_fields = "$trmdb->users.ID";
		}

		if ( $this->query_vars['count_total'] )
			$this->query_fields = 'SQL_CALC_FOUND_ROWS ' . $this->query_fields;

		$this->query_from = "FROM $trmdb->users";
		...
		if ( in_array( $qv['orderby'], array('nicename', 'email', 'url', 'registered') ) ) {
			...
			) p ON ({$trmdb->users}.ID = p.post_author)
			";
			$orderby = 'post_count';
		...
		if ( $qv['number'] ) {
			if ( $qv['offset'] )
				$this->query_limit = $trmdb->prepare("LIMIT %d, %d", $qv['offset'], $qv['number']);
			else
				$this->query_limit = $trmdb->prepare("LIMIT %d", $qv['number']);
		}
		...
		if ( 'authors' == $qv['who'] && $blog_id ) {
			$qv['meta_key'] = $trmdb->get_blog_prefix( $blog_id ) . 'user_level';
			$qv['meta_value'] = 0;
			$qv['meta_compare'] = '!=';
		...
		if ( $blog_id && ( $role || is_multisite() ) ) {
			...
			$cap_meta_query['key'] = $trmdb->get_blog_prefix( $blog_id ) . 'capabilities';
		...
		$meta_query = new TRM_Meta_Query();
		$meta_query->parse_query_vars( $qv );

		if ( !empty( $meta_query->queries ) ) {
			$clauses = $meta_query->get_sql( 'user', $trmdb->users, 'ID', $this );
		...
		if ( !empty( $qv['include'] ) ) {
			$ids = implode( ',', trm_parse_id_list( $qv['include'] ) );
			$this->query_where .= " AND $trmdb->users.ID IN ($ids)";
		} elseif ( !empty($qv['exclude']) ) {
			$ids = implode( ',', trm_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $trmdb->users.ID NOT IN ($ids)";
		}
	...
	function query() {
		...
		if ( is_array( $this->query_vars['fields'] ) || 'all' == $this->query_vars['fields'] ) {
			$this->results = $trmdb->get_results("SELECT $this->query_fields $this->query_from $this->query_where $this->query_orderby $this->query_limit");
		} else {
			$this->results = $trmdb->get_col("SELECT $this->query_fields $this->query_from $this->query_where $this->query_orderby $this->query_limit");
		}

		if ( $this->query_vars['count_total'] )
			$this->total_users = $trmdb->get_var( apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()' ) );
		...
		if ( 'all_with_meta' == $this->query_vars['fields'] ) {
			...
			foreach ( $this->results as $userid )
				$r[ $userid ] = new TRM_User( $userid, '', $this->query_vars['blog_id'] );
 ...
 * @since 3.1.0
 * @uses $trmdb
 * @uses TRM_User_Query See for default arguments and information.
 *
...
function get_users( $args = array() ) {
	...
	$args = trm_parse_args( $args );
	$args['count_total'] = false;

	$user_search = new TRM_User_Query($args);
...
function get_blogs_of_user( $id, $all = false ) {
	...
	$blogs = trm_cache_get( 'blogs_of_user-' . $id, 'users' );

	// Try priming the new cache from the old cache
	if ( false === $blogs ) {
		$cache_suffix = $all ? '_all' : '_short';
		$blogs = trm_cache_get( 'blogs_of_user_' . $id . $cache_suffix, 'users' );
		if ( is_array( $blogs ) ) {
			$blogs = array_keys( $blogs );
			if ( $all )
				trm_cache_set( 'blogs_of_user-' . $id, $blogs, 'users' );
		}
	}
	...
	if ( false === $blogs ) {
		...
		$blogs = $match = array();
		$prefix_length = strlen($trmdb->base_prefix);
		foreach ( (array) $user as $key => $value ) {
			if ( $prefix_length && substr($key, 0, $prefix_length) != $trmdb->base_prefix )
				continue;
			if ( substr($key, -12, 12) != 'capabilities' )
				continue;
			if ( preg_match( '/^' . $trmdb->base_prefix . '((\d+)_)?capabilities$/', $key, $match ) ) {
				if ( count( $match ) > 2 )
					$blogs[] = (int) $match[ 2 ];
				else
					$blogs[] = 1;
...
function is_blog_user( $blog_id = 0 ) {
	...
	$cap_key = $trmdb->base_prefix . $blog_id . '_capabilities';
 ...
 * @since 3.0.0
 * @uses add_metadata()
 * @link http://codex./Function_Reference/add_user_meta
 *
 ...
 * @link http://codex./Function_Reference/delete_user_meta
 *
 * @param int $user_id user ID
 * @param string $meta_key Metadata name.
 ...
 * @since 3.0.0
 * @uses get_metadata()
 * @link http://codex./Function_Reference/get_user_meta
 *
 * @param int $user_id Post ID.
 ...
 * @link http://codex./Function_Reference/update_user_meta
 *
 * @param int $user_id Post ID.
 ...
 * Assumes role names are unique phrases.  Same assumption made by TRM_User_Query::prepare_query()
 * Using $strategy = 'time' this is CPU-intensive and should handle around 10^7 users.
 * Using $strategy = 'memory' this is memory-intensive and should handle around 10^5 users, but see TRM Bug #12257.
 *
 * @since 3.0.0
...
function count_users($strategy = 'time') {
	global $trmdb, $trm_roles;

	// Initialize
	$id = get_current_blog_id();
	$blog_prefix = $trmdb->get_blog_prefix($id);
	$result = array();
	...
	if ( 'time' == $strategy ) {
		...
		if ( ! isset( $trm_roles ) )
			$trm_roles = new TRM_Roles();

		$avail_roles = $trm_roles->get_names();
		...
		//$row = $trmdb->get_row( "SELECT $select_count, COUNT(*) FROM $trmdb->usermeta WHERE meta_key = '{$blog_prefix}capabilities'", ARRAY_N );
		...
		$users_of_blog = $trmdb->get_col( "SELECT meta_value FROM $trmdb->usermeta WHERE meta_key = '{$blog_prefix}capabilities'" );
 ...
 *
 * Used by trm_set_current_user() for back compat. Might be deprecated in the future.
 *
 * @since 2.0.4
 ...
 */
function trm_dropdown_users( $args = '' ) {
	$defaults = array(
	...
	$query_args = trm_array_slice_assoc( $r, array( 'blog_id', 'include', 'exclude', 'orderby', 'order', 'who' ) );
	...
	$output = apply_filters('trm_dropdown_users', $output);
...
function update_user_caches(&$user) {
	trm_cache_add($user->ID, $user, 'users');
	trm_cache_add($user->user_login, $user->ID, 'userlogins');
	trm_cache_add($user->user_email, $user->ID, 'useremail');
	trm_cache_add($user->user_nicename, $user->ID, 'userslugs');
}
...
function clean_user_cache($id) {
	$user = new TRM_User($id);

	trm_cache_delete($id, 'users');
	trm_cache_delete($user->user_login, 'userlogins');
	trm_cache_delete($user->user_email, 'useremail');
	trm_cache_delete($user->user_nicename, 'userslugs');
	trm_cache_delete('blogs_of_user-' . $id, 'users');
}
 ...
 *
 * @since 2.0.0
 * @uses $trmdb Trnder database layer.
 ...
 * @return int|TRM_Error The newly created user's ID or a TRM_Error object if the user could not be created.
 */
function trm_insert_user($userdata) {
	global $trmdb;
	...
	if ( !empty($ID) ) {
		...
		$update = false;
		// Hash the password
		$user_pass = trm_hash_password($user_pass);
	}
	...
	if ( empty($user_login) )
		return new TRM_Error('empty_user_login', __('Cannot create a user with an empty login name.') );

	if ( !$update && username_exists( $user_login ) )
		return new TRM_Error('existing_user_login', __('This username is already registered.') );
	...
	if ( !$update && ! defined( 'TRM_IMPORTING' ) && email_exists($user_email) )
		return new TRM_Error('existing_user_email', __('This email address is already registered.') );
...
//	if ( empty($show_admin_bar_front) )
	...
	$user_nicename_check = $trmdb->get_var( $trmdb->prepare("SELECT ID FROM $trmdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1" , $user_nicename, $user_login));

	if ( $user_nicename_check ) {
		$suffix = 2;
		while ($user_nicename_check) {
			$alt_user_nicename = $user_nicename . "-$suffix";
			$user_nicename_check = $trmdb->get_var( $trmdb->prepare("SELECT ID FROM $trmdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1" , $alt_user_nicename, $user_login));
			$suffix++;
		}
	...
	if ( $update ) {
		$trmdb->update( $trmdb->users, $data, compact( 'ID' ) );
		$user_id = (int) $ID;
	} else {
		$trmdb->insert( $trmdb->users, $data + compact( 'user_login' ) );
		$user_id = (int) $trmdb->insert_id;
	}
...
//	update_user_meta( $user_id, 'show_admin_bar_admin', $show_admin_bar_admin );
	...
	foreach ( _trm_get_user_contactmethods( $user ) as $method => $name ) {
		if ( empty($$method) )
			$$method = '';
	...
	trm_cache_delete($user_login, 'userlogins');
 ...
 *
 * @since 2.0.0
 * @see trm_insert_user() For what fields can be set in $userdata
 * @uses trm_insert_user() Used to update existing user or add new one if user doesn't exist already
 *
 * @param array $userdata An array of user data.
 * @return int The updated user's ID.
 */
function trm_update_user($userdata) {
	$ID = (int) $userdata['ID'];
	...
	if ( ! empty($userdata['user_pass']) ) {
		...
		$userdata['user_pass'] = trm_hash_password($userdata['user_pass']);
	}
	...
	$user_id = trm_insert_user($userdata);
 ...
 * detail creation of a user, use trm_insert_user() to specify more infomation.
 *
 * @since 2.0.0
 * @see trm_insert_user() More complete way to create a new user
 *
 ...
 */
function trm_create_user($username, $password, $email = '') {
	$user_login = esc_sql( $username );
	$user_email = esc_sql( $email    );
	$user_pass = $password;

	$userdata = compact('user_login', 'user_email', 'user_pass');
	return trm_insert_user($userdata);
}
 ...
 */
function _trm_get_user_contactmethods( $user = null ) {
