<?php
/**
 * XML-RPC protocol support for Trnder
 *
 * @package Trnder
 */

/**
 * Trnder XMLRPC server implementation.
 *
 * Implements compatability for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional Trnder API for managing comments, pages, posts,
 * options, etc.
 *
 * Since Trnder 2.6.0, Trnder XMLRPC server can be disabled in the
 ...
 * @since 1.5.0
 */
class trm_xmlrpc_server extends IXR_Server {
	 ...
	 * @since 1.5.0
	 *
	 * @return trm_xmlrpc_server
	 */
	function __construct() {
		$this->methods = array(
			// Trnder API
			'trm.getUsersBlogs'		=> 'this:trm_getUsersBlogs',
			'trm.getPage'			=> 'this:trm_getPage',
			'trm.getPages'			=> 'this:trm_getPages',
			'trm.newPage'			=> 'this:trm_newPage',
			'trm.deletePage'			=> 'this:trm_deletePage',
			'trm.editPage'			=> 'this:trm_editPage',
			'trm.getPageList'		=> 'this:trm_getPageList',
			'trm.getAuthors'			=> 'this:trm_getAuthors',
			'trm.getCategories'		=> 'this:mw_getCategories',		// Alias
			'trm.getTags'			=> 'this:trm_getTags',
			'trm.newCategory'		=> 'this:trm_newCategory',
			'trm.deleteCategory'		=> 'this:trm_deleteCategory',
			'trm.suggestCategories'	=> 'this:trm_suggestCategories',
			'trm.uploadFile'			=> 'this:mw_newMediaObject',	// Alias
			'trm.getCommentCount'	=> 'this:trm_getCommentCount',
			'trm.getPostStatusList'	=> 'this:trm_getPostStatusList',
			'trm.getPageStatusList'	=> 'this:trm_getPageStatusList',
			'trm.getPageTemplates'	=> 'this:trm_getPageTemplates',
			'trm.getOptions'			=> 'this:trm_getOptions',
			'trm.setOptions'			=> 'this:trm_setOptions',
			'trm.getComment'			=> 'this:trm_getComment',
			'trm.getComments'		=> 'this:trm_getComments',
			'trm.deleteComment'		=> 'this:trm_deleteComment',
			'trm.editComment'		=> 'this:trm_editComment',
			'trm.newComment'			=> 'this:trm_newComment',
			'trm.getCommentStatusList' => 'this:trm_getCommentStatusList',
			'trm.getMediaItem'		=> 'this:trm_getMediaItem',
			'trm.getMediaLibrary'	=> 'this:trm_getMediaLibrary',
			'trm.getPostFormats'     => 'this:trm_getPostFormats',
	 ...
	 * @deprecated use trm_xmlrpc_server::login
	 * @see trm_xmlrpc_server::login
	 */
	 ...
	 * @return mixed TRM_User object if authentication passed, false otherwise
	 */
	function login($username, $password) {
		...
		$user = trm_authenticate($username, $password);

		if (is_trm_error($user)) {
			$this->error = new IXR_Error(403, __('Bad login/pass combination.'));
			return false;
		}

		trm_set_current_user( $user->ID );
		return $user;
	}
	...
	function get_custom_fields($post_id) {
		...
		foreach ( (array) has_meta($post_id) as $meta ) {
			...
			if ( strpos($meta['meta_key'], '_trm_') === 0 ) {
				continue;
			}
	 ...
	 */
	function trm_getUsersBlogs( $args ) {
		global $current_site;
		// If this isn't on TRMMU then just use blogger_getUsersBlogs
		if ( !is_multisite() ) {
		...
		do_action( 'xmlrpc_call', 'trm.getUsersBlogs' );
		...
		foreach ( $blogs as $blog ) {
			...
			$struct[] = array(
				...
				'xmlrpc'		=> site_url( 'trm-xm-lrpc.php' )
			);
	...
	function trm_getPage($args) {
		...
		do_action('xmlrpc_call', 'trm.getPage');
		...
		if ( $page->ID && ($page->post_type == 'page') ) {
			...
			$categories = array();
			foreach ( trm_get_post_categories($page->ID) as $cat_id ) {
			...
			$page_template = get_post_meta( $page->ID, '_trm_page_template', true );
			...
			$page_struct = array(
				...
				'trm_slug'				=> $page->post_name,
				'trm_password'			=> $page->post_password,
				'trm_author'				=> $author->display_name,
				'trm_page_parent_id'		=> $page->post_parent,
				'trm_page_parent_title'	=> $parent_title,
				'trm_page_order'			=> $page->menu_order,
				'trm_author_id'			=> $author->ID,
				'trm_author_display_name'	=> $author->display_name,
				'date_created_gmt'		=> new IXR_Date($page_date_gmt),
				'custom_fields'			=> $this->get_custom_fields($page_id),
				'trm_page_template'		=> $page_template
			);
	...
	function trm_getPages($args) {
		...
		do_action('xmlrpc_call', 'trm.getPages');
		...
		if ( $num_pages >= 1 ) {
			...
			for ( $i = 0; $i < $num_pages; $i++ ) {
				$page = trm_xmlrpc_server::trm_getPage(array(
	 ...
	 * @since 2.2.0
	 *
	 * @param array $args Method parameters. See {@link trm_xmlrpc_server::mw_newPost()}
	 * @return unknown
	 */
	function trm_newPage($args) {
		...
		do_action('xmlrpc_call', 'trm.newPage');
	...
	function trm_deletePage($args) {
		...
		do_action('xmlrpc_call', 'trm.deletePage');

		// Get the current page based on the page_id and
		// make sure it is a page and not a post.
		$actual_page = trm_get_single_post($page_id, ARRAY_A);
		...
		$result = trm_delete_post($page_id);
		if ( !$result )
	...
	function trm_editPage($args) {
		...
		do_action('xmlrpc_call', 'trm.editPage');
	...
	function trm_getPageList($args) {
		...
		do_action('xmlrpc_call', 'trm.getPageList');

		// Get list of pages ids and titles
		$page_list = $trmdb->get_results("
			SELECT ID page_id,
	...
	function trm_getAuthors($args) {
		...
		do_action('xmlrpc_call', 'trm.getAuthors');
	...
	function trm_getTags( $args ) {
		...
		do_action( 'xmlrpc_call', 'trm.getKeywords' );
	...
	function trm_newCategory($args) {
		...
		do_action('xmlrpc_call', 'trm.newCategory');
		...
		$cat_id = trm_insert_category($new_category, true);
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return mixed See {@link trm_delete_term()} for return info.
	 */
	function trm_deleteCategory($args) {
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.deleteCategory');

		if ( !current_user_can('manage_categories') )
			return new IXR_Error( 401, __( 'Sorry, you do not have the right to delete a category.' ) );

		return trm_delete_term( $category_id, 'category' );
	}
	 ...
	 * @return array
	 */
	function trm_suggestCategories($args) {
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.suggestCategories');
	...
	function trm_getComment($args) {
		...
		do_action('xmlrpc_call', 'trm.getComment');
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return array. Contains a collection of comments. See {@link trm_xmlrpc_server::trm_getComment()} for a description of each item contents
	 */
	function trm_getComments($args) {
		$raw_args = $args;
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.getComments');
    ...
    // FIXME: we already have the comments, why query them again?
		for ( $i = 0; $i < $num_comments; $i++ ) {
			$comment = trm_xmlrpc_server::trm_getComment(array(
	 ...
	 * See {@link trm_delete_comment()} for more information on
	 * this behavior.
	 *
	 * @since 2.7.0
	 *
	 ...
	 *  - password
	 *  - comment_id
	 * @return mixed {@link trm_delete_comment()}
	 */
	function trm_deleteComment($args) {
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.deleteComment');

		if ( ! get_comment($comment_ID) )
			return new IXR_Error( 404, __( 'Invalid comment ID.' ) );

		return trm_delete_comment($comment_ID);
	}
	...
	function trm_editComment($args) {
		...
		do_action('xmlrpc_call', 'trm.editComment');
		...
		$result = trm_update_comment($comment);
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return mixed {@link trm_new_comment()}
	 */
	function trm_newComment($args) {
		global $trmdb;
		...
		if ( $logged_in ) {
			$comment['comment_author'] = $trmdb->escape( $user->display_name );
			$comment['comment_author_email'] = $trmdb->escape( $user->user_email );
			$comment['comment_author_url'] = $trmdb->escape( $user->user_url );
		...
		do_action('xmlrpc_call', 'trm.newComment');

		return trm_new_comment($comment);
	}
	 ...
	 * @return array
	 */
	function trm_getCommentStatusList($args) {
		$this->escape( $args );
		...
		do_action('xmlrpc_call', 'trm.getCommentStatusList');
	 ...
	 * @return array
	 */
	function trm_getCommentCount( $args ) {
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.getCommentCount');

		$count = trm_count_comments( $post_id );
		return array(
	 ...
	 * @return array
	 */
	function trm_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		do_action('xmlrpc_call', 'trm.getPostStatusList');
	 ...
	 * @return array
	 */
	function trm_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		do_action('xmlrpc_call', 'trm.getPageStatusList');
	 ...
	 * @return array
	 */
	function trm_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 *  - 'metadata'
	 */
	function trm_getMediaItem($args) {
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.getMediaItem');
		...
		$link = trm_get_attachment_url($attachment->ID);
		$thumbnail_link = trm_get_attachment_thumb_url($attachment->ID);
		...
		$attachment_struct = array(
			...
			'metadata'				=> trm_get_attachment_metadata($attachment->ID),
		);
	 ...
	 *  - password
	 *  - filter
	 * @return array. Contains a collection of media items. See {@link trm_xmlrpc_server::trm_getMediaItem()} for a description of each item contents
	 */
	function trm_getMediaLibrary($args) {
		$raw_args = $args;
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'trm.getMediaLibrary');
		...
		foreach ($attachments as $attachment )
			$attachments_struct[] = $this->trm_getMediaItem( array( $raw_args[0], $raw_args[1], $raw_args[2], $attachment->ID ) );
	...
	/**
	  ...
	  * @return array
	  */
	function trm_getPostFormats( $args ) {
		$this->escape( $args );
		...
		do_action( 'xmlrpc_call', 'trm.getPostFormats' );
	...
	function _multisite_getUsersBlogs($args) {
		...
		$path = $current_blog->path . 'trm-xm-lrpc.php';
		$protocol = is_ssl() ? 'https' : 'http';

		$rpc = new IXR_Client("$protocol://{$domain}{$path}");
		$rpc->query('trm.getUsersBlogs', $args[1], $args[2]);
	...
	function blogger_getPost($args) {
		...
		$post_data = trm_get_single_post($post_ID, ARRAY_A);

		$categories = implode(',', trm_get_post_categories($post_ID));
	...
	function blogger_getRecentPosts($args) {
		...
		$posts_list = trm_get_recent_posts( $query );
		...
		foreach ($posts_list as $entry) {
			...
			$categories = implode(',', trm_get_post_categories($entry['ID']));
	...
	function blogger_newPost($args) {
		...
		$post_ID = trm_insert_post($post_data);
	...
	function blogger_editPost($args) {
		...
		$actual_post = trm_get_single_post($post_ID,ARRAY_A);
		...
		$result = trm_update_post($postdata);
	...
	function blogger_deletePost($args) {
		...
		$result = trm_delete_post($post_ID);
	...
	function mw_newPost($args) {
		...
		if ( !empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				$post_type = 'page';
				if ( !empty( $content_struct['trm_page_template'] ) )
					$page_template = $content_struct['trm_page_template'];
		...
		if ( isset( $content_struct['trm_post_format'] ) ) {
			$content_struct['trm_post_format'] = sanitize_key( $content_struct['trm_post_format'] );
			if ( !array_key_exists( $content_struct['trm_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format' ) );
			}
		}

		// Let Trnder generate the post_name (slug) unless
		// one has been provided.
		$post_name = "";
		if ( isset($content_struct['trm_slug']) )
			$post_name = $content_struct['trm_slug'];

		// Only use a password if one was given.
		if ( isset($content_struct['trm_password']) )
			$post_password = $content_struct['trm_password'];

		// Only set a post parent if one was provided.
		if ( isset($content_struct['trm_page_parent_id']) )
			$post_parent = $content_struct['trm_page_parent_id'];

		// Only set the menu_order if it was provided.
		if ( isset($content_struct['trm_page_order']) )
			$menu_order = $content_struct['trm_page_order'];

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset($content_struct['trm_author_id']) && ($user->ID != $content_struct['trm_author_id']) ) {
			switch ( $post_type ) {
				case "post":
			...
			$post_author = $content_struct['trm_author_id'];
		}
		...
		$post_ID = trm_insert_post($postdata, true);
		if ( is_trm_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
		...
		// in this function
		if ( isset( $content_struct['trm_post_format'] ) )
			trm_set_post_terms( $post_ID, array( 'post-format-' . $content_struct['trm_post_format'] ), 'post_format' );
	...
	function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $trmdb->get_results( "SELECT ID, guid FROM {$trmdb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( strpos( $post_content, $file->guid ) !== false )
					$trmdb->update($trmdb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID) );
			}
		}
	}
	...
	function mw_editPost($args) {
		...
		// Let Trnder manage slug if none was provided.
		$post_name = "";
		...
		// We've got all the data -- post it:
		$netrmost = compact('ID', 'post_content', 'post_title', 'post_category', 'post_status', 'post_excerpt', 'comment_status', 'ping_status', 'post_date', 'post_date_gmt', 'to_ping', 'post_name', 'post_password', 'post_parent', 'menu_order', 'post_author', 'tags_input', 'page_template');

		$result = trm_update_post($netrmost, true);
		if ( is_trm_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
	...
	function mw_getPost($args) {
		...
		if ($postdata['post_date'] != '') {
			...
			$categories = array();
			$catids = trm_get_post_categories($post_ID);
			foreach($catids as $catid)
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = trm_get_post_tags( $post_ID );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag )
					$tagnames[] = $tag->name;
			...
			$resp = array(
				...
				'trm_slug' => $postdata['post_name'],
				'trm_password' => $postdata['post_password'],
				'trm_author_id' => $author->ID,
				'trm_author_display_name'	=> $author->display_name,
				'date_created_gmt' => new IXR_Date($post_date_gmt),
				'post_status' => $postdata['post_status'],
				'custom_fields' => $this->get_custom_fields($post_ID),
				'trm_post_format' => $post_format,
				'sticky' => $sticky
			);
	...
	function mw_getRecentPosts($args) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = array();
			$catids = trm_get_post_categories($entry['ID']);
			foreach( $catids as $catid )
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = trm_get_post_tags( $entry['ID'] );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$struct[] = array(
				...
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'trm_slug' => $entry['post_name'],
				'trm_password' => $entry['post_password'],
				'trm_author_id' => $author->ID,
				'trm_author_display_name' => $author->display_name,
				'date_created_gmt' => new IXR_Date($post_date_gmt),
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-trendr-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @param array $args Method parameters.
	...
	function mw_newMediaObject($args) {
		...
		$blog_ID     = (int) $args[0];
		$username  = $trmdb->escape($args[1]);
		$password   = $trmdb->escape($args[2]);
		$data        = $args[3];
		...
		if ( !empty($data['overwrite']) && ($data['overwrite'] == true) ) {
			...
			trm_delete_attachment($old_file->ID);

			// Make sure the new name is different by pre-pending the
			// previous post id.
			$filename = preg_replace('/^trmid\d+-/', '', $name);
			$name = "trmid{$old_file->ID}-{$filename}";
		}

		$upload = trm_upload_bits($name, NULL, $bits);
		...
		// Save the data
		$id = trm_insert_attachment( $attachment, $upload[ 'file' ], $post_id );
		trm_update_attachment_metadata( $id, trm_generate_attachment_metadata( $id, $upload['file'] ) );

		return apply_filters( 'trm_handle_upload', array( 'file' => $name, 'url' => $upload[ 'url' ], 'type' => $type ), 'upload' );
	}
	...
	function mt_getPostCategories($args) {
		...
		$categories = array();
		$catids = trm_get_post_categories(intval($post_ID));
	...
	function mt_setPostCategories($args) {
		...
		trm_set_post_categories($post_ID, $catids);
	...
	function mt_getTrackbackPings($args) {
		...
		$comments = $trmdb->get_results( $trmdb->prepare("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $trmdb->comments WHERE comment_post_ID = %d", $post_ID) );
	...
	function mt_publishPost($args) {
		...
		// retain old cats
		$cats = trm_get_post_categories($post_ID);
	...
	function pingback_ping($args) {
		...
		if ( $post_ID = url_to_postid($pagelinkedto) ) {
			...
			if ( intval($urltest['fragment']) ) {
				...
				$sql = $trmdb->prepare("SELECT ID FROM $trmdb->posts WHERE post_title RLIKE %s", like_escape( $title ) );
				if (! ($post_ID = $trmdb->get_var($sql)) ) {
		...
		if ( $trmdb->get_results( $trmdb->prepare("SELECT * FROM $trmdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom) ) )
		...
		$linea = trm_remote_fopen( $pagelinkedfrom );
		if ( !$linea )
		...
		foreach ( $p as $para ) {
			if ( strpos($para, $pagelinkedto) !== false ) { // it exists, but is it a link?
				...
				$excerpt = preg_replace('|\</?trmcontext\>|', '', $para);

				// prevent really long link text
				if ( strlen($context[1]) > 100 )
					$context[1] = substr($context[1], 0, 100) . '...';

				$marker = '<trmcontext>'.$context[1].'</trmcontext>';    // set up our marker
				$excerpt= str_replace($context[0], $marker, $excerpt); // swap out the link for our marker
				$excerpt = strip_tags($excerpt, '<trmcontext>');        // strip all tags but our context marker
		...
		$pagelinkedfrom = $trmdb->escape( $pagelinkedfrom );
