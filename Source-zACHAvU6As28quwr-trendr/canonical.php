<?php
/**
 * Canonical API to handle Trnder Redirecting
 *
 ...
 * not exist based on exact Trnder query. Will instead try to parse the URL
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	global $trm_rewrite, $is_IIS, $trm_query, $trmdb;
	...
	// See: http://trac./ticket/5017
	// See: http://trac./ticket/7173
	// Disabled, for now:
	...
	if ( is_singular() && 1 > $trm_query->post_count && ($id = get_query_var('p')) ) {

		$vars = $trmdb->get_results( $trmdb->prepare("SELECT post_type, post_parent FROM $trmdb->posts WHERE ID = %d", $id) );
	...
	// These tests give us a TRM-generated permalink
	if ( is_404() ) {
	...
	} elseif ( is_object($trm_rewrite) && $trm_rewrite->using_permalinks() ) {
		...
		if ( is_attachment() && !empty($_GET['attachment_id']) && ! $redirect_url ) {
			...
			if ( $redirect_url = get_permalink( $trm_query->get_queried_object_id() ) )
				$redirect['query'] = remove_query_arg('name', $redirect['query']);
		} elseif ( is_page() && !empty($_GET['page_id']) && ! $redirect_url ) {
			if ( $redirect_url = get_permalink(get_query_var('page_id')) )
				$redirect['query'] = remove_query_arg('page_id', $redirect['query']);
		} elseif ( is_page() && !is_feed() && isset($trm_query->queried_object) && 'page' == get_option('show_on_front') && $trm_query->queried_object->ID == get_option('page_on_front')  && ! $redirect_url ) {
		...
		// now moving on to non ?m=X year/month/day links
			...
			if ( ( false !== $author ) && $trmdb->get_var( $trmdb->prepare( "SELECT ID FROM $trmdb->posts WHERE $trmdb->posts.post_author = %d AND $trmdb->posts.post_status = 'publish' LIMIT 1", $author->ID ) ) ) {
			...
			$term_count = 0;
			foreach ( $trm_query->tax_query->queries as $tax_query )
				$term_count += count( $tax_query['terms'] );

			$obj = $trm_query->get_queried_object();
			if ( $term_count <= 1 && !empty($obj->term_id) && ( $tax_url = get_term_link((int)$obj->term_id, $obj->taxonomy) ) && !is_trm_error($tax_url) ) {
				if ( !empty($redirect['query']) ) {
					...
					$rewrite_vars = array_diff( array_keys($trm_query->query), array_keys($_GET) );
		...
		} elseif ( is_single() && strpos($trm_rewrite->permalink_structure, '%category%') !== false ) {
			$category = get_category_by_path(get_query_var('category_name'));
			$post_terms = trm_get_object_terms($trm_query->get_queried_object_id(), 'category', array('fields' => 'tt_ids'));
			if ( (!$category || is_trm_error($category)) || ( !is_trm_error($post_terms) && !empty($post_terms) && !in_array($category->term_taxonomy_id, $post_terms) ) )
				$redirect_url = get_permalink($trm_query->get_queried_object_id());
		}
		...
		if ( get_query_var('paged') || is_feed() || get_query_var('cpage') ) {
			while ( preg_match( "#/$trm_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] ) || preg_match( '#/(comments/?)?(feed|rss|rdf|atom|rss2)(/+)?$#', $redirect['path'] ) || preg_match( '#/comment-page-[0-9]+(/+)?$#', $redirect['path'] ) ) {
				// Strip off paging and feed
				$redirect['path'] = preg_replace("#/$trm_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path']); // strip off any existing paging
			...
			$addl_path = '';
			if ( is_feed() && in_array( get_query_var('feed'), $trm_rewrite->feeds ) ) {
			...
			if ( get_query_var('paged') > 0 ) {
				...
				if ( !is_feed() ) {
					if ( $paged > 1 && !is_single() ) {
						$addl_path = ( !empty( $addl_path ) ? trailingslashit($addl_path) : '' ) . user_trailingslashit("$trm_rewrite->pagination_base/$paged", 'paged');
					} elseif ( !is_single() ) {
						$addl_path = !empty( $addl_path ) ? trailingslashit($addl_path) : '';
					}
				} elseif ( $paged > 1 ) {
			...
			if ( !empty( $addl_path ) && $trm_rewrite->using_index_permalinks() && strpos($redirect['path'], '/index.php/') === false )
	...
	// trailing slashes
	if ( is_object($trm_rewrite) && $trm_rewrite->using_permalinks() && !is_404() && (!is_front_page() || ( is_front_page() && (get_query_var('paged') > 1) ) ) ) {
		$user_ts_type = '';
		if ( get_query_var('paged') > 0 ) {
			$user_ts_type = 'paged';
		} else {
...
function redirect_guess_404_permalink() {
	...
	$where = $trmdb->prepare("post_name LIKE %s", like_escape( get_query_var('name') ) . '%');

	// if any of post_type, year, monthnum, or day are set, use them to refine the query
	if ( get_query_var('post_type') )
		$where .= $trmdb->prepare(" AND post_type = %s", get_query_var('post_type'));
	if ( get_query_var('year') )
		$where .= $trmdb->prepare(" AND YEAR(post_date) = %d", get_query_var('year'));
	if ( get_query_var('monthnum') )
		$where .= $trmdb->prepare(" AND MONTH(post_date) = %d", get_query_var('monthnum'));
	if ( get_query_var('day') )
		$where .= $trmdb->prepare(" AND DAYOFMONTH(post_date) = %d", get_query_var('day'));

	$post_id = $trmdb->get_var("SELECT ID FROM $trmdb->posts WHERE $where AND post_status = 'publish'");
	if ( !$post_id )
		return false;
