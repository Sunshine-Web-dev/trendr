...
function get_header( $name = null ) {
	...
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . TRMINC . '/theme-compat/header.php');
}
...
function get_footer( $name = null ) {
	...
	// Backward compat code will be removed in a future release
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . TRMINC . '/theme-compat/footer.php');
}
...
function get_sidebar( $name = null ) {
	...
	// Backward compat code will be removed in a future release
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . TRMINC . '/theme-compat/sidebar.php');
}
 ...
 * form into the sidebar and also by the search widget in Trnder.
 *
 ...
 */
function trm_loginout($redirect = '', $echo = true) {
	if ( ! is_user_logged_in() )
		$link = '<a href="' . esc_url( trm_login_url($redirect) ) . '">' . __('Log in') . '</a>';
	else
		$link = '<a href="' . esc_url( trm_logout_url($redirect) ) . '">' . __('Log out') . '</a>';
 ...
 *
 * @since 2.7.0
 * @uses trm_nonce_url() To protect against CSRF
 * @uses site_url() To generate the log in URL
 * @uses apply_filters() calls 'logout_url' hook on final logout url
 *
 * @param string $redirect Path to redirect to on logout.
 */
function trm_logout_url($redirect = '') {
	$args = array( 'action' => 'logout' );
	if ( !empty($redirect) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg($args, site_url('enter.php', 'login'));
	$logout_url = trm_nonce_url( $logout_url, 'log-out' );
 ...
 */
function trm_login_url($redirect = '', $force_reauth = false) {
	$login_url = site_url('enter.php', 'login');
...
/**
 * Provides a simple login form for use anywhere within Trnder. By default, it echoes
 ...
 */
function trm_login_form( $args = array() ) {
	$defaults = array( 'echo' => true,
						...
						'id_submit' => 'trm-submit',
						'remember' => true,
						'value_username' => '',
						'value_remember' => false, // Set this to true to default the "Remember me" checkbox to checked
					);
	$args = trm_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );

	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . site_url( 'enter.php', 'login' ) . '" method="post">
			...
			<p class="login-submit">
				<input type="submit" name="trm-submit" id="' . esc_attr( $args['id_submit'] ) . '" class="button-primary" value="' . esc_attr( $args['label_log_in'] ) . '" tabindex="100" />
 ...
 * @param string $redirect Path to redirect to on login.
 */
function trm_lostpassword_url($redirect = '') {
	$args = array( 'action' => 'lostpassword' );
	if ( !empty($redirect) ) {
		$args['redirect_to'] = $redirect;
	}

	$lostpassword_url = add_query_arg($args, site_url('enter.php', 'login'));
 ...
 * @param boolean $echo Default to echo and not return the link.
 */
function trm_register( $before = '<li>', $after = '</li>', $echo = true ) {

	if ( ! is_user_logged_in() ) {
		if ( get_option('users_can_register') )
			$link = $before . '<a href="' . site_url('enter.php?action=register', 'login') . '">' . __('Register') . '</a>' . $after;
		else
			$link = '';
	} else {
...
/**
 * Theme container function for the 'trm_meta' action.
 *
 * The 'trm_meta' action can have several purposes, depending on how you use it,
 * but one purpose might have been to allow for theme switching.
 *
 * @since 1.5.0
 * @link http://trac./ticket/1458 Explanation of 'trm_meta' action.
 * @uses do_action() Calls 'trm_meta' hook.
 */
function trm_meta() {
	do_action('trm_meta');
}
 ...
 * <li><strong>trmurl</strong> - Blog URI path to Trnder.</li>
...
function get_bloginfo( $show = '', $filter = 'raw' ) {
	...
	switch( $show ) {
		...
		case 'comments_rss2_url':
			...
			break;
		case 'pingback_url':
			$output = get_option('siteurl') .'/trm-xm-lrpc.php';
			break;
		case 'stylesheet_url':
 ...
 */
function trm_title($sep = '&raquo;', $display = true, $seplocation = '') {
	global $trmdb, $trm_locale;
	...
	$t_sep = '%TRM_TITILE_SEP%'; // Temporary separator, for accurate flipping, if necessary
	...
	if ( is_archive() && !empty($m) ) {
		...
		$my_month = $trm_locale->get_month(substr($m, 4, 2));
	...
	if ( is_archive() && !empty($year) ) {
		$title = $year;
		if ( !empty($monthnum) )
			$title .= $t_sep . $trm_locale->get_month($monthnum);
		if ( !empty($day) )
	...
	$title = apply_filters('trm_title', $title, $sep, $seplocation);
 ...
 * for category page title and with less overhead than {@link trm_title()}.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * overhead than {@link trm_title()}, because of its limited implementation.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * It has less overhead than {@link trm_title()}, because of its limited implementation.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * is needed, should be better than {@link trm_title()}.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
...
function single_month_title($prefix = '', $display = true ) {
	...
	if ( !empty($monthnum) && !empty($year) ) {
		$my_year = $year;
		$my_month = $trm_locale->get_month($monthnum);
	} elseif ( !empty($m) ) {
		$my_year = substr($m, 0, 4);
		$my_month = $trm_locale->get_month(substr($m, 4, 2));
	}
 ...
 */
function trm_get_archives($args = '') {
	global $trmdb, $trm_locale;
	...
	if ( 'monthly' == $type ) {
		$query = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $trmdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date DESC $limit";
		$key = md5($query);
		$cache = trm_cache_get( 'trm_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $trmdb->get_results($query);
			$cache[ $key ] = $arcresults;
			trm_cache_set( 'trm_get_archives', $cache, 'general' );
		} else {
		...
		if ( $arcresults ) {
			...
			foreach ( (array) $arcresults as $arcresult ) {
				...
				$text = sprintf(__('%1$s %2$d'), $trm_locale->get_month($arcresult->month), $arcresult->year);
				if ( $show_post_count )
		...
		$query = "SELECT YEAR(post_date) AS `year`, count(ID) as posts FROM $trmdb->posts $join $where GROUP BY YEAR(post_date) ORDER BY post_date DESC $limit";
		$key = md5($query);
		$cache = trm_cache_get( 'trm_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $trmdb->get_results($query);
		...
		$query = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth`, count(ID) as posts FROM $trmdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date), DAYOFMONTH(post_date) ORDER BY post_date DESC $limit";
		$key = md5($query);
		$cache = trm_cache_get( 'trm_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $trmdb->get_results($query);
		...
		$week = _trm_mysql_week( '`post_date`' );
		$query = "SELECT DISTINCT $week AS `week`, YEAR( `post_date` ) AS `yr`, DATE_FORMAT( `post_date`, '%Y-%m-%d' ) AS `yyyymmdd`, count( `ID` ) AS `posts` FROM `$trmdb->posts` $join $where GROUP BY $week, YEAR( `post_date` ) ORDER BY `post_date` DESC $limit";
		$key = md5($query);
		$cache = trm_cache_get( 'trm_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $trmdb->get_results($query);
		...
		$query = "SELECT * FROM $trmdb->posts $join $where ORDER BY $orderby $limit";
		$key = md5($query);
		$cache = trm_cache_get( 'trm_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $trmdb->get_results($query);
...
function get_calendar($initial = true, $echo = true) {
	global $trmdb, $m, $monthnum, $year, $trm_locale, $posts;

	$cache = array();
	$key = md5( $m . $monthnum . $year );
	if ( $cache = trm_cache_get( 'get_calendar', 'calendar' ) ) {
	...
	if ( !$posts ) {
		$gotsome = $trmdb->get_var("SELECT 1 as test FROM $trmdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1");
		if ( !$gotsome ) {
			$cache[ $key ] = '';
			trm_cache_set( 'get_calendar', $cache, 'calendar' );
			return;
		}
	}
	...
	if ( !empty($monthnum) && !empty($year) ) {
		...
		$thismonth = $trmdb->get_var("SELECT DATE_FORMAT((DATE_ADD('{$thisyear}0101', INTERVAL $d DAY) ), '%m')");
	} elseif ( !empty($m) ) {
		$thisyear = ''.intval(substr($m, 0, 4));
		if ( strlen($m) < 6 )
				$thismonth = '01';
	...
	$previous = $trmdb->get_row("SELECT MONTH(post_date) AS month, YEAR(post_date) AS year
		FROM $trmdb->posts
		WHERE post_date < '$thisyear-$thismonth-01'
		AND post_type = 'post' AND post_status = 'publish'
			ORDER BY post_date DESC
			LIMIT 1");
	$next = $trmdb->get_row("SELECT MONTH(post_date) AS month, YEAR(post_date) AS year
		FROM $trmdb->posts
	...
	$calendar_output = '<table id="trm-calendar">
	<caption>' . sprintf($calendar_caption, $trm_locale->get_month($thismonth), date('Y', $unixmonth)) . '</caption>
	<thead>
	<tr>';
	...
	for ( $wdcount=0; $wdcount<=6; $wdcount++ ) {
		$myweek[] = $trm_locale->get_weekday(($wdcount+$week_begins)%7);
	}

	foreach ( $myweek as $wd ) {
		$day_name = (true == $initial) ? $trm_locale->get_weekday_initial($wd) : $trm_locale->get_weekday_abbrev($wd);
		$wd = esc_attr($wd);
	...
	if ( $previous ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="prev"><a href="' . get_month_link($previous->year, $previous->month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s'), $trm_locale->get_month($previous->month), date('Y', mktime(0, 0 , 0, $previous->month, 1, $previous->year)))) . '">&laquo; ' . $trm_locale->get_month_abbrev($trm_locale->get_month($previous->month)) . '</a></td>';
	} else {
	...
	if ( $next ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="next"><a href="' . get_month_link($next->year, $next->month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s'), $trm_locale->get_month($next->month), date('Y', mktime(0, 0 , 0, $next->month, 1, $next->year))) ) . '">' . $trm_locale->get_month_abbrev($trm_locale->get_month($next->month)) . ' &raquo;</a></td>';
	} else {
	...
	// Get days with posts
	$dayswithposts = $trmdb->get_results("SELECT DISTINCT DAYOFMONTH(post_date)
		FROM $trmdb->posts WHERE post_date >= '{$thisyear}-{$thismonth}-01 00:00:00'
		AND post_type = 'post' AND post_status = 'publish'
	...
	$ak_post_titles = $trmdb->get_results("SELECT ID, post_title, DAYOFMONTH(post_date) as dom "
		."FROM $trmdb->posts "
...
function delete_get_calendar_cache() {
	trm_cache_delete( 'get_calendar', 'calendar' );
}
...
function the_weekday() {
	global $trm_locale, $post;
	$the_weekday = $trm_locale->get_weekday(mysql2date('w', $post->post_date, false));
...
function the_weekday_date($before='',$after='') {
	global $trm_locale, $post, $day, $previousweekday;
	$the_weekday_date = '';
	if ( $currentday != $previousweekday ) {
		$the_weekday_date .= $before;
		$the_weekday_date .= $trm_locale->get_weekday(mysql2date('w', $post->post_date, false));
...
/**
 * Fire the trm_head action
 *
 * @since 1.2.0
 * @uses do_action() Calls 'trm_head' hook.
 */
function trm_head() {
	do_action('trm_head');
}
...
/**
 * Fire the trm_footer action
 *
 * @since 1.5.1
 * @uses do_action() Calls 'trm_footer' hook.
 */
function trm_footer() {
	do_action('trm_footer');
}
 ...
 */
//function rsd_link() {
	//echo '<link rel="EditURI" type="application/rsd+xml" title="RSD" href="' . get_bloginfo('trmurl') . "/trm-xm-lrpc.php?rsd\" />\n";
//}
...
//function wlwmanifest_link() {
	//echo '<link rel="wlwmanifest" type="application/wlwmanifest+xml" href="'
		//. get_bloginfo('trmurl') . '/Source-zACHAvU6As28quwr-trendr/wlwmanifest.xml" /> ' . "\n";
//}
 ...
 *
 * Checks to see if the user has deleted the tinymce files to slim down there Trnder install.
 *
 * @since 2.1.0
 *
...
function rich_edit_exists() {
	global $trm_rich_edit_exists;
	if ( !isset($trm_rich_edit_exists) )
		$trm_rich_edit_exists = file_exists(ABSPATH . TRMINC . '/js/tinymce/tiny_mce.js');
	return $trm_rich_edit_exists;
}
...
function user_can_richedit() {
	global $trm_rich_edit, $pagenow, $is_iphone;

	if ( !isset( $trm_rich_edit) ) {
	...
	return apply_filters('user_can_richedit', $trm_rich_edit);
}
 ...
 */
function trm_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // defaults
	if ( $user = trm_get_current_user() ) { // look for cookie
		$ed = get_user_setting('editor', 'tinymce');
		$r = ( in_array($ed, array('tinymce', 'html', 'test') ) ) ? $ed : $r;
	}
	return apply_filters( 'trm_default_editor', $r ); // filter
}
...
<?php
	if ( $richedit ) {
		$trm_default_editor = trm_default_editor(); ?>
		<div class="zerosize"><input accesskey="e" type="button" onclick="switchEditors.go('<?php echo $id; ?>')" /></div>
<?php	if ( 'html' == $trm_default_editor ) {
			add_filter('the_editor_content', 'trm_htmledit_pre'); ?>
...
<?php	} else {
			...
			add_filter('the_editor_content', 'trm_richedit_pre'); ?>
...
<?php
	// queue scripts
	if ( $richedit )
		add_action( 'admin_print_footer_scripts', 'trm_tiny_mce', 25 );
	elseif ( $extended )
		add_action( 'admin_print_footer_scripts', 'trm_quicktags', 25 );

}

/**
 * Retrieve the contents of the search Trnder query variable.
 *
 ...
 * <code>
 * trm_admin_css_color('classic', __('Classic'), admin_url("css/colors-classic.css"),
 ...
 */
function trm_admin_css_color($key, $name, $url, $colors = array()) {
	global $_trm_admin_css_colors;

	if ( !isset($_trm_admin_css_colors) )
		$_trm_admin_css_colors = array();

	$_trm_admin_css_colors[$key] = (object) array('name' => $name, 'url' => $url, 'colors' => $colors);
}
...
function register_admin_color_schemes() {
	trm_admin_css_color( 'classic', __( 'Blue' ), admin_url( 'css/colors-classic.css' ),
		array( '#5589aa', '#cfdfe9', '#d1e5ee', '#eff8ff' ) );
	trm_admin_css_color( 'fresh', __( 'Gray' ), admin_url( 'css/colors-fresh.css' ),
		array( '#7c7976', '#c6c6c6', '#e0e0e0', '#f1f1f1' ) );
}

/**
 * Display the URL of a Trnder admin CSS file.
 *
 * @see TRM_Styles::_css_href and its style_loader_src filter.
 *
 * @since 2.3.0
 *
 * @param string $file file relative to Backend-WeaprEcqaKejUbRq-trendr/ without its ".css" extension.
 */
function trm_admin_css_uri( $file = 'Backend-WeaprEcqaKejUbRq-trendr' ) {
	...
	return apply_filters( 'trm_admin_css_uri', $_file, $file );
}
 ...
 * 'trm_print_styles' action has *not* yet been called, the CSS file will be
 * enqueued. If the trm_print_styles action *has* been called, the CSS link will
 * be printed. Printing may be forced by passing TRUE as the $force_echo
 * (second) parameter.
 *
 * For backward compatibility with Trnder 2.3 calling method: If the $file
 * (first) parameter does not correspond to a registered CSS file, we assume
 * $file is a file relative to Backend-WeaprEcqaKejUbRq-trendr/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @package Trnder
 * @since 2.3.0
 * @uses $trm_styles Trnder Styles Object
 *
 * @param string $file Optional. Style handle name or file name (without ".css" extension) relative
 * 	 to Backend-WeaprEcqaKejUbRq-trendr/. Defaults to 'Backend-WeaprEcqaKejUbRq-trendr'.
 * @param bool $force_echo Optional.  Force the stylesheet link to be printed rather than enqueued.
 */
function trm_admin_css( $file = 'Backend-WeaprEcqaKejUbRq-trendr', $force_echo = false ) {
	global $trm_styles;
	...
	if ( $trm_styles->query( $handle ) ) {
		if ( $force_echo || did_action( 'trm_print_styles' ) ) // we already printed the style queue.  Print this one immediately
			trm_print_styles( $handle );
		else // Add to style queue
			trm_enqueue_style( $handle );
		return;
	}

	echo apply_filters( 'trm_admin_css', "<link rel='stylesheet' href='" . esc_url( trm_admin_css_uri( $file ) ) . "' type='text/css' />\n", $file );
	if ( is_rtl() )
		echo apply_filters( 'trm_admin_css', "<link rel='stylesheet' href='" . esc_url( trm_admin_css_uri( "$file-rtl" ) ) . "' type='text/css' />\n", "$file-rtl" );
}
...
/**
 * Display the XHTML generator that is generated on the trm_head hook.
 *
 * @since 2.5.0
 */
//function trm_generator() {
	//the_generator( apply_filters( 'trm_generator_type', 'xhtml' ) );
//}
