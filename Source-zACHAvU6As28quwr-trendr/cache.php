 ...
 * Object Cache API
 *
 * @link http://codex./Function_Reference/TRM_Cache
 *
 * @package Trnder
 * @subpackage Cache
 */
 ...
 *
 * @since 2.0.0
 * @uses $trm_object_cache Object Cache Class
 * @see TRM_Object_Cache::add()
 *
...
function trm_cache_add($key, $data, $flag = '', $expire = 0) {
	...
	return $trm_object_cache->add($key, $data, $flag, $expire);
}

/**
 * Closes the cache.
 *
 * This function has ceased to do anything since Trnder 2.5. The
 * functionality was removed along with the rest of the persistant cache. This
 * does not mean that plugins can't implement this function when they need to
 * make sure that the cache is cleaned up after Trnder no longer needs it.
 *
 * @since 2.0.0
 *
 ...
 *
 * @since 2.0.0
 * @global TRM_Object_Cache $trm_object_cache Trnder Object Cache
 */
function trm_cache_init() {
	$GLOBALS['trm_object_cache'] = new TRM_Object_Cache();
}
...
function trm_cache_replace($key, $data, $flag = '', $expire = 0) {
	...
	return $trm_object_cache->replace($key, $data, $flag, $expire);
}
...
function trm_cache_set($key, $data, $flag = '', $expire = 0) {
	...
	return $trm_object_cache->set($key, $data, $flag, $expire);
}
...
function trm_cache_add_global_groups( $groups ) {
	...
	return $trm_object_cache->add_global_groups($groups);
}
...
function trm_cache_reset() {
	...
	return $trm_object_cache->reset();
}

/**
 * Trnder Object Cache
 *
 * The Trnder Object Cache is used to save on trips to the database. The
 * Object Cache stores all of the cache data to memory and makes the cache
 * contents available by using a key, which is used to name and later retrieve
 * the cache contents.
 *
 * The Object Cache can be replaced by other caching mechanisms by placing files
 * in the trm-src folder which is looked at in trm-main. If that file
...
class TRM_Object_Cache {
	 ...
	 *
	 * @uses TRM_Object_Cache::get Checks to see if the cache already has data.
	 * @uses TRM_Object_Cache::set Sets the data after the checking the cache
	 *		contents existance.
	 *
	 * @since 2.0.0
	 *
	 ...
	 * naming guidelines outside of core Trnder usage.
	 *
	 ...
	 *
	 * @since 2.0.8
	 * @return null|TRM_Object_Cache If cache is disabled, returns null.
	 */
