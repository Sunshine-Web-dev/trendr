 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0', TRMINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'Trnder/' . $GLOBALS['trm_version']);
...
/**
 * Retrieve URL headers and content using TRM HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = trm_remote_request($url, array('headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT));
	if ( is_trm_error($resp) ) {
	...
	// Also note, TRM_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( trm_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = trm_remote_retrieve_response_code( $resp );
	$response->response_code = trm_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = trm_remote_retrieve_body( $resp );
...
class RSSCache {
	...
	function RSSCache ($base='', $age='') {
		$this->BASE_CACHE = TRM_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function trm_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with trm_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
