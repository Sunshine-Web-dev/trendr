...
class TRM_Admin_Bar {
	...
	function initialize() {
		...
		add_action( 'trm_head', 'trm_admin_bar_header' );

		add_action( 'admin_head', 'trm_admin_bar_header' );
		...
		trm_enqueue_script( 'admin-bar' );
		trm_enqueue_style( 'admin-bar' );
	...
	function recursive_render( $id, &$menu_item ) { ?>
		...
		<li id="<?php echo esc_attr( "Backend-WeaprEcqaKejUbRq-trendr-bar-$id" ); ?>" class="<?php echo esc_attr( $menuclass ); ?>">
	...
	function add_menus() {
		add_action( 'admin_bar_menu', 'trm_admin_bar_my_account_menu', 10 );
		add_action( 'admin_bar_menu', 'trm_admin_bar_my_sites_menu', 20 );
		add_action( 'admin_bar_menu', 'trm_admin_bar_dashboard_view_site_menu', 25 );
		add_action( 'admin_bar_menu', 'trm_admin_bar_edit_menu', 30 );
		add_action( 'admin_bar_menu', 'trm_admin_bar_shortlink_menu', 80 );
		add_action( 'admin_bar_menu', 'trm_admin_bar_updates_menu', 70 );

		if ( !is_network_admin() && !is_user_admin() ) {
			add_action( 'admin_bar_menu', 'trm_admin_bar_new_content_menu', 40 );
			add_action( 'admin_bar_menu', 'trm_admin_bar_comments_menu', 50 );
			add_action( 'admin_bar_menu', 'trm_admin_bar_appearance_menu', 60 );
		}
	...
	function load_user_locale_translations() {
		...
		$this->adminbar_locale_filter = lambda( '$_', '$GLOBALS["trm_admin_bar"]->user->locale;' );
	...
	function unload_user_locale_translations() {
		...
		$lan['default'] =$this->previous_translations;
		*/
	}
}
?>
