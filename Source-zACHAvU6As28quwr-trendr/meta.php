 ...
 * Metadata API
 *
 * Functions for retrieving and manipulating metadata of various Trnder object types.  Metadata
...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
	...
	if ( $unique && $trmdb->get_var( $trmdb->prepare(
	...
	$trmdb->insert( $table, array(
		$column => $object_id,
		'meta_key' => $meta_key,
		'meta_value' => $meta_value
	) );

	trm_cache_delete($object_id, $meta_type . '_meta');
	// users cache stores usermeta that must be cleared.
	if ( 'user' == $meta_type )
		clean_user_cache($object_id);

	do_action( "added_{$meta_type}_meta", $trmdb->insert_id, $object_id, $meta_key, $_meta_value );
...
function update_metadata($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '') {
	...
	if ( ! $meta_id = $trmdb->get_var( $trmdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) ) )
...
function delete_metadata($meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false) {
	...
	$query = $trmdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( !$delete_all )
		$query .= $trmdb->prepare(" AND $type_column = %d", $object_id );

	if ( $meta_value )
		$query .= $trmdb->prepare(" AND meta_value = %s", $meta_value );

	$meta_ids = $trmdb->get_col( $query );
	if ( !count( $meta_ids ) )
		return false;
...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
	...
	$meta_cache = trm_cache_get($object_id, $meta_type . '_meta');
...
function update_meta_cache($meta_type, $object_ids) {
	...
	foreach ( $object_ids as $id ) {
		$cached_object = trm_cache_get( $id, $cache_key );
	...
	// Get meta info
	$id_list = join(',', $ids);
	$meta_list = $trmdb->get_results( $trmdb->prepare("SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list)",
		$meta_type), ARRAY_A );
	...
	foreach ( $ids as $id ) {
		if ( ! isset($cache[$id]) )
			$cache[$id] = array();
		trm_cache_add( $id, $cache[$id], $cache_key );
	}
...
function get_meta_sql( $meta_query = false, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new TRM_Meta_Query( $meta_query );
...
class TRM_Meta_Query {
	...
	*/
	...
	function parse_query_vars( $qv ) {
		...
		// TRM_Query sets 'meta_value' = '' by default
	...
	function get_sql( $type, $primary_table, $primary_id_column, $context = null ) {
		...
		foreach ( $this->queries as $k => $q ) {
			...
			$where[$k] = '';
			if ( !empty( $meta_key ) )
				$where[$k] = $trmdb->prepare( "$alias.meta_key = %s", $meta_key );
			...
			$where[$k] = ' (' . $where[$k] . $trmdb->prepare( "CAST($alias.meta_value AS {$meta_type}) {$meta_compare} {$meta_compare_string})", $meta_value );
		}
...
function _get_meta_table($type) {
	...
	if ( empty($trmdb->$table_name) )
		return false;

	return $trmdb->$table_name;
}
