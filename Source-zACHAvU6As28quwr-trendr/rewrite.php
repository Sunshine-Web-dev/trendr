 ...
 *
 * @see TRM_Rewrite::add_rule() for long description.
 * @since 2.1.0
 *
...
function add_rewrite_rule($regex, $redirect, $after = 'bottom') {
	global $trm_rewrite;
	$trm_rewrite->add_rule($regex, $redirect, $after);
}
...
function add_rewrite_tag($tagname, $regex) {
	...
	global $trm_rewrite, $trm;
	$trm->add_query_var($qv);
	$trm_rewrite->add_rewrite_tag($tagname, $regex, $qv . '=');
}

/**
 * Add permalink structure.
 *
 * @see TRM_Rewrite::add_permastruct()
 * @since 3.0.0
 *
...
function add_permastruct( $name, $struct, $with_front = true, $ep_mask = EP_NONE ) {
	global $trm_rewrite;
	return $trm_rewrite->add_permastruct( $name, $struct, $with_front, $ep_mask );
}
...
function add_feed($feedname, $function) {
	global $trm_rewrite;
	if ( ! in_array($feedname, $trm_rewrite->feeds) ) //override the file if it is
		$trm_rewrite->feeds[] = $feedname;
 ...
 *
 * Be sure to flush the rewrite rules (trm_rewrite->flush_rules()) when your plugin gets
 * activated (register_activation_hook()) and deactivated (register_deactivation_hook())
 *
 * @since 2.1.0
 * @see TRM_Rewrite::add_endpoint() Parameters and more description.
 * @uses $trm_rewrite
 *
...
function add_rewrite_endpoint($name, $places) {
	global $trm_rewrite;
	$trm_rewrite->add_endpoint($name, $places);
}
 ...
 * @return string
 */
function _trm_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
...
function url_to_postid($url) {
	...
	$rewrite = $trm_rewrite->trm_rewrite_rules();
	...
	foreach ( (array)$rewrite as $match => $query) {
		...
		if ( preg_match("!^$match!", $request_match, $matches) ) {
			...
			$query = addslashes(TRM_MatchesMapRegex::apply($query, $matches));
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array($key, $trm->public_query_vars) )
					$query[$key] = $value;
			}

			// Do the query
			$query = new TRM_Query($query);
...
/**
 * Trnder Rewrite Component.
 *
 * The Trnder Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * Trnder Query class.
 *
 * The Rewrite along with TRM class function as a front controller for Trnder.
...
class TRM_Rewrite {
	/**
	 * Default permalink structure for Trnder.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var string
	 ...
	 * Front URL path.
	 *
	 * The difference between the root property is that Trnder might be
	 * located at example/Trnder/index.php, if permalinks are turned off. The
	 * Trnder/index.php will be the front portion. If permalinks are turned
	...
	/**
	 * Root URL path to Trnder (without domain).
	 *
	 * The difference between front property is that Trnder might be located
	 * at example.com/Trnder/. The root is the 'Trnder/' portion.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var string
	...
	/**
	 * Rules that don't redirect to TRM's index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for Trnder.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for Trnder pages.
	 *
	 * @since 2.5.0
	 * @access public
	 * @var bool
	...
	/**
	 * Preg_replace values for the search, see {@link TRM_Rewrite::$rewritecode}.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var array
	...
	function page_uri_index() {
		...
		$posts = get_page_hierarchy( $trmdb->get_results("SELECT ID, post_name, post_parent FROM $trmdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'") );
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $trmdb->get_results( $trmdb->prepare( "SELECT ID, post_name, post_parent FROM $trmdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 *
	 * The main TRM_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of {@link TRM_Rewrite::generate_rewrite_rules()} that
	 * allows for shorter list of parameters. See the method for longer
	 * description of what generating rewrite rules does.
	 *
	 * @uses TRM_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 * @since 1.5.0
	 * @access public
	 *
	 ...
	 * reference to the current TRM_Rewrite instance to further manipulate the
	...
	function rewrite_rules() {
		...
		$default_feeds = array(	'.*trm-atom.php$'	=>	$this->index . '?feed=atom',
								'.*trm-rdf.php$'		=>	$this->index . '?feed=rdf',
								'.*trm-rss.php$'		=>	$this->index . '?feed=rss',
								'.*trm-rss2.php$'	=>	$this->index . '?feed=rss2',
								'.*trm-feed.php$'	=>	$this->index . '?feed=feed',
								'.*trm-commentsrss2.php$'	=>	$this->index . '?feed=rss2&withcomments=1');

		// Registration rules
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*trm-signup.php$'] = $this->index . '?signup=true';
			$registration_pages['.*trm-activate.php$'] = $this->index . '?activate=true';
		}
	 ...
	 * TRM_Rewrite::rewrite_rules()} is that this method stores the rewrite rules
	 ...
	 *
	 * Will add  the non_trm_rules property rules to the .htaccess file before
	 * the Trnder rewrite rules one.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	...
	function mod_rewrite_rules() {
		...
		//add in the rules that don't redirect to TRM's index.php (and thus shouldn't be handled by TRM at all)
		foreach ( (array) $this->non_trm_rules as $match => $query) {
	...
	<system.webServer>
		...
		if ( !is_multisite() ) {
			$rules .= '
				<rule name="trendr" patternSyntax="Wildcard">
					<match url="*" />
						<conditions>
			...
			if (is_subdomain_install()) {
				$rules .= '
				<rule name="trendr - Rule 1" stopProcessing="true">
					<match url="^index\.php$" ignoreCase="false" />
					<action type="None" />
				</rule>
				<rule name="trendr - Rule 2" stopProcessing="true">
					<match url="^files/(.+)" ignoreCase="false" />
					<action type="Rewrite" url="Source-zACHAvU6As28quwr-trendr/ms-files.php?file={R:1}" appendQueryString="false" />
				</rule>
				<rule name="trendr - Rule 3" stopProcessing="true">
				...
				<rule name="trendr - Rule 4" stopProcessing="true">
					<match url="." ignoreCase="false" />
					<action type="Rewrite" url="index.php" />
				</rule>';
			} else {
				...
				<rule name="trendr - Rule 5" stopProcessing="true">
					<match url="^([_0-9a-zA-Z-]+/)?(trm-(content|admin|includes).*)" ignoreCase="false" />
					<action type="Rewrite" url="{R:2}" />
				</rule>
				<rule name="trendr - Rule 6" stopProcessing="true">
					<match url="^([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
					<action type="Rewrite" url="{R:2}" />
				</rule>
				<rule name="trendr - Rule 7" stopProcessing="true">
					<match url="." ignoreCase="false" />
					<action type="Rewrite" url="index.php" />
				</rule>';
			}
	...
	function add_rule($regex, $redirect, $after = 'bottom') {
		...
		if ( $front != $this->index ) { //it doesn't redirect to TRM's index.php
	...
	function add_external_rule($regex, $redirect) {
		$this->non_trm_rules[$regex] = $redirect;
	}
	 ...
	 * Remove rewrite rules and then recreate rewrite rules.
	 *
	 * Calls {@link TRM_Rewrite::trm_rewrite_rules()} after removing the
	 ...
	 */
	function init() {
		$this->extra_rules = $this->non_trm_rules = $this->endpoints = array();
	 ...
	 * {@link TRM_Rewrite::init()} after the option is updated.
	 *
