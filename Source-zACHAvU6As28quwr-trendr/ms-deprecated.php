<?php
/**
 * Deprecated functions from Trnder MU and the multisite feature. You shouldn't
 ...
 * @since MU
 * @deprecated 3.0.0
 * @deprecated Use trm_generate_password()
 * @see trm_generate_password()
 */
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0', 'trm_generate_password()' );
	return trm_generate_password( $len );
}
 ...
 * @since MU
 * @deprecated 3.0.0
 * @deprecated Use trm_die()
 * @see trm_die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0', 'trm_die()' );
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	...
	global $trmdb;
	$blogs = $trmdb->get_results( $trmdb->prepare("SELECT blog_id, domain, path FROM $trmdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", $trmdb->siteid), ARRAY_A );

	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $trmdb->get_var( "SELECT COUNT(ID) FROM " . $trmdb->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
	unset( $blogs );
	$blogs = $blog_list;
