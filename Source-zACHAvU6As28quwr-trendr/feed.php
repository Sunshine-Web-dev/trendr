 ...
 * @subpackage Feed
 * @since 2.2.0
 * @uses apply_filters() Calls 'get_trm_title_rss' hook on title.
 * @uses trm_title() See function for $sep parameter usage.
 *
 * @param string $sep Optional.How to separate the title. See trm_title() for more info.
 * @return string Error message on failure or blog title on success.
 */
function get_trm_title_rss($sep = '&#187;') {
	$title = trm_title($sep, false);
	if ( is_trm_error( $title ) )
		return $title->get_error_message();
	$title = apply_filters('get_trm_title_rss', $title);
	return $title;
}
 ...
 * @subpackage Feed
 * @since 2.2.0
 * @uses apply_filters() Calls 'trm_title_rss' on the blog title.
 * @see trm_title() $sep parameter usage.
 *
 * @param string $sep Optional.
 */
function trm_title_rss($sep = '&#187;') {
	echo apply_filters('trm_title_rss', get_trm_title_rss($sep));
}
 ...
 *
 * In the case of Trnder, text is defined as containing no markup,
 ...
 * @return TRM_Error|SimplePie TRM_Error object on failure or SimplePie object on success
 */
function fetch_feed($url) {
	require_once (ABSPATH . TRMINC . '/class-feed.php');

	$feed = new SimplePie();
	$feed->set_feed_url($url);
	$feed->set_cache_class('TRM_Feed_Cache');
	$feed->set_file_class('TRM_SimplePie_File');
	$feed->set_cache_duration(apply_filters('trm_feed_cache_transient_lifetime', 43200, $url));
	do_action_ref_array( 'trm_feed_options', array( &$feed, $url ) );
	$feed->init();
	$feed->handle_content_type();

	if ( $feed->error() )
		return new TRM_Error('simplepie-error', $feed->error());
