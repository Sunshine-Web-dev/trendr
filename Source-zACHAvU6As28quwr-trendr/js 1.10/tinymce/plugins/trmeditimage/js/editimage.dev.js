...
var tinymce = null, tinyMCEPopup, tinyMCE, trmImage;
...
trmImage = {
	...
	img_seturl : function(u) {
		...
		if ( 'current' == u ) {
			...
			if ( rel ) {
				rel = rel.replace( /attachment|trm-att-[0-9]+/gi, '' );
	...
	init : function() {
		...
		window.setTimeout( function(){trmImage.setup();}, 500 );
	},
	...
	setup : function() {
		...
		if ( tinyMCEPopup.editor.getParam('trmeditimage_disable_captions', false) )
		...
		if ( DL = dom.getParent(el, 'dl') ) {
			...
			tinymce.each(DL.childNodes, function(e) {
				if ( e.nodeName == 'DD' && dom.hasClass(e, 'trm-caption-dd') ) {
					caption = e.innerHTML;
					return;
				}
			});
	...
	update : function() {
		...
		if ( do_caption ) {
			...
			if ( DL ) {
				...
				if ( (DT = ed.dom.getParent(el, 'dt')) && (DD = DT.nextSibling) && ed.dom.hasClass(DD, 'trm-caption-dd') )
					ed.dom.setHTML(DD, f.img_cap.value);

			} else {
				if ( (id = f.img_classes.value.match( /trm-image-([0-9]{1,6})/ )) && id[1] )
				...
				html = '<dl id="'+cap_id+'" class="trm-caption '+t.align+'" style="width: '+cap_width+
				'px"><dt class="trm-caption-dt">'+html+'</dt><dd class="trm-caption-dd">'+f.img_cap.value+'</dd></dl>';
	...
	updateImageData : function() {
		var f = document.forms[0], t = trmImage, w = f.width.value, h = f.height.value;
	...
	getImageData : function() {
		var t = trmImage, f = document.forms[0];
...
window.onload = function(){trmImage.init();}
trmImage.preInit();
