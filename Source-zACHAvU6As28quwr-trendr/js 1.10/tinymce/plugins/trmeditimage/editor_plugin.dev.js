...
(function() {
	tinymce.create('tinymce.plugins.trmEditImage', {
		...
		init : function(ed, url) {
			...
			ed.onInit.add(function(ed) {
				tinymce.dom.Event.add(ed.getBody(), 'dragstart', function(e) {
					if ( !tinymce.isGecko && e.target.nodeName == 'IMG' && ed.dom.getParent(e.target, 'dl.trm-caption') )
			...
			ed.onMouseDown.add(function(ed, e) {
				...
				if ( e.target.nodeName == 'IMG' && ed.dom.getAttrib(e.target, 'class').indexOf('mceItem') == -1 ) {
					ed.plugins.trendr._showButtons(e.target, 'trm_editbtns');
					if ( tinymce.isGecko && (p = ed.dom.getParent(e.target, 'dl.trm-caption')) && ed.dom.hasClass(p.parentNode, 'mceTemp') )
		...
		_do_shcode : function(co) {
			return co.replace(/(?:<p>)?\[(?:trm_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:trm_)?caption\](?:<\/p>)?[\s\u00a0]*/g, function(a,b,c){
				...
				return '<div class="'+div_cls+'" draggable><dl id="'+id+'" class="trm-caption '+cls+'" style="width: '+(10+parseInt(w))+
				'px"><dt class="trm-caption-dt">'+c+'</dt><dd class="trm-caption-dd">'+cap+'</dd></dl></div>';
			});
		},
