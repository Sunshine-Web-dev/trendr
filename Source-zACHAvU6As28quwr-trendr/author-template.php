 ...
 *
 * These functions must be used within the Trnder Loop.
 *
 * @link http://codex./Author_Templates
 *
 * @package Trnder
 * @subpackage Template
 */
 ...
 * @since 0.71
 * @see get_the_author()
 * @link http://codex./Template_Tags/the_author
 *
 * @param string $deprecated Deprecated.
 ...
 * @link http://codex./Template_Tags/the_author_meta
 * @since 2.8.0
 ...
 * author's name.
 *
 * @link http://codex./Template_Tags/the_author_link
 * @since 2.1
 ...
 *
 * @link http://codex./Template_Tags/the_author_posts
 * @since 0.71
 ...
 *
 * @link http://codex./Template_Tags/the_author_posts_link
 * @since 1.2.0
 * @uses $authordata The current author's DB object.
 ...
 *
 * @since 2.1.0
 * @uses $trm_rewrite TRM_Rewrite
 * @return string The URL to the author's page.
 */
function get_author_posts_url($author_id, $author_nicename = '') {
	global $trm_rewrite;
	$auth_ID = (int) $author_id;
	$link = $trm_rewrite->get_author_permastruct();
 ...
 * </ul>
 *
 * @link http://codex./Template_Tags/trm_list_authors
 * @since 1.2.0
 * @param array $args The argument array.
 * @return null|string The output, if echo is set to false.
 */
function trm_list_authors($args = '') {
	global $trmdb;
	...
	$query_args = trm_array_slice_assoc( $args, array( 'orderby', 'order', 'number' ) );
	$query_args['fields'] = 'ids';
	$authors = get_users( $query_args );

	$author_count = array();
	foreach ( (array) $trmdb->get_results("SELECT DISTINCT post_author, COUNT(ID) AS count FROM $trmdb->posts WHERE post_type = 'post' AND " . get_private_posts_cap_sql( 'post' ) . " GROUP BY post_author") as $row )
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = trm_cache_get('is_multi_author', 'posts') ) ) {
		$rows = (array) $trmdb->get_col("SELECT DISTINCT post_author FROM $trmdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
		$is_multi_author = 1 < count( $rows ) ? 1 : 0;
		trm_cache_set('is_multi_author', $is_multi_author, 'posts');
	}
...
function __clear_multi_author_cache() {
	trm_cache_delete('is_multi_author', 'posts');
}
