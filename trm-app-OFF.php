<?php
/**
 * Atom Publishing Protocol support for Trnder
 *
 * @version 1.0.5-dc
 */

/**
 * Trnder is handling an Atom Publishing Protocol request.
 *
 * @var bool
 */
define('APP_REQUEST', true);

/** Set up Trnder environment */
require_once('./initiate.php');

/** Atom Publishing Protocol Class */
require_once(ABSPATH . TRMINC . '/atomlib.php');

/** Admin Image API for metadata updating */
require_once(ABSPATH . '/Backend-WeaprEcqaKejUbRq-trendr/includes/image.php');

$_SERVER['PATH_INFO'] = preg_replace( '/.*\/trm-app\.php/', '', $_SERVER['REQUEST_URI'] );
...
/**
 * Trnder AtomPub API implementation.
 *
 * @package Trnder
...
EOD;
	...
	function create_post() {
		...
		$trm_cats = get_categories(array('hide_empty' => false));

		$post_category = array();

		foreach ( $trm_cats as $cat ) {
		...
		$postID = trm_insert_post($post_data);
		if ( is_trm_error( $postID ) )
		...
		// could we clean up trm_set_post_categories or cache to not print
		// this could affect our ability to send back the right headers
		@trm_set_post_categories($postID, $post_category);
	...
	function create_attachment() {
		...
		$file = trm_upload_bits( $slug, NULL, $bits);

		log_app('trm_upload_bits returns:',print_r($file,true));

		$url = $file['url'];
		$file = $file['file'];

		do_action('trm_create_file_in_uploads', $file); // replicate
		...
		// Save the data
		$postID = trm_insert_attachment($attachment, $file);
	...
	function delete_attachment($postID) {
		...
		$location = get_post_meta($entry['ID'], '_trm_attached_file', true);
		$filetype = trm_check_filetype($location);
	...
	function put_file($postID) {
		...
		$result = trm_update_post($post_data);

		if ( !$result )
			$this->internal_error(__('Sorry, your entry could not be posted. Something wrong happened.'));

		trm_update_attachment_metadata( $postID, trm_generate_attachment_metadata( $postID, $location ) );
	...
	function set_current_entry($postID) {
		...
		$entry = trm_get_single_post($postID,ARRAY_A);
	...
	function get_feed($page = 1, $post_type = 'post') {
		global $post, $trm, $trm_query, $posts, $trmdb, $blog_id;
		...
		trm('posts_per_page=' . $count . '&offset=' . ($count * ($page-1) . '&orderby=modified'));

		$post = $GLOBALS['post'];
		$posts = $GLOBALS['posts'];
		$trm = $GLOBALS['trm'];
		$trm_query = $GLOBALS['trm_query'];
		$trmdb = $GLOBALS['trmdb'];
		$blog_id = (int) $GLOBALS['blog_id'];
		log_app('function',"query_posts(# " . print_r($trm_query, true) . "#)");

		log_app('function',"total_count(# $trm_query->max_num_pages #)");
		$last_page = $trm_query->max_num_pages;
...
EOD;
	...
	function auth_required($msg) {
		...
		nocache_headers();
		header('WWW-Authenticate: Basic realm="Trnder Atom Protocol"');
...
EOD;
	...
	function authenticate() {
		...
		if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
			...
			// See http://trac.trendr.org/ticket/7361
			list($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']) =
		...
		if (isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER['PHP_AUTH_PW'])) {
			...
			$user = trm_authenticate($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']);
			if ( $user && !is_trm_error($user) ) {
	...
	function process_conditionals() {
		...
		switch($this->params[0]) {
			case $this->ENTRY_PATH:
				global $post;
				$post = trm_get_single_post($this->params[1]);
				$trm_last_modified = get_post_modified_time('D, d M Y H:i:s', true);
				$post = NULL;
				break;
		...
		@header("Last-Modified: $trm_last_modified");
		@header("ETag: $trm_etag");
