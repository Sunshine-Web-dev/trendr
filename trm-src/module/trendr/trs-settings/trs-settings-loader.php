<?php
// Exit if accessed directly
if ( !defined( 'ABSPATH' ) ) exit;

class TRS_Settings_Component extends TRS_Component {

	/**
	 * Start the settings component creation process
	 *
	 * @since 1.5
	 */
	function __construct() {
		parent::start(
			'settings',
			__( 'Settings', 'trendr' ),
			TRS_PLUGIN_DIR
		);
	}

	/**
	 * Include files
	 *
	 * @global obj $trs
	 */
	function includes() {
		// Files to include
		$includes = array(
			'actions',
			'screens',
			'template',
			'functions',
		);

		parent::includes( $includes );
	}

	/**
	 * Setup globals
	 *
	 * The TRS_SETTINGS_SLUG constant is deprecated, and only used here for
	 * backwards compatibility.
	 *
	 * @since 1.5
	 * @global obj $trs
	 */
	function setup_globals() {
		global $trs;

		// Define a slug, if necessary
		if ( !defined( 'TRS_SETTINGS_SLUG' ) )
			define( 'TRS_SETTINGS_SLUG', $this->id );

		// All globals for settings component.
		$globals = array(
			'slug'          => TRS_SETTINGS_SLUG,
			'has_directory' => false,
		);

		parent::setup_globals( $globals );
	}

	/**
	 * Setup BuddyBar navigation
	 *
	 * @global obj $trs
	 */
	function setup_nav() {
		global $trs;

		// Add the settings navigation item
		$main_nav = array(
			'name'                    => __( 'Settings', 'trendr' ),
			'slug'                    => $this->slug,
			'position'                => 100,
			'show_for_displayed_user' => trs_core_can_edit_settings(),
			'screen_function'         => 'trs_core_screen_general_settings',
			'default_subnav_slug'     => 'general'
		);

		// Determine user to use
		if ( isset( $trs->displayed_user->domain ) )
			$user_domain = $trs->displayed_user->domain;
		elseif ( isset( $trs->loggedin_user->domain ) )
			$user_domain = $trs->loggedin_user->domain;
		else
			return;

		$settings_link = trailingslashit( $user_domain . $this->slug );

		// Add General Settings nav item
		$sub_nav[] = array(
			'name'            => __( 'General', 'trendr' ),
			'slug'            => 'general',
			'parent_url'      => $settings_link,
			'parent_slug'     => $this->slug,
			'screen_function' => 'trs_core_screen_general_settings',
			'position'        => 10,
			'user_has_access' => trs_core_can_edit_settings()
		);

		// Add Notifications nav item
		$sub_nav[] = array(
			'name'            => __( 'Notifications', 'trendr' ),
			'slug'            => 'notifications',
			'parent_url'      => $settings_link,
			'parent_slug'     => $this->slug,
			'screen_function' => 'trs_core_screen_notification_settings',
			'position'        => 20,
			'user_has_access' => trs_core_can_edit_settings()
		);

		// Add Delete Account nav item
		if ( !is_super_admin() && empty( $trs->site_options['trs-disable-account-deletion'] ) ) {
			$sub_nav[] = array(
				'name'            => __( 'Delete Account', 'trendr' ),
				'slug'            => 'delete-account',
				'parent_url'      => $settings_link,
				'parent_slug'     => $this->slug,
				'screen_function' => 'trs_core_screen_delete_account',
				'position'        => 90,
				'user_has_access' => trs_is_my_profile()
			);
		}

		parent::setup_nav( $main_nav, $sub_nav );
	}

	/**
	 * Set up the admin bar
	 *
	 * @global obj $trs
	 */
	function setup_admin_bar() {
		global $trs;

		// Prevent debug notices
		$trm_admin_nav = array();

		// Menus for logged in user
		if ( is_user_logged_in() ) {

			// Setup the logged in user variables
			$user_domain   = $trs->loggedin_user->domain;
			$settings_link = trailingslashit( $user_domain . $this->slug );

			// Add main Settings menu
			$trm_admin_nav[] = array(
				'parent' => $trs->my_account_menu_id,
				'id'     => 'my-account-' . $this->id,
				'title'  => __( 'Settings', 'trendr' ),
				'href'   => trailingslashit( $settings_link )
			);

			// General Account
			$trm_admin_nav[] = array(
				'parent' => 'my-account-' . $this->id,
				'id'     => 'my-account-' . $this->id . '-general',
				'title'  => __( 'General', 'trendr' ),
				'href'   => trailingslashit( $settings_link . 'general' )
			);

			// Notifications
			$trm_admin_nav[] = array(
				'parent' => 'my-account-' . $this->id,
				'id'     => 'my-account-' . $this->id . '-notifications',
				'title'  => __( 'Notifications', 'trendr' ),
				'href'   => trailingslashit( $settings_link . 'notifications' )
			);

			// Delete Account
			if ( !is_super_admin() && empty( $trs->site_options['trs-disable-account-deletion'] ) ) {
				$trm_admin_nav[] = array(
					'parent' => 'my-account-' . $this->id,
					'id'     => 'my-account-' . $this->id . '-delete-account',
					'title'  => __( 'Delete Account', 'trendr' ),
					'href'   => trailingslashit( $settings_link . 'delete-account' )
				);
			}
		}

		parent::setup_admin_bar( $trm_admin_nav );
	}
}
// Create the settingss component
$trs->settings = new TRS_Settings_Component();

?>
