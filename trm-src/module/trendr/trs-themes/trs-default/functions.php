 ...
 * filter hooks in trendr and trendr to change core functionality.
 *
 * The first function, trs_dtheme_setup(), sets up the theme by registering support
 * for various features in trendr, such as post thumbnails and navigation menus, and
 * for trendr, action buttons and javascript localisation.
 *
 * When using a child theme (see http://codex.trendr.org/Theme_Development, http://codex.trendr.org/Child_Themes
 ...
 *
 * For more information on hooks, actions, and filters, see http://codex.trendr.org/Plugin_API.
 *
 * @package trendr
...
/**
 * Sets up theme defaults and registers support for various trendr and trendr features.
 *
 ...
 *
 * @see http://codex.trendr.org/Function_Reference/trm_enqueue_script
 * @since 1.5
 */
 ...
 *
 * @see http://codex.trendr.org/Function_Reference/trm_enqueue_style
			...
			<div class="comment-meta">
				<p>
					<?php
						...
						printf( __( '<a href="%1$s" rel="nofollow">%2$s</a> said on <a href="%3$s"><span class="time">%4$s</span></a>', 'trendr' ), get_comment_author_url(), get_comment_author(), get_comment_link(), get_comment_date() );
					?>
				</p>
			</div>
...
/**
 * trm_nav_menu() callback from the main navigation in header.php
 *
 * Used when the custom menus haven't been configured.
 *
 * @global object $trs Global trendr settings object
 * @param array Menu arguments from trm_nav_menu()
 * @see trm_nav_menu()
 * @since 1.5
 */
...
if ( !function_exists( 'trs_dtheme_page_menu_args' ) ) :
/**
 * Get our trm_nav_menu() fallback, trs_dtheme_main_nav(), to show a home link.
 *
 * @param array $args Default values for trm_page_menu()
 * @see trm_page_menu()
 * @since 1.5
 */
function trs_dtheme_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'trm_page_menu_args', 'trs_dtheme_page_menu_args' );
endif;
...
if ( !function_exists( 'trs_dtheme_content_nav' ) ) :
/**
 * Display navigation to next/previous pages when applicable
 *
 * @global unknown $trm_query
 * @param string $nav_id DOM ID for this navigation
 * @since 1.5
 */
function trs_dtheme_content_nav( $nav_id ) {
	global $trm_query;

	if ( $trm_query->max_num_pages > 1 ) : ?>
		<div id="<?php echo $nav_id; ?>" class="navigation">
			<div class="alignleft"><?php next_posts_link( __( '&larr; Previous Entries', 'trendr' ) ); ?></div>
			<div class="alignright"><?php previous_posts_link( __( 'Next Entries &rarr;', 'trendr' ) ); ?></div>
		</div><!-- #<?php echo $nav_id; ?> -->
	<?php endif;
}
endif;
 ...
 * to body.js.
 *
 * This technique is borrowed from trendr, Backend-WeaprEcqaKejUbRq-trendr/admin-header.php.
 *
 * @package trendr
 * @since 1.5.1
