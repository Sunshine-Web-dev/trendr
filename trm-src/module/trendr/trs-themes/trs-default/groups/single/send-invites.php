<?php do_action( 'trs_before_group_send_invites_content' ) ?>

<?php if ( trs_get_total_friend_count( trs_loggedin_user_id() ) ) : ?>

	<form action="<?php trs_group_send_invite_form_action() ?>" method="post" id="send-invite-form" class="standard-form" role="main">
		...
		<div class="main-column">
			...
			<?php do_action( 'trs_before_group_send_invites_list' ) ?>

			<?php /* The ID 'friend-list' is important for AJAX support. */ ?>
			<ul id="friend-list" class="article-piece">
			<?php if ( trs_group_has_invites() ) : ?>
				...
				<?php while ( trs_group_invites() ) : trs_group_the_invite(); ?>
					...
					<li id="<?php trs_group_invite_item_id() ?>">
						...
						<?php do_action( 'trs_group_send_invites_item' ) ?>

						<div class="action">
							<a class="button remove" href="<?php trs_group_invite_user_remove_invite_url() ?>" id="<?php trs_group_invite_item_id() ?>"><?php _e( 'Remove Invite', 'trendr' ) ?></a>

							<?php do_action( 'trs_group_send_invites_item_action' ) ?>
						</div>
					</li>
			...
			<?php do_action( 'trs_after_group_send_invites_list' ) ?>
		...
		<div class="submit">
			<input type="submit" name="submit" id="submit" value="<?php _e( 'Send Invites', 'trendr' ) ?>" />
		</div>

		<?php trm_nonce_field( 'groups_send_invites', '_key_send_invites') ?>

		<?php /* This is important, don't forget it */ ?>
		<input type="hidden" name="group_id" id="group_id" value="<?php trs_group_id() ?>" />
...
<?php do_action( 'trs_after_group_send_invites_content' ) ?>
