...
function trs_forums_load_btrsress() {
	global $trs, $trmdb, $trm_roles, $current_user, $trm_users_object;
	global $bb, $bbdb, $bb_table_prefix, $bb_current_user;
	global $bb_roles, $trm_taxonomy_object, $bb_queries;
	...
	define( 'BB_PATH', TRS_PLUGIN_DIR . '/trs-forums/btrsress/' );
	define( 'BACKPRESS_PATH', TRS_PLUGIN_DIR . '/trs-forums/btrsress/bb-includes/backpress/' );
	define( 'BB_URL', TRS_PLUGIN_URL . '/trs-forums/btrsress/' );
	...
	$trm_users_object = new TRS_Forums_BB_Auth;

	if ( !isset( $trm_taxonomy_object ) )
		$trm_taxonomy_object = new BB_Taxonomy( $bbdb );

	$trm_taxonomy_object->register_taxonomy( 'bb_topic_tag', 'bb_topic' );

	// Set a site id if there isn't one already
	if ( !isset( $bb->site_id ) )
		$bb->site_id = trs_get_root_blog_id();
	...
	if ( !$tables_installed = (boolean) $bbdb->get_results( 'DESCRIBE `' . $bbdb->forums . '`;', ARRAY_A ) ) {
		...
		trs_bb_dbDelta( str_replace( ' IF NOT EXISTS', '', str_replace( '`', '', $bb_queries ) ) );
		...
		foreach ( (array)$site_admins as $site_admin )
			update_user_meta( trs_core_get_userid( $site_admin ), $bb_table_prefix . 'capabilities', array( 'keymaster' => true ) );
...
add_action( 'btrsress_init', 'trs_forums_load_btrsress' );

/* TRM to bbP wrapper functions */
...
/**
 * bbPress needs the DB class to be TRSDB, but we want to use TRMDB, so we can
 * extend it and use this.
 *
 * The class is pluggable, so that plugins that swap out TRMDB with a custom
 * database class (such as HyperDB and SharDB) can provide their own versions
 * of TRSDB which extend the appropriate base class.
 */
if ( ! class_exists( 'TRSDB' ) ) :
	class TRSDB extends TRMDB {
		var $db_servers = array();

		function TRSDB( $dbuser, $dtrsassword, $dbname, $dbhost ) {
			$this->__construct( $dbuser, $dtrsassword, $dbname, $dbhost );
		}
		 ...
		 *
		 * Overriden here to work around differences between bbPress', and trendr', implementation differences.
		 * In particular, when trendr tries to run bbPress' SQL installation script, the collation check always
		 * failed. The capability is long supported by trendr' minimum required MySQL version, so this is safe.
		 */
		...
		function init( $args ) {
			...
			return trm_parse_args( $args, $defaults );
		}
...
endif; // class_exists( 'TRSDB' )

/* BTRSress needs this function to convert vars */
...
/**
 * Copied from Backend-WeaprEcqaKejUbRq-trendr/includes/upgrade.php, this will take care of creating
 * the bbPress stand-alone tables without loading a conflicting TRM Admin.
 *
 * @param unknown_type $queries
 * @param unknown_type $execute
 * @return unknown
 */
function trs_bb_dbDelta($queries, $execute = true) {
	global $trmdb;
			...
			if ( array_key_exists(strtolower($table), $cqueries) ) {
				...
				if ($tableindices) {
					...
					foreach ($tableindices as $tableindex) {
						...
						$index_ary[$keyname]['columns'][] = array('fieldname' => $tableindex->Column_name, 'sutrsart' => $tableindex->Sub_part);
					...
					foreach ($index_ary as $index_name => $index_data) {
						...
						foreach ($index_data['columns'] as $column_data) {
							...
							if ($column_data['sutrsart'] != '') {
								$index_columns .= '('.$column_data['sutrsart'].')';
							}
						}
