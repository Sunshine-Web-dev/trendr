...
Class TRS_XProfile_Group {
	...
	function trs_xprofile_group( $id = null ) {
		$this->__construct( $id );
	}
	...
	function populate( $id ) {
		...
		$sql = $trmdb->prepare( "SELECT * FROM {$trs->profile->table_name_groups} WHERE id = %d", $id );

		if ( !$group = $trmdb->get_row( $sql ) )
			return false;
	...
	function save() {
		...
		if ( $this->id )
			$sql = $trmdb->prepare( "UPDATE {$trs->profile->table_name_groups} SET name = %s, description = %s WHERE id = %d", $this->name, $this->description, $this->id );
		else
			$sql = $trmdb->prepare( "INSERT INTO {$trs->profile->table_name_groups} (name, description, can_delete) VALUES (%s, %s, 1)", $this->name, $this->description );

		if ( is_trm_error( $trmdb->query( $sql ) ) )
			return false;
	...
	function delete() {
		...
		/* Delete field group */
		if ( !$trmdb->query( $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_groups} WHERE id = %d", $this->id ) ) ) {
			return false;
		} else {
			/* Remove the group's fields. */
			if ( TRS_XProfile_Field::delete_for_group( $this->id ) ) {
				/* Remove profile data for the groups fields */
				for ( $i = 0, $count = count( $this->fields ); $i < $count; ++$i ) {
					TRS_XProfile_ProfileData::delete_for_field( $this->fields[$i]->id );
				}
			}
	 ...
	 * get()
	 *
	 * Populates the TRS_XProfile_Group object with profile field groups, fields, and field data
	 *
	...
	function get( $args = '' ) {
		...
		if ( $profile_group_id )
			$where_sql = $trmdb->prepare( 'WHERE g.id = %d', $profile_group_id );
		elseif ( $exclude_groups )
			$where_sql = $trmdb->prepare( "WHERE g.id NOT IN ({$exclude_groups})");

		if ( $hide_empty_groups )
			$groups = $trmdb->get_results( $trmdb->prepare( "SELECT DISTINCT g.* FROM {$trs->profile->table_name_groups} g INNER JOIN {$trs->profile->table_name_fields} f ON g.id = f.group_id {$where_sql} ORDER BY g.group_order ASC" ) );
		else
			$groups = $trmdb->get_results( $trmdb->prepare( "SELECT DISTINCT g.* FROM {$trs->profile->table_name_groups} g {$where_sql} ORDER BY g.group_order ASC" ) );
		...
		$exclude_fields_sql = '';
		if ( $exclude_fields )
			$exclude_fields_sql = $trmdb->prepare( "AND id NOT IN ({$exclude_fields})" );

		// Fetch the fields
		$fields = $trmdb->get_results( $trmdb->prepare( "SELECT id, name, description, type, group_id, is_required FROM {$trs->profile->table_name_fields} WHERE group_id IN ( {$group_ids} ) AND parent_id = 0 {$exclude_fields_sql} ORDER BY field_order" ) );
		...
		if ( $fetch_field_data ) {
			...
			if ( !empty( $field_ids ) )
				$field_data = $trmdb->get_results( $trmdb->prepare( "SELECT field_id, value FROM {$trs->profile->table_name_data} WHERE field_id IN ( {$field_ids_sql} ) AND user_id = %d", $user_id ) );
			...
			if ( !empty( $field_data ) && !is_trm_error( $field_data ) ) {
	...
	function admin_validate() {
		...
		if ( empty( $_POST['group_name'] ) ) {
			$message = __( 'Please make sure you give the group a name.', 'trendr' );
			return false;
		} else {
			return true;
		}
	...
	function update_position( $field_group_id, $position ) {
		...
		return $trmdb->query( $trmdb->prepare( "UPDATE {$trs->profile->table_name_groups} SET group_order = %d WHERE id = %d", $position, $field_group_id ) );
	}
	...
	*/
	...
	function render_admin_form() {
		...
		if ( !$this->id ) {
			$title	= __( 'Add New Field Group', 'trendr' );
			$action	= "admin.php?page=trs-profile-setup&amp;mode=add_group";
			$button	= __( 'Create Field Group', 'trendr' );
		} else {
			$title = __( 'Edit Field Group', 'trendr' );
			$action = "admin.php?page=trs-profile-setup&amp;mode=edit_group&amp;group_id=" . $this->id;
			$button	= __( 'Save Changes', 'trendr' );
		}
?>
		<div class="wrap">
			...
			<p><?php _e( 'Fields marked * are required', 'trendr' ) ?></p>
...
<?php endif; ?>
			<div id="poststuff">
				<form action="<?php echo esc_attr( $action ); ?>" method="post">
					<div id="titlediv">
						<h3><label for="group_name"><?php _e( "Field Group Title", 'trendr') ?> *</label></h3>
						<div id="titlewrap">
...
<?php if ( '0' != $this->can_delete ) : ?>
					<div id="titlediv">
						<h3><label for="description"><?php _e( "Group Description", 'trendr' ); ?></label></h3>
						<div id="titlewrap">
...
<?php endif; ?>
					...
					<p class="submit">
						...
						<?php _e( 'or', 'trendr' ); ?> <a href="admin.php?page=trs-profile-setup" class="deletion"><?php _e( 'Cancel', 'trendr' ); ?></a>
					</p>
				</form>
			</div>
		</div>
...
Class TRS_XProfile_Field {
	...
	function trs_xprofile_field( $id = null, $user_id = null, $get_data = true ) {
	...
	function populate( $id, $user_id, $get_data ) {
		global $trmdb, $userdata, $trs;

		$user_id = 0;
		if ( is_null( $user_id ) )
			$user_id = $userdata->ID;

		$sql = $trmdb->prepare( "SELECT * FROM {$trs->profile->table_name_fields} WHERE id = %d", $id );

		if ( $field = $trmdb->get_row( $sql ) ) {
			$this->id               = $field->id;
	...
	function delete() {
		...
		if ( !$trmdb->query( $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_fields} WHERE id = %d OR parent_id = %d", $this->id, $this->id ) ) )
			return false;

		/* delete the data in the DB for this field */
		TRS_XProfile_ProfileData::delete_for_field( $this->id );
	...
	function save() {
		...
		if ( $this->id != null )
			$sql = $trmdb->prepare( "UPDATE {$trs->profile->table_name_fields} SET group_id = %d, parent_id = 0, type = %s, name = %s, description = %s, is_required = %d, order_by = %s, field_order = %d WHERE id = %d", $this->group_id, $this->type, $this->name, $this->description, $this->is_required, $this->order_by, $this->field_order, $this->id );
		else
			$sql = $trmdb->prepare( "INSERT INTO {$trs->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, order_by, field_order ) VALUES (%d, %d, %s, %s, %s, %d, %s, %d )", $this->group_id, $this->parent_id, $this->type, $this->name, $this->description, $this->is_required, $this->order_by, $this->field_order );
		 ...
		 */
		if ( $trmdb->query( $sql ) !== null ) {
			if ( $this->id )
				$field_id = $this->id;
			else
				$field_id = $trmdb->insert_id;
			...
			if ( 'radio' == $this->type || 'selectbox' == $this->type || 'checkbox' == $this->type || 'multiselectbox' == $this->type ) {
				...
				if ( $options ) {
					foreach ( (array)$options as $option_key => $option_value ) {
						...
						if ( '' != $option_value ) {
							if ( !$trmdb->query( $trmdb->prepare( "INSERT INTO {$trs->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, option_order, is_default_option) VALUES (%d, %d, 'option', %s, '', 0, %d, %d)", $this->group_id, $parent_id, $option_value, $counter, $is_default ) ) )
								return false;
						}
	...
	function get_field_data( $user_id ) {
		return new TRS_XProfile_ProfileData( $this->id, $user_id );
	}
	...
	function get_children( $for_editing = false ) {
		...
		$sql = $trmdb->prepare( "SELECT * FROM {$trs->profile->table_name_fields} WHERE parent_id = %d AND group_id = %d $sort_sql", $parent_id, $this->group_id );

		if ( !$children = $trmdb->get_results( $sql ) )
			return false;

		return apply_filters( 'trs_xprofile_field_get_children', $children );
	}

	function delete_children() {
		global $trmdb, $trs;

		$sql = $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_fields} WHERE parent_id = %d", $this->id );
	...
	function get_type( $field_id ) {
		...
		if ( $field_id ) {
			$sql = $trmdb->prepare( "SELECT type FROM {$trs->profile->table_name_fields} WHERE id = %d", $field_id );

			if ( !$field_type = $trmdb->get_var( $sql ) )
				return false;
	...
	function delete_for_group( $group_id ) {
		...
		if ( $group_id ) {
			$sql = $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_fields} WHERE group_id = %d", $group_id );

			if ( $trmdb->get_var( $sql ) === false )
				return false;
	...
	function get_id_from_name( $field_name ) {
		...
		if ( empty( $trs->profile->table_name_fields ) || !isset( $field_name ) )
			return false;

		return $trmdb->get_var( $trmdb->prepare( "SELECT id FROM {$trs->profile->table_name_fields} WHERE name = %s", $field_name ) );
	}
	...
	function update_position( $field_id, $position, $field_group_id ) {
		...
		if ( $parent = $trmdb->query( $trmdb->prepare( "UPDATE {$trs->profile->table_name_fields} SET field_order = %d, group_id = %d WHERE id = %d", $position, $field_group_id, $field_id ) ) ) {;
			/* Update any children of this $field_id */
			$children = $trmdb->query( $trmdb->prepare( "UPDATE {$trs->profile->table_name_fields} SET group_id = %d WHERE parent_id = %d", $field_group_id, $field_id ) );
...
?>
			<div id="<?php echo $type; ?>" class="options-box" style="<?php if ( $this->type != $type ) { ?>display: none;<?php } ?> margin-left: 15px;">
				<h4><?php _e( 'Please enter options for this Field:', 'trendr' ); ?></h4>
				<p><label for="sort_order_<?php echo $type; ?>"><?php _e( 'Order By:', 'trendr' ); ?></label>
					<select name="sort_order_<?php echo $type; ?>" id="sort_order_<?php echo $type; ?>" >
						<option value="default" <?php if ( 'default' == $this->order_by ) {?> selected="selected"<?php } ?> ><?php _e( 'Order Entered', 'trendr' ); ?></option>
						<option value="asc" <?php if ( 'asc' == $this->order_by ) {?> selected="selected"<?php } ?>><?php _e( 'Name - Ascending', 'trendr' ); ?></option>
						<option value="desc" <?php if ( 'desc' == $this->order_by ) {?> selected="selected"<?php } ?>><?php _e( 'Name - Descending', 'trendr' ); ?></option>
					</select>
<?php
				...
				if ( !empty( $options ) ) {
					for ( $i = 0, $count = count( $options ); $i < $count; ++$i ) {
						...
						if ( 'multiselectbox' == $type || 'checkbox' == $type )
							$default_name = '[' . $j . ']';
?>
						<p><?php _e('Option', 'trendr'); ?> <?php echo $j; ?>:
						   ...
						   <input type="<?php echo $default_input; ?>" name="isDefault_<?php echo $type; ?>_option<?php echo $default_name; ?>" <?php if ( (int) $options[$i]->is_default_option ) {?> checked="checked"<?php } ?> " value="<?php echo $j; ?>" /> <?php _e( 'Default Value', 'trendr' ); ?>
<?php
					if ( $j != 1 && $options[$i]->id != -1 ) : ?>
							<a href="admin.php?page=trs-profile-setup&amp;mode=delete_option&amp;option_id=<?php echo $options[$i]->id ?>" class="ajax-option-delete" id="delete-<?php echo $options[$i]->id; ?>">[x]</a>
<?php				endif; ?>
						</p>
<?php				} /* end for */ ?>
...
?>
					...
					<p><?php _e( 'Option', 'trendr' ); ?> 1: <input type="text" name="<?php echo $type; ?>_option[1]" id="<?php echo $type; ?>_option1" />
					<input type="<?php echo $default_input; ?>" name="isDefault_<?php echo $type; ?>_option<?php echo $default_name; ?>" id="isDefault_<?php echo $type; ?>_option" value="1" /> <?php _e( 'Default Value', 'trendr' ); ?>
...
<?php			} /* end if */ ?>
				...
				<p><a href="javascript:add_option('<?php echo $type; ?>')"><?php _e('Add Another Option', 'trendr'); ?></a></p>
			</div>
	...
	function render_admin_form( $message = '' ) {
		if ( !$this->id ) {
			$title				= __( 'Add Field', 'trendr' );
			$action				= "admin.php?page=trs-profile-setup&amp;group_id=" . $this->group_id . "&amp;mode=add_field#tabs-" . $this->group_id;
			...
			$title				= __( 'Edit Field', 'trendr' );
			$action				= "admin.php?page=trs-profile-setup&amp;mode=edit_field&amp;group_id=" . $this->group_id . "&amp;field_id=" . $this->id . "#tabs-" . $this->group_id;
		}
?>
	<div class="wrap">
...
<?php	} ?>
		...
		<form action="<?php echo $action; ?>" method="post">
			<div id="poststuff">
				<div id="titlediv">
					<h3><label for="title"><?php _e( 'Field Title', 'trendr' ); ?> *</label></h3>
					<div id="titlewrap">
...
?>
				<div id="titlediv">
					<h3><label for="description"><?php _e("Field Description", 'trendr'); ?></label></h3>
					<div id="titlewrap">
				...
				<div id="titlediv">
					<h3><label for="required"><?php _e( "Is This Field Required?", 'trendr' ); ?> *</label></h3>
					<select name="required" id="required" style="width: 30%">
						<option value="0"<?php if ( $this->is_required == '0' ) { ?> selected="selected"<?php } ?>><?php _e( 'Not Required', 'trendr' ); ?></option>
						<option value="1"<?php if ( $this->is_required == '1' ) { ?> selected="selected"<?php } ?>><?php _e( 'Required', 'trendr' ); ?></option>
					</select>
				</div>

				<div id="titlediv">
					<h3><label for="fieldtype"><?php _e("Field Type", 'trendr'); ?> *</label></h3>
					<select name="fieldtype" id="fieldtype" onchange="show_options(this.value)" style="width: 30%">
						<option value="textbox"<?php if ( $this->type == 'textbox' ) {?> selected="selected"<?php } ?>><?php _e( 'Text Box', 'trendr' ); ?></option>
						<option value="textarea"<?php if ( $this->type == 'textarea' ) {?> selected="selected"<?php } ?>><?php _e( 'Multi-line Text Box', 'trendr' ); ?></option>
						<option value="datebox"<?php if ( $this->type == 'datebox' ) {?> selected="selected"<?php } ?>><?php _e( 'Date Selector', 'trendr' ); ?></option>
						<option value="radio"<?php if ( $this->type == 'radio' ) {?> selected="selected"<?php } ?>><?php _e( 'Radio Buttons', 'trendr' ); ?></option>
						<option value="selectbox"<?php if ( $this->type == 'selectbox' ) {?> selected="selected"<?php } ?>><?php _e( 'Drop Down Select Box', 'trendr' ); ?></option>
						<option value="multiselectbox"<?php if ( $this->type == 'multiselectbox' ) {?> selected="selected"<?php } ?>><?php _e( 'Multi Select Box', 'trendr' ); ?></option>
						<option value="checkbox"<?php if ( $this->type == 'checkbox' ) {?> selected="selected"<?php } ?>><?php _e( 'Checkboxes', 'trendr' ); ?></option>
					</select>
				</div>
...
<?php	} ?>
				<p class="submit">
					...
					<input type="submit" value="<?php _e( 'Save', 'trendr' ); ?>" name="saveField" id="saveField" style="font-weight: bold" class="button-primary" />
					<?php _e( 'or', 'trendr' ); ?> <a href="admin.php?page=trs-profile-setup" class="deletion"><?php _e( 'Cancel', 'trendr' ); ?></a>
				</p>

			</div>

			<?php trm_nonce_field( 'xprofile_delete_option' ); ?>
...
<?php
	...
	function admin_validate() {
		...
		if ( '' == $_POST['title'] || '' == $_POST['required'] || '' == $_POST['fieldtype'] ) {
			$message = __( 'Please make sure you fill out all required fields.', 'trendr' );
			return false;
		} else if ( empty( $_POST['field_file'] ) && $_POST['fieldtype'] == 'radio' && empty( $_POST['radio_option'][1] ) ) {
			$message = __( 'Radio button field types require at least one option. Please add options below.', 'trendr' );
			return false;
		} else if ( empty( $_POST['field_file'] ) && $_POST['fieldtype'] == 'selectbox' && empty( $_POST['selectbox_option'][1] ) ) {
			$message = __( 'Select box field types require at least one option. Please add options below.', 'trendr' );
			return false;
			...
			$message = __( 'Checkbox field types require at least one option. Please add options below.', 'trendr' );
			return false;
		} else {
			return true;
		}
...
Class TRS_XProfile_ProfileData {
	var $id;
	var $user_id;
	var $field_id;
	var $value;
	...
	function trs_xprofile_profiledata( $field_id = null, $user_id = null ) {
	...
	function populate( $field_id, $user_id )  {
		...
		$sql = $trmdb->prepare( "SELECT * FROM {$trs->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $field_id, $user_id );

		if ( $profiledata = $trmdb->get_row( $sql ) ) {
	...
	function exists() {
		...
		$retval = $trmdb->get_row( $trmdb->prepare( "SELECT id FROM {$trs->profile->table_name_data} WHERE user_id = %d AND field_id = %d", $this->user_id, $this->field_id ) );
	...
	function is_valid_field() {
		...
		$retval = $trmdb->get_row( $trmdb->prepare( "SELECT id FROM {$trs->profile->table_name_fields} WHERE id = %d", $this->field_id ) );
	...
	function save() {
		...
		if($this->field_id==2){
					...
					$url = "https://maps.googleapis.com/maps/api/geocode/json?address={$address}&key={$trschk['apikey']}";
		        	...
		        	$location['address']=$this->value;
		        	$location['lat']=$lati;
		        	$location['long']=$longi;
		        	trs_xprofile_update_meta(get_current_user_id(),$this->field_id,'data','location',$location);
				}
		$this->value        = apply_filters( 'xprofile_data_value_before_save',        $this->value,           $this->id );

		$this->last_updated = apply_filters( 'xprofile_data_last_updated_before_save', trs_core_current_time(), $this->id );
		...
		if ( $this->is_valid_field() ) {
			if ( $this->exists() && !empty( $this->value ) && strlen( trim( $this->value ) ) ) {
				$result   = $trmdb->query( $trmdb->prepare( "UPDATE {$trs->profile->table_name_data} SET value = %s, last_updated = %s WHERE user_id = %d AND field_id = %d", $this->value, $this->last_updated, $this->user_id, $this->field_id ) );
				...
				$result   = $trmdb->query( $trmdb->prepare("INSERT INTO {$trs->profile->table_name_data} (user_id, field_id, value, last_updated) VALUES (%d, %d, %s, %s)", $this->user_id, $this->field_id, $this->value, $this->last_updated ) );
	...
	function delete() {
		...
		if ( !$trmdb->query( $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $this->field_id, $this->user_id ) ) )
			return false;
	...
	/**
	 * TRS_XProfile_ProfileData::get_all_for_user()
	 *
	...
	function get_all_for_user( $user_id ) {
		...
		$results      = $trmdb->get_results( $trmdb->prepare( "SELECT g.id as field_group_id, g.name as field_group_name, f.id as field_id, f.name as field_name, f.type as field_type, d.value as field_data, u.user_login, u.user_nicename, u.user_email FROM {$trs->profile->table_name_groups} g LEFT JOIN {$trs->profile->table_name_fields} f ON g.id = f.group_id INNER JOIN {$trs->profile->table_name_data} d ON f.id = d.field_id LEFT JOIN {$trmdb->users} u ON d.user_id = u.ID WHERE d.user_id = %d AND d.value != ''", $user_id ) );
		$profile_data = array();
	...
	function get_value_byid( $field_id, $user_ids = null ) {
		...
		if ( !$user_ids )
			$user_ids = $trs->displayed_user->id;

		if ( is_array( $user_ids ) ) {
			$user_ids = implode( ',', (array) $user_ids );
			$data = $trmdb->get_results( $trmdb->prepare( "SELECT user_id, value FROM {$trs->profile->table_name_data} WHERE field_id = %d AND user_id IN ({$user_ids})", $field_id ) );
		} else {
			$data = $trmdb->get_var( $trmdb->prepare( "SELECT value FROM {$trs->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $field_id, $user_ids ) );
		}
	...
	function get_value_byfieldname( $fields, $user_id = null ) {
		...
		if ( is_array( $fields ) ) {
			for ( $i = 0, $count = count( $fields ); $i < $count; ++$i ) {
				if ( $i == 0 )
					$field_sql .= $trmdb->prepare( "AND ( f.name = %s ", $fields[$i] );
				else
					$field_sql .= $trmdb->prepare( "OR f.name = %s ", $fields[$i] );
			}

			$field_sql .= ')';
		} else {
			$field_sql .= $trmdb->prepare( "AND f.name = %s", $fields );
		}

		$sql = $trmdb->prepare( "SELECT d.value, f.name FROM {$trs->profile->table_name_data} d, {$trs->profile->table_name_fields} f WHERE d.field_id = f.id AND d.user_id = %d AND f.parent_id = 0 $field_sql", $user_id );

		if ( !$values = $trmdb->get_results( $sql ) )
			return false;
	...
	function delete_for_field( $field_id ) {
		...
		if ( !$trmdb->query( $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_data} WHERE field_id = %d", $field_id ) ) )
			return false;
	...
	function get_last_updated( $user_id ) {
		...
		$last_updated = $trmdb->get_var( $trmdb->prepare( "SELECT last_updated FROM {$trs->profile->table_name_data} WHERE user_id = %d ORDER BY last_updated LIMIT 1", $user_id ) );
	...
	function delete_data_for_user( $user_id ) {
		...
		return $trmdb->query( $trmdb->prepare( "DELETE FROM {$trs->profile->table_name_data} WHERE user_id = %d", $user_id ) );
	}
	...
	function get_random( $user_id, $exclude_fullname ) {
		...
		if ( $exclude_fullname )
			$exclude_sql = $trmdb->prepare( " AND pf.id != 1" );

		return $trmdb->get_results( $trmdb->prepare( "SELECT pf.type, pf.name, pd.value FROM {$trs->profile->table_name_data} pd INNER JOIN {$trs->profile->table_name_fields} pf ON pd.field_id = pf.id AND pd.user_id = %d {$exclude_sql} ORDER BY RAND() LIMIT 1", $user_id ) );
	}
	...
	function get_fullname( $user_id = 0 ) {
		...
		$field_name = trs_xprofile_fullname_field_name();
