...
function trs_xprofile_delete_meta( $user_id,$object_id, $object_type, $meta_key = false, $meta_value = false ) {
	global $trmdb, $trs;
	...
	if ( !$meta_key )
		$trmdb->query( $trmdb->prepare( "DELETE FROM " . $trs->profile->table_name_meta . " WHERE user_id = %d AND object_id = %d AND object_type = %s", $object_id, $object_type ) );
	else if ( $meta_value )
		$trmdb->query( $trmdb->prepare( "DELETE FROM " . $trs->profile->table_name_meta . " WHERE user_id = %d AND object_id = %d AND object_type = %s AND meta_key = %s AND meta_value = %s", $object_id, $object_type, $meta_key, $meta_value ) );
	else
		$trmdb->query( $trmdb->prepare( "DELETE FROM " . $trs->profile->table_name_meta . " WHERE user_id = %d object_id = %d AND object_type = %s AND meta_key = %s", $object_id, $object_type, $meta_key ) );

	// Delete the cached object
	trm_cache_delete( 'trs_xprofile_meta_' . $user_id.'_'.$object_type . '_' . $object_id . '_' . $meta_key, 'trs' );

	return true;
}

function trs_xprofile_get_meta( $user_id,$object_id, $object_type, $meta_key = '') {
	global $trmdb, $trs;
	...
	if ( !empty( $meta_key ) ) {
		...
		if ( !$metas = trm_cache_get( 'trs_xprofile_meta_' . $user_id.'_'.$object_type . '_' . $object_id . '_' . $meta_key, 'trs' ) ) {
			$metas = $trmdb->get_col( $trmdb->prepare( "SELECT meta_value FROM " . $trs->profile->table_name_meta . " WHERE user_id= %d AND object_id = %d AND object_type = %s AND meta_key = %s", $user_id,$object_id, $object_type, $meta_key ) );
			trm_cache_set( 'trs_xprofile_meta_' . $user_id.'_'.$object_type . '_' . $object_id . '_' . $meta_key, $metas, 'trs' );
		}
	} else {
		$metas = $trmdb->get_col( $trmdb->prepare( "SELECT meta_value FROM " . $trs->profile->table_name_meta . " WHERE user_id= %d AND object_id = %d AND object_type = %s", $user_id,$object_id, $object_type ) );
	}
...
function trs_xprofile_update_meta( $user_id,$object_id, $object_type, $meta_key, $meta_value ) {
	global $trmdb, $trs;
	...
	$cur = $trmdb->get_row( $trmdb->prepare( "SELECT * FROM " . $trs->profile->table_name_meta . " WHERE user_id = %d AND object_id = %d AND object_type = %s AND meta_key = %s", $user_id,$object_id, $object_type, $meta_key ) );

	if ( empty( $cur ) )
		$trmdb->query( $trmdb->prepare( "INSERT INTO " . $trs->profile->table_name_meta . " ( user_id,object_id, object_type, meta_key, meta_value ) VALUES ( %d, %d, %s, %s, %s )", $user_id,$object_id, $object_type,  $meta_key, $meta_value ) );
	else if ( $cur->meta_value != $meta_value )
		$trmdb->query( $trmdb->prepare( "UPDATE " . $trs->profile->table_name_meta . " SET meta_value = %s WHERE user_id = %d AND object_id = %d AND object_type = %s AND meta_key = %s", $meta_value, $user_id,$object_id, $object_type, $meta_key ) );
	else
		return false;

	// Update the cached object and recache
	trm_cache_set( 'trs_xprofile_meta_' . $user_id.'_'.$object_type . '_' . $object_id . '_' . $meta_key, $meta_value, 'trs' );

	return true;
}

function trs_xprofile_update_fieldgroup_meta( $user_id,$field_group_id, $meta_key, $meta_value ) {
	return trs_xprofile_update_meta( $user_id,$field_group_id, 'group', $meta_key, $meta_value );
}

function trs_xprofile_update_field_meta( $user_id,$field_id, $meta_key, $meta_value ) {
	return trs_xprofile_update_meta( $user_id,$field_id, 'field', $meta_key, $meta_value );
}

function trs_xprofile_update_fielddata_meta( $user_id,$field_data_id, $meta_key, $meta_value ) {
	return trs_xprofile_update_meta( $user_id,$field_data_id, 'data', $meta_key, $meta_value );
}
