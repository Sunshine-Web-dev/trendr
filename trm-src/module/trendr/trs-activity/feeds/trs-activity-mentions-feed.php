...
<rss version="2.0"
	...
	<?php do_action('trs_activity_mentions_feed'); ?>
>

<channel>
	<title><?php trs_site_name() ?> | <?php echo $trs->displayed_user->fullname; ?> | <?php _e( 'Mentions', 'trendr' ) ?></title>
	<atom:link href="<?php self_link(); ?>" rel="self" type="application/rss+xml" />
	<link><?php echo home_url( trs_get_activity_root_slug() . '/#mentions/' ) ?></link>
	<description><?php echo $trs->displayed_user->fullname; ?> - <?php _e( 'Mentions', 'trendr' ) ?></description>
	...
	<?php do_action('trs_activity_mentions_feed_head'); ?>

	<?php if ( trs_has_activities( 'max=50&display_comments=stream&search_terms=@' . trs_core_get_username( $trs->displayed_user->id, $trs->displayed_user->userdata->user_nicename, $trs->displayed_user->userdata->user_login ) ) ) : ?>
		<?php while ( trs_activities() ) : trs_the_activity(); ?>
			<item>
				...
				<?php do_action('trs_activity_mentions_feed_item'); ?>
			</item>
		<?php endwhile; ?>
