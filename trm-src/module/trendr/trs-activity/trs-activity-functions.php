...
function trs_activity_add_user_favorite( $activity_id, $user_id = 0 ) {
	...
	// Update user meta
	trs_update_user_meta( $user_id, 'trs_favorite_activities', $my_favs );

	// Update activity meta counts
	//if ( true === trs_activity_update_meta( $activity_id, 'favorite_count', $fav_count ) ) {
	if ( trs_activity_update_meta( $activity_id, 'favorite_count', $fav_count ) ) {
		// Execute additional code
		    //$trs_activity_add_user_favorite = $trs_activity_add_user_favorite ?: array();
	...
	if ( ! empty( $fav_count ) ) {
		...
		if ( trs_activity_update_meta( $activity_id, 'favorite_count', (int) $fav_count - 1 ) ) {
			...
			if ( trs_update_user_meta( $user_id, 'trs_favorite_activities', $my_favs ) ) {
				...
				/**
				 * Fires if trs_update_user_meta() is successful and before returning a true value for success.
				 *
				 * @since 1.2.1
				 *
...
function trs_activity_add( $args = '' ) {
	...
	$defaults = array(
		...
		'content'           => '',    // Optional: The content of the activity item e.g. "trendr is awesome guys!"
...
/**
 * Take content, remove all images and replace them with one fullnail image.
 *
 * @since 1.2.0
 *
 ...
 *
 * @uses esc_attr()
 * @uses apply_filters() To call the 'trs_activity_fullnail_content_images' hook
 *
 * @return string $content The content with images stripped and replaced with a single full.
 */
function trs_activity_fullnail_content_images( $content, $link = false ) {
	global $post;
	...
	if ( !empty( $matches ) && !empty( $matches[0] ) ) {
		...
		if ( !empty( $src ) ) {
			...
			$image = '<img src="' . esc_attr( $src ) . '" width="' . $new_width . '" height="' . $new_height . '" alt="' . __( 'Thumbnail', 'trendr' ) . '" class="align-left fullnail" />';
	...
	return apply_filters( 'trs_activity_fullnail_content_images', $content, $matches );
}
