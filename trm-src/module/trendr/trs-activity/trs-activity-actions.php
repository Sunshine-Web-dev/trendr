...
function trs_activity_action_post_update() {
	...
	// Get activity info
	$content = apply_filters( 'trs_activity_post_update_content', $_POST['whats-new']             );
	$object  = apply_filters( 'trs_activity_post_update_object',  $_POST['whats-new-post-object'] );
	$item_id = apply_filters( 'trs_activity_post_update_item_id', $_POST['whats-new-post-in']     );

	// No activity content so provide feedback and redirect
	if ( empty( $content ) ) {
		trs_core_add_message( __( 'You have not entered any content to share!', 'trendr' ), 'error' );
		trs_core_redirect( trm_get_referer() );
	}
	...
	if ( !empty( $activity_id ) )
		...
		trs_core_add_message( __( 'There was an error when posting your data, please try again.', 'trendr' ), 'error' );
