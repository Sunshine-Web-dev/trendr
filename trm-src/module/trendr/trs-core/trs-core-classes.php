...
class TRS_Core_User {
	...
	function populate() {
		...
		$this->last_active  = trs_core_get_last_activity( trs_get_user_meta( $this->id, '', true ), __( 'active %s', 'trendr' ) );
	}
...
//Removed to enable rootdefine ( 'TRS_ENABLE_ROOT_PROFILES', true ); on many users 
// members to load fast// 1 second delay removed

		//$sql['select_main'] = "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.display_name, u.user_email";
		$sql['select_main'] = "SELECT DISTINCT u.ID as id, u.user_registered,u.user_email";

		//if ( 'active' == $type || 'online' == $type || 'newest' == $type  )
			//removed last_active for this " "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.display_name, u.user_email , um.meta_value as last_activity
//FROM trm_users u 
//LEFT JOIN trm_usermeta um ON um.user_id = u.ID
...
//LIMIT 0, 20;
	...
	/**
	 * Get trendr user details for a specified user.
	 *
	 * @global trmdb $trmdb trendr database object
	...
	function __construct( $args = '' ) {
		...
		if ( false !== $this->wrapper ) {
			...
			if ( !empty( $wrapper_class ) )
				$this->wrapper_class = ' class="global-knob ' . $wrapper_class . '"';
			else
				$this->wrapper_class = ' class="global-knob"';
