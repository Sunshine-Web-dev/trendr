 ...
 * Modified for Trnder by: Andy Peatling - http://apeatling.trendr.com/
 */
...
function trs_core_set_portrait_constants() {
	...
	if ( !defined( 'TRS_AVATAR_FULL_WIDTH' ) )
		define( 'TRS_AVATAR_FULL_WIDTH', 195 );

	if ( !defined( 'TRS_AVATAR_FULL_HEIGHT' ) )
		define( 'TRS_AVATAR_FULL_HEIGHT', 195 );
	...
	//if ( !defined( 'TRS_AVATAR_ORIGINAL_MAX_FILESIZE' ) ) {
		//if ( !isset( $trs->site_options['fileupload_maxk'] ) )
			//define( 'TRS_AVATAR_ORIGINAL_MAX_FILESIZE', 5600000 ); // 1mb
		//else
			//define( 'TRS_AVATAR_ORIGINAL_MAX_FILESIZE', $trs->site_options['fileupload_maxk'] * 1024 );
	//}

	//if ( !defined( 'TRS_AVATAR_DEFAULT' ) )
		//define( 'TRS_AVATAR_DEFAULT', TRS_PLUGIN_URL . '/trs-core/images/image.png' );
if ( !defined('TRM_CONTENT_URL') )
	define( 'TRM_CONTENT_URL', get_option('siteurl') . '/trm-src'); 
 ...
 *
 * Fetches an portrait from a Trnder object. Supports user/group/blog as
 * default, but can be extended to include your own custom components too.
 *
 * @global object $trs Global Trnder settings object
 * @global $current_blog Trnder global containing information and settings for the current blog being viewed.
...
function trs_core_fetch_portrait( $args = '' ) {
	...
	if ( $width )
		...
		$html_width = ( 'full' == $type ) ? ' width="' . trs_core_portrait_thumb_width() . '"' : ' width="' . trs_core_portrait_full_width() . '"';

	// Set portrait height
	if ( $height )
		$html_height = " height='{$height}'";
	else
		$html_height = ( 'full' == $type ) ? ' height="' . trs_core_portrait_thumb_height() . '"' : ' height="' . trs_core_portrait_full_height() . '"';
	 ...
	 * or thumbnail image.
	 */
	$portrait_size              = ( 'full' == $type ) ? '-portrait' : '-trsthumb';
 ...
 *
 * @global object $trs Trnder global settings
 * @param mixed $args
...
function trs_core_delete_existing_portrait( $args = '' ) {
	...
	if ( $av_dir = opendir( $portrait_folder_dir ) ) {
		while ( false !== ( $portrait_file = readdir($av_dir) ) ) {
			if ( ( preg_match( "/-portrait/", $portrait_file ) || preg_match( "/-trsfull/", $portrait_file ) ) && '.' != $portrait_file && '..' != $portrait_file )
 ...
 * @param $file
 */
function trs_core_fix_image_orientation($file) {
    ...
    if (!empty($exif['Orientation'])) {
        switch ($exif['Orientation']) {
            case 3:
                trs_core_image_rotate($file, 180);
                break;
            case 6:
                trs_core_image_rotate($file, -90);
                break;
            case 8:
                trs_core_image_rotate($file, 90);
                break;
 ...
 * @param int $degrees
 */
function trs_core_image_rotate($file, $degrees = 0) {
    // we need a gd resource to process so we create one here
    $img = trm_load_image($file);
    $rotated = imagerotate($img, $degrees, 0);
    imagejpeg($rotated, $file, 95);
    imagedestroy($img);
    imagedestroy($rotated);
}

function trs_core_portrait_save_crop ($args){
    global $trs;
    ...
    $original_file = trs_core_portrait_upload_path() . preg_replace('~^.*/uploads~', '', $original_file);

    if ( !file_exists( $original_file ) ) {
        die('no original file ' . $original_file);
        return false;
    }
    ...
    if ( !file_exists( $portrait_folder_dir ) ) {
        die($portrait_folder_dir . ' not found');
        return false;
    }
    ...
    $full_filename  = trm_hash( $original_file . time() ) . '-portrait.jpg';
    $thumb_filename = trm_hash( $original_file . time() ) . '-trsthumb.jpg';


    $exploded = explode(',', $crop, 2); // limit to 2 parts, i.e: find the first comma
    $encoded = $exploded[1]; // pick up the 2nd part
    $decoded = base64_decode($encoded);
    $crop_file = imagecreatefromstring($decoded);
    if(!$crop_file){
        die('didn\'t create resource from string ' . $crop);
        return false;
    }
    imagejpeg($crop_file, $portrait_folder_dir . '/' . $full_filename, 95);
    imagejpeg($crop_file, $portrait_folder_dir . '/' . $thumb_filename, 95);
...
function trs_core_portrait_handle_upload( $file, $upload_dir_filter ) {
	...
	$trs_core_pre_portrait_handle_upload = apply_filters( 'trs_core_pre_portrait_handle_upload', $trs_core_pre_portrait_handle_upload );
    ...
    // fix orientation functionality
    trs_core_fix_image_orientation($trs->portrait_admin->original['file']);

	// Get image size
	//$size = @getimagesize( $trs->portrait_admin->original['file'] );
	$size = ( $trs->portrait_admin->original['file'] );
	...
	if(!empty($_POST['turkey-upload'])) {
	    header('Content-type: application/json');
        header("HTTP/1.1 200 OK");
        $response = array(
            'image' =>$trs->portrait_admin->image->url,
            '_key' => trm_create_nonce( 'trs_portrait_cropstore' ),
            '_http_referer'=> esc_attr( $_SERVER['REQUEST_URI']),
            'debug' => $_SESSION
        );
 ...
 *
 * Attempts to filter get_portrait function and let Trnder have a go
...
function trs_core_fetch_portrait_filter( $portrait, $user, $size, $default, $alt = '' ) {
	...
	// Do not filter if inside Trnder options page
	...
	if ( !$alt )
		$alt = __( 'Portrait of %s', 'trendr' );

	// Let Trnder handle the fetching of the portrait
	$trs_portrait = trs_core_fetch_portrait( array( 'item_id' => $id, 'width' => $size, 'height' => $size, 'alt' => $alt ) );

	// If Trnder found an portrait, use it. If not, use the result of get_portrait
...
function trs_core_portrait_upload_path() {
	...
	if ( isset( $trs->portrait->upload_path ) ) {
		...
		if ( defined( 'TRS_AVATAR_UPLOAD_PATH' ) ) {
			...
			if ( !trs_is_root_blog() ) {
				// Switch dynamically in order to support TRS_ENABLE_MULTIBLOG
 ...
 * @since 1.5
 *
 * @param str $type 'full' for thumbs, otherwise full
 * @param str $h_or_w 'height' for height, otherwise width
 * @return int $dim The dimension
 */
function trs_core_portrait_dimension( $type = 'full', $h_or_w = 'height' ) {
	global $trs;
...
function trs_core_portrait_thumb_width() {
	return apply_filters( 'trs_core_portrait_thumb_width', trs_core_portrait_dimension( 'full', 'width' ) );
}
...
function trs_core_portrait_thumb_height() {
	return apply_filters( 'trs_core_portrait_thumb_height', trs_core_portrait_dimension( 'full', 'height' ) );
}
