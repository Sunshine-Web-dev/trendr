 ...
 * trendr Abstraction
 *
 * The functions within this file will detect the version of trendr you are running
 * and will alter the environment so trendr can run regardless.
 *
 * The code below mostly contains function mappings. This code is subject to change once
 * the 3.0 trendr version merge takes place.
 */
...
/**
 * Parso the trendr core version number into the major release
 *
 * @since trendr (1.5.2)
 * @global string $trm_version
 * @return string 
 */
function trs_get_major_trm_version() {
	global $trm_version;
	
	return (float) $trm_version;
}

/**
 * Only add abstraction functions if trendr is not in multisite mode
 */
if ( !is_multisite() ) {
	global $trmdb;

	$trmdb->base_prefix = $trmdb->prefix;
	$trmdb->blogid      = TRS_ROOT_BLOG;
...
function trs_core_get_status_sql( $prefix = false ) {
	if ( !is_multisite() )
