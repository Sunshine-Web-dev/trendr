...
var MedVideoHandler = function () {
	...
	var createMarkup = function () {
		var html = '<input type="text" id="med_video_url" name="med_video_url" placeholder="' + TRS_DThemeMed.paste_video_url + '" value="" />' +
			'<input type="button" id="med_video_url_preview" value="' + TRS_DThemeMed.preview + '" />';
	...
	var createVideoPreview = function () {
		...
		$.post(ajaxurl, {"action":"med_preview_video", "data":url}, function (data) {
			...
			$('.med_action_container').html(
				'<p><input type="button" class="button-primary med_primary_button" id="med_submit" value="' + TRS_DThemeMed.add_video + '" /> ' +
...
var createRemoteImagePreview = function () {
		...
		$('.med_action_container').html(
			'<p><input type="button" class="button-primary med_primary_button" id="med_submit" value="' + TRS_DThemeMed.add_photos + '" /> ' +
			'<input type="button" class="button" id="med_cancel" value="' + TRS_DThemeMed.cancel + '" /></p>'
		);
		$("#med_cancel_action").hide();
	};
...
var MedLinkHandler = function () {
	...
	var createMarkup = function () {
		var html = '<input type="text" id="med_link_preview_url" name="med_link_preview_url" placeholder="' + TRS_DThemeMed.paste_link_url + '" value="" />' +
			'<input type="button" id="med_link_url_preview" value="' + TRS_DThemeMed.preview + '" />';
		$container.empty().append(html);
	...
	var createPreviewMarkup = function (data) {
			...
			'<tr>' +
				...
				'<td>' +
					...
					'<div class="med_thumbnail_chooser">' +
						...
						'<span class="med_thumbnail_chooser_label">' + TRS_DThemeMed.choose_thumbnail + '</span>' +
						'<span class="med_right">&nbsp;<img class="med_thumbnail_chooser_right" src="' + _med_data.root_url + '/img/system/right.gif" /></span>' +
						'<br /><input type="checkbox" id="med_link_no_thumbnail" /> <label for="med_link_no_thumbnail">' + TRS_DThemeMed.no_thumbnail + '</label>' +
					'</div>' +
				'</td>' +
			'</tr>' +
		'</table>';
		...
		$('.med_action_container').html(
			'<p><input type="button" class="button-primary med_primary_button" id="med_submit" value="' + TRS_DThemeMed.add_link + '" /> ' +
			'<input type="button" class="button" id="med_cancel" value="' + TRS_DThemeMed.cancel + '" /></p>'
		);
		$("#med_cancel_action").hide();
...
var MedPhotoHandler = function () {
	...
	var createMarkup = function () {
		var html = '<div id="med_tmp_photo"> </div>' +
			...
			'<input type="button" id="med_add_remote_image" value="' + TRS_DThemeMed.add_remote_image + '" /><div id="med_remote_image_container"></div>' +
			'<input type="button" id="med_remote_image_preview" value="' + TRS_DThemeMed.preview + '" />';
		...
		var uploader = new qq.FileUploader({
			...
			"onSubmit": function (id) {
				if (!parseInt(TRS_DThemeMed._max_images, 10)) return true; // Skip check
				id = parseInt(id, 10);
				if (!id) id = $("img.med_preview_photo_item").length;
				if (!id) return true;
				if (id < parseInt(TRS_DThemeMed._max_images, 10)) return true;
				if (!$("#med-too_many_photos").length) $("#med_tmp_photo").append(
					'<p id="med-too_many_photos">' + TRS_DThemeMed.images_limit_exceeded + '</p>'
				);
				return false;
			},
			"onComplete": createPhotoPreview,
			template: '<div class="qq-uploader">' +
                '<div class="qq-upload-drop-area"><span>' + TRS_DThemeMed.drop_files + '</span></div>' +
                '<div class="qq-upload-button">' + TRS_DThemeMed.upload_file + '</div>' +
		...
		$("#med_add_remote_image").click(function () {
			...
			$("#med_add_remote_image").val(TRS_DThemeMed.add_another_remote_image);
...
function createMarkup () {
	var html = '<div class="med_actions_container med-theme-' + _med_data.theme.replace(/[^-_a-z0-9]/ig, '') + ' med-alignment-' + _med_data.alignment.replace(/[^-_a-z0-9]/ig, '') + '">' +
		'<div class="med_toolbar_container">' +
			'<a href="#photos" class="med_toolbarItem" title="' + TRS_DThemeMed.add_photos + '" id="med_addPhotos"><span>' + TRS_DThemeMed.add_photos + '</span></a>' +
			'&nbsp;' +
			'<a href="#videos" class="med_toolbarItem" title="' + TRS_DThemeMed.add_videos + '" id="med_addVideos"><span>' + TRS_DThemeMed.add_videos + '</span></a>' +
			'&nbsp;' +
			'<a href="#links" class="med_toolbarItem" title="' + TRS_DThemeMed.add_links + '" id="med_addLinks"><span>' + TRS_DThemeMed.add_links + '</span></a>' +
		'</div>' +
		...
		'</div>' +
		'<input type="button" id="med_cancel_action" value="' + TRS_DThemeMed.cancel + '" style="display:none" />' +
	'</div>';
