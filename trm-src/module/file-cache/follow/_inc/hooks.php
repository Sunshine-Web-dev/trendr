 ...
 * TRS Follow Hooks
 *
 * Functions in this file allow this component to hook into Trnder so it
...
add_action( 'trs_before_member_header', 'trs_follow_add_profile_follow_button' );
...
function trs_follow_add_activity_tab() {
	...
	if ( empty( $counts['following'] ) )
		return false;
	?>
	<li id="activity-following"><a href="<?php echo trs_loggedin_user_domain() . TRS_ACTIVITY_SLUG . '/' . TRS_FOLLOWING_SLUG . '/' ?>" title="<?php _e( 'Posts from people I am following.', 'trs-follow' ) ?>"><?php printf( __( 'Following', 'trs-follow' ), (int)$counts['following'] ) ?></a></li><?php
}
...
function trs_follow_add_following_tab() {
	...
	if ( empty( $counts['following'] ) )
		return false;
	?>
	<li id="members-following"><a href="<?php echo trs_loggedin_user_domain() . TRS_FOLLOWING_SLUG ?>"><?php printf( __( 'Following %d', 'trs-follow' ), $counts['following'] ) ?></a></li><?php
}
