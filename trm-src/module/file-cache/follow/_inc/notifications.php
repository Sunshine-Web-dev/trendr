...
/**
 * When we're on the notification's 'read' page, remove 'follow_r' query arg.
 *
 ...
 * 'follow_r' query argument to each notification to determine whether we
 * need to clear it.
 *
 * @since 1.2.1
 */
function trs_follow_notifications_remove_queryarg_from_userlink( $retval ) {
	if ( trs_is_current_action( 'read' ) ) {
		...
		$retval = str_replace( '?follow_r', '', $retval );
	}
...
function trs_follow_new_follow_email_notification( $args = '' ) {
	...
	$follower_link = trs_core_get_user_domain( $r['follower_id'] ) . '?follow_r';
