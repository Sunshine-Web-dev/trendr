...
var trs_like_ajax_request = null;                                // TODO implement this. Global variable to prevent multiple AJAX requests
var id, type;
jq(document).ready(function trsLike() {
    "use strict";
    jq('.like').live('click', function() {

        id = jq(this).attr('id');                           // Used to get the id of the entity liked or unliked

      //  console.log('id: ' + id);
        type = jq(this).attr('class')                           //
            .replace('trs-primary-action ','')                   // end space needed to avoid double space in var type
            ...
            function( data ) {
                ...
                if (type == 'activity_update like') {
                    ...
                    jq('#' + id).removeClass('like')
                        .addClass('unlike')
                        .attr('title', trslikeTerms.unlike_message)
                    ...
                    jq('#' + id).removeClass('unlike')
                        .addClass('like')
                        .attr('title', trslikeTerms.like_message)
                    ...
                    jq('#' + id).removeClass('like')
                        .addClass('unlike')
                        .attr('title', trslikeTerms.unlike_message)      // may want different (smaller) message for comments
                        .attr('id', id.replace("like", "unlike") );
                ...
                if (data == 'Unlike <span>1</span>') {
                    ...
                    jq('li#activity-' + id + ' .activity-meta')
                        .append('<p class="users-who-like" id="users-who-like-' + id + '"><small>' + trslikeTerms.you_like_this +'</small></p>')
                }
