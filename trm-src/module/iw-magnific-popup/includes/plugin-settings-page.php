...
function iwmp_options_page() {
    ...
    if( ! current_user_can( 'manage_options' ) ) {
        trm_die( 'You do not have sufficient permission to access this page.' );
    }
    ...
    if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
        if ( ! isset( $_POST['_iwmp_nonce'] ) || ! trm_verify_nonce( $_POST['_iwmp_nonce'], 'iwmp_settings_nonce' ) ) {
            trm_die( 'Cheating, Huh?' );
        }
