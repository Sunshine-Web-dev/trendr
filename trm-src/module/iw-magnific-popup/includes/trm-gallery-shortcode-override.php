...
function iwmp_gallery_shortcode( $attr ) {
	...
	$valid_tags = trm_kses_allowed_html( 'post' );
	...
	foreach ( $attachments as $id => $attachment ) {
		if ( ! empty( $link ) && 'file' === $link )
			$image_output = trm_get_attachment_link( $id, $size, false, false );
		elseif ( ! empty( $link ) && 'none' === $link )
			$image_output = trm_get_attachment_image( $id, $size, false );
		else
			$image_output = trm_get_attachment_link( $id, $size, true, false );

		$image_meta  = trm_get_attachment_metadata( $id );
...
add_filter('trm_get_attachment_link', 'iwmp_add_title_to_attachment', 10, 5);
