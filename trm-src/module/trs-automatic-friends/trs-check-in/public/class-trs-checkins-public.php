...
class Trs_Checkins_Public {
	...
	public function render_location_pickup_html() {
		...
		if ( is_user_logged_in() ) {
			...
			if ( $trs_checkins->apikey ) {
					$checkin_html .= '<div class="trschk-marker-container"><span class="trschk-allow-checkin">Add a location to your posts. <i class="fa fa-map-marker" aria-hidden="true"></i></span></div>';
	...
	public function enqueue_styles() {
		...
		if ( trs_is_groups_component() || trs_is_activity_component() || trs_is_profile_component() || strpos( filter_input( INPUT_SERVER, 'REQUEST_URI' ), 'checkin' ) ) {
			...
			trm_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/trs-checkins-public.css', array(), $this->version, 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		if ( trs_is_groups_component() || trs_is_activity_component() || trs_is_profile_component() || strpos( filter_input( INPUT_SERVER, 'REQUEST_URI' ), 'checkin' ) ) {
			...
			trm_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/trs-checkins-public.js', array( 'jquery', 'jquery-ui-datepicker' ), $this->version, false );
			if ( is_user_logged_in() ) {
