jQuery( document ).ready(
	function($){
		...
		var acc = document.getElementsByClassName( "trschk-myplace-accordion" );
		var i;
		...
		$( document ).on(
			'click', '.trschk-myplace-accordion', function(){
				return false;
			}
		);

		$( '#trschk-add-place-visit-date' ).datepicker( { dateFormat: 'yy-mm-dd', minDate: 0 } );
		...
		$( document ).on(
			'click', '#trschk-cancel-checkin', function(){
				...
				$.ajax(
					{
						...
						success: function( response ) {
							...
							$( '#trschk-show-places-panel' ).click();
							$( '.trschk-allow-checkin' ).click();
						},
					}
				);
			}
