(function($){
    ...
    $(document).ready(function() {
        $(<?php echo $trm_pjax_options[TRM_PJAX_CONFIG_PREFIX.'menu-selector']; ?>).pjax('<?php echo $trm_pjax_options[TRM_PJAX_CONFIG_PREFIX.'content-selector']; ?>').live('click', function(e) {       
            alert('asdasda');
            $('<?php echo $trm_pjax_options[TRM_PJAX_CONFIG_PREFIX.'menu-active-class']; ?>')<?php foreach($active_class_array AS $ac ){ echo '.removeClass'; } ?>removeClass('');
        ...
        $('body').bind('pjax:end', function(event, request, settings) {
            ...
            <?php if( $trm_pjax_options[TRM_PJAX_CONFIG_PREFIX.'show-notice'] == 1 ) : ?>
