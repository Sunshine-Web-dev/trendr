...
class QueryMonitor extends QM_Plugin {
	...
	public function __construct( $file ) {
		...
		add_action( 'trm_footer',      array( $this, 'action_footer' ), 999 );
	...
	public function activate( $sitewide = false ) {
		...
		if ( !file_exists( $db = TRM_CONTENT_DIR . '/db.php' ) and function_exists( 'symlink' ) )
			@symlink( $this->plugin_path( 'trm-content/db.php' ), $db );
	...
	public function deactivate() {
		...
		if ( class_exists( 'QueryMonitorDB' ) )
			unlink( TRM_CONTENT_DIR . '/db.php' );

	}

	public function action_admin_bar_menu( TRM_Admin_Bar $trm_admin_bar ) {

		if ( !$this->show_query_monitor() )
			return;

		$class = implode( ' ', array( 'hide-if-js', QM_Util::trmv() ) );
	...
	public function js_admin_bar_menu() {
		...
		$class = implode( ' ', apply_filters( 'query_monitor_class', array( QM_Util::trmv() ) ) );
	...
	public function show_query_monitor() {
		...
		if ( isset( $_REQUEST['trm_customize'] ) and 'on' == $_REQUEST['trm_customize'] )
