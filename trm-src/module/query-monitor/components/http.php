	...
	function http_debug( $param, $action ) {
		...
		switch ( $action ) {
			...
			case 'response':
				...
				if ( !empty( $class ) )
					$this->data['http'][$args['_qm_key']]['transport'] = str_replace( 'trm_http_', '', strtolower( $class ) );
				else
		...
		if ( !empty( $data['http'] ) ) {
			...
			foreach ( $data['http'] as $row ) {
				...
				if ( isset( $row['response'] ) ) {
					...
					if ( is_trm_error( $row['response'] ) ) {
						$response = $row['response']->get_error_message();
						$css      = 'qm-warn';
					} else {
						$response = trm_remote_retrieve_response_code( $row['response'] );
						$msg      = trm_remote_retrieve_response_message( $row['response'] );
						$css      = '';
				...
				foreach ( $stack as & $trace ) {
					foreach ( array( 'TRM_Http', 'trm_remote_', 'fetch_rss', 'fetch_feed', 'SimplePie', 'download_url' ) as $skip ) {
