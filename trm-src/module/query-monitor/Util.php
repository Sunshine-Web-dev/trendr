...
class QM_Util {
	...
	public static function convert_hr_to_bytes( $size ) {
		...
		# Annoyingly, trm_convert_hr_to_bytes() is defined in a file that's only
	...
	public static function get_file_component( $file ) {
		...
		if ( empty( self::$file_dirs ) ) {
			self::$file_dirs['plugin']     = self::standard_dir( TRM_PLUGIN_DIR );
			self::$file_dirs['muplugin']   = self::standard_dir( TRMMU_PLUGIN_DIR );
			...
			self::$file_dirs['other']      = self::standard_dir( TRM_CONTENT_DIR );
	...
	public static function trmv() {
		return 'qm-trm-' . ( floatval( $GLOBALS['trm_version'] ) * 10 );
	}
