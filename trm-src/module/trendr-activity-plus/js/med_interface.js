...
$(function() {
	...
	// 		 if( jq('#post-refine select').val() != -1){
	// 					param+=","+ jq('#post-refine select').val();
	// 		 }
...
var MedVideoHandler = function () {
	...
	var createMarkup = function () {
		// var html = '<div class="video-scope" width=50%><input type="text" id="med_video_url" name="med_video_url" placeholder="' + lanMed.paste_video_url + '" value="" />' +
		// 	'<input type="button" id="med_video_url_preview" value="' + lanMed.preview + '" /></div>';
			var html =	'<div class="video-scope"  width=50%><input type="button" id="med_add_remote_video" value="' + lanMed.add_remote_video + '" /><div id="med_remote_video_container"></div>' +
				'<input type="button" id="med_remote_video_preview" value="' + lanMed.preview + '" /></div>';

			html +=
			'<div class="photo-scope"  width=50%><input type="button" id="med_add_remote_image" value="' + lanMed.add_remote_image + '" /><div id="med_remote_image_container"></div>' +
			'<input type="button" id="med_remote_image_preview" value="' + lanMed.preview + '" /></div>';
		$container.empty().append(html);
		$("#med_remote_image_preview").hide();
		$("#med_remote_video_preview").hide();
		$(window).off("resize.med").on("resize.med", resize);
		resize();
		//asamir add handle remote video
		$("#med_add_remote_video").click(function () {
			$('.video-scope').show();
			$('.photo-scope').hide();
			$("#med_remote_image_preview").hide();
			$("#med_remote_video_preview").show();


			// $("#med_add_remote_video").val(lanMed.add_another_remote_image);

			$("#med_remote_video_container").append(
				'<input type="text" id="med_video_url" name="med_video_url" placeholder="' + lanMed.paste_video_url + '" value="" />'
			);
	...
	var processForSave = function () {
		if($("#med_video_url").val()){
				return {
					"med_video_url": $("#med_video_url").val()
				};
		}else if($('input.med_photos_to_add').length && $('input.med_photos_to_add').length >0){
			var $imgs = $('input.med_photos_to_add');
			var imgArr = [];
			$imgs.each(function () {
				imgArr[imgArr.length] = $(this).val();
