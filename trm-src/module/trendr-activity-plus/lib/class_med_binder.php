...
EOFontIconCSS;
	...
	function js_load_scripts () {
		...
		trm_localize_script('med_interface_script', 'lanMed', array(
			'add_photos' => __('Add media', 'med'),
			'add_remote_image' => __('Add image URL', 'med'),
			'add_remote_video' => __('Add video URL', 'med'),
			...
			'upload_file' => __('Upload a Media file', 'med'),
	...
	//"(^|[^0-9A-Z&/]+)(#|\uFF03)([0-9A-Z_]*[A-Z_]+[a-z0-9_\\u00c0-\\u00d6\\u00d8-\\u00f6\\u00f8-\\u00ff]*)"
       ...
       $content = str_replace( $hashtag , ' <a href="' . $trs->root_domain . "/" . $trs->activity->slug . "/" . TRS_ACTIVITY_HASHTAGS_SLUG . "/" . urlencode( htmlspecialchars( $hashtag_noHash ) ) . '" rel="nofollow" class="hashtag" data-turbolinks="false">' . $hashtag . '</a>', $content ) ;
    ...
    $userlink = trs_core_get_userlink( $user_id );
         ...
         //groups_post_update(array('content' => $content, 'group_id' => $gid))
				:
				//trs_activity_post_update(array('content' => $content,          'component' => 'activity',
          //'type' => $type,
         // 'user_id' => $user_id))
			//;
       trs_activity_add(
