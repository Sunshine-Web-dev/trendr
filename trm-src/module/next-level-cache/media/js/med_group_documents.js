...
var MedDocumentHandler = function () {
	...
	var createDocumentPreview = function (id, fileName, resp) {
		...
		$('.med_action_container').html(
			'<p><input type="button" class="button-primary med_primary_button" id="med_submit" value="' + lanMedDocs.add_documents + '" /> ' +
...
$(".med_toolbar_container").append(
	'&nbsp;' +
	'<a href="#documents" title="' + lanMedDocs.add_documents + '" class="med_toolbarItem" id="med_addDocuments"><span>' + lanMedDocs.add_documents + '</span></a>'
);
...
$('#med_addDocuments').click(function () {
	...
	if (parseInt(group_id)) {
		...
		alert(lanMedDocs.no_group_selected);
	}
	return false;
});
