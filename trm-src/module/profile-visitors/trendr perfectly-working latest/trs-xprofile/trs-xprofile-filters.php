...
add_filter( 'trs_get_the_profile_group_name',          'trm_filter_kses',       1 );
add_filter( 'trs_get_the_profile_group_description',   'trm_filter_kses',       1 );
add_filter( 'trs_get_the_profile_field_value',         'xprofile_filter_kses', 1 );
add_filter( 'trs_get_the_profile_field_name',          'trm_filter_kses',       1 );
add_filter( 'trs_get_the_profile_field_edit_value',    'trm_filter_kses',       1 );
add_filter( 'trs_get_the_profile_field_description',   'trm_filter_kses',       1 );

add_filter( 'trs_get_the_profile_field_value',         'trmtexturize'        );
add_filter( 'trs_get_the_profile_field_value',         'convert_smilies', 2 );
add_filter( 'trs_get_the_profile_field_value',         'convert_chars'      );
add_filter( 'trs_get_the_profile_field_value',         'trmautop'            );
add_filter( 'trs_get_the_profile_field_value',         'make_clickable', 8  );
add_filter( 'trs_get_the_profile_field_value',         'force_balance_tags' );

add_filter( 'trs_get_the_profile_field_edit_value',    'force_balance_tags' );
add_filter( 'trs_get_the_profile_field_edit_value',    'esc_html'           );

add_filter( 'trs_get_the_profile_group_name',          'stripslashes' );
add_filter( 'trs_get_the_profile_group_description',   'stripslashes' );
add_filter( 'trs_get_the_profile_field_value',         'stripslashes' );
add_filter( 'trs_get_the_profile_field_edit_value',    'stripslashes' );
add_filter( 'trs_get_the_profile_field_name',          'stripslashes' );
add_filter( 'trs_get_the_profile_field_description',   'stripslashes' );

add_filter( 'xprofile_get_field_data',                'trm_filter_kses', 1 );
add_filter( 'xprofile_field_name_before_save',        'trm_filter_kses', 1 );
add_filter( 'xprofile_field_description_before_save', 'trm_filter_kses', 1 );
...
/* Custom trendr filters */

add_filter( 'trs_get_the_profile_field_value',         'xprofile_filter_format_field_value', 1, 2 );
add_filter( 'trs_get_the_site_member_profile_data',    'xprofile_filter_format_field_value', 1, 2 );
add_filter( 'trs_get_the_profile_field_value',         'xprofile_filter_link_profile_data', 9, 2 );
...
function xprofile_filter_kses( $content ) {
	...
	return trm_kses( $content, $xprofile_allowedtags );
}
...
function xprofile_sanitize_data_value_before_save ( $field_value, $field_id, $reserialize = true ) {
	...
	if ( !is_array( $field_value ) ) {
		...
		$filtered_field_value = trm_rel_nofollow( force_balance_tags( $kses_field_value ) );
	...
	// Filter each array item independently
		...
		foreach ( (array)$field_value as $value ) {
			...
			$filtered_value 	= trm_rel_nofollow( force_balance_tags( $kses_field_value ) );
 ...
 * then the date will be formatted by trs_format_time().
 *
 * @since 1.0.0
 *
...
function xprofile_filter_format_field_value( $field_value, $field_type = '' ) {
	...
	if ( 'datebox' == $field_type ) {
		...
		if ( is_numeric( $field_value ) )
			$field_value = trs_format_time( $field_value, true, false );

		// If MySQL timestamp
		else
			$field_value = trs_format_time( strtotime( $field_value ), true, false );
		...
		foreach ( (array)$values as $value ) {
			...
			if ( preg_match( '@(https?://([-\w\.]+)+(:\d+)?(/([\w/_\.]*(\?\S+)?)?)?)@', $value ) ) {
				...
				if ( count( explode( ' ', $value ) ) > 5 ) {
					...
					$new_values[] = '<a href="' . site_url( trs_get_members_root_slug() ) . '/?s=' . strip_tags( $value ) . '" rel="nofollow">' . $value . '</a>';
				}
			}
		}
...
function xprofile_filter_comments( $comments, $post_id ) {
	...
	if ( $fullnames = TRS_XProfile_ProfileData::get_value_byid( 1, $user_ids ) ) {
