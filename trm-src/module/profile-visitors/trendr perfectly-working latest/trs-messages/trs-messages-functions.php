...
function messages_new_message( $args = '' ) {
	...
	$defaults = array (
		'sender_id'  => $trs->loggedin_user->id,
		...
		'subject'    => false,
		'content'    => false,
		'date_sent'  => trs_core_current_time()
	);
	...
	$message            = new TRS_Messages_Message;
	...
	if ( !empty( $thread_id ) ) {
		$thread = new TRS_Messages_Thread( $thread_id );
		...
		if ( empty( $message->subject ) )
			$message->subject = sprintf( __( 'Re: %s', 'trendr' ), $thread->messages[0]->subject );
	...
	// No thread ID, so make some adjustments
		...
		if ( empty( $message->subject ) )
			$message->subject = __( 'No Subject', 'trendr' );
	...
	if ( $message->send() ) {
		...
		foreach ( (array)$message->recipients as $recipient )
			trs_core_add_notification( $message->id, $recipient->user_id, 'messages', 'new_message' );
...
function messages_send_notice( $subject, $message ) {
	...
	// Has access to send notices, lets do it.
		...
		$notice            = new TRS_Messages_Notice;
		$notice->subject   = $subject;
		$notice->message   = $message;
		$notice->date_sent = trs_core_current_time();
		$notice->is_active = 1;
		$notice->save(); // send it.
...
function messages_delete_thread( $thread_ids ) {
	...
	if ( is_array( $thread_ids ) ) {
		...
		for ( $i = 0, $count = count( $thread_ids ); $i < $count; ++$i ) {
			if ( !$status = TRS_Messages_Thread::delete( $thread_ids[$i]) )
				$error = 1;
		}
		...
		if ( !TRS_Messages_Thread::delete( $thread_ids ) )
			return false;
...
function messages_check_thread_access( $thread_id, $user_id = 0 ) {
	...
	return TRS_Messages_Thread::check_access( $thread_id, $user_id );
}

function messages_mark_thread_read( $thread_id ) {
	return TRS_Messages_Thread::mark_as_read( $thread_id );
}

function messages_mark_thread_unread( $thread_id ) {
	return TRS_Messages_Thread::mark_as_unread( $thread_id );
}

function messages_add_callback_values( $recipients, $subject, $content ) {
	@setcookie( 'trs_messages_send_to', $recipients, time() + 60 * 60 * 24, COOKIEPATH );
	@setcookie( 'trs_messages_subject', $subject,    time() + 60 * 60 * 24, COOKIEPATH );
	@setcookie( 'trs_messages_content', $content,    time() + 60 * 60 * 24, COOKIEPATH );
}

function messages_remove_callback_values() {
	@setcookie( 'trs_messages_send_to', false, time() - 1000, COOKIEPATH );
	@setcookie( 'trs_messages_subject', false, time() - 1000, COOKIEPATH );
	@setcookie( 'trs_messages_content', false, time() - 1000, COOKIEPATH );
}
...
function messages_get_unread_count( $user_id = 0 ) {
	...
	return TRS_Messages_Thread::get_inbox_count( $user_id );
}

function messages_is_user_sender( $user_id, $message_id ) {
	return TRS_Messages_Message::is_user_sender( $user_id, $message_id );
}

function messages_get_message_sender( $message_id ) {
	return TRS_Messages_Message::get_message_sender( $message_id );
}

function messages_is_valid_thread( $thread_id ) {
	return TRS_Messages_Thread::is_valid( $thread_id );
}
...
function messages_format_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	if ( 'new_message' == $action ) {
		$link  = trs_loggedin_user_domain() . trs_get_messages_slug() . '/inbox';
		$title = __( 'Inbox', 'trendr' );

		if ( (int)$total_items > 1 ) {
			$text = sprintf( __('You have %d new messages', 'trendr' ), (int)$total_items );
			$filter = 'trs_messages_multiple_new_message_notification';
		} else {
			$text = sprintf( __('You have %d new message', 'trendr' ), (int)$total_items );
			$filter = 'trs_messages_single_new_message_notification';
		}
	}
