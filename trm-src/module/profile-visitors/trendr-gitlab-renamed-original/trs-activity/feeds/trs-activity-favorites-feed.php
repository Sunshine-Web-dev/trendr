...
<rss version="2.0"
	...
	<?php do_action('trs_activity_favorites_feed'); ?>
>

<channel>
	<title><?php trs_site_name() ?> | <?php echo $trs->displayed_user->fullname; ?> | <?php _e( 'Favorite Activity', 'trendr' ) ?></title>
	<atom:link href="<?php self_link(); ?>" rel="self" type="application/rss+xml" />
	<link><?php echo home_url( trs_get_activity_root_slug() . '/#my-favorites/' ) ?></link>
	<description><?php echo $trs->displayed_user->fullname; ?> - <?php _e( 'Favorite Activity', 'trendr' ) ?></description>
	...
	<?php do_action('trs_activity_favorites_feed_head'); ?>

	<?php
		$favs = trs_activity_get_user_favorites( $trs->displayed_user->id );
		$fav_ids = implode( ',', (array)$favs );
	?>

	<?php if ( trs_has_activities( 'include=' . $fav_ids . '&max=50&display_comments=stream' ) ) : ?>
		<?php while ( trs_activities() ) : trs_the_activity(); ?>
			...
			<item>
				...
				<?php do_action('trs_activity_favorites_feed_item'); ?>
