...
function trs_activity_add( $args = '' ) {
	...
	//asamir hanlde automtic pf Posts
		if( isset($_POST['period_in_min'])){
			$period_in_min = preg_replace( "/\D/" , "" , $_POST['period_in_min'] );
					if(is_numeric($period_in_min) && $period_in_min != 0)
					{
						$period_in_min = $period_in_min*60;

					 $fp_promote_details = array();
					 $fp_promote_details["creator"] = get_current_user_id();
					 $fp_promote_details["createdat"] = time();
					 $fp_promote_details["expire_in"] =$period_in_min; //per s
					 trs_activity_update_meta( $activity->id , 'fp_promote' , $fp_promote_details );
					 trs_activity_update_meta( $activity->id , 'fp_promote_Expire' , (int)$fp_promote_details["createdat"]+(int)$fp_promote_details["expire_in"] );
					 trs_activity_update_meta( $activity->id , 'fp_promote_type' , 'ad' );
				}
	}
	return $activity->id;
}
...
function get_All_featured_post(){
		global $trs, $trmdb;
		return $trmdb->get_results( $trmdb->prepare( "SELECT a.id,(m.meta_value - unix_timestamp()) as expire_in,m.meta_value as expire_date,IFNULL(m2.meta_value,0) views,m3.meta_value type
						FROM trm_trs_activity a
						...
						left outer join trm_trs_activity_meta m3 on m3.activity_id = a.id and m3.meta_key = 'fp_promote_type'
