...
// var_dump($filter);
		...
		//asamir execude the pf items
		if(isset($GLOBALS['featured_post'])){
				if ( $exclude ) $exclude.=','.$GLOBALS['featured_post'];
				else $exclude = $GLOBALS['featured_post'];
		}
...
// echo "<b>".$select_sql.$from_sql.$where_sql.$sort. $pag_sql."</b><hr>";
		...
		if(isset($GLOBALS['featured_post'])){
			$pfwhere_sql = " a.id IN (".$GLOBALS['featured_post'].")";
			$pfactivities = $trmdb->get_results( $select_sql. " ". $from_sql . " WHERE ". $pfwhere_sql );

			array_unshift($activities,$pfactivities[0]);
		}
		// append pf
