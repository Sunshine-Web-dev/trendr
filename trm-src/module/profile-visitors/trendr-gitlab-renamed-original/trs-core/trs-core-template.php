...
function trs_search_form_type_select() {
	...
	$selection_box  = '<label for="search-which" class="accessibly-hidden">' . __( 'Search these:', 'trendr' ) . '</label>';
...
function trs_is_user_change_portrait() {
	if ( trs_is_profile_component() && trs_is_current_action( 'change-portrait' ) )
		return true;
	...
	function trs_get_the_body_class( $trm_classes, $custom_classes = false ) {
		...
		if ( !trs_is_blog_page() ) :
			if ( trs_is_user_profile() )
				$trs_classes[] = 'xprofile';

			if ( trs_is_activity_component() )
				$trs_classes[] = 'activity';
