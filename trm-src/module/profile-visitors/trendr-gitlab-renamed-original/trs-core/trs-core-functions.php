 ...
 *
 * @global object $trmdb WordPress database object
...
function trs_core_activation_notice() {
	...
	if ( !defined( 'TRS_SILENCE_THEME_NOTICE' ) && !in_array( 'trendr', (array)$ct->tags ) ) {
		trs_core_add_admin_notice( sprintf( __( "You'll need to <a href='%s'>activate a <strong>trendr-compatible theme</strong></a> to take advantage of all of trendr's features. We've bundled a default theme, but you can always <a href='%s'>install some other compatible themes</a> or <a href='%s'>update your existing WordPress theme</a>.", 'trendr' ), admin_url( 'themes.php' ), network_admin_url( 'theme-install.php?type=tag&s=trendr&tab=search' ), network_admin_url( 'plugin-install.php?type=term&tab=search&s=%22trs-template-pack%22' ) ) );
	}
	...
	if ( !empty( $orphaned_components ) ) {
		...
		$notice    = sprintf( __( 'The following active trendr Components do not have associated WordPress Pages: %2$s. <a href="%1$s" class="button-secondary">Repair</a>', 'trendr' ), $admin_url, '<strong>' . implode( '</strong>, <strong>', $orphaned_components ) . '</strong>' );
	...
	if ( !empty( $dupe_names ) ) {
		$admin_url = trs_get_admin_url( add_query_arg( array( 'page' => 'trs-page-settings' ), 'admin.php' ) );
		$notice    = sprintf( __( 'Each trendr Component needs its own WordPress page. The following WordPress Pages have more than one component associated with them: %2$s. <a href="%1$s" class="button-secondary">Repair</a>', 'trendr' ), $admin_url, '<strong>' . implode( '</strong>, <strong>', $dupe_names ) . '</strong>' );
...
function trs_core_time_since( $older_date, $newer_date = false ) {
	...
	$ago_text       = apply_filters( 'trs_core_time_since_ago_text',       __( '%s ago',    'trendr' ) );

	// array of time period chunks
	$chunks = array(
		array( 60 * 60 * 24 * 365 , __( 'year',   'trendr' ), __( 'years',   'trendr' ) ),
		array( 60 * 60 * 24 * 30 ,  __( 'month',  'trendr' ), __( 'months',  'trendr' ) ),
		array( 60 * 60 * 24 * 7,    __( 'week',   'trendr' ), __( 'weeks',   'trendr' ) ),
		array( 60 * 60 * 24 ,       __( 'day',    'trendr' ), __( 'days',    'trendr' ) ),
		array( 60 * 60 ,            __( 'hour',   'trendr' ), __( 'hours',   'trendr' ) ),
		array( 60 ,                 __( 'minute', 'trendr' ), __( 'minutes', 'trendr' ) ),
		array( 1,                   __( 'second', 'trendr' ), __( 'seconds', 'trendr' ) )
	);
...
/**
 * Initializes {@link TRS_Embed} after everything is loaded.
 *
 * @global object $trs trendr global settings
 * @package trendr Core
 * @since 1.5
 */
function trs_embed_init() {
	global $trs;

	if ( empty( $trs->embed ) )
		$trs->embed = new TRS_Embed();
}
add_action( 'trs_init', 'trs_embed_init', 9 );
 ...
 *
 * trendr stores a number of pieces of userdata in the WordPress central usermeta table. In
...
/**
 * Output the correct URL based on trendr and WordPress configuration
 *
 * @package trendr
 * @since 1.5
 *
...
function trs_admin_url( $path = '', $scheme = 'admin' ) {
	...
	/**
	 * Return the correct URL based on trendr and WordPress configuration
	 *
	 * @package trendr
	 * @since 1.5
	 *
