	...
	switch( $step ) {
		...
		case 'new':
			if ( isset( $_REQUEST['doinstall'] ) && 1 == (int)$_REQUEST['doinstall'] ) {
				...
				switch ( $result ) {
					case 1:
						_e( '<p>All done! Configuration settings have been saved to the file <code>bb-config.php</code> in the root of your WordPress install.</p>', 'trendr' );
						break;
					default:
						// Just write the contents to screen
						_e( '<p>A configuration file could not be created. No problem, but you will need to save the text shown below into a file named <code>bb-config.php</code> in the root directory of your WordPress installation before you can start using the forum functionality.</p>', 'trendr' ); ?>
...
function trs_forums_btrsress_install() {
	...
	if ( is_multisite() )
		$file .= "\n" .   '$bb->wordpress_mu_primary_blog_id = ' . trs_get_root_blog_id() . ';';
