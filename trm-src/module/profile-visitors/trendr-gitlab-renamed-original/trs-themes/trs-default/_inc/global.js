...
//asamir send period param to server
	...
	jq('#post-refine select , body > ul> li > #activity_media_filter > input[type="checkbox"]').change( function() {
...
//asamir to handle the checkbox search with the dropdown filter
	...
	/* Activity Tab Set */
	if ( null != jq.cookie('trs-activity-scope') && jq('div.activity-type-tabs').length ) {
...
jq.cookie( 'trs-activity-filter',filter, {path: location.pathname} );
var checkboxs ="";
	jq('body > ul> li > #activity_media_filter > input[type="checkbox"]').each(function(index,elem){
				if(elem.checked){
  ...
  jq.cookie( 'trs-activity-filter-checkbox',(checkboxs), {path: location.pathname} );
	jq.cookie( 'trs-activity-scope', scope, {path: location.pathname} );
