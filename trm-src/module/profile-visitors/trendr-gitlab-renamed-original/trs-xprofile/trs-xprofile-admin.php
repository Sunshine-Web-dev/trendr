 ...
 * @uses trs_xprofile_install() runs the installation of DB tables for the xprofile component
 * @uses trm_enqueue_script() Adds a JS file to the JS queue ready for output
...
function xprofile_add_admin_menu() {
	...
	$hook = add_submenu_page( 'trs-general-settings', __( 'Profile Fields', 'trendr' ), __( 'Profile Fields', 'trendr' ), 'manage_options', 'trs-profile-setup', 'xprofile_admin' );

	add_action( "admin_print_styles-$hook", 'trs_core_add_admin_menu_styles' );
}
add_action( trs_core_admin_hook(), 'xprofile_add_admin_menu' );
...
function xprofile_admin( $message = '', $type = 'error' ) {
	...
	$groups = TRS_XProfile_Group::get( array(
		'fetch_fields' => true
	));
...
?>
	...
	<div class="wrap">
		...
		<h2>
			...
			<?php _e( 'Extended Profile Fields', 'trendr'); ?>

			<a id="add_group" class="button add-new-h2" href="admin.php?page=trs-profile-setup&amp;mode=add_group"><?php _e( 'Add New Group', 'trendr' ); ?></a>
		</h2>

		<p><?php _e( 'Your users will distinguish themselves through their profile page. You must give them profile fields that allow them to describe themselves in a way that is relevant to the theme of your social network.', 'trendr'); ?></p>
		<p><?php echo sprintf( __( 'NOTE: Any fields in the "%s" group will appear on the signup page.', 'trendr' ), esc_html( stripslashes( trs_get_option( 'trs-xprofile-base-group-name' ) ) ) ) ?></p>

		<form action="" id="profile-field-form" method="post">

			<?php trm_nonce_field( 'trs_reorder_fields', '_key_reorder_fields' ); ?>

			<?php trm_nonce_field( 'trs_reorder_groups', '_key_reorder_groups', false );
...
<?php
			if ( !empty( $groups ) ) :
				foreach ( $groups as $group ) { ?>
					...
					<li id="group_<?php echo $group->id; ?>"><a href="#tabs-<?php echo $group->id; ?>" class="ui-tab"><?php echo esc_attr( $group->name ); ?><?php if ( !$group->can_delete ) : ?> <?php _e( '(Primary)', 'trendr'); endif; ?></a></li>
					...
					<div id="tabs-<?php echo $group->id; ?>" class="tab-wrapper">
						<div class="tab-toolbar">
							<div class="tab-toolbar-left">
								<a class="button" href="admin.php?page=trs-profile-setup&amp;group_id=<?php echo esc_attr( $group->id ); ?>&amp;mode=add_field"><?php _e( 'Add New Field', 'trendr' ); ?></a>
								<a class="button edit" href="admin.php?page=trs-profile-setup&amp;mode=edit_group&amp;group_id=<?php echo esc_attr( $group->id ); ?>"><?php _e( 'Edit Group', 'trendr' ); ?></a>

<?php				if ( $group->can_delete ) : ?>

								<a class="submitdelete deletion" href="admin.php?page=trs-profile-setup&amp;mode=delete_group&amp;group_id=<?php echo esc_attr( $group->id ); ?>"><?php _e( 'Delete Group', 'trendr' ); ?></a>
...
?>
							...
							<p class="nodrag nofields"><?php _e( 'There are no fields in this group.', 'trendr' ); ?></p>
...
?>
				...
				<div id="message" class="error"><p><?php _e( 'You have no groups.', 'trendr' ); ?></p></div>
				<p><a href="admin.php?page=trs-profile-setup&amp;mode=add_group"><?php _e( 'Add New Group', 'trendr' ); ?></a></p>
...
function xprofile_admin_manage_group( $group_id = null ) {
	...
	if ( isset( $_POST['save_group'] ) ) {
		if ( TRS_XProfile_Group::admin_validate( $_POST ) ) {
			$group->name		= trm_filter_kses( $_POST['group_name'] );
			$group->description	= !empty( $_POST['group_description'] ) ? trm_filter_kses( $_POST['group_description'] ) : '';

			if ( !$group->save() ) {
				$message = __( 'There was an error saving the group. Please try again', 'trendr' );
				$type    = 'error';
			} else {
				$message = __( 'The group was saved successfully.', 'trendr' );
				$type    = 'success';

				if ( 1 == $group_id )
					trs_update_option( 'trs-xprofile-base-group-name', $group->name );
...
function xprofile_admin_delete_group( $group_id ) {
	...
	if ( !$group->delete() ) {
		$message = __( 'There was an error deleting the group. Please try again', 'trendr' );
		$type    = 'error';
	} else {
		$message = __( 'The group was deleted successfully.', 'trendr' );
		$type    = 'success';
...
function xprofile_admin_manage_field( $group_id, $field_id = null ) {
	global $trs, $trmdb, $message, $groups;

	$field = new TRS_XProfile_Field( $field_id );
	$field->group_id = $group_id;

	if ( isset( $_POST['saveField'] ) ) {
		if ( TRS_XProfile_Field::admin_validate() ) {
			$field->name        = trm_filter_kses( $_POST['title'] );
			$field->description = !empty( $_POST['description'] ) ? trm_filter_kses( $_POST['description'] ) : '';
			$field->is_required = trm_filter_kses( $_POST['required'] );
			$field->type        = trm_filter_kses( $_POST['fieldtype'] );

			if ( !empty( $_POST["sort_order_{$field->type}"] ) )
				$field->order_by = trm_filter_kses( $_POST["sort_order_{$field->type}"] );

			$field->field_order = $trmdb->get_var( $trmdb->prepare( "SELECT field_order FROM {$trs->profile->table_name_fields} WHERE id = %d", $field_id ) );

			if ( !$field->field_order ) {
				$field->field_order = (int) $trmdb->get_var( $trmdb->prepare( "SELECT max(field_order) FROM {$trs->profile->table_name_fields} WHERE group_id = %d", $group_id ) );
				$field->field_order++;
			}

			if ( !$field->save() ) {
				$message = __( 'There was an error saving the field. Please try again', 'trendr' );
				$type = 'error';

				unset( $_GET['mode'] );
				xprofile_admin( $message, $type );
			} else {
				$message = __( 'The field was saved successfully.', 'trendr' );
				$type = 'success';

				if ( 1 == $field_id )
					trs_update_option( 'trs-xprofile-fullname-field-name', $field->name );

				unset( $_GET['mode'] );

				do_action( 'xprofile_fields_saved_field', $field );

				$groups = TRS_XProfile_Group::get();
				xprofile_admin( $message, $type );
			}
		} else {
...
function xprofile_admin_delete_field( $field_id, $type = 'field' ) {
	...
	if ( 'field' == $type )
		$type = __('field', 'trendr');
	else
		$type = __('option', 'trendr');

	$field = new TRS_XProfile_Field( $field_id );

	if ( !$field->delete() ) {
		$message = sprintf( __('There was an error deleting the %s. Please try again', 'trendr' ), $type );
		$type = 'error';
	} else {
		$message = sprintf( __('The %s was deleted successfully!', 'trendr' ), $type );
		$type = 'success';
...
function xprofile_ajax_reorder_fields() {
	...
	// Check the nonce
	check_admin_referer( 'trs_reorder_fields', '_key_reorder_fields' );
...
add_action( 'trm_ajax_xprofile_reorder_fields', 'xprofile_ajax_reorder_fields' );
...
function xprofile_ajax_reorder_field_groups() {
	...
	// Check the nonce
	check_admin_referer( 'trs_reorder_groups', '_key_reorder_groups' );
...
add_action( 'trm_ajax_xprofile_reorder_groups', 'xprofile_ajax_reorder_field_groups' );
...
function xprofile_admin_field( $admin_field, $admin_group, $class='' ) {
	...
	$field = $admin_field; ?>
						<fieldset id="field_<?php echo esc_attr( $field->id ); ?>" class="sortable<?php echo ' ' . $field->type; if ( $class ) echo ' ' . $class; ?>">
							<legend><span><?php trs_the_profile_field_name(); ?> <?php if( !$field->can_delete ) : ?> <?php _e( '(Primary)', 'trendr' ); endif; ?> <?php if ( trs_get_the_profile_field_is_required() ) : ?><?php _e( '(Required)', 'trendr' ) ?><?php endif; ?></span></legend>
...
<?php
	switch ( $field->type ) {
		case 'textbox' : ?>
								...
								<input type="text" name="<?php trs_the_profile_field_input_name() ?>" id="<?php trs_the_profile_field_input_name() ?>" value="" />

<?php		break; case 'textarea' : ?>

								<textarea rows="5" cols="40" name="<?php trs_the_profile_field_input_name() ?>" id="<?php trs_the_profile_field_input_name() ?>"></textarea>
...
<?php } ?>
								<div class="actions">
									<a class="button edit" href="admin.php?page=trs-profile-setup&amp;group_id=<?php echo esc_attr( $admin_group->id ); ?>&amp;field_id=<?php echo esc_attr( $field->id ); ?>&amp;mode=edit_field"><?php _e( 'Edit', 'trendr' ); ?></a>
									<?php if ( !$field->can_delete ) : ?>&nbsp;<?php else : ?><a class="button delete" href="admin.php?page=trs-profile-setup&amp;field_id=<?php echo esc_attr( $field->id ); ?>&amp;mode=delete_field"><?php _e( 'Delete', 'trendr' ); ?></a><?php endif; ?>
