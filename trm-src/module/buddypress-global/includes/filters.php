...
add_filter( 'template_include', 'buddyboss_global_search_override_trm_native_results', 999 ); //don't leave any chance!.

/**
 * Force native trm search section to load page template so we can hook stuff into it.
 *
 * @since 1.0.0
 * @param mixed $value
 * @return mixed
...
function buddyboss_global_search_override_trm_native_results($template) {
...
/**
 * Load dummy post for trm native search result. magic starts here.
 * @since 1.0.0 
 * @param mixed $value
 * @return mixed
 **/
...
function buddyboss_global_search_result_page_dummy_post_load($template) {
	...
	$dummy = array(
               ...
               'post_modified_gmt'     => 0,
               'post_content'          => '',
               'post_title'            => __('Search Results',"trendr-global-search"),
               'post_excerpt'          => '',
	...
	// Set the $post global
	//$post = new TRM_Post( (object) $dummy );
   
	// Copy the new post global into the main $trm_query
	$trm_query->post       = $post;
	$trm_query->posts      = array( $post );
	$trm_query->post_count      = 1;
	$trm_query->max_num_pages      = 0;
		
	return $template;
}

/**
 * Force native trm search page not to look any data into db to save query and performance
 * @since 1.0.0
 * @param mixed $value
 * @return mixed
 **/
