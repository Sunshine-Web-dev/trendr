...
function trs_dtheme_post_update() {
	...
	if($_POST['story']=="true"){
		...
		$s1 .= $s2;
		$trmd->insert ('trm_localities', array("Lat_Long_pk"=>$s1)); 
		/*$trmdb->insert( 'trm_trs_stories', array("user_id" => $trs->loggedin_user->id, "content" => $_POST['content'],"location"=>$_POST["location_address"] ,"date_recorded"=>trs_core_current_time(),"primary_link"=>trs_core_get_user_domain( $trs->loggedin_user->id ),"type"=>"post_text","categories"=>$_POST["categories"], "Latitude"=>$_POST["latitude"], "Longitude"=>$_POST["longitude"], "Lat_Long_fk"=>$test));*/
...
function trs_stories_load_more(){
	...
	for($i=$_POST['index']-1;$i<$real_showed_count;$i++){
			?>
			<div  id="stores_<?php echo $real_result[$i]->id;?>" class="stores_div" style="width:90%; height:auto;display: inline-block;border:1px solid #999;padding:5px;margin-bottom: 5px">
							<div id="story-contour-image">
							<a href="<?php echo $real_result[$i]->primary_link; ?>"><?php echo trs_core_fetch_portrait( array( 'item_id' => $real_result[$i]->user_id, 'type' => 'full', 'width' => 20, 'height' => 20, 'html' => true, 'alt' => __( 'Profile picture of %s', 'trendr' ) ) ) ;
...
function trs_search_filter(){
	...
	trs_xprofile_update_meta( get_current_user_id(),'5', 'data', 'latitude',$_POST['latitude']);
	trs_xprofile_update_meta( get_current_user_id(),'6', 'data', 'longitude',$_POST['longitude']);
	global $trmdb,$trs;
	$users = get_users( array( 'fields' => array( 'ID' ) ) );
	//print_r($users);
	$users_nearby=[];
...
function trs_my_search_filter(){
	...
	?>
	<div class="add_new_stories stores_div"  style="width:50px;height: 50px;display: inline-block;text-align: center;">
	...
	if ( $result ) { ?>
		<div class="message-box new-message">
			<div class="message-metadata">
				...
				<?php echo trs_loggedin_user_portrait( 'type=full&width=30&height=30' ); ?>
...
function trs_dtheme_ajax_messages_autocomplete_results() {
	...
	if ( !empty( $user_ids ) ) {
		foreach ( $user_ids as $user_id ) {
			...
			echo '<span id="link-' . $username . '" href="' . trs_core_get_user_domain( $user_id ) . '"></span>' . trs_core_fetch_portrait( array( 'item_id' => $user_id, 'type' => 'full', 'width' => 15, 'height' => 15 ) ) . ' &nbsp;' . trs_core_get_user_displayname( $user_id ) . ' (' . $username . ')
			';
		}
	}
}
