$('html').on(' click touchstart  scroll   touchmove touchend touchcancel touchleave touch','.trs-img',function(){
...
javascript:alert($(window).scrollTop());
    ...
    .all.selected .broadcast-knobs, .all.selected .ac-reply-content, .all.selected .broadcast-top, .all.selected .broadcast-inn p{display: inline;padding:0;}
    .all.selected ul.publish-piece { column-count: 1;width:100%;
column-gap: 0px;
...
$(document).on("click", ".article-piece li", function(){
  $('.article-piece li').removeClass('onClick'); /* remove focus state from all input elements */
    $(this).addClass('onClick'); /* add focus state to currently clicked element */
$(".article-piece li[id^='activity-']").each(function() {
  var id = parseInt(this.id.replace("activity-", ""), 10);
  var activity = $("#activity-" + id);
  //var flightNumber = $("#id_flight_number_for_" + id);
  // do stuff with airline and flightNumber <input>s
    alert(this.id);
              $('.trs-img ').addClass('selected');
});

});
$(document).ready(function() {
  $('.trs-img').focus(function() {
    $('.trs-img').removeClass('onFocus'); /* remove focus state from all input elements */
...
(function() {
  ...
  $(".broadcast-inn").click(function(e) {
