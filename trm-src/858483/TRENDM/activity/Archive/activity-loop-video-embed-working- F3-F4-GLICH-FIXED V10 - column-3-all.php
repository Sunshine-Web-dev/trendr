...
////f3.active
    ...
    //$('.all ul.publish-piece').removeClass('top'); /* remove focus state from all input elements */
     $('.frame').contents().find(".photo_post.selected  ").hide();
...
$(document).on("click", ".all .photo_post", function(){
    $('.photo_post').removeClass('selected');
    $(this).addClass('selected'); 
    var currentId = $(this).attr('id');
   $('.all ul.publish-piece').css('column-count','1'); 
                    //$("#l-c #frame ").contents().find('.container ').removeClass('all');
