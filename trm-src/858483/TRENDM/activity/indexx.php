...
while ($rowvactivity1=mysqli_fetch_array($vqueryactivity))
{
	...
	<video class ="videos"  playsinline autoplay  style=" width: 100%;height: 1000%;padding:10px;" preload="auto">
  ...
  <source src="<?php echo site_url().'/trm-src/uploads/med/'.$finalvideo;?>" type="video/ogg">
	   ...
	   <button onclick = change_all()>mute/unmute all </button>
	   <br><br><br><br><br><br>
	<?php
}
}
...
var videos = document.querySelectorAll(".videos");
for (let i = 0; i < videos.length; i++) {
      videos[i].addEventListener("click", change_all);
	  videos[i].volume = 0;
}
