...
function create_stories_table(){
    ...
    $sql = "CREATE TABLE IF NOT EXISTS {$trs_prefix}trs_stories (
              ...
              categories text NOT NULL) $charset_collate;";
    $trmdb->query($sql);
...
trm_redirect( TRS_ACTIVITY_SLUG  ) ;    
...
//register menus // links
   function connect_link() {
        $output = '';
        $message = __("Commune ", 'trscp');
        $output .= '<div class="c-link">';
        $output .= '<a href="' . site_url() . '/members/" >' . $message . '</a>';
        $output .= '</div>';
         
        echo $output;
    }
       add_action( 'header_menu',  'connect_link' );
   function explore_link() {
        $output = '';
        $message = __("Discover", 'trscp');
        $output .= '<div class="e-link">';
        $output .= '<a href="' .  '/activity" >' . $message . '</a>';
        $output .= '</div>';
         
        echo $output;
    }
       add_action( 'header_menu',  'explore_link' );

   function interact_link() {
        $output = '';
        $message = __("Interact", 'trscp');
        $output .= '<div class="i-link">';
        $output .= '<a href="' .  '/all-stories" >' . $message . '</a>';
        $output .= '</div>';
         
        echo $output;
    }
       add_action( 'header_menu',  'interact_link' );


//register profile// links

    function edit_profile() {
        $output = '';
        if ( trs_is_my_profile()  ) {
                $message = __("Profile", 'trscp');
    
            $output .= '<div class="profile-button">';
            $output .= '<a href="' . trs_displayed_user_domain() . 'profile/edit/group/1" class="button">' . $message . '</a>';
            $output .= '</div>';
        }
       ...
       add_action( 'trs_before_member_header_meta', 'edit_profile'  );

    function edit_message() {
        $output = '';
        if ( trs_is_my_profile()  ) {
                $message = __("Inbox ", 'trscp');
    
            $output .= '<div class="user-messages">';
            $output .= '<a href="' . trs_displayed_user_domain() . 'messages/" class="button">' . $message . '</a>';
            $output .= '</div>';
        }
       ...
       add_action( 'trs_before_member_header_meta', 'edit_message'  );
    function edit_settings() {
        $output = '';
        if ( trs_is_my_profile()  ) {
                $message = __("Settings", 'trscp');
    
            $output .= '<div class="user-settings">';
            $output .= '<a href="' . trs_displayed_user_domain() . 'settings/" class="button">' . $message . '</a>';
            $output .= '</div>';
        }
       ...
       add_action( 'trs_before_member_header_meta', 'edit_settings'  );
     function edit_portrait() {
        $output = '';

        if ( trs_is_my_profile()  ) {
                $message = __(" ", 'trscp');
                $output .= '<div class="portrait-edit">';
            $output .= '<a href="' . trs_displayed_user_domain() . 'profile/change-profile-photo/" >' . $message . '</a>';
            $output .= '</div>';
        }
  
        echo $output;
    }
       add_action( 'trs_profile_knobs', 'edit_portrait' );
        ...
        if ( trs_is_my_profile()  ) {
            ...
            $output .= '<a href="' . trs_displayed_user_domain() . 'profile/change-cover/" >' . $message . 'Upload Cover Photo</a>';
            $output .= '</div>';
        }
       ...
       add_action( 'trs_before_member_header_meta', 'edit_cover' );
