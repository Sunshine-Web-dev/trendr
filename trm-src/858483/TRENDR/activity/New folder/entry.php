...
<?php $activity_id = trs_get_activity_id();?><?php $activity_blog_id = trs_activity_get_meta( $activities_template->activity, $activity_id); ?>
			<a class="broadcast-inn" href="<?php echo esc_url(trs_activity_get_permalink($activity_blog_id) .$activity_id); ?>">
	 ...
	 if ( trs_get_activity_id() == $GLOBALS['featured_post']) { ?>
				...
				<?php if ( trs_activity_can_favorite() ) : ?>
					...
					<?php else : ?>
						...
						<a href="<?php trs_activity_unfavorite_link(); ?>" class="button unfav trs-secondary-action" title="<?php esc_attr_e( 'Remove Favorite', 'trendr' ); ?>"><?php _e( 'Unfavorite', 'trendr' ) ?></a>
