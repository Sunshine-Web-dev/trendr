<?php
/*
Plugin Name: Trendr OPcache Cache Plugin
Plugin URI: https://github.com/elcobvg/trendr-opcache
 ...
 */
function trm_cache_add($key, $data, $group = 'default', $expire = 0)
{
    return TRM_Object_Cache::instance()->add($key, $data, $group, $expire);
}
 ...
 */
function trm_cache_decr($key, $offset = 1, $group = 'default')
{
    return TRM_Object_Cache::instance()->decr($key, $offset, $group);
}
 ...
 */
function trm_cache_delete($key, $group = 'default')
{
    return TRM_Object_Cache::instance()->delete($key, $group);
}
...
{
    return TRM_Object_Cache::instance()->flush();
}
 ...
 */
function trm_cache_get($key, $group = 'default', $force = false, &$found = null)
{
    return TRM_Object_Cache::instance()->get($key, $group, $force, $found);
}
 ...
 */
function trm_cache_get_multi($groups)
{
    return TRM_Object_Cache::instance()->get_multi($groups);
}
 ...
 * @return false|int False on failure, the item's new value on success.
 */
function trm_cache_incr($key, $offset = 1, $group = 'default')
{
    return TRM_Object_Cache::instance()->incr($key, $offset, $group);
}
 ...
 *
 * @global TRM_Object_Cache $trm_object_cache WordPress Object Cache
 */
function trm_cache_init()
{
    $GLOBALS['trm_object_cache'] = TRM_Object_Cache::instance();
}
 ...
 */
function trm_cache_replace($key, $data, $group = 'default', $expire = 0)
{
    return TRM_Object_Cache::instance()->replace($key, $data, $group, $expire);
}
 ...
 * @return bool False on failure, true on success
 */
function trm_cache_set($key, $data, $group = 'default', $expire = 0)
{
    return TRM_Object_Cache::instance()->set($key, $data, $group, $expire);
}
 ...
 */
function trm_cache_switch_to_blog($blog_id)
{
    TRM_Object_Cache::instance()->switch_to_blog($blog_id);
}
...
{
    TRM_Object_Cache::instance()->add_global_groups($groups);
}


/**
 * Pass thru to trm_cache_add_global_groups.
 *
 * @param string|array $groups A group or an array of groups to add
 */
function trm_cache_add_non_persistent_groups($groups)
{
    trm_cache_add_global_groups($groups);
}
 ...
 */
function trm_cache_reset()
{
    _deprecated_function(__FUNCTION__, '3.5', 'trm_cache_switch_to_blog()');
    return false;
}
...
class TRM_Object_Cache
{
    ...
    /**
     * Singleton. Return instance of TRM_Object_Cache
     *
     * @return TRM_Object_Cache
     */
    ...
    {
        if (!defined('TRM_OPCACHE_KEY_SALT')) {
            /**
             * Set in config if you are using some sort of shared
             * config where base_name is the same on all sites
             */
            define('TRM_OPCACHE_KEY_SALT', 'trm-opcache');
        }

        $this->directory   = TRM_CONTENT_DIR . '/cache';
    ...
    {
        if (trm_suspend_cache_addition() || $this->exists($key, $group)) {
            return false;
        }
...
//	    rmdir( $directory_name );
    ...
    {
        return $this->directory . DIRECTORY_SEPARATOR . $group . DIRECTORY_SEPARATOR . TRM_OPCACHE_KEY_SALT . '-' . md5( $this->base_name . $key) . '.php';
    }
