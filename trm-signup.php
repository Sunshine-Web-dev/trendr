...
add_action( 'trm_head', 'signuppageheaders' ) ;

require( './trm-header.php' );

if ( is_array( get_site_option( 'illegal_names' )) && isset( $_GET[ 'new' ] ) && in_array( $_GET[ 'new' ], get_site_option( 'illegal_names' ) ) == true ) {
	trm_redirect( network_home_url() );
	die();
}
...
add_action( 'trm_head', 'do_signup_header' );
...
if ( !is_main_site() ) {
	trm_redirect( network_home_url( 'trm-signup.php' ) );
	die();
}
...
function trmmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'trm_head', 'trmmu_signup_stylesheet' );
get_header();
...
function validate_user_form() {
	return trmmu_validate_user_signup($_POST['user_name'], $_POST['user_email']);
}
...
function signup_another_blog($blogname = '', $blog_title = '', $errors = '') {
	...
	<form id="setupform" method="post" action="trm-signup.php">
...
function validate_another_blog_signup() {
	global $trmdb, $blogname, $blog_title, $errors, $domain, $path;
	$current_user = trm_get_current_user();
	if ( !is_user_logged_in() )
		die();
...
function confirm_another_blog_signup($domain, $path, $blog_title, $user_name, $user_email = '', $meta = '') {
	...
	<p>
		<?php printf( __( '<a href="http://%1$s">http://%2$s</a> is your new site.  <a href="%3$s">Log in</a> as &#8220;%4$s&#8221; using your existing password.' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "enter.php", $user_name ) ?>
	</p>
	<?php
...
function validate_user_signup() {
	...
	trmmu_signup_user($user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function validate_blog_signup() {
	...
	$result = trmmu_validate_user_signup($_POST['user_name'], $_POST['user_email']);
	extract($result);
	...
	$result = trmmu_validate_blog_signup($_POST['blogname'], $_POST['blog_title']);
	extract($result);
	...
	trmmu_signup_blog($domain, $path, $blog_title, $user_name, $user_email, $meta);
...
if ( $active_signup == 'none' ) {
	...
	if ( is_ssl() )
		...
		$proto = 'http://';
	$login_url = site_url( 'enter.php?redirect_to=' . urlencode($proto . $_SERVER['HTTP_HOST'] . '/trm-signup.php' ));
