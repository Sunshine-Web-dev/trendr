...
if ( !current_user_can('manage_network_themes') )
	trm_die( __( 'You do not have sufficient permissions to manage network themes.' ) );
...
$referer = remove_query_arg( $temp_args, trm_get_referer() );
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'enable':
			...
			trm_redirect( add_query_arg( 'enabled', '1', $referer ) );
			exit;
			break;
		case 'disable':
			...
			update_site_option( 'allowedthemes', $allowed_themes );
			trm_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
			break;
		case 'enable-selected':
			...
			if ( empty($themes) ) {
				trm_redirect( add_query_arg( 'error', 'none', $referer ) );
				exit;
			}
			foreach( (array) $themes as $theme )
				$allowed_themes[ $theme ] = true;
			update_site_option( 'allowedthemes', $allowed_themes );
			trm_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
			break;
		case 'disable-selected':
			...
			update_site_option( 'allowedthemes', $allowed_themes );
			trm_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
			break;
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) )
				trm_die( __('You do not have sufficient permissions to delete themes for this site.') );
			check_admin_referer( 'bulk-themes' );
			...
			foreach ( $themes as $key => $theme ) {
				$data = get_theme_data( TRM_CONTENT_DIR . '/858483/' . $theme . '/style.css' );
				if ( $data['Name'] == $main_theme ) {
					unset( $themes[$key] );
				} else {
					$files_to_delete = array_merge( $files_to_delete, list_files( TRM_CONTENT_DIR . "/858483/$theme" ) );					
					$theme_info[ $theme ] = $data;
				}
			}
			
			if ( empty( $themes ) ) {
				trm_redirect( add_query_arg( 'error', 'main', $referer ) );
				exit;
			}
			...
			<div class="wrap">
				...
				<form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" style="display:inline;">
					...
					?>
					<?php trm_nonce_field('bulk-themes') ?>
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array) $files_to_delete as $file )
							echo '<li>' . esc_html( str_replace( TRM_CONTENT_DIR . "/858483", '', $file) ) . '</li>';
					?>
					</ul>
				</div>
			</div>
			...
			trm_redirect( network_admin_url( "themes.php?deleted=".count( $themes )."&paged=$paged&s=$s" ) );
			exit;
			break;
	}
}
...
add_contextual_help($current_screen,
	...
	'<p>' . __('<a href="http://codex.trendr.org/Network_Admin_Themes_Screen" target="_blank">Documentation on Network Themes</a>') . '</p>' .
...
<?php $trm_list_table->search_box( __( 'Search Installed Themes' ), 'theme' ); ?>
</form>
