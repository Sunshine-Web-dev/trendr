...
if ( empty( $_GET['action'] ) ) {
	trm_redirect( admin_url( 'index.php' ) );
	exit;
}
...
function confirm_delete_users( $users ) {
	...
	<?php
	trm_nonce_field( 'ms-users-delete' );
	...
	foreach ( ( $allusers = (array) $_POST['allusers'] ) as $key => $val ) {
		if ( $val != '' && $val != '0' ) {
			$delete_user = new TRM_User( $val );

			if ( ! current_user_can( 'delete_user', $delete_user->ID ) )
				trm_die( sprintf( __( 'Warning! User %s cannot be deleted.' ), $delete_user->user_login ) );

			if ( in_array( $delete_user->user_login, $site_admins ) )
				trm_die( sprintf( __( 'Warning! User cannot be deleted. The user %s is a network admnistrator.' ), $delete_user->user_login ) );
...
switch ( $_GET['action'] ) {
	case 'siteoptions':
		...
		if ( empty( $_POST ) )
			trm_die( sprintf( __( 'You probably need to go back to the <a href="%s">options page</a>.' ), esc_url( admin_url( 'settings.php' ) ) ) );

		if ( isset($_POST['TRMLANG']) && ( '' === $_POST['TRMLANG'] || in_array( $_POST['TRMLANG'], get_available_languages() ) ) )
			update_site_option( 'TRMLANG', $_POST['TRMLANG'] );
		...
		do_action( 'update_trmmu_options' );

		trm_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
		exit();
	break;
	...
	case 'deleteblog':
		...
		if ( $id != '0' && $id != $current_site->blog_id && current_user_can( 'delete_site', $id ) ) {
			trmmu_delete_blog( $id, true );
			trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'delete' ), trm_get_referer() ) );
		} else {
			trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'not_deleted' ), trm_get_referer() ) );
		}
		...
		if ( ( isset( $_POST['action'] ) || isset( $_POST['action2'] ) ) && isset( $_POST['allblogs'] ) ) {
			...
			foreach ( (array) $_POST['allblogs'] as $key => $val ) {
				if ( $val != '0' && $val != $current_site->blog_id ) {
					switch ( $doaction ) {
						case 'delete':
							if ( ! current_user_can( 'delete_site', $val ) )
								trm_die( __( 'You are not allowed to delete the site.' ) );
							$blogfunction = 'all_delete';
							trmmu_delete_blog( $val, true );
						break;
					...
					trm_die( __( 'You are not allowed to change the current site.' ) );
				}
			}

			trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $blogfunction ), trm_get_referer() ) );
		} else {
			trm_redirect( network_admin_url( 'sites.php' ) );
		}
		exit();
	break;
	...
	case 'archiveblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'archive' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'unarchiveblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'unarchive' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'activateblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'activate' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'deactivateblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'deactivate' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'unspamblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'unspam' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'spamblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'spam' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'unmatureblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'unmature' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'matureblog':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'mature' ), trm_get_referer() ) );
		exit();
	break;
	...
	case 'confirm':
		...
		<html xmlns="http://www.w3.org/1999/xhtml" <?php if ( function_exists( 'language_attributes' ) ) language_attributes(); ?>>
			<head>
				<title><?php _e( 'Trnder &rsaquo; Confirm your action' ); ?></title>
			...
			<body>
				<h1 id="logo"><img alt="Trnder" src="<?php echo esc_attr( admin_url( 'images/trendr-logo.png' ) ); ?>" /></h1>
				<form action="edit.php?action=<?php echo esc_attr( $_GET['action2'] ) ?>" method="post">
					<input type="hidden" name="action" value="<?php echo esc_attr( $_GET['action2'] ) ?>" />
					<input type="hidden" name="id" value="<?php echo esc_attr( $id ); ?>" />
					<input type="hidden" name="_http_referer" value="<?php echo esc_attr( trm_get_referer() ); ?>" />
					<?php trm_nonce_field( $_GET['action2'], '_key', false ); ?>
	...
	case 'deleteuser':
		...
		if ( $id != '0' && $id != '1' ) {
			...
			trm_redirect( network_admin_url( 'users.php' ) );
		}
		exit();
	break;
	...
	case 'allusers':
		...
		if ( ( isset( $_POST['action']) || isset($_POST['action2'] ) ) && isset( $_POST['allusers'] ) ) {
			...
			foreach ( (array) $_POST['allusers'] as $key => $val ) {
				if ( !empty( $val ) ) {
					switch ( $doaction ) {
						...
						case 'spam':
							$user = new TRM_User( $val );
							if ( in_array( $user->user_login, get_super_admins() ) )
								trm_die( sprintf( __( 'Warning! User cannot be modified. The user %s is a network administrator.' ), esc_html( $user->user_login ) ) );
			...
			trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $userfunction ), trm_get_referer() ) );
		} else {
	...
	case 'dodelete':
		...
		trm_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $deletefunction ), network_admin_url( 'users.php' ) ) );
		exit();
	break;
	...
	default:
		...
		trm_redirect( network_admin_url( 'index.php' ) );
		exit();
	break;
}
?>
