...
add_contextual_help($current_screen,
	...
	'<p>' . __('<a href="http://codex.trendr.org/Network_Admin_Updates_Screen" target="_blank">Documentation on Update Network</a>') . '</p>' .
...
switch ( $action ) {
	case "upgrade":
		...
		if ( $n < 5 ) {
			...
			update_site_option( 'trmmu_upgrade_site', $trm_db_version );
		}

		$blogs = $trmdb->get_results( "SELECT * FROM {$trmdb->blogs} WHERE site_id = '{$trmdb->siteid}' AND spam = '0' AND deleted = '0' AND archived = '0' ORDER BY registered DESC LIMIT {$n}, 5", ARRAY_A );
		if ( empty( $blogs ) ) {
		...
		foreach ( (array) $blogs as $details ) {
			...
			echo "<li>$siteurl</li>";
			$response = trm_remote_get( trailingslashit( $siteurl ) . "Backend-WeaprEcqaKejUbRq-trendr/upgrade.php?step=upgrade_db", array( 'timeout' => 120, 'httpversion' => '1.1' ) );
			if ( is_trm_error( $response ) )
				trm_die( sprintf( __( 'Warning! Problem updating %1$s. Your server may not be able to connect to sites running on it. Error message: <em>%2$s</em>' ), $siteurl, $response->get_error_message() ) );
			do_action( 'after_mu_upgrade', $response );
			do_action( 'trmmu_upgrade_site', $details[ 'blog_id' ] );
		}
		echo "</ul>";
