...
add_contextual_help($current_screen,
	...
	'<p>' . __('<a href="http://codex.trendr.org/Network_Admin_Settings_Screen" target="_blank">Documentation on Network Settings</a>') . '</p>' .
...
<div class="wrap">
	...
	<form method="post" action="edit.php?action=siteoptions">
		<?php trm_nonce_field( 'siteoptions' ); ?>
			...
			<tr valign="top">
				...
				<td>
					...
					<?php if ( is_subdomain_install() )
						_e( 'If registration is disabled, please set <code>NOBLOGREDIRECT</code> in <code>trm-setup.php</code> to a URL you will redirect visitors to if they visit a non-existent site.' );
					?>
				</td>
			</tr>
...
<?php
		...
		if ( ! empty( $languages ) ) {
			$lang = get_site_option( 'TRMLANG' );
?>
		<h3><?php _e( 'Language Settings' ); ?></h3>
		<table class="form-table">
				<tr valign="top">
					<th><label for="TRMLANG"><?php _e( 'Default Language' ); ?></label></th>
					<td>
						<select name="TRMLANG" id="TRMLANG">
							<?php mu_dropdown_languages( $languages, get_site_option( 'TRMLANG' ) ); ?>
						</select>
					</td>
				</tr>
		</table>
...
?>
		...
		<?php do_action( 'trmmu_options' ); // Add more options here ?>
