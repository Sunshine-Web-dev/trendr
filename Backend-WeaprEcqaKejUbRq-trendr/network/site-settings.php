...
if ( isset($_REQUEST['action']) && 'update-site' == $_REQUEST['action'] && is_array( $_POST['option'] ) ) {
	...
	foreach ( (array) $_POST['option'] as $key => $val ) {
		if ( $key === 0 || is_array( $val ) || in_array($key, $skip_options) )
			continue; // Avoids "0 is a protected TRM option and may not be modified" error when edit blog options
		if ( $c == $count )
	...
	do_action( 'trmmu_update_blog_options' );
	restore_current_blog();
	trm_redirect( add_query_arg( array( 'update' => 'updated', 'id' => $id ), 'site-settings.php') );
	exit;
}
...
<form method="post" action="site-settings.php?action=update-site">
	...
	<table class="form-table">
		...
		$options = $trmdb->get_results( "SELECT * FROM {$blog_prefix}options WHERE option_name NOT LIKE '\_%' AND option_name NOT LIKE '%user_roles'" );
		...
		do_action( 'trmmueditblogaction', $id );
		?>
	</table>
	<?php submit_button(); ?>
</form>
