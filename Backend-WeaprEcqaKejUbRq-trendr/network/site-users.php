...
if ( ! current_user_can('manage_sites') )
	trm_die(__('You do not have sufficient permissions to edit this site.'));
...
add_contextual_help($current_screen,
	...
	'<p>' . __('<strong>Settings</strong> - This page shows a list of all settings associated with this site. Some are created by Trnder and others are created by plugins you activate. Note that some fields are grayed out and say Serialized Data. You cannot modify these values due to the way the setting is stored in the database.') . '</p>' .
...
// @todo This is a hack. Eventually, add API to TRM_Roles allowing retrieval of roles for a particular blog.
if ( ! empty($trm_roles->use_db) ) {
	$editblog_roles = get_blog_option( $id, "{$blog_prefix}user_roles" );
} else {
	// Roles are stored in memory, not the DB.
	$editblog_roles = $trm_roles->roles;
}
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'newuser':
			check_admin_referer( 'add-user', '_key_add-new-user' );
			$user = $_POST['user'];
			if ( !is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				$update = 'err_new';
			} else {
				$password = trm_generate_password( 12, false);
				$user_id = trmmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
		...
		case 'adduser':
			...
			if ( !empty( $_POST['newuser'] ) ) {
				...
				$userid = $trmdb->get_var( $trmdb->prepare( "SELECT ID FROM " . $trmdb->users . " WHERE user_login = %s", $newuser ) );
				if ( $userid ) {
					$user = $trmdb->get_var( "SELECT user_id FROM " . $trmdb->usermeta . " WHERE user_id='$userid' AND meta_key='{$blog_prefix}capabilities'" );
					if ( $user == false )
	...
	trm_redirect( add_query_arg( 'update', $update, trm_get_referer() ) );
	exit();
}
...
<form action="<?php echo network_admin_url('site-users.php?action=newuser'); ?>" id="newuser" method="post">
	...
	<?php trm_nonce_field( 'add-user', '_key_add-new-user' ) ?>
