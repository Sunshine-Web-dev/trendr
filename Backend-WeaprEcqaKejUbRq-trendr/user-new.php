...
%%s' ), get_bloginfo('name'), site_url(), esc_html( $_REQUEST[ 'role' ] ) );
	...
	add_filter( 'trmmu_signup_user_notification_email', 'admin_created_user_email' );
...
if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
	check_admin_referer( 'add-user', '_key_add-user' );
	...
	if ( false !== strpos($_REQUEST[ 'email' ], '@') ) {
		...
		if ( is_super_admin() ) {
			...
			trm_redirect( add_query_arg( array('update' => 'enter_email'), 'user-new.php' ) );
			die();
		}
	}

	if ( !$user_details ) {
		trm_redirect( add_query_arg( array('update' => 'does_not_exist'), 'user-new.php' ) );
		die();
	}
	...
	if ( ( $username != null && !is_super_admin( $user_id ) ) && ( array_key_exists($blog_id, get_blogs_of_user($user_id)) ) ) {
		...
		if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
			...
			trm_mail( $new_user_email, sprintf( __( '[%s] Joining confirmation' ), get_option( 'blogname' ) ),  sprintf($message, get_option('blogname'), site_url(), $_REQUEST[ 'role' ], site_url("/newbloguser/$newuser_key/")));
	...
	check_admin_referer( 'create-user', '_key_create-user' );
	...
	if ( !is_multisite() ) {
		...
		$user_details = trmmu_validate_user_signup( $_REQUEST[ 'user_login' ], $_REQUEST[ 'email' ] );
		unset( $user_details[ 'errors' ]->errors[ 'user_email_used' ] );
		if ( is_trm_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
			$add_user_errors = $user_details[ 'errors' ];
		} else {
			$new_user_login = apply_filters('pre_user_login', sanitize_user(stripslashes($_REQUEST['user_login']), true));
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				add_filter( 'trmmu_signup_user_notification', '__return_false' ); // Disable confirmation email
			}
			trmmu_signup_user( $new_user_login, $_REQUEST[ 'email' ], array( 'add_to_blog' => $trmdb->blogid, 'new_role' => $_REQUEST[ 'role' ] ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				$key = $trmdb->get_var( $trmdb->prepare( "SELECT activation_key FROM {$trmdb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $_REQUEST[ 'email' ] ) );
				trmmu_activate_signup( $key );
				$redirect = add_query_arg( array('update' => 'addnoconfirmation'), 'user-new.php' );
			} else {
...
add_contextual_help($current_screen,
    ...
    '<p>' . __('<a href="http://codex.trendr.org/Users_Add_New_Screen" target="_blank">Documentation on Adding New Users</a>') . '</p>' .
...
<?php if ( isset($add_user_errors) && is_trm_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<?php trm_nonce_field( 'add-user', '_key_add-user' ) ?>
...
<table class="form-table">
	...
	<tr class="form-field">
		...
		<td><select name="role" id="adduser-role">
			<?php trm_dropdown_roles( get_option('default_role') ); ?>
			</select>
		</td>
	</tr>
...
<?php trm_nonce_field( 'create-user', '_key_create-user' ) ?>
<?php
...
<?php } // !is_multisite ?>
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			trm_dropdown_roles($new_user_role);
			?>
			</select>
		</td>
	</tr>
