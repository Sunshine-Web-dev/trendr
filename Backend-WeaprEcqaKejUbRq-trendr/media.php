...
case 'editattachment' :
	...
	if ( !current_user_can('edit_post', $attachment_id) )
		trm_die ( __('You are not allowed to edit this attachment.') );
...
case 'edit' :
	...
	if ( empty( $_GET['attachment_id'] ) ) {
		trm_redirect( admin_url('upload.php') );
		exit();
	}
	...
	if ( empty($att->ID) ) trm_die( __('You attempted to edit an attachment that doesn&#8217;t exist. Perhaps it was deleted?') );
	if ( $att->post_status == 'trash' ) trm_die( __('You can&#8217;t edit this attachment because it is in the Trash. Please move it out of the Trash and try again.') );

	add_filter('attachment_fields_to_edit', 'media_single_attachment_fields_to_edit', 10, 2);

	trm_enqueue_script( 'trm-ajax-response' );
	trm_enqueue_script('image-edit');
	...
	'<p>' . __('<a href="http://codex.trendr.org/Media_Add_New_Screen#Edit_Media" target="_blank">Documentation on Edit Media</a>') . '</p>' .
...
<?php trm_original_referer_field(true, 'previous'); ?>
