...
if ( ! current_user_can( 'manage_options' ) )
	trm_die(__( 'You do not have sufficient permissions to delete this site.'));

if ( isset( $_GET['h'] ) && $_GET['h'] != '' && get_option( 'delete_blog_hash' ) != false ) {
	if ( get_option( 'delete_blog_hash' ) == $_GET['h'] ) {
		trmmu_delete_blog( $trmdb->blogid );
		trm_die( sprintf( __( 'Thank you for using %s, your site has been deleted. Happy trails to you until we meet again.' ), $current_site->site_name ) );
	} else {
		trm_die( __( "I'm sorry, the link you clicked is stale. Please select another option." ) );
	}
}
...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'deleteblog' && isset( $_POST['confirmdelete'] ) && $_POST['confirmdelete'] == '1' ) {
	...
	$hash = trm_generate_password( 20, false );
...
###SITE_NAME###" ) );
	...
	trm_mail( get_option( 'admin_email' ), "[ " . get_option( 'blogname' ) . " ] ".__( 'Delete My Site' ), $content );
	?>
...
<?php } else {
	...
	<form method="post" name="deletedirect">
		<?php trm_nonce_field( 'delete-blog' ) ?>
