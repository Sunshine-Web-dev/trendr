...
add_contextual_help($current_screen,
    ...
    '<p>' . __('<a href="http://codex.trendr.org/Users_Screen" target="_blank">Documentation on Managing Users</a>') . '</p>' .
    '<p>' . __('<a href="http://codex.trendr.org/Roles_and_Capabilities" target="_blank">Descriptions of Roles and Capabilities</a>') . '</p>' .
    '<p>' . __('<a href="http://trendr.org/support/" target="_blank">Support Forums</a>') . '</p>'
);

if ( empty($_REQUEST) ) {
	$referer = '<input type="hidden" name="trm_http_referer" value="'. esc_attr(stripslashes($_SERVER['REQUEST_URI'])) . '" />';
} elseif ( isset($_REQUEST['trm_http_referer']) ) {
	$redirect = remove_query_arg(array('trm_http_referer', 'updated', 'delete_count'), stripslashes($_REQUEST['trm_http_referer']));
	$referer = '<input type="hidden" name="trm_http_referer" value="' . esc_attr($redirect) . '" />';
} else {
	$redirect = 'users.php';
	$referer = '';
}
...
switch ( $trm_list_table->current_action() ) {
...
case 'promote':
	...
	if ( ! current_user_can( 'promote_users' ) )
		trm_die( __( 'You can&#8217;t edit that user.' ) );
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $current_user->ID && !$trm_roles->role_objects[$_REQUEST['new_role']]->has_cap('promote_users') ) {
	...
	trm_redirect(add_query_arg('update', $update, $redirect));
	exit();
...
case 'dodelete':
	if ( is_multisite() )
		trm_die( __('User deletion is not allowed from this screen.') );
	...
	if ( ! current_user_can( 'delete_users' ) )
		trm_die(__('You can&#8217;t delete users.'));
	...
	foreach ( (array) $userids as $id) {
		...
		if ( ! current_user_can( 'delete_user', $id ) )
			trm_die(__( 'You can&#8217;t delete that user.' ) );
		...
		case 'reassign':
			if ( current_user_can('delete_user', $id) )
				trm_delete_user($id, $_REQUEST['reassign_user']);
			break;
		}
		++$delete_count;
	}
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new TRM_Error( 'edit_users', __( 'You can&#8217;t delete users.' ) );
...
<?php trm_nonce_field('delete-users') ?>
<?php echo $referer; ?>
...
<?php if ( $go_delete ) : ?>
	...
	<ul style="list-style:none;">
		...
		trm_dropdown_users( array( 'name' => 'reassign_user', 'exclude' => array_diff( $userids, array($current_user->ID) ) ) ); ?></li>
	</ul></fieldset>
...
case 'doremove':
	...
	if ( ! is_multisite() )
		trm_die( __( 'You can&#8217;t remove users.' ) );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new TRM_Error('edit_users', __('You can&#8217;t remove users.'));
...
?>
<form action="" method="post" name="updateusers" id="updateusers">
<?php trm_nonce_field('remove-users') ?>
<?php echo $referer; ?>
...
default:
	...
	if ( !empty($_GET['_http_referer']) ) {
		trm_redirect(remove_query_arg(array('_http_referer', '_key'), stripslashes($_SERVER['REQUEST_URI'])));
		exit;
	}

	$trm_list_table->prepare_items();
	$total_pages = $trm_list_table->get_pagination_arg( 'total_pages' );
	if ( $pagenum > $total_pages && $total_pages > 0 ) {
		trm_redirect( add_query_arg( 'paged', $total_pages ) );
		exit;
	}
	include('./admin-header.php');
...
<?php $trm_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
