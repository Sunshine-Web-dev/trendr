...
class Custom_Image_Header {
	...
	function help() {
		...
		'<p>' . __( '<a href="http://codex.trendr.org/Appearance_Header_Screen" target="_blank">Documentation on Custom Header</a>' ) . '</p>' .
	...
	function js_includes() {
		...
		if ( ( 1 == $step || 3 == $step ) && $this->header_text() )
			trm_enqueue_script('farbtastic');
		elseif ( 2 == $step )
			trm_enqueue_script('imgareaselect');
	}
	...
	function css_includes() {
		...
		if ( ( 1 == $step || 3 == $step ) && $this->header_text() )
			trm_enqueue_style('farbtastic');
		elseif ( 2 == $step )
			trm_enqueue_style('imgareaselect');
	}
	...
	function take_action() {
		...
		if ( isset( $_POST['resetheader'] ) ) {
			check_admin_referer( 'custom-header-options', '_key-custom-header-options' );
	...
	function process_default_headers() {
		...
		if ( !isset($_trm_default_headers) )
			return;

		$this->default_headers = $_trm_default_headers;
...
<td>
	...
	<p>
		...
		<?php trm_nonce_field( 'custom-header-upload', '_key-custom-header-upload' ) ?>
...
trm_nonce_field( 'custom-header-options', '_key-custom-header-options' ); ?>
...
<?php }
	 ...
	 */
	function step_2() {
		check_admin_referer('custom-header-upload', '_key-custom-header-upload');
		if ( ! current_theme_supports( 'custom-header-uploads' ) )
			trm_die( 'Cheatin&#8217; uh?' );

		$overrides = array('test_form' => false);
		$file = trm_handle_upload($_FILES['import'], $overrides);

		if ( isset($file['error']) )
			trm_die( $file['error'],  __( 'Image Upload Error' ) );
		...
		// Save the data
		$id = trm_insert_attachment($object, $file);
		...
		if ( $width == HEADER_IMAGE_WIDTH && $height == HEADER_IMAGE_HEIGHT ) {
			...
			update_post_meta( $id, '_trm_attachment_is_custom_header', get_option('stylesheet' ) );

			set_theme_mod('header_image', esc_url($url));
			do_action('trm_create_file_in_uploads', $file, $id); // For replication
			return $this->finished();
		} elseif ( $width > HEADER_IMAGE_WIDTH ) {
			$oitar = $width / HEADER_IMAGE_WIDTH;
			$image = trm_crop_image($file, 0, 0, $width, $height, HEADER_IMAGE_WIDTH, $height / $oitar, false, str_replace(basename($file), 'midsize-'.basename($file), $file));
			if ( is_trm_error( $image ) )
				trm_die( __( 'Image could not be processed.  Please go back and try again.' ), __( 'Image Processing Error' ) );

			$image = apply_filters('trm_create_file_in_uploads', $image, $id); // For replication
...
<form method="post" action="<?php echo esc_attr(add_query_arg('step', 3)); ?>">
	...
	<?php trm_nonce_field( 'custom-header-crop-image' ) ?>
	...
	function step_3() {
		...
		$cropped = trm_crop_image( $attachment_id, (int) $_POST['x1'], (int) $_POST['y1'], (int) $_POST['width'], (int) $_POST['height'], HEADER_IMAGE_WIDTH, HEADER_IMAGE_HEIGHT );
		if ( is_trm_error( $cropped ) )
			trm_die( __( 'Image could not be processed.  Please go back and try again.' ), __( 'Image Processing Error' ) );

		$cropped = apply_filters('trm_create_file_in_uploads', $cropped, $attachment_id); // For replication
		...
		trm_insert_attachment($object, $cropped);
		trm_update_attachment_metadata( $attachment_id, trm_generate_attachment_metadata( $attachment_id, $cropped ) );
		update_post_meta( $attachment_id, '_trm_attachment_is_custom_header', get_option('stylesheet' ) );

		set_theme_mod('header_image', $url);

		// cleanup
		$medium = str_replace(basename($original), 'midsize-'.basename($original), $original);
		@unlink( apply_filters( 'trm_delete_file', $medium ) );
		@unlink( apply_filters( 'trm_delete_file', $original ) );
	...
	function admin_page() {
		if ( ! current_user_can('edit_theme_options') )
			trm_die(__('You do not have permission to customize headers.'));
		$step = $this->step();
		if ( 1 == $step )
			$this->step_1();
		elseif ( 2 == $step )
