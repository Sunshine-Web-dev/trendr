...
$trm_list_table = _get_list_table('TRM_Themes_List_Table');

if ( current_user_can( 'switch_themes' ) && isset($_GET['action'] ) ) {
	if ( 'activate' == $_GET['action'] ) {
		check_admin_referer('switch-theme_' . $_GET['template']);
		switch_theme($_GET['template'], $_GET['stylesheet']);
		trm_redirect( admin_url('themes.php?activated=true') );
		exit;
		...
		trm_redirect( admin_url('themes.php?deleted=true') );
		exit;
	}
}
...
$help = '<p>' . __('Aside from the default theme included with your Trnder installation, themes are designed and developed by third parties.') . '</p>';
...
if ( current_user_can('install_themes') )
	$help .= '<p>' . sprintf(__('If you would like to see more themes to choose from, click on the &#8220;Install Themes&#8221; tab and you will be able to browse or search for additional themes from the <a href="%s" target="_blank">Trnder.org Theme Directory</a>. Themes in the Trnder.org Theme Directory are designed and developed by third parties, and are compatible with the license Trnder uses. Oh, and they&#8217;re free!'), 'http://trendr.org/extend/858483/') . '</p>';
...
<?php elseif ( isset($_GET['activated']) ) :
		if ( isset($trm_registered_sidebars) && count( (array) $trm_registered_sidebars ) && current_user_can('edit_theme_options') ) { ?>
	...
	if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
		foreach ( (array) $submenu['themes.php'] as $item) {
			...
			if ( !empty($submenu[$item[2]]) ) {
				...
				if ( file_exists(ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/' . $item[2]) ) {
...
<?php if ( !empty( $_REQUEST['s'] ) || !empty( $_REQUEST['filter'] ) || $trm_list_table->has_items() ) : ?>
		...
		<div class="feature-container">
			...
			<ol class="feature-group">
				<?php foreach ( $features as $key => $feature ) :
						...
						?>
				<li>
					<input type="checkbox" name="features[]" id="feature-id-<?php echo $key; ?>" value="<?php echo $key; ?>" <?php checked( in_array( $key, $trm_list_table->features ) ); ?>/>
