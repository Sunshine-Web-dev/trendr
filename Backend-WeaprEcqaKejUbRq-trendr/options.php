...
trm_reset_vars(array('action', 'option_page'));
...
if ( is_multisite() ) {
	if ( ! empty($_GET[ 'adminhash' ] ) ) {
		...
		trm_redirect( admin_url( $redirect ) );
		exit;
	} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' == $_GET['dismiss'] ) {
		delete_option( 'adminhash' );
		delete_option( 'new_admin_email' );
		trm_redirect( admin_url( 'options-general.php?updated=true' ) );
		exit;
	}
}
...
if ( !is_multisite() ) {
	...
	$whitelist_options['general'][] = 'TRMLANG';
...
if ( 'update' == $action ) {
	...
	if ( !isset( $whitelist_options[ $option_page ] ) )
		trm_die( __( 'Error: options page not found.' ) );

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! is_super_admin() )
			trm_die( __( 'You do not have sufficient permissions to modify unregistered settings for this site.' ) );
	...
	if ( $options ) {
		foreach ( $options as $option ) {
			if ( $unregistered )
				_deprecated_argument( 'options.php', '2.7', sprintf( __( 'The <code>%1$s</code> setting is unregistered. Unregistered settings are deprecated. See http://codex.trendr.org/Settings_API' ), $option, $option_page ) );
	 ...
	 */
	$goback = add_query_arg( 'settings-updated', 'true',  trm_get_referer() );
	trm_redirect( $goback );
	exit;
}
...
<?php screen_icon(); ?>
  ...
  <?php trm_nonce_field('options-options') ?>
  <input type="hidden" name="action" value="update" />
  <input type='hidden' name='option_page' value='options' />
  <table class="form-table">
<?php
$options = $trmdb->get_results( "SELECT * FROM $trmdb->options ORDER BY option_name" );
