...
case 'spam'    :
	...
	if ( !$comment = get_comment_to_edit( $comment_id ) ) {
		trm_redirect( admin_url('edit-comments.php?error=1') );
		die();
	}

	if ( !current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		trm_redirect( admin_url('edit-comments.php?error=2') );
		die();
	}

	// No need to re-approve/re-trash/re-spam a comment.
	if ( $action == str_replace( '1', 'approve', $comment->comment_approved ) ) {
		trm_redirect( admin_url( 'edit-comments.php?same=' . $comment_id ) );
		die();
 	}
...
case 'unapprovecomment' :
	...
	if ( '' != trm_get_referer() && ! $noredir && false === strpos(trm_get_referer(), 'comment.php') )
		$redir = trm_get_referer();
	elseif ( '' != trm_get_original_referer() && ! $noredir )
		$redir = trm_get_original_referer();
