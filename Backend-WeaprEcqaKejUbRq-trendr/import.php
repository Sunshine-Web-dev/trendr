<?php
/**
 * Import Trnder Administration Screen
 *
 * @package Trnder
...
if ( !current_user_can('import') )
	trm_die(__('You do not have sufficient permissions to import content in this site.'));
...
add_contextual_help($current_screen,
	...
	'<p>' . __('In previous versions of Trnder, all the importers were built-in, but they have been turned into plugins as of version 3.0 since most people only use them once or infrequently.') . '</p>' .
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Tools_Import_Screen" target="_blank">Documentation on Import</a>') . '</p>' .
...
if ( current_user_can('install_plugins') )
	$popular_importers = array(
		...
		'trmcat2tag' => array(__('Categories and Tags Converter'), __('Install the category/tag converter to convert existing categories to tags or tags to categories, selectively.'), 'install', 'trm-cat2tag' ),
		...
		'trendr' => array( 'Trnder', __('Install the Trnder importer to import posts, pages, comments, custom fields, categories, and tags from a Trnder export file.'), 'install' )
	);

if ( ! empty( $_GET['invalid'] ) && !empty($popular_importers[$_GET['invalid']][3]) ) {
	trm_redirect( admin_url('import.php?import=' . $popular_importers[$_GET['invalid']][3]) );
	exit;
}
...
<?php endif; ?>
<p><?php _e('If you have posts or comments in another system, Trnder can import those into this site. To get started, choose a system to import from below:'); ?></p>

<?php

// Load all importers so that they can register.
$import_loc = 'Backend-WeaprEcqaKejUbRq-trendr/import';
...
<?php
	...
	foreach ($importers as $id => $data) {
		...
		if ( 'install' == $data[2] ) {
			...
			if ( file_exists( TRM_PLUGIN_DIR . '/' . $plugin_slug ) ) {
				// Looks like Importer is installed, But not active
				$plugins = get_plugins( '/' . $plugin_slug );
				if ( !empty($plugins) ) {
					$keys = array_keys($plugins);
					$plugin_file = $plugin_slug . '/' . $keys[0];
					$action = '<a href="' . esc_url(trm_nonce_url(admin_url('plugins.php?action=activate&plugin=' . $plugin_file . '&from=import'), 'activate-plugin_' . $plugin_file)) .
			...
			$action = "<a href='" . esc_url("admin.php?import=$id") . "' title='" . esc_attr( trmtexturize(strip_tags($data[1])) ) ."'>{$data[0]}</a>";
		}
