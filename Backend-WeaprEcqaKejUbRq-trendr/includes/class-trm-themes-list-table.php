 ...
 * @access private
 */
class TRM_Themes_List_Table extends TRM_List_Table {
	...
	function no_items() {
		...
		if ( is_multisite() ) {
			...
			if ( current_user_can( 'install_themes' ) ) {
				printf( __( 'You only have one theme installed right now. Live a little! You can choose from over 1,000 free themes in the Trnder.org Theme Directory at any time: just click on the <a href="%s">Install Themes</a> tab above.' ), admin_url( 'theme-install.php' ) );
	...
	function tablenav( $which = 'top' ) {
		...
		<div class="tablenav <?php echo $which; ?>">
		   ...
		   <img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" class="ajax-loading list-ajax-loading" alt="" />
		  <br class="clear" />
		</div>
		<?php
	}
	...
	function display() {
		// trm_nonce_field( "fetch-list-" . get_class( $this ), '_ajax_fetch_list_nonce' );
?>
...
<?php if ( !empty( $theme_name ) ) :
	...
	if ( ! is_multisite() && current_user_can( 'delete_themes' ) )
		$actions[] = '<a class="submitdelete deletion" href="' . trm_nonce_url( "themes.php?action=delete&amp;template=$stylesheet", 'delete-theme_' . $stylesheet ) . '" onclick="' . "return confirm( '" . esc_js( sprintf( __( "You are about to delete this theme '%s'\n  'Cancel' to stop, 'OK' to delete." ), $theme_name ) ) . "' );" . '">' . __( 'Delete' ) . '</a>';
...
<span class='action-links'><?php echo $actions ?></span>
	...
	<p><?php printf( __( 'The template files are located in <code>%2$s</code>. The stylesheet files are located in <code>%3$s</code>. <strong>%4$s</strong> uses templates from <strong>%5$s</strong>. Changes made to the templates will affect both themes.' ), $title, str_replace( TRM_CONTENT_DIR, '', $template_dir ), str_replace( TRM_CONTENT_DIR, '', $stylesheet_dir ), $title, $parent_theme ); ?></p>
<?php } else { ?>
	<p><?php printf( __( 'All of this theme&#8217;s files are located in <code>%2$s</code>.' ), $title, str_replace( TRM_CONTENT_DIR, '', $template_dir ), str_replace( TRM_CONTENT_DIR, '', $stylesheet_dir ) ); ?></p>
<?php } ?>
<?php if ( $tags ) : ?>
