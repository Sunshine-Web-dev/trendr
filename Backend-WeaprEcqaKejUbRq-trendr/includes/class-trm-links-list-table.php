 ...
 * @access private
 */
class TRM_Links_List_Table extends TRM_List_Table {
	...
	function prepare_items() {
		...
		trm_reset_vars( array( 'action', 'cat_id', 'linkurl', 'name', 'image', 'description', 'visible', 'target', 'category', 'link_id', 'submit', 'orderby', 'order', 'links_show_cat_id', 'rating', 'rel', 'notes', 'linkcheck[]', 's' ) );
...
<?php
	...
	function display_rows() {
		...
		foreach ( $this->items as $link ) {
			...
			$link->link_category = trm_get_link_cats( $link->link_id );
			...
			foreach ( $columns as $column_name => $column_display_name ) {
				...
				switch ( $column_name ) {
					...
					case 'name':
						...
						$actions['delete'] = "<a class='submitdelete' href='" . trm_nonce_url( "link.php?action=delete&amp;link_id=$link->link_id", 'delete-bookmark_' . $link->link_id ) . "' onclick=\"if ( confirm( '" . esc_js( sprintf( __( "You are about to delete this link '%s'\n  'Cancel' to stop, 'OK' to delete." ), $link->link_name ) ) . "' ) ) { return true;}return false;\">" . __( 'Delete' ) . "</a>";
