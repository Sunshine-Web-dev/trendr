<?php
/**
 * Trnder Theme Install Administration API
 *
 * @package Trnder
...
function install_themes_feature_list( ) {
	if ( !$cache = get_transient( 'trmorg_theme_feature_list' ) )
		set_transient( 'trmorg_theme_feature_list', array( ),  10800);
	...
	set_transient( 'trmorg_theme_feature_list', $feature_list, 10800 );
...
<form method="post" enctype="multipart/form-data" action="<?php echo self_admin_url('update.php?action=upload-theme') ?>">
	<?php trm_nonce_field( 'theme-upload') ?>
...
function display_theme($theme, $actions = null, $show_details = true) {
	...
	$name = trm_kses($theme->name, $themes_allowedtags);
	$desc = trm_kses($theme->description, $themes_allowedtags);
	//if ( strlen($desc) > 30 )
...
<p><strong><?php _e('Version:') ?></strong> <?php echo trm_kses($theme->version, $themes_allowedtags) ?></p>
<p><strong><?php _e('Author:') ?></strong> <?php echo trm_kses($theme->author, $themes_allowedtags) ?></p>
...
<p><strong><?php _e('Requires Trnder Version:') ?></strong> <?php printf(__('%s or higher'), $theme->requires) ?></p>
...
<?php }
	/*
	 ...
	 public 'preview_url' => string 'http://trm-themes.com/?magazine-basic' (length=36)
	 public 'screenshot_url' => string 'http://trm-themes.com/trm-src/858483/magazine-basic/screenshot.png' (length=68)
	 public 'rating' => float 80
	 public 'num_ratings' => int 1
	 public 'homepage' => string 'http://trendr.org/extend/858483/magazine-basic' (length=49)
	 ...
	 public 'download_link' => string 'http://trendr.org/extend/858483/download/magazine-basic.1.1.zip' (length=66)
	 */
}
...
function install_theme_information() {
	...
	foreach ( (array)$api->sections as $section_name => $content )
		$api->sections[$section_name] = trm_kses($content, $themes_allowedtags);

	foreach ( array('version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug') as $key ) {
		if ( isset($api->$key) )
			$api->$key = trm_kses($api->$key, $themes_allowedtags);
	}
	...
	if ( !empty($api->tested) && version_compare($GLOBALS['trm_version'], $api->tested, '>') )
		echo '<div class="updated"><p>' . __('<strong>Warning:</strong> This theme has <strong>not been tested</strong> with your current version of Trnder.') . '</p></div>';
	else if ( !empty($api->requires) && version_compare($GLOBALS['trm_version'], $api->requires, '<') )
		echo '<div class="updated"><p>' . __('<strong>Warning:</strong> This theme has not been marked as <strong>compatible</strong> with your version of Trnder.') . '</p></div>';
...
case 'install':
	...
	$buttons .= '<a class="button-primary" id="install" href="' . trm_nonce_url(self_admin_url('update.php?action=install-theme&theme=' . $api->slug), 'install-theme_' . $api->slug) . '" target="_parent">' . __('Install Now') . '</a>';
	endif;
	break;
case 'update_available':
	if ( current_user_can('update_themes') ) :
	$buttons .= '<a class="button-primary" id="install"	href="' . trm_nonce_url(self_admin_url('update.php?action=upgrade-theme&theme=' . $update_file), 'upgrade-theme_' . $update_file) . '" target="_parent">' . __('Install Update Now') . '</a>';
	endif;
	break;
