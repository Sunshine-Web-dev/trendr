...
foreach ($menu as $menu_page) {
	if ( false !== $pos = strpos($menu_page[2], '?') ) {
		...
		trm_parse_str($hook_args, $hook_args);
...
$_trm_submenu_nopriv = array();
$_trm_menu_nopriv = array();
	...
	foreach ($$sub_loop as $parent => $sub) {
		foreach ($sub as $index => $data) {
			if ( ! current_user_can($data[1]) ) {
				...
				$_trm_submenu_nopriv[$parent][$data[2]] = true;
			}
		}
		unset($index, $data);
...
foreach ( $menu as $id => $data ) {
	...
	if ( $new_parent != $old_parent ) {
		$_trm_real_parent_file[$old_parent] = $new_parent;
		...
		if ( isset($_trm_submenu_nopriv[$old_parent]) )
			$_trm_submenu_nopriv[$new_parent] = $_trm_submenu_nopriv[$old_parent];
	}
}
...
foreach ( $menu as $id => $data ) {
	...
	if ( empty($submenu[$data[2]]) ) {
		...
		if ( isset( $_trm_menu_nopriv[$data[2]] ) ) {
			unset($menu[$id]);
		}
	}
}
...
foreach ( $menu as $id => $data ) {
	if ( 0 == strcmp('trm-menu-separator', $data[4] ) ) {
...
if ( 'trm-menu-separator' == $menu[ $last_menu_key ][ 4 ] )
