<?php
/**
 * Trnder Administration Media API.
 *
 * @package Trnder
...
function update_gallery_tab($tabs) {
	...
	if ( $post_id )
		$attachments = intval( $trmdb->get_var( $trmdb->prepare( "SELECT count(*) FROM $trmdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent = %d", $post_id ) ) );
...
function get_image_send_to_editor($id, $caption, $title, $align, $url='', $rel = false, $size='medium', $alt = '') {
	...
	$rel = $rel ? ' rel="attachment trm-att-' . esc_attr($id).'"' : '';
 ...
 * @param array $overrides allows you to override the {@link trm_handle_upload()} behavior
...
function media_handle_upload($file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false )) {
	...
	$file = trm_handle_upload($_FILES[$file_id], $overrides, $time);

	if ( isset($file['error']) )
		return new TRM_Error( 'upload_error', $file['error'] );
	...
	if ( $image_meta = @trm_read_image_metadata($file) ) {
	...
	// Save the data
	$id = trm_insert_attachment($attachment, $file, $post_id);
	if ( !is_trm_error($id) ) {
		trm_update_attachment_metadata( $id, trm_generate_attachment_metadata( $id, $file ) );
	}
 ...
 * @param array $post_data allows you to overwrite some of the attachment
 * @return int|object The ID of the attachment or a TRM_Error on failure
 */
function media_handle_sideload($file_array, $post_id, $desc = null, $post_data = array()) {
	$overrides = array('test_form'=>false);

	$file = trm_handle_sideload($file_array, $overrides);
	if ( isset($file['error']) )
		return new TRM_Error( 'upload_error', $file['error'] );
 ...
 */
function trm_iframe($content_func /* ... */) {
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml" <?php do_action('admin_xml_ns'); ?> <?php language_attributes(); ?>>
<head>
<meta http-equiv="Content-Type" content="<?php bloginfo('html_type'); ?>; charset=<?php echo get_option('blog_charset'); ?>" />
<title><?php bloginfo('name') ?> &rsaquo; <?php _e('Uploads'); ?> &#8212; <?php _e('Trnder'); ?></title>
<?php
...
if ( ( is_array( $content_func ) && ! empty( $content_func[1] ) && 0 === strpos( (string) $content_func[1], 'media' ) )
	...
	trm_enqueue_style( 'media' );
trm_enqueue_style( 'ie' );
?>
<script type="text/javascript">
//<![CDATA[
...
var userSettings = {'url':'<?php echo SITECOOKIEPATH; ?>','uid':'<?php if ( ! isset($current_user) ) $current_user = trm_get_current_user(); echo $current_user->ID; ?>','time':'<?php echo time(); ?>'};
...
function media_upload_form_handler() {
	...
	if ( !empty($_POST['attachments']) ) foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
		...
		if ( isset($attachment['image_alt']) ) {
			$image_alt = get_post_meta($attachment_id, '_trm_attachment_image_alt', true);
			if ( $image_alt != stripslashes($attachment['image_alt']) ) {
				$image_alt = trm_strip_all_tags( stripslashes($attachment['image_alt']), true );
				// update_meta expects slashed
				update_post_meta( $attachment_id, '_trm_attachment_image_alt', addslashes($image_alt) );
			}
		}
		...
		if ( $post != $_post )
			trm_update_post($post);

		foreach ( get_attachment_taxonomies($post) as $t ) {
			if ( isset($attachment[$t]) )
				trm_set_object_terms($attachment_id, array_map('trim', preg_split('/,+/', $attachment[$t])), $t, false);
		}
	}
	...
	if ( isset($send_id) ) {
		...
		if ( !empty($attachment['url']) ) {
			...
			if ( strpos($attachment['url'], 'attachment_id') || get_attachment_link($send_id) == $attachment['url'] )
				$rel = " rel='attachment trm-att-" . esc_attr($send_id) . "'";
...
function media_upload_image() {
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' )
		return trm_iframe( 'media_upload_type_url_form', 'image', $errors, $id );

	return trm_iframe( 'media_upload_type_form', 'image', $errors, $id );
}
 ...
 * @return string|TRM_Error Populated HTML img tag on success
 */
...
function media_upload_audio() {
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' )
		return trm_iframe( 'media_upload_type_url_form', 'audio', $errors, $id );

	return trm_iframe( 'media_upload_type_form', 'audio', $errors, $id );
}
...
function media_upload_video() {
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' )
		return trm_iframe( 'media_upload_type_url_form', 'video', $errors, $id );

	return trm_iframe( 'media_upload_type_form', 'video', $errors, $id );
}
...
function media_upload_file() {
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' )
		return trm_iframe( 'media_upload_type_url_form', 'file', $errors, $id );

	return trm_iframe( 'media_upload_type_form', 'file', $errors, $id );
}
...
function media_upload_gallery() {
	...
	trm_enqueue_script('admin-gallery');
	return trm_iframe( 'media_upload_gallery_form', $errors );
}
...
function media_upload_library() {
	...
	return trm_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields($post, $url_type = '') {
	...
	$file = trm_get_attachment_url($post->ID);
...
function image_attachment_fields_to_edit($form_fields, $post) {
	if ( substr($post->post_mime_type, 0, 5) == 'image' ) {
		$alt = get_post_meta($post->ID, '_trm_attachment_image_alt', true);
		if ( empty($alt) )
			$alt = '';
...
function get_attachment_fields_to_edit($post, $errors = null) {
	...
	$image_url = trm_get_attachment_url($post->ID);
	...
	$form_fields = array(
		...
		'image_url'	=> array(
			...
			'value'      => trm_get_attachment_url($post->ID),
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array($GLOBALS['trm_the_query']->posts) )
			foreach ( $GLOBALS['trm_the_query']->posts as $attachment )
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( ( $attachment_id = intval( $attachment_id ) ) && $thumb_url = trm_get_attachment_image_src( $attachment_id, 'thumbnail', true ) )
	...
	$keys = array_keys( trm_match_mime_types( array_keys( $post_mime_types ), $post->post_mime_type ) );
	...
	$display_title = $show_title ? "<div class='filename new'><span class='title'>" . trm_html_excerpt( $display_title, 60 ) . "</span></div>" : '';
	...
	$media_dims = '';
	$meta = trm_get_attachment_metadata( $post->ID );
	...
	if ( gd_edit_image_support( $post->post_mime_type ) ) {
		$nonce = trm_create_nonce( "image_editor-$post->ID" );
		$image_edit_button = "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" . esc_attr__( 'Edit Image' ) . "' /> <img src='" . esc_url( admin_url( 'images/trmspin_light.gif' ) ) . "' class='imgedit-wait-spin' alt='' />";
	}
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( !EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . trm_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-attachment_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Delete Permanently' ) . '</a>';
		} elseif ( !MEDIA_TRASH ) {
			$delete = "<a href='#' class='del-link' onclick=\"document.getElementById('del_attachment_$attachment_id').style.display='block';return false;\">" . __( 'Delete' ) . "</a>
			 ...
			 <a href='" . trm_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-attachment_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . __( 'Continue' ) . "</a>
			 <a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __( 'Cancel' ) . "</a>
			 </div>";
		} else {
			$delete = "<a href='" . trm_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-attachment_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Move to Trash' ) . "</a>
			<a href='" . trm_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-attachment_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . __( 'Undo' ) . "</a>";
		}
	} else {
		$delete = '';
	}
	...
	if ( 'image' == $type && $calling_post_id && current_theme_supports( 'post-thumbnails', get_post_type( $calling_post_id ) ) && get_post_thumbnail_id( $calling_post_id ) != $attachment_id ) {
		$ajax_nonce = trm_create_nonce( "set_post_thumbnail-$calling_post_id" );
		$thumbnail = "<a class='trm-post-thumbnail' id='trm-post-thumbnail-" . $attachment_id . "' href='#' onclick='TRMSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html__( "Use as featured image" ) . "</a>";
	}
...
function media_upload_form( $errors = null ) {
	...
	$upload_size_unit = $max_upload_size =  trm_max_upload_size();
...
<?php if (isset($errors['upload_error']) && is_trm_error($errors['upload_error'])) { ?>
...
$post_params = array(
		...
		"_key" => trm_create_nonce('media-form'),
		"type" => $type,
		"tab" => $tab,
		"short" => "1",
);
...
<?php trm_nonce_field('media-form'); ?>
...
<?php } ?>
	...
	getImageData : function() {
		...
		document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" alt="" />';
...
<p class="ml-submit">
<input type="button" class="button" style="display:none;" onMouseDown="trmgallery.update();" name="insert-gallery" id="insert-gallery" value="<?php esc_attr_e( 'Insert gallery' ); ?>" />
<input type="button" class="button" style="display:none;" onMouseDown="trmgallery.update();" name="update-gallery" id="update-gallery" value="<?php esc_attr_e( 'Update gallery settings' ); ?>" />
</p>
</div>
</form>
<?php
...
function media_upload_library_form($errors) {
	global $trmdb, $trm_query, $trm_locale, $type, $tab, $post_mime_types;
	...
	list($post_mime_types, $avail_post_mime_types) = trm_edit_attachments_query();
...
<?php
$type_links = array();
$_num_posts = (array) trm_count_attachments();
$matches = trm_match_mime_types(array_keys($post_mime_types), array_keys($_num_posts));
...
foreach ( $post_mime_types as $mime_type => $label ) {
	...
	if ( !trm_match_mime_types($mime_type, $avail_post_mime_types) )
		continue;

	if ( isset($_GET['post_mime_type']) && trm_match_mime_types($mime_type, $_GET['post_mime_type']) )
		$class = ' class="current"';
...
$page_links = paginate_links( array(
	...
	'prev_text' => __('&laquo;'),
	'next_text' => __('&raquo;'),
	'total' => ceil($trm_query->found_posts / 10),
	'current' => $_GET['paged']
));
...
$arc_query = "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $trmdb->posts WHERE post_type = 'attachment' ORDER BY post_date DESC";

$arc_result = $trmdb->get_results( $arc_query );
...
foreach ($arc_result as $arc_row) {
	...
	echo esc_html( $trm_locale->get_month($arc_row->mmonth) . " $arc_row->yyear" );
	echo "</option>\n";
}
?>
</select>
