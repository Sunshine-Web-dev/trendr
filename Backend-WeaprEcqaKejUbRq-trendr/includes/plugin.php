 ...
 * Version: Must be set in the plugin for Trnder 2.3+
 ...
 * reading.
 *
 * @link http://trac.trendr.org/ticket/5651 Previous Optimizations.
 * @link http://trac.trendr.org/ticket/7372 Further and better Optimizations.
 * @since 1.5.0
 *
...
function _get_plugin_data_markup_translate($plugin_file, $plugin_data, $markup = true, $translate = true) {
	...
	$plugin_data['AuthorName'] = $plugin_data['Author'] = trm_kses( $plugin_data['Author'], $plugins_allowedtags );
	...
	if ( $markup ) {
		...
		$plugin_data['Description'] = trmtexturize( $plugin_data['Description'] );
		if ( ! empty($plugin_data['Author']) )
			$plugin_data['Description'] .= ' <cite>' . sprintf( __('By %s'), $plugin_data['Author'] ) . '.</cite>';
	}

	// Sanitize all displayed data. Author and AuthorName sanitized above.
	$plugin_data['Title']       = trm_kses( $plugin_data['Title'],       $plugins_allowedtags );
	$plugin_data['Version']     = trm_kses( $plugin_data['Version'],     $plugins_allowedtags );
	$plugin_data['Description'] = trm_kses( $plugin_data['Description'], $plugins_allowedtags );
	$plugin_data['Name']        = trm_kses( $plugin_data['Name'],        $plugins_allowedtags );
...
function get_plugin_files($plugin) {
	$plugin_file = TRM_PLUGIN_DIR . '/' . $plugin;
	$dir = dirname($plugin_file);
	$plugin_files = array($plugin);
	if ( is_dir($dir) && $dir != TRM_PLUGIN_DIR ) {
 ...
 *
 * Trnder only supports plugin files in the base plugins directory
 * (trm-src/module) and in one directory above the plugins directory
 * (trm-src/module/my-plugin). The file it looks for has the plugin data and
...
function get_plugins($plugin_folder = '') {
	...
	if ( ! $cache_plugins = trm_cache_get('plugins', 'plugins') )
		$cache_plugins = array();
	...
	$trm_plugins = array ();
	$plugin_root = TRM_PLUGIN_DIR;
	if ( !empty($plugin_folder) )
		$plugin_root .= $plugin_folder;

	// Files in trm-src/module directory
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$trm_plugins[plugin_basename( $plugin_file )] = $plugin_data;
	}

	uasort( $trm_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $trm_plugins;
	trm_cache_set('plugins', $cache_plugins, 'plugins');
 ...
 *
 * Trnder only includes mu-plugin files in the base mu-plugins directory (trm-src/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	$trm_plugins = array();
	// Files in trm-src/mu-plugins directory
	$plugin_files = array();

	if ( ! is_dir( TRMMU_PLUGIN_DIR ) )
		return $trm_plugins;
	if ( $plugins_dir = @ opendir( TRMMU_PLUGIN_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
			if ( substr( $file, -4 ) == '.php' )
				$plugin_files[] = $file;
		}
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( TRMMU_PLUGIN_DIR . "/$plugin_file" ) )
			continue;

		$plugin_data = get_plugin_data( TRMMU_PLUGIN_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.

		if ( empty ( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;

		$trm_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $trm_plugins['index.php'] ) && filesize( TRMMU_PLUGIN_DIR . '/index.php') <= 30 ) // silence is golden
		unset( $trm_plugins['index.php'] );
...
/**
 * Check the trm-src directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// These exist in the trm-src directory
	if ( $plugins_dir = @ opendir( TRM_CONTENT_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( TRM_CONTENT_DIR . "/$plugin_file" ) )
			continue;
		$plugin_data = get_plugin_data( TRM_CONTENT_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.
		if ( empty( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;
...
/**
 * Returns drop-ins that Trnder uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php' => array( __( 'Advanced caching plugin.'       ), 'TRM_CACHE' ), // TRM_CACHE
...
function is_network_only_plugin( $plugin ) {
	$plugin_data = get_plugin_data( TRM_PLUGIN_DIR . '/' . $plugin );
	if ( $plugin_data )
 ...
 * @return TRM_Error|null TRM_Error on invalid file or null on success.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( is_trm_error($valid) )
		return $valid;

	if ( !in_array($plugin, $current) ) {
		if ( !empty($redirect) )
			trm_redirect(add_query_arg('_error_nonce', trm_create_nonce('plugin-activation-error_' . $plugin), $redirect)); // we'll override this later if the plugin can be included without fatal error
		ob_start();
		include_once(TRM_PLUGIN_DIR . '/' . $plugin);
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new TRM_Error('unexpected_output', __('The plugin generated unexpected output.'), $output);
		}
		ob_end_clean();
	}
 ...
 *
 * When TRM_Error is returned, it does not mean that one of the plugins had
 ...
 * @return bool|TRM_Error True when finished or TRM_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !empty($errors) )
		return new TRM_Error('plugins_invalid', __('One of the plugins is invalid.'), $errors);
...
function delete_plugins($plugins, $redirect = '' ) {
	...
	ob_start();
	$url = trm_nonce_url('plugins.php?action=delete-selected&verify-delete=1&' . implode('&', $checked), 'bulk-plugins');
	if ( false === ($credentials = request_filesystem_credentials($url)) ) {
		$data = ob_get_contents();
		ob_end_clean();
		if ( ! empty($data) ){
			include_once( ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/admin-header.php');
			echo $data;
	...
	if ( ! is_object($trm_filesystem) )
		return new TRM_Error('fs_unavailable', __('Could not access filesystem.'));

	if ( is_trm_error($trm_filesystem->errors) && $trm_filesystem->errors->get_error_code() )
		return new TRM_Error('fs_error', __('Filesystem error.'), $trm_filesystem->errors);

	//Get the base plugin folder
	$plugins_dir = $trm_filesystem->trm_plugins_dir();
	if ( empty($plugins_dir) )
		return new TRM_Error('fs_no_plugins_dir', __('Unable to locate Trnder Plugin directory.'));
	...
	foreach( $plugins as $plugin_file ) {
		...
		if ( strpos($plugin_file, '/') && $this_plugin_dir != $plugins_dir ) //base check on if plugin includes directory separator AND that its not the root plugin folder
			...
			$deleted = $trm_filesystem->delete($plugins_dir . $plugin_file);
	...
	if ( ! empty($errors) )
		return new TRM_Error('could_not_remove_plugin', sprintf(__('Could not fully remove the plugin(s) %s.'), implode(', ', $errors)) );
 ...
 * @return TRM_Error|int 0 on success, TRM_Error on failure.
 */
function validate_plugin($plugin) {
	if ( validate_file($plugin) )
		return new TRM_Error('plugin_invalid', __('Invalid plugin path.'));
	if ( ! file_exists(TRM_PLUGIN_DIR . '/' . $plugin) )
		return new TRM_Error('plugin_not_found', __('Plugin file does not exist.'));

	$installed_plugins = get_plugins();
	if ( ! isset($installed_plugins[$plugin]) )
		return new TRM_Error('no_plugin_header', __('The plugin does not have a valid header.'));
	return 0;
}
...
function is_uninstallable_plugin($plugin) {
	...
	if ( isset( $uninstallable_plugins[$file] ) || file_exists( TRM_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) )
		return true;
...
function uninstall_plugin($plugin) {
	...
	$uninstallable_plugins = (array) get_option('uninstall_plugins');
	if ( file_exists( TRM_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) ) {
		...
		define('TRM_UNINSTALL_PLUGIN', $file);
		include TRM_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php';
...
function add_object_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '') {
	global $_trm_last_object_menu;

	$_trm_last_object_menu++;

	return add_menu_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $_trm_last_object_menu);
}
...
function add_utility_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '') {
	global $_trm_last_utility_menu;

	$_trm_last_utility_menu++;

	return add_menu_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $_trm_last_utility_menu);
}
 ...
 * that the user has the required capability as well.
 *
 * @param string $parent_slug The slug name for the parent menu (or the file name of a standard Trnder admin page)
 * @param string $page_title The text to be displayed in the title tags of the page when the menu is selected
 * @param string $menu_title The text to be used for the menu
 * @param string $capability The capability required for this menu to be displayed to the user.
 * @param string $menu_slug The slug name to refer to this menu by (should be unique for this menu)
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '' ) {
	global $submenu;
	global $menu;
	global $_trm_real_parent_file;
	global $_trm_submenu_nopriv;
	...
	if ( isset( $_trm_real_parent_file[$parent_slug] ) )
		$parent_slug = $_trm_real_parent_file[$parent_slug];

	if ( !current_user_can( $capability ) ) {
		$_trm_submenu_nopriv[$parent_slug][$menu_slug] = true;
		return false;
	}
	...
	$_registered_pages[$hookname] = true;
	// backwards-compatibility for plugins using add_management page.  See Backend-WeaprEcqaKejUbRq-trendr/admin.php for redirect from edit.php to tools.php
...
function get_admin_page_parent( $parent = '' ) {
	...
	global $plugin_page;
	global $_trm_real_parent_file;
	global $_trm_menu_nopriv;
	global $_trm_submenu_nopriv;

	if ( !empty ( $parent ) && 'admin.php' != $parent ) {
		if ( isset( $_trm_real_parent_file[$parent] ) )
			$parent = $_trm_real_parent_file[$parent];
		return $parent;
	}

	/*
	if ( !empty ( $parent_file ) ) {
		if ( isset( $_trm_real_parent_file[$parent_file] ) )
			$parent_file = $_trm_real_parent_file[$parent_file];
	...
	*/
	...
	if ( $pagenow == 'admin.php' && isset( $plugin_page ) ) {
		...
		if ( isset( $_trm_menu_nopriv[$plugin_page] ) ) {
			$parent_file = $plugin_page;
			if ( isset( $_trm_real_parent_file[$parent_file] ) )
					$parent_file = $_trm_real_parent_file[$parent_file];
			return $parent_file;
	...
	if ( isset( $plugin_page ) && isset( $_trm_submenu_nopriv[$pagenow][$plugin_page] ) ) {
		$parent_file = $pagenow;
		if ( isset( $_trm_real_parent_file[$parent_file] ) )
			$parent_file = $_trm_real_parent_file[$parent_file];
		return $parent_file;
...
function user_can_access_admin_page() {
	...
	if ( !isset( $plugin_page ) && isset( $_trm_submenu_nopriv[$parent][$pagenow] ) )
		return false;

	if ( isset( $plugin_page ) ) {
		if ( isset( $_trm_submenu_nopriv[$parent][$plugin_page] ) )
			return false;
	...
	if ( empty( $parent) ) {
		if ( isset( $_trm_menu_nopriv[$pagenow] ) )
			return false;
		if ( isset( $_trm_submenu_nopriv[$pagenow][$pagenow] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_trm_submenu_nopriv[$pagenow][$plugin_page] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_trm_menu_nopriv[$plugin_page] ) )
			return false;
		foreach (array_keys( $_trm_submenu_nopriv ) as $key ) {
			if ( isset( $_trm_submenu_nopriv[$key][$pagenow] ) )
				return false;
			if ( isset( $plugin_page ) && isset( $_trm_submenu_nopriv[$key][$plugin_page] ) )
			return false;
		}
		return true;
	}

	if ( isset( $plugin_page ) && ( $plugin_page == $parent ) && isset( $_trm_menu_nopriv[$plugin_page] ) )
		return false;
...
function settings_fields($option_group) {
	...
	trm_nonce_field("$option_group-options");
}
