...
class TRM_List_Table {
	...
	function ajax_user_can() {
		die( 'function TRM_List_Table::ajax_user_can() must be over-ridden in a sub-class.' );
	}

	/**
	 * Prepares the list of items for displaying.
	 * @uses TRM_List_Table::set_pagination_args()
	 *
	 * @since 3.1.0
	 * @access public
	 * @abstract
	...
	function prepare_items() {
		die( 'function TRM_List_Table::prepare_items() must be over-ridden in a sub-class.' );
	}
	...
	function set_pagination_args( $args ) {
		...
		if ( ! headers_sent() && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) && $args['total_pages'] > 0 && $this->get_pagenum() > $args['total_pages'] ) {
			trm_redirect( add_query_arg( 'paged', $args['total_pages'] ) );
			exit;
		}
...
<?php
	...
	function months_dropdown( $post_type ) {
		...
		$months = $trmdb->get_results( $trmdb->prepare( "
...
<?php
		foreach ( $months as $arc_row ) {
			...
			printf( "<option %s value='%s'>%s</option>\n",
				...
				$trm_locale->get_month( $month ) . " $year"
			);
		}
?>
		</select>
...
<?php
	...
	function get_columns() {
		die( 'function TRM_List_Table::get_columns() must be over-ridden in a sub-class.' );
	}
...
?>
<table class="trm-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>" cellspacing="0">
	<thead>
	<tr>
...
<?php
	...
	function display_tablenav( $which ) {
		if ( 'top' == $which )
			trm_nonce_field( 'bulk-' . $this->_args['plural'] );
?>
