 ...
 * @return object|bool TRM_Error on failure, true on success.
 */
function _trm_translate_postdata( $update = false, $post_data = null ) {
...
function edit_post( $post_data = null ) {
	...
	if ( !current_user_can( $ptype->cap->edit_post, $post_ID ) ) {
		if ( 'page' == $post_data['post_type'] )
			trm_die( __('You are not allowed to edit this page.' ));
		else
	...
	$post_data = _trm_translate_postdata( true, $post_data );
	if ( is_trm_error($post_data) )
		trm_die( $post_data->get_error_message() );
	...
	trm_set_post_lock( $post_ID, $GLOBALS['current_user']->ID );
...
function bulk_edit_posts( $post_data = null ) {
	...
	if ( !current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' == $ptype->name )
			trm_die( __('You are not allowed to edit pages.'));
		else
			trm_die( __('You are not allowed to edit posts.'));
	}
	...
	if ( isset($post_data['post_parent']) && ($parent = (int) $post_data['post_parent']) ) {
		$pages = $trmdb->get_results("SELECT ID, post_parent FROM $trmdb->posts WHERE post_type = 'page'");
		$children = array();
	...
	foreach ( $post_IDs as $post_ID ) {
		...
		if ( trm_check_post_lock( $post_ID ) ) {
			$locked[] = $post_ID;
			continue;
		}
		...
		foreach ( $tax_names as $tax_name ) {
			...
			if ( $taxonomy_obj->hierarchical )
				$current_terms = (array) trm_get_object_terms( $post_ID, $tax_name, array('fields' => 'ids') );
			else
				$current_terms = (array) trm_get_object_terms( $post_ID, $tax_name, array('fields' => 'names') );

			$post_data['tax_input'][$tax_name] = array_merge( $current_terms, $new_terms );
		}

		if ( isset($new_cats) && in_array( 'category', $tax_names ) ) {
			$cats = (array) trm_get_post_categories($post_ID);
		...
		$updated[] = trm_update_post( $post_data );
...
function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
	...
	if ( $create_in_db ) {
		...
		$old_posts = $trmdb->get_col( "SELECT ID FROM $trmdb->posts WHERE post_status = 'auto-draft' AND DATE_SUB( NOW(), INTERVAL 7 DAY ) > post_date" );
		foreach ( (array) $old_posts as $delete )
			trm_delete_post( $delete, true ); // Force delete
		$post_id = trm_insert_post( array( 'post_title' => __( 'Auto Draft' ), 'post_type' => $post_type, 'post_status' => 'auto-draft' ) );
		$post = get_post( $post_id );
...
function get_post_to_edit( $id ) {
	...
	if ( $post->post_type == 'page' )
		$post->page_template = get_post_meta( $id, '_trm_page_template', true );
...
function post_exists($title, $content = '', $date = '') {
	...
	$query = "SELECT ID FROM $trmdb->posts WHERE 1=1";
	$args = array();
	...
	if ( !empty ( $args ) )
		return $trmdb->get_var( $trmdb->prepare($query, $args) );
...
function trm_write_post() {
	...
	if ( !current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' == $ptype->name )
			return new TRM_Error( 'edit_pages', __( 'You are not allowed to create pages on this site.' ) );
		else
			return new TRM_Error( 'edit_posts', __( 'You are not allowed to create posts or drafts on this site.' ) );
	}
	...
	$translated = _trm_translate_postdata( false );
	if ( is_trm_error($translated) )
		return $translated;
...
/**
 * Calls trm_write_post() and handles the errors.
 *
 * @since 2.0.0
 *
 * @return unknown
 */
function write_post() {
	$result = trm_write_post();
	if ( is_trm_error( $result ) )
		trm_die( $result->get_error_message() );
	else
		return $result;
}
...
function add_meta( $post_ID ) {
	...
	if ( ('0' === $metavalue || !empty ( $metavalue ) ) && ((('#NONE#' != $metakeyselect) && !empty ( $metakeyselect) ) || !empty ( $metakeyinput) ) ) {
		...
		$trmdb->insert( $trmdb->postmeta, array( 'post_id' => $post_ID, 'meta_key' => $metakey, 'meta_value' => $metavalue ) );
		$meta_id = $trmdb->insert_id;
...
function delete_meta( $mid ) {
	...
	$post_id = $trmdb->get_var( $trmdb->prepare("SELECT post_id FROM $trmdb->postmeta WHERE meta_id = %d", $mid) );

	do_action( 'delete_postmeta', $mid );
	trm_cache_delete($post_id, 'post_meta');
	$rval = $trmdb->query( $trmdb->prepare("DELETE FROM $trmdb->postmeta WHERE meta_id = %d", $mid) );
...
function get_post_meta_by_id( $mid ) {
	...
	$meta = $trmdb->get_row( $trmdb->prepare("SELECT * FROM $trmdb->postmeta WHERE meta_id = %d", $mid) );
	if ( empty($meta) )
		return false;
...
function has_meta( $postid ) {
	...
	return $trmdb->get_results( $trmdb->prepare("SELECT meta_key, meta_value, meta_id, post_id
			FROM $trmdb->postmeta WHERE post_id = %d
...
function update_meta( $meta_id, $meta_key, $meta_value ) {
	...
	$post_id = $trmdb->get_var( $trmdb->prepare("SELECT post_id FROM $trmdb->postmeta WHERE meta_id = %d", $meta_id) );
	...
	$rval = $trmdb->update( $trmdb->postmeta, $data, $where );
	trm_cache_delete($post_id, 'post_meta');
...
function _relocate_children( $old_ID, $new_ID ) {
	...
	$children = $trmdb->get_col( $trmdb->prepare("
		SELECT post_id
		FROM $trmdb->postmeta
		WHERE meta_key = '_trm_attachment_temp_parent'
		AND meta_value = %d", $old_ID) );

	foreach ( $children as $child_id ) {
		$trmdb->update($trmdb->posts, array('post_parent' => $new_ID), array('ID' => $child_id) );
		delete_post_meta($child_id, '_trm_attachment_temp_parent');
	}
}
...
function get_available_post_statuses($type = 'post') {
	$stati = trm_count_posts($type);

	return array_keys(get_object_vars($stati));
}

/**
 * Run the trm query to fetch the posts for listing on the edit posts page
 *
 * @since 2.5.0
 *
 ...
 * @return array
 */
function trm_edit_posts_query( $q = false ) {
	if ( false === $q )
		$q = $_GET;
...
function get_available_post_mime_types($type = 'attachment') {
	...
	$types = $trmdb->get_col($trmdb->prepare("SELECT DISTINCT post_mime_type FROM $trmdb->posts WHERE post_type = %s", $type));
	return $types;
}
 ...
 * @return unknown
 */
function trm_edit_attachments_query( $q = false ) {
	if ( false === $q )
		$q = $_GET;
...
function get_sample_permalink($id, $title = null, $name = null) {
	...
	$post->post_name = trm_unique_post_slug($post->post_name, $post->ID, $post->post_status, $post->post_type, $post->post_parent);
 ...
 * @return string html
 */
function _trm_post_thumbnail_html( $thumbnail_id = NULL ) {
	global $content_width, $_trm_additional_image_sizes, $post_ID;
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		...
		$content_width = 266;
		if ( !isset( $_trm_additional_image_sizes['post-thumbnail'] ) )
			$thumbnail_html = trm_get_attachment_image( $thumbnail_id, array( $content_width, $content_width ) );
		else
			$thumbnail_html = trm_get_attachment_image( $thumbnail_id, 'post-thumbnail' );
		if ( !empty( $thumbnail_html ) ) {
			$ajax_nonce = trm_create_nonce( "set_post_thumbnail-$post_ID" );
			$content = sprintf($set_thumbnail_link, $thumbnail_html);
			$content .= '<p class="hide-if-no-js"><a href="#" id="remove-post-thumbnail" onclick="TRMRemoveThumbnail(\'' . $ajax_nonce . '\');return false;">' . esc_html__( 'Remove featured image' ) . '</a></p>';
		}
 ...
 */
function trm_check_post_lock( $post_id ) {
	...
	$time_window = apply_filters( 'trm_check_post_lock_window', AUTOSAVE_INTERVAL * 2 );
 ...
 */
function trm_set_post_lock( $post_id ) {
	if ( !$post = get_post( $post_id ) )
		return false;
 ...
 * @since 2.6.0
 *
 * @uses _trm_translate_postdata()
 * @uses _trm_post_revision_fields()
 *
 * @return unknown
 */
function trm_create_post_autosave( $post_id ) {
	$translated = _trm_translate_postdata( true );
	if ( is_trm_error( $translated ) )
		return $translated;

	// Only store one autosave.  If there is already an autosave, overwrite it.
	if ( $old_autosave = trm_get_post_autosave( $post_id ) ) {
		$new_autosave = _trm_post_revision_fields( $_POST, true );
		$new_autosave['ID'] = $old_autosave->ID;
		$new_autosave['post_author'] = get_current_user_id();
		return trm_update_post( $new_autosave );
	}

	// _trm_put_post_revision() expects unescaped.
	$_POST = stripslashes_deep($_POST);

	// Otherwise create the new autosave as a special post revision
	return _trm_put_post_revision( $_POST, true );
}
...
function post_preview() {
	...
	if ( 'auto-draft' == $status )
		trm_die( __('Preview not available. Please save as a draft first.') );
	...
	if ( 'draft' == $post->post_status ) {
		...
		$id = trm_create_post_autosave( $post->ID );
		if ( ! is_trm_error($id) )
			$id = $post->ID;
	}
	...
	if ( $_POST['post_status'] == 'draft'  ) {
		...
		$nonce = trm_create_nonce('post_preview_' . $id);
 ...
 * @since 2.7.0
 *
 * TinyMCE is loaded separately from other Javascript by using trm-tinymce.php. It outputs concatenated
 ...
 */
function trm_tiny_mce( $teeny = false, $settings = false ) {
	...
	*/
	...
	if ( $teeny ) {
		$plugins = apply_filters( 'teeny_mce_plugins', array('inlinepopups', 'fullscreen', 'trendr', 'trmlink', 'trmdialogs') );
		$ext_plugins = '';
	} else {
		$plugins = array( 'inlinepopups', 'spellchecker', 'tabfocus', 'paste', 'media', 'trendr', 'trmfullscreen', 'trmeditimage', 'trmgallery', 'trmlink', 'trmdialogs' );
		...
		array( 'myplugin' => 'http://my-site.com/trm-src/module/myfolder/mce_plugin.js' )
		...
		*/
		...
		if ( ! empty($mce_external_plugins) ) {
			...
			/tinymce/langs/trm-langs.php and should define a variable $strings that
			...
			*/
			...
			foreach ( $mce_external_plugins as $name => $url ) {
				...
				if ( ! in_array($name, $loaded_langs) ) {
					$path = str_replace( TRM_PLUGIN_URL, '', $plugurl );
					$path = TRM_PLUGIN_DIR . $path . '/langs/';
	...
	if ( $teeny ) {
		...
		$mce_buttons = apply_filters('mce_buttons', array('bold', 'italic', 'strikethrough', '|', 'bullist', 'numlist', 'blockquote', '|', 'justifyleft', 'justifycenter', 'justifyright', '|', 'link', 'unlink', 'trm_more', '|', 'spellchecker', 'fullscreen', 'trm_adv' ));
		$mce_buttons = implode($mce_buttons, ',');

		$mce_buttons_2 = array( 'formatselect', 'underline', 'justifyfull', 'forecolor', '|', 'pastetext', 'pasteword', 'removeformat', '|', 'charmap', '|', 'outdent', 'indent', '|', 'undo', 'redo', 'trm_help' );
	...
	$initArray = array (
		...
		'trmeditimage_disable_captions' => $no_captions,
		'trm_fullscreen_content_css' => "$baseurl/module/trmfullscreen/css/trm-fullscreen.css",
	...
	if ( 'en' != $language )
		include_once(ABSPATH . TRMINC . '/js/tinymce/langs/trm-langs.php');
	...
	do_action('before_trm_tiny_mce', $initArray); ?>
...
<?php
	if ( $compressed )
		echo "<script type='text/javascript' src='$baseurl/trm-tinymce.php?c=1&amp;$version'></script>\n";
	else
	...
	if ( 'en' != $language && isset($lang) )
		...
		echo "<script type='text/javascript' src='$baseurl/langs/trm-langs-en.js?$version'></script>\n";
?>
...
<?php
	...
	if ( ! $compressed ) {
?>
(function(){var t=tinyMCEPreInit,sl=tinymce.ScriptLoader,ln=t.mceInit.language,th=t.mceInit.theme,pl=t.mceInit.plugins;sl.markDone(t.base+'/langs/'+ln+'.js');sl.markDone(t.base+'/858483/'+th+'/langs/'+ln+'.js');sl.markDone(t.base+'/858483/'+th+'/langs/'+ln+'_dlg.js');tinymce.each(pl.split(','),function(n){if(n&&n.charAt(0)!='-'){sl.markDone(t.base+'/module/'+n+'/langs/'+ln+'.js');sl.markDone(t.base+'/module/'+n+'/langs/'+ln+'_dlg.js');}});})();
<?php } ?>
...
do_action('after_trm_tiny_mce', $initArray);
}

// Load additional inline scripts based on active plugins.
function trm_preload_dialogs($init) {
	$plugins = preg_split('/[ ,-]+/', $init['plugins']);

	if ( in_array( 'trmdialogs', $plugins, true ) ) {
		trm_print_scripts('trmdialogs-popup');
		trm_print_styles('trm-jquery-ui-dialog');
	}

	if ( in_array( 'trmlink', $plugins, true ) ) {
		require_once ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/includes/internal-linking.php';
		?><div style="display:none;"><?php trm_link_dialog(); ?></div><?php
		trm_print_scripts('trmlink');
		trm_print_styles('trmlink');
	}

	// Distraction Free Writing mode
	if ( in_array( 'trmfullscreen', $plugins, true ) ) {
		trm_fullscreen_html();
		trm_print_scripts('trm-fullscreen');
	}

	trm_print_scripts('word-count');
}

function trm_quicktags() {
	global $tinymce_version;

	trm_preload_dialogs( array( 'plugins' => 'trmdialogs,trmlink,trmfullscreen' ) );

	if ( !user_can_richedit() ) {
		trm_enqueue_style( 'tinymce-buttons', includes_url('js/tinymce/858483/advanced/skins/trm_theme/ui.css'), array(), $tinymce_version );
		trm_print_styles('tinymce-buttons');
	}
}

function trm_print_editor_js() {
	trm_print_scripts('editor');
}

function trm_fullscreen_html() {
...
?>
<div id="trm-fullscreen-body">
<div id="fullscreen-topbar">
	<div id="trm-fullscreen-toolbar">
		<div id="trm-fullscreen-close"><a href="#" onclick="fullscreen.off();return false;"><?php _e('Exit fullscreen'); ?></a></div>
		<div id="trm-fullscreen-central-toolbar" style="width:<?php echo $width; ?>px;">

		<div id="trm-fullscreen-mode-bar"><div id="trm-fullscreen-modes">
		...
		<div id="trm-fullscreen-button-bar"><div id="trm-fullscreen-buttons" class="trm_themeSkin">
<?php
	...
	$buttons = apply_filters( 'trm_fullscreen_buttons', $buttons );
...
<?php		continue;
		...
		<div<?php if ( $args['both'] ) { ?> class="trm-fullscreen-both"<?php } ?>>
		<a title="<?php echo $args['title']; ?>" onclick="<?php echo $args['onclick']; ?>return false;" class="mceButton mceButtonEnabled mce_<?php echo $button; ?>" href="#" id="trm_fs_<?php echo $button; ?>" role="button" aria-pressed="false">
...
<div id="trm-fullscreen-wrap" style="width:<?php echo $dfw_width; ?>px;">
	<label id="trm-fullscreen-title-prompt-text" for="trm-fullscreen-title"><?php echo apply_filters( 'enter_title_here', __( 'Enter title here' ), $post ); ?></label>
	<input type="text" id="trm-fullscreen-title" value="" autocomplete="off" />

	<div id="trm-fullscreen-container">
		<textarea id="trm_mce_fullscreen"></textarea>
	</div>

	<div id="trm-fullscreen-status">
		<div id="trm-fullscreen-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></div>
		<div id="trm-fullscreen-tagline"><?php _e('Just write.'); ?></div>
	</div>
</div>
</div>
