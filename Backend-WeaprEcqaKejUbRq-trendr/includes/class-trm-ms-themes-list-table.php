 ...
 * @access private
 */
class TRM_MS_Themes_List_Table extends TRM_List_Table {
	...
	function prepare_items() {
		...
		if ( !$this->is_site_themes ) {
			...
			$allowed_themes = trmmu_get_blog_allowedthemes( $this->site_id );
	...
	function single_row( $key, $theme ) {
		...
		if ( empty( $theme['enabled'] ) )
			$actions['enable'] = '<a href="' . esc_url( trm_nonce_url($url . 'action=enable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'enable-theme_' . $theme_key) ) . '" title="' . esc_attr__('Enable this theme') . '" class="edit">' . ( $this->is_site_themes ? __( 'Enable' ) : __( 'Network Enable' ) ) . '</a>';
		else
			$actions['disable'] = '<a href="' . esc_url( trm_nonce_url($url . 'action=disable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'disable-theme_' . $theme_key) ) . '" title="' . esc_attr__('Disable this theme') . '">' . ( $this->is_site_themes ? __( 'Disable' ) : __( 'Network Disable' ) ) . '</a>';
		...
		if ( empty( $theme['enabled'] ) && current_user_can( 'delete_themes' ) && ! $this->is_site_themes && $theme_key != get_option( 'stylesheet' ) && $theme_key != get_option( 'template' ) )
			$actions['delete'] = '<a href="' . esc_url( trm_nonce_url( 'themes.php?action=delete-selected&amp;checked[]=' . $theme_key . '&amp;theme_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bulk-themes' ) ) . '" title="' . esc_attr__( 'Delete this theme' ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		...
		if ( $this->is_site_themes )
			remove_action( "after_theme_row_$theme_key", 'trm_theme_update_row' );
