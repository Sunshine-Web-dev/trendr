<?php
/**
 * Template Trnder Administration API.
 *
 ...
 */
function trm_category_checklist( $post_id = 0, $descendants_and_self = 0, $selected_cats = false, $popular_cats = false, $walker = null, $checked_ontop = true ) {
	trm_terms_checklist($post_id,
	 	array(
			'taxonomy' => 'category',
...
/**
 * Taxonomy independent version of trm_category_checklist
 *
 * @since 3.0.0
 *
 * @param int $post_id
 * @param array $args
 */
function trm_terms_checklist($post_id = 0, $args = array()) {
 	$defaults = array(
	...
	extract( trm_parse_args($args, $defaults), EXTR_SKIP );
	...
	elseif ( $post_id )
		$args['selected_cats'] = trm_get_object_terms($post_id, $taxonomy, array_merge($args, array('fields' => 'ids')));
	else
 ...
 * @return unknown
 */
function trm_popular_terms_checklist( $taxonomy, $default = 0, $number = 10, $echo = true ) {
	global $post_ID;

	if ( $post_ID )
		$checked_terms = trm_get_object_terms($post_ID, $taxonomy, array('fields'=>'ids'));
	else
		$checked_terms = array();
 ...
 */
function trm_link_category_checklist( $link_id = 0 ) {
	$default = 1;

	if ( $link_id ) {
		$checked_categories = trm_get_link_cats( $link_id );
...
function get_column_headers( $screen ) {
	...
	global $_trm_column_headers;

	if ( !isset( $_trm_column_headers[ $screen->id ] ) ) {
		$_trm_column_headers[ $screen->id ] = apply_filters( 'manage_' . $screen->id . '_columns', array() );
	}

	return $_trm_column_headers[ $screen->id ];
}
...
<div class="hidden" id="inline_' . $post->ID . '">
	...
	if ( $post->post_type == 'page' )
		echo '<div class="page_template">' . esc_html( get_post_meta( $post->ID, '_trm_page_template', true ) ) . '</div>';
	...
	foreach ( $taxonomy_names as $taxonomy_name) {
		...
		if ( $taxonomy->hierarchical && $taxonomy->show_ui )
				echo '<div class="post_category" id="'.$taxonomy_name.'_'.$post->ID.'">' . implode( ',', trm_get_object_terms( $post->ID, $taxonomy_name, array('fields'=>'ids')) ) . '</div>';
 ...
 */
function trm_comment_reply($position = '1', $checkbox = false, $mode = 'single', $table_row = true) {
	// allow plugin to replace the popup content
	$content = apply_filters( 'trm_comment_reply', '', array('position' => $position, 'checkbox' => $checkbox, 'mode' => $mode) );
	...
	if ( $mode == 'single' ) {
		$trm_list_table = _get_list_table('TRM_Post_Comments_List_Table');
	} else {
		$trm_list_table = _get_list_table('TRM_Comments_List_Table');
	}

?>
<form method="get" action="">
<?php if ( $table_row ) : ?>
<table style="display:none;"><tbody id="com-reply"><tr id="replyrow" style="display:none;"><td colspan="<?php echo $trm_list_table->get_column_count(); ?>" class="colspanchange">
<?php else : ?>
...
<?php endif; ?>
	...
	<img class="waiting" style="display:none;" src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" alt="" />
	...
	<?php trm_nonce_field( 'replyto-comment', '_ajax_nonce-replyto-comment', false ); ?>
	<?php trm_comment_form_unfiltered_html_nonce(); ?>
<?php if ( $table_row ) : ?>
</td></tr></tbody></table>
<?php else : ?>
</div></div>
 ...
 * @since 2.9.0
 */
function trm_comment_trashnotice() {
?>
...
function _list_meta_row( $entry, &$count ) {
	...
	if ( !$update_nonce )
		$update_nonce = trm_create_nonce( 'add-meta' );
	...
	$delete_nonce = trm_create_nonce( 'delete-meta_' . $entry['meta_id'] );
	...
	$r .= "</div>";
	$r .= trm_nonce_field( 'change-meta', '_ajax_nonce', false, false );
	$r .= "</td>";
...
<?php trm_nonce_field( 'add-meta', '_ajax_nonce-add-meta', false ); ?>
</td></tr>
</tbody>
</table>
<?php
...
function touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0 ) {
	global $trm_locale, $post, $comment;
	...
	for ( $i = 1; $i < 13; $i = $i +1 ) {
		...
		$month .= '>' . $trm_locale->get_month_abbrev( $trm_locale->get_month( $i ) ) . "</option>\n";
	}
	$month .= '</select>';
...
function parent_dropdown( $default = 0, $parent = 0, $level = 0 ) {
	global $trmdb, $post_ID;
	$items = $trmdb->get_results( $trmdb->prepare("SELECT ID, post_parent, post_title FROM $trmdb->posts WHERE post_parent = %d AND post_type = 'page' ORDER BY menu_order", $parent) );
...
function the_attachment_links( $id = false ) {
	...
	$icon = trm_get_attachment_image( $post->ID, 'thumbnail', true );
	$attachment_data = trm_get_attachment_metadata( $id );
...
<table>
	...
	<tr>
		...
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><?php echo esc_textarea( trm_get_attachment_url() ); ?></textarea></td>
	</tr>
<?php if ( $icon ) : ?>
	<tr>
		<th scope="row"><?php $thumb ? _e( 'Thumbnail linked to file' ) : _e( 'Image linked to file' ); ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo trm_get_attachment_url(); ?>"><?php echo $icon ?></a></textarea></td>
	</tr>
	<tr>
		<th scope="row"><?php $thumb ? _e( 'Thumbnail linked to page' ) : _e( 'Image linked to page' ); ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo get_attachment_link( $post->ID ) ?>" rel="attachment trm-att-<?php echo $post->ID; ?>"><?php echo $icon ?></a></textarea></td>
	</tr>
<?php else : ?>
	<tr>
		<th scope="row"><?php _e( 'Link to file' ) ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo trm_get_attachment_url(); ?>" class="attachmentlink"><?php echo basename( trm_get_attachment_url() ); ?></a></textarea></td>
	</tr>
	<tr>
		<th scope="row"><?php _e( 'Link to page' ) ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo get_attachment_link( $post->ID ) ?>" rel="attachment trm-att-<?php echo $post->ID ?>"><?php the_title(); ?></a></textarea></td>
	</tr>
<?php endif; ?>
</table>
</form>
 ...
 */
function trm_dropdown_roles( $selected = false ) {
	$p = '';
	$r = '';
 ...
 * @return unknown
 */
function trm_convert_hr_to_bytes( $size ) {
 ...
 * @return unknown
 */
function trm_convert_bytes_to_hr( $bytes ) {
 ...
 * @return unknown
 */
function trm_max_upload_size() {
	$u_bytes = trm_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = trm_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
 ...
 */
function trm_import_upload_form( $action ) {
	$bytes = apply_filters( 'import_upload_size_limit', trm_max_upload_size() );
	$size = trm_convert_bytes_to_hr( $bytes );
...
?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr(trm_nonce_url($action, 'import-upload')); ?>">
<p>
...
function add_meta_box($id, $title, $callback, $page, $context = 'advanced', $priority = 'default', $callback_args=null) {
	global $trm_meta_boxes;

	if ( !isset($trm_meta_boxes) )
		$trm_meta_boxes = array();
	if ( !isset($trm_meta_boxes[$page]) )
		$trm_meta_boxes[$page] = array();
	if ( !isset($trm_meta_boxes[$page][$context]) )
		$trm_meta_boxes[$page][$context] = array();

	foreach ( array_keys($trm_meta_boxes[$page]) as $a_context ) {
		foreach ( array('high', 'core', 'default', 'low') as $a_priority ) {
			if ( !isset($trm_meta_boxes[$page][$a_context][$a_priority][$id]) )
				continue;

			// If a core box was previously added or removed by a plugin, don't add.
			if ( 'core' == $priority ) {
				// If core box previously deleted, don't add
				if ( false === $trm_meta_boxes[$page][$a_context][$a_priority][$id] )
					return;
				// If box was added with default priority, give it core priority to maintain sort order
				if ( 'default' == $a_priority ) {
					$trm_meta_boxes[$page][$a_context]['core'][$id] = $trm_meta_boxes[$page][$a_context]['default'][$id];
					unset($trm_meta_boxes[$page][$a_context]['default'][$id]);
				}
				return;
			}
			...
			// else if we're adding to the sorted priortiy, we don't know the title or callback. Glab them from the previously added context/priority.
				...
				$title = $trm_meta_boxes[$page][$a_context][$a_priority][$id]['title'];
				$callback = $trm_meta_boxes[$page][$a_context][$a_priority][$id]['callback'];
				$callback_args = $trm_meta_boxes[$page][$a_context][$a_priority][$id]['args'];
			}
			// An id can be in only one priority and one context
			if ( $priority != $a_priority || $context != $a_context )
				unset($trm_meta_boxes[$page][$a_context][$a_priority][$id]);
		}
	}
	...
	if ( !isset($trm_meta_boxes[$page][$context][$priority]) )
		$trm_meta_boxes[$page][$context][$priority] = array();

	$trm_meta_boxes[$page][$context][$priority][$id] = array('id' => $id, 'title' => $title, 'callback' => $callback, 'args' => $callback_args);
}
...
function do_meta_boxes($page, $context, $object) {
	...
	do {
		...
		if ( !isset($trm_meta_boxes) || !isset($trm_meta_boxes[$page]) || !isset($trm_meta_boxes[$page][$context]) )
			break;

		foreach ( array('high', 'sorted', 'core', 'default', 'low') as $priority ) {
			if ( isset($trm_meta_boxes[$page][$context][$priority]) ) {
				foreach ( (array) $trm_meta_boxes[$page][$context][$priority] as $box ) {
...
function remove_meta_box($id, $page, $context) {
	...
	foreach ( array('high', 'core', 'default', 'low') as $priority )
		$trm_meta_boxes[$page][$context][$priority][$id] = false;
}
...
function meta_box_prefs($screen) {
	...
	if ( empty($trm_meta_boxes[$screen->id]) )
		return;

	$hidden = get_hidden_meta_boxes($screen);

	foreach ( array_keys($trm_meta_boxes[$screen->id]) as $context ) {
		foreach ( array_keys($trm_meta_boxes[$screen->id][$context]) as $priority ) {
			foreach ( $trm_meta_boxes[$screen->id][$context][$priority] as $box ) {
				if ( false == $box || ! $box['title'] )
					continue;
 ...
 * @since 2.7.0
 *
 * @global $trm_settings_sections Storage array of all settings sections added to admin pages
 *
...
function add_settings_section($id, $title, $callback, $page) {
	...
	if ( !isset($trm_settings_sections) )
		$trm_settings_sections = array();
	if ( !isset($trm_settings_sections[$page]) )
		$trm_settings_sections[$page] = array();
	if ( !isset($trm_settings_sections[$page][$id]) )
		$trm_settings_sections[$page][$id] = array();

	$trm_settings_sections[$page][$id] = array('id' => $id, 'title' => $title, 'callback' => $callback);
}
 ...
 * @since 2.7.0
 *
 * @global $trm_settings_fields Storage array of settings fields and info about their pages/sections
 *
...
function add_settings_field($id, $title, $callback, $page, $section = 'default', $args = array()) {
	...
	if ( !isset($trm_settings_fields) )
		$trm_settings_fields = array();
	if ( !isset($trm_settings_fields[$page]) )
		$trm_settings_fields[$page] = array();
	if ( !isset($trm_settings_fields[$page][$section]) )
		$trm_settings_fields[$page][$section] = array();

	$trm_settings_fields[$page][$section][$id] = array('id' => $id, 'title' => $title, 'callback' => $callback, 'args' => $args);
}
...
function do_settings_sections($page) {
	global $trm_settings_sections, $trm_settings_fields;

	if ( !isset($trm_settings_sections) || !isset($trm_settings_sections[$page]) )
		return;

	foreach ( (array) $trm_settings_sections[$page] as $section ) {
		echo "<h3>{$section['title']}</h3>\n";
		call_user_func($section['callback'], $section);
		if ( !isset($trm_settings_fields) || !isset($trm_settings_fields[$page]) || !isset($trm_settings_fields[$page][$section['id']]) )
			continue;
 ...
 *
 * @global $trm_settings_fields Storage array of settings fields and their pages/sections
 *
 * @since 2.7.0
 *
...
function do_settings_fields($page, $section) {
	...
	if ( !isset($trm_settings_fields) || !isset($trm_settings_fields[$page]) || !isset($trm_settings_fields[$page][$section]) )
		return;

	foreach ( (array) $trm_settings_fields[$page][$section] as $field ) {
		echo '<tr valign="top">';
 ...
 * @since 3.0.0
 *
 * @global array $trm_settings_errors Storage array of errors registered during this pageload
 *
...
function add_settings_error( $setting, $code, $message, $type = 'error' ) {
	global $trm_settings_errors;

	if ( !isset($trm_settings_errors) )
		$trm_settings_errors = array();
	...
	$trm_settings_errors[] = $new_error;
}

/**
 * Fetch settings errors registered by add_settings_error()
 *
 * Checks the $trm_settings_errors array for any errors declared during the current
...
function get_settings_errors( $setting = '', $sanitize = FALSE ) {
	...
	} elseif ( count( $trm_settings_errors ) ) {
		$settings_errors = $trm_settings_errors;
	} else {
		return;
	}
	...
	<div id="find-posts" class="find-box" style="display:none;">
		...
		<div class="find-box-inside">
			<div class="find-box-search">
				...
				<?php trm_nonce_field( 'find-posts', '_ajax_nonce', false ); ?>
...
function iframe_header( $title = '', $limit_styles = false ) {
	...
	global $hook_suffix, $current_screen, $current_user, $admin_body_class, $trm_locale;
...
<title><?php bloginfo('name') ?> &rsaquo; <?php echo $title ?> &#8212; <?php _e('Trnder'); ?></title>
<?php
trm_enqueue_style( 'global' );
if ( ! $limit_styles )
	trm_enqueue_style( 'Backend-WeaprEcqaKejUbRq-trendr' );
trm_enqueue_style( 'colors' );
?>
...
function _media_states( $post ) {
	...
	if ( current_theme_supports( 'custom-header') ) {
		$meta_header = get_post_meta($post->ID, '_trm_attachment_is_custom_header', true );
		if ( ! empty( $meta_header ) && $meta_header == $stylesheet )
			$media_states[] = __( 'Header Image' );
	}

	if ( current_theme_supports( 'custom-background') ) {
		$meta_background = get_post_meta($post->ID, '_trm_attachment_is_custom_background', true );
...
function screen_meta($screen) {
	global $trm_meta_boxes, $_trm_contextual_help, $trm_list_table, $trm_current_screen_options;
	...
	$show_screen = false;
	if ( !empty($trm_meta_boxes[$screen->id]) || !empty($columns) )
		$show_screen = true;
	...
	if ( !isset($_trm_contextual_help) )
		$_trm_contextual_help = array();
	...
	if ( !empty($trm_current_screen_options) )
		$show_screen = true;
...
<div id="screen-options-wrap" class="hidden">
	...
	<?php if ( isset($trm_meta_boxes[$screen->id]) ) : ?>
...
?>
	...
	echo $screen_options;
	echo $settings; ?>
<div><?php trm_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false ); ?></div>
</form>
</div>

<?php endif; // $show_screen

	$_trm_contextual_help = apply_filters('contextual_help_list', $_trm_contextual_help, $screen);
	?>
	<div id="contextual-help-wrap" class="hidden">
	<?php
	$contextual_help = '';
	if ( isset($_trm_contextual_help[$screen->id]) ) {
		$contextual_help .= '<div class="metabox-prefs">' . $_trm_contextual_help[$screen->id] . "</div>\n";
	} else {
		$contextual_help .= '<div class="metabox-prefs">';
		$default_help = __('<a href="http://codex.trendr.org/" target="_blank">Documentation</a>');
		$default_help .= '<br />';
		$default_help .= __('<a href="http://trendr.org/support/" target="_blank">Support Forums</a>');
...
function add_contextual_help($screen, $help) {
	...
	$_trm_contextual_help[$screen->id] = $help;
}

function screen_layout($screen) {
	global $screen_layout_columns, $trm_current_screen_options;
	...
	if ( !isset($trm_current_screen_options['layout_columns']) ) {
		$screen_layout_columns = 0;
		return '';
	}

	$screen_layout_columns = get_user_option("screen_layout_$screen->id");
	$num = $trm_current_screen_options['layout_columns']['max'];

	if ( ! $screen_layout_columns ) {
		if ( isset($trm_current_screen_options['layout_columns']['default']) )
			$screen_layout_columns = $trm_current_screen_options['layout_columns']['default'];
		else
...
function add_screen_option( $option, $args = array() ) {
	global $trm_current_screen_options;

	if ( !isset($trm_current_screen_options) )
		$trm_current_screen_options = array();

	$trm_current_screen_options[$option] = $args;
}
...
function screen_options($screen) {
	...
	if ( !isset($trm_current_screen_options['per_page']) )
		return '';

	$per_page_label = $trm_current_screen_options['per_page']['label'];

	if ( empty($trm_current_screen_options['per_page']['option']) ) {
		$option = str_replace( '-', '_', "{$screen->id}_per_page" );
	} else {
		$option = $trm_current_screen_options['per_page']['option'];
	}

	$per_page = (int) get_user_option( $option );
	if ( empty( $per_page ) || $per_page < 1 ) {
		if ( isset($trm_current_screen_options['per_page']['default']) )
			$per_page = $trm_current_screen_options['per_page']['default'];
		else
			$per_page = 20;
	}
	...
	if ( !empty($per_page_label) )
		$return .= "<input type='text' class='screen-per-page' name='trm_screen_options[value]' id='$option' maxlength='3' value='$per_page' /> <label for='$option'>$per_page_label</label>\n";
	$return .= get_submit_button( __( 'Apply' ), 'button', 'screen-options-apply', false );
	$return .= "<input type='hidden' name='trm_screen_options[option]' value='" . esc_attr($option) . "' />";
	$return .= "</div>\n";
	return $return;
}
	...
	var testCompression = {
		get : function(test) {
			...
			if (x) {
				...
				x.open('GET', ajaxurl + '?action=trm-compression-test&test='+test+'&'+(new Date()).getTime(), true);
				x.send('');
			}
		},
