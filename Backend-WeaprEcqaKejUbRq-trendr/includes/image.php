 ...
 */
function trm_create_thumbnail( $file, $max_side, $deprecated = '' ) {
	if ( !empty( $deprecated ) )
		_deprecated_argument( __FUNCTION__, '1.2' );
	$thumbpath = image_resize( $file, $max_side, $max_side );
	return apply_filters( 'trm_create_thumbnail', $thumbpath );
}
 ...
 * @return string|TRM_Error|false New filepath on success, TRM_Error or false on failure.
 */
function trm_crop_image( $src_file, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false ) {
	if ( is_numeric( $src_file ) ) // Handle int as attachment ID
		$src_file = get_attached_file( $src_file );

	$src = trm_load_image( $src_file );

	if ( !is_resource( $src ) )
		return new TRM_Error( 'error_loading_image', $src, $src_file );

	$dst = trm_imagecreatetruecolor( $dst_w, $dst_h );
	...
	if ( imagejpeg( $dst, $dst_file, apply_filters( 'jpeg_quality', 90, 'trm_crop_image' ) ) )
		return $dst_file;
	else
		return false;
}
 ...
 */
function trm_generate_attachment_metadata( $attachment_id, $file ) {
	...
	if ( preg_match('!^image/!', get_post_mime_type( $attachment )) && file_is_displayable_image($file) ) {
		...
		list($uwidth, $uheight) = trm_constrain_dimensions($metadata['width'], $metadata['height'], 128, 96);
		$metadata['hwstring_small'] = "height='$uheight' width='$uwidth'";

		// Make the file path relative to the upload dir
		$metadata['file'] = _trm_relative_upload_path($file);
		...
		foreach ( get_intermediate_image_sizes() as $s ) {
			...
			if ( isset( $_trm_additional_image_sizes[$s]['width'] ) )
				$sizes[$s]['width'] = intval( $_trm_additional_image_sizes[$s]['width'] ); // For theme-added sizes
			else
				$sizes[$s]['width'] = get_option( "{$s}_size_w" ); // For default sizes set in options
			if ( isset( $_trm_additional_image_sizes[$s]['height'] ) )
				$sizes[$s]['height'] = intval( $_trm_additional_image_sizes[$s]['height'] ); // For theme-added sizes
			else
				$sizes[$s]['height'] = get_option( "{$s}_size_h" ); // For default sizes set in options
			if ( isset( $_trm_additional_image_sizes[$s]['crop'] ) )
				$sizes[$s]['crop'] = intval( $_trm_additional_image_sizes[$s]['crop'] ); // For theme-added sizes
			else
		...
		$image_meta = trm_read_image_metadata( $file );
		if ( $image_meta )
			$metadata['image_meta'] = $image_meta;

	}

	return apply_filters( 'trm_generate_attachment_metadata', $metadata, $attachment_id );
}
...
function get_udims( $width, $height) {
	return trm_constrain_dimensions( $width, $height, 128, 96 );
}
 ...
 * @return int|float
 */
function trm_exif_frac2dec($str) {
 ...
 */
function trm_read_image_metadata( $file ) {
	if ( ! file_exists( $file ) )
		return false;
	...
	if ( is_callable( 'exif_read_data' ) && in_array( $sourceImageType, apply_filters( 'trm_read_image_metadata_types', array( IMAGETYPE_JPEG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM ) ) ) ) {
		...
		if ( ! empty($exif['FNumber'] ) )
			$meta['aperture'] = round( trm_exif_frac2dec( $exif['FNumber'] ), 2 );
		if ( ! empty($exif['Model'] ) )
			$meta['camera'] = utf8_encode( trim( $exif['Model'] ) );
		if ( ! empty($exif['DateTimeDigitized'] ) )
			$meta['created_timestamp'] = trm_exif_date2ts($exif['DateTimeDigitized'] );
		if ( ! empty($exif['FocalLength'] ) )
			$meta['focal_length'] = trm_exif_frac2dec( $exif['FocalLength'] );
		if ( ! empty($exif['ISOSpeedRatings'] ) )
			$meta['iso'] = utf8_encode( trim( $exif['ISOSpeedRatings'] ) );
		if ( ! empty($exif['ExposureTime'] ) )
			$meta['shutter_speed'] = trm_exif_frac2dec( $exif['ExposureTime'] );
	}

	return apply_filters( 'trm_read_image_metadata', $meta, $file, $sourceImageType );
