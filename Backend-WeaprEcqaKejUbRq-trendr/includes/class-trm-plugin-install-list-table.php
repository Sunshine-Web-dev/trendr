 ...
 * @access private
 */
class TRM_Plugin_Install_List_Table extends TRM_List_Table {
	...
	function prepare_items() {
		include( ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/includes/plugin-install.php' );

		global $tabs, $tab, $paged, $type, $term;

		trm_reset_vars( array( 'tab' ) );
		...
		if ( is_trm_error( $api ) )
			trm_die( $api->get_error_message() . '</p> <p class="hide-if-no-js"><a href="#" onclick="document.location.reload(); return false;">' . __( 'Try again' ) . '</a>' );
	...
	function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			$title = trm_kses( $plugin['name'], $plugins_allowedtags );
			...
			$version = trm_kses( $plugin['version'], $plugins_allowedtags );
			...
			$author = trm_kses( $author, $plugins_allowedtags );
