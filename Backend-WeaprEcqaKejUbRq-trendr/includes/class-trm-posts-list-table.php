 ...
 * @access private
 */
class TRM_Posts_List_Table extends TRM_List_Table {
	...
	function __construct() {
		global $post_type_object, $post_type, $trmdb;
		...
		elseif ( in_array( $_REQUEST['post_type'], get_post_types( array( 'show_ui' => true ) ) ) )
			...
			trm_die( __( 'Invalid post type' ) );
		$_REQUEST['post_type'] = $post_type;

		$post_type_object = get_post_type_object( $post_type );

		if ( !current_user_can( $post_type_object->cap->edit_others_posts ) ) {
			$this->user_posts_count = $trmdb->get_var( $trmdb->prepare( "
				SELECT COUNT( 1 ) FROM $trmdb->posts
		...
		if ( 'post' == $post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
			...
			$this->sticky_posts_count = $trmdb->get_var( $trmdb->prepare( "SELECT COUNT( 1 ) FROM $trmdb->posts WHERE post_type = %s AND post_status != 'trash' AND ID IN ($sticky_posts)", $post_type ) );
		}
	...
	function prepare_items() {
		global $post_type_object, $post_type, $avail_post_stati, $trm_query, $per_page, $mode;

		$avail_post_stati = trm_edit_posts_query();

		$this->hierarchical_display = ( $post_type_object->hierarchical && 'menu_order title' == $trm_query->query['orderby'] );

		$total_items = $this->hierarchical_display ? $trm_query->post_count : $trm_query->found_posts;
		...
		if ( $this->hierarchical_display )
			...
			$total_pages = $trm_query->max_num_pages;
	...
	function get_views() {
		...
		$status_links = array();
		$num_posts = trm_count_posts( $post_type, 'readable' );
		$class = '';
		$allposts = '';
...
<?php
		if ( 'top' == $which && !is_singular() ) {
			...
			if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
				...
				trm_dropdown_categories( $dropdown_options );
			}
...
<?php
	...
	function display_rows( $posts = array() ) {
		global $trm_query, $post_type_object, $per_page;

		if ( empty( $posts ) )
			$posts = $trm_query->posts;
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$trmdb->update( $trmdb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	...
	function single_row( $a_post, $level = 0 ) {
		...
		$post_format_class = ( $post_format && !is_trm_error($post_format) ) ? 'format-' . sanitize_html_class( $post_format ) : 'format-default';
	?>
...
<?php
				...
				if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
					if ( 'trash' == $post->post_status )
						$actions['untrash'] = "<a title='" . esc_attr( __( 'Restore this item from the Trash' ) ) . "' href='" . trm_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-' . $post->post_type . '_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
	...
	<?php endif; // $bulk
		...
		if ( post_type_supports( $screen->post_type, 'author' ) ) :
			...
			if ( is_super_admin() || current_user_can( $post_type_object->cap->edit_others_posts ) ) :
				...
				if ( $authors = trm_dropdown_users( $users_opt ) ) :
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name )?>-checklist">
				<?php trm_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ) ?>
			</ul>
	...
	<?php
		...
		trm_dropdown_pages( $dropdown_args );
	?>
			</label>
	...
	?>
		<p class="submit inline-edit-save">
			...
			<?php if ( ! $bulk ) {
				trm_nonce_field( 'inlineeditnonce', '_inline_edit', false );
