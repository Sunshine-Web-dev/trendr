...
<?php } ?>
<img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" class="ajax-loading" id="draft-ajax-loading" alt="" />
</div>
...
?>
<a class="preview button" href="<?php echo $preview_link; ?>" target="trm-preview" id="post-preview" tabindex="4"><?php echo $preview_button; ?></a>
<input type="hidden" name="trm-preview" id="trm-preview" value="" />
</div>
...
<img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" class="ajax-loading" id="ajax-loading" alt="" />
<?php
	...
	<div id="taxonomy-<?php echo $taxonomy; ?>" class="categorydiv">
		...
		<div id="<?php echo $taxonomy; ?>-pop" class="tabs-panel" style="display: none;">
			<ul id="<?php echo $taxonomy; ?>checklist-pop" class="categorychecklist form-no-clear" >
				<?php $popular_ids = trm_popular_terms_checklist($taxonomy); ?>
			</ul>
		</div>
		...
		<div id="<?php echo $taxonomy; ?>-all" class="tabs-panel">
			...
			<ul id="<?php echo $taxonomy; ?>checklist" class="list:<?php echo $taxonomy?> categorychecklist form-no-clear">
				<?php trm_terms_checklist($post->ID, array( 'taxonomy' => $taxonomy, 'popular_cats' => $popular_ids ) ) ?>
			</ul>
		</div>
	<?php if ( current_user_can($tax->cap->edit_terms) ) : ?>
			<div id="<?php echo $taxonomy; ?>-adder" class="trm-hidden-children">
				<h4>
				...
				<p id="<?php echo $taxonomy; ?>-add" class="category-add trm-hidden-child">
					<label class="screen-reader-text" for="new<?php echo $taxonomy; ?>"><?php echo $tax->labels->add_new_item; ?></label>
					<input type="text" name="new<?php echo $taxonomy; ?>" id="new<?php echo $taxonomy; ?>" class="form-required form-input-tip" value="<?php echo esc_attr( $tax->labels->new_item_name ); ?>" tabindex="3" aria-required="true"/>
					<label class="screen-reader-text" for="new<?php echo $taxonomy; ?>_parent">
						<?php echo $tax->labels->parent_item_colon; ?>
					</label>
					<?php trm_dropdown_categories( array( 'taxonomy' => $taxonomy, 'hide_empty' => 0, 'name' => 'new'.$taxonomy.'_parent', 'orderby' => 'name', 'hierarchical' => 1, 'show_option_none' => '&mdash; ' . $tax->labels->parent_item . ' &mdash;', 'tab_index' => 3 ) ); ?>
					<input type="button" id="<?php echo $taxonomy; ?>-add-submit" class="add:<?php echo $taxonomy ?>checklist:<?php echo $taxonomy ?>-add button category-add-sumbit" value="<?php echo esc_attr( $tax->labels->add_new_item ); ?>" tabindex="3" />
					<?php trm_nonce_field( 'add-'.$taxonomy, '_ajax_nonce-add-'.$taxonomy, false ); ?>
...
<p><?php _e('Excerpts are optional hand-crafted summaries of your content that can be used in your theme. <a href="http://codex.trendr.org/Excerpt" target="_blank">Learn more about manual excerpts.</a>'); ?></p>
<?php
}
...
<p><?php _e('Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. If you link other Trnder sites they&#8217;ll be notified automatically using <a href="http://codex.trendr.org/Introduction_to_Blogging#Managing_Comments" target="_blank">pingbacks</a>, no other action necessary.'); ?></p>
<?php
if ( ! empty($pings) )
	echo $pings;
}
...
<p><?php _e('Custom fields can be used to add extra metadata to a post that you can <a href="http://codex.trendr.org/Using_Custom_Fields" target="_blank">use in your theme</a>.'); ?></p>
<?php
}
...
<p class="meta-options">
	...
	<label for="ping_status" class="selectit"><input name="ping_status" type="checkbox" id="ping_status" value="open" <?php checked($post->ping_status, 'open'); ?> /> <?php printf( __( 'Allow <a href="%s" target="_blank">trackbacks and pingbacks</a> on this page.' ), __( 'http://codex.trendr.org/Introduction_to_Blogging#Managing_Comments' ) ); ?></label>
...
function post_comment_meta_box($post) {
	...
	$total = $trmdb->get_var($trmdb->prepare("SELECT count(1) FROM $trmdb->comments WHERE comment_post_ID = '%d' AND ( comment_approved = '0' OR comment_approved = '1')", $post_ID));
	...
	trm_nonce_field( 'get-comments', 'add_comment_nonce', false );

	$trm_list_table = _get_list_table('TRM_Post_Comments_List_Table');
	$trm_list_table->display( true );
?>
<p class="hide-if-no-js"><a href="#commentstatusdiv" id="show-comments" onclick="commentsBox.get(<?php echo $total; ?>);return false;"><?php _e('Show comments'); ?></a> <img class="waiting" style="display:none;" src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" alt="" /></p>
<?php
...
function page_attributes_meta_box($post) {
	...
	if ( $post_type_object->hierarchical ) {
		$pages = trm_dropdown_pages(array('post_type' => $post->post_type, 'exclude_tree' => $post->ID, 'selected' => $post->post_parent, 'name' => 'parent_id', 'show_option_none' => __('(no parent)'), 'sort_column'=> 'menu_order, post_title', 'echo' => 0));
		if ( ! empty($pages) ) {
?>
...
if ( !empty($_GET['action']) && 'edit' == $_GET['action'] && current_user_can('manage_links') ) { ?>
	<a class="submitdelete deletion" href="<?php echo trm_nonce_url("link.php?action=delete&amp;link_id=$link->link_id", 'delete-bookmark_' . $link->link_id); ?>" onclick="if ( confirm('<?php echo esc_js(sprintf(__("You are about to delete this link '%s'\n  'Cancel' to stop, 'OK' to delete."), $link->link_name )); ?>') ) {return true;}return false;"><?php _e('Delete'); ?></a>
<?php } ?>
</div>
...
<div id="categories-all" class="tabs-panel">
	<ul id="categorychecklist" class="list:category categorychecklist form-no-clear">
		...
		if ( isset($link->link_id) )
			trm_link_category_checklist($link->link_id);
		else
			trm_link_category_checklist();
		?>
	</ul>
</div>
...
<div id="categories-pop" class="tabs-panel" style="display: none;">
	<ul id="categorychecklist-pop" class="categorychecklist form-no-clear">
		<?php trm_popular_terms_checklist('link_category'); ?>
	</ul>
</div>

<div id="category-adder" class="trm-hidden-children">
	<h4><a id="category-add-toggle" href="#category-add"><?php _e( '+ Add New Category' ); ?></a></h4>
	<p id="link-category-add" class="trm-hidden-child">
		...
		<?php trm_nonce_field( 'add-link-category', '_ajax_nonce', false ); ?>
...
function post_thumbnail_meta_box() {
	...
	echo _trm_post_thumbnail_html( $thumbnail_id );
}
