<?php
/**
 * Trnder Plugin Install Administration API
 *
 * @package Trnder
...
/**
 * Retrieve plugin installer pages from Trnder Plugins API.
 *
 ...
 * @return object plugins_api response object on success, TRM_Error on failure.
 */
function plugins_api($action, $args = null) {
	...
	// Allows a plugin to override the Trnder.org API entirely.
	...
	if ( false === $res ) {
		$request = trm_remote_post('module/info/1.0/', array( 'timeout' => 15, 'body' => array('action' => $action, 'request' => serialize($args))) );
		if ( is_trm_error($request) ) {
			$res = new TRM_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.'), $request->get_error_message() );
		} else {
			$res = unserialize( trm_remote_retrieve_body( $request ) );
			if ( false === $res )
				$res = new TRM_Error('plugins_api_failed', __('An unknown error occurred.'), trm_remote_retrieve_body( $request ) );
		}
	} elseif ( !is_trm_error($res) ) {
		$res->external = true;
	}
...
/**
 * Retrieve popular Trnder plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	?>
	<p><?php _e('Plugins extend and expand the functionality of Trnder. You may automatically install plugins from the <a href="http://trendr.org/extend/module/">Trnder Plugin Directory</a> or upload a plugin in .zip format via this page.') ?></p>
	...
	if ( is_trm_error($api_tags) ) {
		echo $api_tags->get_error_message();
	} else {
		//Set up the tags in a way which can be interprated by trm_generate_tag_cloud()
		$tags = array();
		foreach ( (array)$api_tags as $tag )
			$tags[ $tag['name'] ] = (object) array(
									...
									'count' => $tag['count'] );
		echo trm_generate_tag_cloud($tags, array( 'single_text' => __('%d plugin'), 'multiple_text' => __('%d plugins') ) );
	}
...
?>
	...
	<form method="post" enctype="multipart/form-data" action="<?php echo self_admin_url('update.php?action=upload-plugin') ?>">
		<?php trm_nonce_field( 'plugin-upload') ?>
...
function display_plugins_table() {
	global $trm_list_table;

	$trm_list_table->display();
}
...
function install_plugin_install_status($api, $loop = false) {
		...
		foreach ( (array)$update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can('update_plugins') )
					$url = trm_nonce_url(self_admin_url('update.php?action=upgrade-plugin&plugin=' . $update_file), 'upgrade-plugin_' . $update_file);
				break;
			}
		}
	}
	...
	if ( 'install' == $status ) {
		if ( is_dir( TRM_PLUGIN_DIR  . '/' . $api->slug ) ) {
			$installed_plugin = get_plugins('/' . $api->slug);
			if ( empty($installed_plugin) ) {
				if ( current_user_can('install_plugins') )
					$url = trm_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $api->slug), 'install-plugin_' . $api->slug);
			} else {
...
function install_plugin_information() {
	...
	foreach ( (array)$api->sections as $section_name => $content )
		$api->sections[$section_name] = trm_kses($content, $plugins_allowedtags);
	foreach ( array('version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug') as $key )
		$api->$key = trm_kses($api->$key, $plugins_allowedtags);
...
<?php endif; if ( ! empty($api->requires) ) : ?>
			<li><strong><?php _e('Requires Trnder Version:') ?></strong> <?php printf(__('%s or higher'), $api->requires) ?></li>
...
<?php endif; if ( ! empty($api->slug) && empty($api->external) ) : ?>
			<li><a target="_blank" href="http://trendr.org/extend/module/<?php echo $api->slug ?>/"><?php _e('Trnder.org Plugin Page &#187;') ?></a></li>
...
<?php endif; ?>
	...
	<?php
		if ( !empty($api->tested) && version_compare( substr($GLOBALS['trm_version'], 0, strlen($api->tested)), $api->tested, '>') )
			echo '<div class="updated"><p>' . __('<strong>Warning:</strong> This plugin has <strong>not been tested</strong> with your current version of Trnder.') . '</p></div>';

		else if ( !empty($api->requires) && version_compare( substr($GLOBALS['trm_version'], 0, strlen($api->requires)), $api->requires, '<') )
			echo '<div class="updated"><p>' . __('<strong>Warning:</strong> This plugin has <strong>not been marked as compatible</strong> with your version of Trnder.') . '</p></div>';
		...
		foreach ( (array)$api->sections as $section_name => $content ) {
			...
			$content = links_add_base_url($content, 'http://trendr.org/extend/module/' . $api->slug . '/');
