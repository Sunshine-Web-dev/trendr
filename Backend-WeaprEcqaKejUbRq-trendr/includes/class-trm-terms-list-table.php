 ...
 * @access private
 */
class TRM_Terms_List_Table extends TRM_List_Table {
	...
	function __construct() {
		...
		trm_reset_vars( array( 'action', 'taxonomy', 'post_type' ) );

		if ( empty( $taxonomy ) )
			$taxonomy = 'post_tag';

		if ( !taxonomy_exists( $taxonomy ) )
			trm_die( __( 'Invalid taxonomy' ) );
	...
	function prepare_items() {
		...
		$this->set_pagination_args( array(
			'total_items' => trm_count_terms( $taxonomy, compact( 'search' ) ),
	...
	function display_rows_or_placeholder() {
		...
		$args = trm_parse_args( $this->callback_args, array(
			'page' => 1,
			'number' => 20,
			'search' => '',
			'hide_empty' => 0
	...
	function column_name( $tag ) {
		...
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term )
			$actions['delete'] = "<a class='delete-tag' href='" . trm_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ) . "'>" . __( 'Delete' ) . "</a>";
...
?>
	...
	?>
		...
		<p class="inline-edit-save submit">
			...
			<?php trm_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
