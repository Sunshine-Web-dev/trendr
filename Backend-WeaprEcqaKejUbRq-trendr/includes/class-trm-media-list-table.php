 ...
 * @access private
 */
class TRM_Media_List_Table extends TRM_List_Table {
	...
	function prepare_items() {
		global $lost, $trmdb, $trm_query, $post_mime_types, $avail_post_mime_types;
		...
		list( $post_mime_types, $avail_post_mime_types ) = trm_edit_attachments_query( $q );

 		$this->is_trash = isset( $_REQUEST['status'] ) && 'trash' == $_REQUEST['status'];

		$this->set_pagination_args( array(
			'total_items' => $trm_query->found_posts,
			'total_pages' => $trm_query->max_num_pages,
			'per_page' => $trm_query->query_vars['posts_per_page'],
		) );
	}

	function get_views() {
		global $trmdb, $post_mime_types, $avail_post_mime_types;

		$type_links = array();
		$_num_posts = (array) trm_count_attachments();
		$_total_posts = array_sum($_num_posts) - $_num_posts['trash'];
		if ( !isset( $total_orphans ) )
				$total_orphans = $trmdb->get_var( "SELECT COUNT( * ) FROM $trmdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent < 1" );
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			if ( !empty($_GET['post_mime_type']) && trm_match_mime_types($mime_type, $_GET['post_mime_type']) )
				$class = ' class="current"';
...
?>
		<td <?php echo $attributes ?>><?php
			if ( $thumb = trm_get_attachment_image( $post->ID, array( 80, 60 ), true ) ) {
				if ( $this->is_trash ) {
					echo $thumb;
				} else {
?>
		...
		if ( $this->detached ) {
			...
			if ( current_user_can( 'delete_post', $post->ID ) )
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = "<a class='submitdelete' href='" . trm_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-attachment_' . $post->ID ) . "'>" . __( 'Trash' ) . "</a>";
				} else {
					$delete_ays = !MEDIA_TRASH ? " onclick='return showNotice.warn();'" : '';
					$actions['delete'] = "<a class='submitdelete'$delete_ays href='" . trm_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-attachment_' . $post->ID ) . "'>" . __( 'Delete Permanently' ) . "</a>";
				}
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash )
					$actions['untrash'] = "<a class='submitdelete' href='" . trm_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-attachment_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
