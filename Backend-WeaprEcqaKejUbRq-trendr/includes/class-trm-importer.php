...
class TRM_Importer {
	...
	/**
	 * Returns array with imported permalinks from Trnder database
	 *
	 * @param string $bid
	 * @return array
	 */
	function get_imported_posts( $importer_name, $bid ) {
		...
		do {
			...
			$sql = $trmdb->prepare( "SELECT post_id, meta_value FROM $trmdb->postmeta WHERE meta_key = '%s' LIMIT %d,%d", $meta_key, $offset, $limit );
	...
	/**
	 * Return count of imported permalinks from Trnder database
	 *
	 * @param string $bid
	 * @return int
	 */
	function count_imported_posts( $importer_name, $bid ) {
		...
		$meta_key = $importer_name . '_' . $bid . '_permalink';
		$sql = $trmdb->prepare( "SELECT COUNT( post_id ) AS cnt FROM $trmdb->postmeta WHERE meta_key = '%s'", $meta_key );

		$result = $trmdb->get_results( $sql );
	...
	/**
	 * Set array with imported comments from Trnder database
	 *
	 * @param string $bid
	 * @return array
	 */
	function get_imported_comments( $bid ) {
		...
		do {
			$sql = $trmdb->prepare( "SELECT comment_ID, comment_agent FROM $trmdb->comments LIMIT %d,%d", $offset, $limit );
			$results = $trmdb->get_results( $sql );
	...
	function set_user( $user_id ) {
		...
		if ( !$user_id || !trm_set_current_user( $user_id ) ) {
	...
	function get_page( $url, $username = '', $password = '', $head = false ) {
		...
		return trm_remote_request( $url, $args );
	}
	...
	function stop_the_insanity() {
		global $trmdb, $trm_actions;
		// Or define( 'TRM_IMPORTING', true );
		$trmdb->queries = array();
		// Reset $trm_actions to keep it from growing out of control
		$trm_actions = array();
	}
}
