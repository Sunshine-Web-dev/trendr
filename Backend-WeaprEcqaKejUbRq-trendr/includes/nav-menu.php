...
class Walker_Nav_Menu_Edit extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $item, $depth, $args) {
		global $_trm_nav_menu_max_depth;
		$_trm_nav_menu_max_depth = $depth > $_trm_nav_menu_max_depth ? $depth : $_trm_nav_menu_max_depth;
		...
		if ( 'taxonomy' == $item->type ) {
			...
			if ( is_trm_error( $original_title ) )
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-attrm-title-<?php echo $item_id; ?>">
						<?php _e( 'Title Attribute' ); ?><br />
						<input type="text" id="edit-menu-item-attrm-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-attrm-title" name="menu-item-attrm-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
					</label>
				</p>
 ...
 */
function _trm_ajax_menu_quick_search( $request = array() ) {
	$args = array();
	...
	if ( 'get-post-item' == $type ) {
		if ( post_type_exists( $object_type ) ) {
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', array( get_post( $object_id ) ) ), 0, (object) $args );
			...
			if ( isset( $request['ID'] ) ) {
				$object_id = (int) $request['ID'];
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', array( get_term( $object_id, $object_type ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
		...
		if ( 'posttype' == $matches[1] && get_post_type_object( $matches[2] ) ) {
			...
			while ( have_posts() ) {
				the_post();
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', array( get_post( get_the_ID() ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo json_encode(
						array(
							'ID' => get_the_ID(),
			...
			if ( empty( $terms ) || is_trm_error( $terms ) )
				return;
			foreach( (array) $terms as $term ) {
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', array( $term ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo json_encode(
						array(
							'ID' => $term->term_id,
 ...
 * @since 3.0.0
 **/
function trm_nav_menu_setup() {
	// Register meta boxes
	if ( trm_get_nav_menus() )
		add_meta_box( 'nav-menu-theme-locations', __( 'Theme Locations' ), 'trm_nav_menu_locations_meta_box' , 'nav-menus', 'side', 'default' );
	add_meta_box( 'add-custom-links', __('Custom Links'), 'trm_nav_menu_item_link_meta_box', 'nav-menus', 'side', 'default' );
	trm_nav_menu_post_type_meta_boxes();
	trm_nav_menu_taxonomy_meta_boxes();

	// Register advanced menu items (columns)
	add_filter( 'manage_nav-menus_columns', 'trm_nav_menu_manage_columns');
 ...
 * @since 3.0.0
 **/
function trm_initial_nav_menu_meta_boxes() {
	global $trm_meta_boxes;

	if ( get_user_option( 'metaboxhidden_nav-menus' ) !== false || ! is_array($trm_meta_boxes) )
		return;

	$initial_meta_boxes = array( 'nav-menu-theme-locations', 'add-custom-links', 'add-page', 'add-category' );
	$hidden_meta_boxes = array();

	foreach ( array_keys($trm_meta_boxes['nav-menus']) as $context ) {
		foreach ( array_keys($trm_meta_boxes['nav-menus'][$context]) as $priority ) {
			foreach ( $trm_meta_boxes['nav-menus'][$context][$priority] as $box ) {
 ...
 * @since 3.0.0
 */
function trm_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( $post_type ) {
			$id = $post_type->name;
			add_meta_box( "add-{$id}", $post_type->labels->name, 'trm_nav_menu_item_post_type_meta_box', 'nav-menus', 'side', 'default', $post_type );
		}
	}
}
 ...
 * @since 3.0.0
 */
function trm_nav_menu_taxonomy_meta_boxes() {
	...
	foreach ( $taxonomies as $tax ) {
		...
		if ( $tax ) {
			$id = $tax->name;
			add_meta_box( "add-{$id}", $tax->labels->name, 'trm_nav_menu_item_taxonomy_meta_box', 'nav-menus', 'side', 'default', $tax );
		}
	}
}
 ...
 * @since 3.0.0
 */
function trm_nav_menu_locations_meta_box() {
			...
			<label class="howto" for="locations-<?php echo $location; ?>">
				...
				<select name="menu-locations[<?php echo $location; ?>]" id="locations-<?php echo $location; ?>">
					...
					<option<?php selected( isset( $menu_locations[ $location ] ) && $menu_locations[ $location ] == $menu->term_id ); ?>
						...
						$truncated_name = trm_html_excerpt( $menu->name, 40 );
	...
	<p class="button-controls">
		<img class="waiting" src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" alt="" />
 ...
 * @since 3.0.0
 */
function trm_nav_menu_item_link_meta_box() {
 ...
 */
function trm_nav_menu_item_post_type_meta_box( $object, $post_type ) {
	global $_nav_menu_placeholder, $nav_menu_selected_id;
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		...
		?>">
			<ul id="<?php echo $post_type_name; ?>checklist-most-recent" class="categorychecklist form-no-clear">
				...
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', $most_recent), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>" id="tabs-panel-posttype-<?php echo $post_type_name; ?>-search">
			...
			<?php if ( ! empty( $search_results ) && ! is_trm_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', $search_results), 0, (object) $args );
				?>
			<?php elseif ( is_trm_error( $search_results ) ) : ?>
		...
		?>">
			...
			<ul id="<?php echo $post_type_name; ?>checklist" class="list:<?php echo $post_type_name?> categorychecklist form-no-clear">
				...
				$checkbox_items = walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', $posts), 0, (object) $args );
 ...
 */
function trm_nav_menu_item_taxonomy_meta_box( $object, $taxonomy ) {
	global $nav_menu_selected_id;
	...
	if ( ! $terms || is_trm_error($terms) ) {
		echo '<p>' . __( 'No items.' ) . '</p>';
		return;
	}

	$num_pages = ceil( trm_count_terms( $taxonomy_name , array_merge( $args, array('number' => '', 'offset' => '') ) ) / $per_page );
	...
	<div id="taxonomy-<?php echo $taxonomy_name; ?>" class="taxonomydiv">
		...
		?>">
			<ul id="<?php echo $taxonomy_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				...
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', $popular_terms), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>">
			...
			<ul id="<?php echo $taxonomy_name; ?>checklist" class="list:<?php echo $taxonomy_name?> categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', $terms), 0, (object) $args );
				?>
			</ul>
			<?php if ( ! empty( $page_links ) ) : ?>
				<div class="add-menu-item-pagelinks">
 ...
 */
function trm_save_nav_menu_items( $menu_id = 0, $menu_data = array() ) {
	$menu_id = (int) $menu_id;
	$items_saved = array();
	...
	if ( 0 == $menu_id || is_nav_menu( $menu_id ) ) {
		...
		foreach( (array) $menu_data as $_possible_db_id => $_item_object_data ) {
			...
			$args = array(
				...
				'menu-item-attrm-title' => ( isset( $_item_object_data['menu-item-attrm-title'] ) ? $_item_object_data['menu-item-attrm-title'] : '' ),
				'menu-item-target' => ( isset( $_item_object_data['menu-item-target'] ) ? $_item_object_data['menu-item-target'] : '' ),
				'menu-item-classes' => ( isset( $_item_object_data['menu-item-classes'] ) ? $_item_object_data['menu-item-classes'] : '' ),
				'menu-item-xfn' => ( isset( $_item_object_data['menu-item-xfn'] ) ? $_item_object_data['menu-item-xfn'] : '' ),
			);

			$items_saved[] = trm_update_nav_menu_item( $menu_id, $_actual_db_id, $args );
 ...
 */
function _trm_nav_menu_meta_box_object( $object = null ) {
 ...
 * @return string|TRM_Error $output The menu formatted to edit or error object on failure.
 */
function trm_get_nav_menu_to_edit( $menu_id = 0 ) {
	$menu = trm_get_nav_menu_object( $menu_id );

	// If the menu exists, get its items.
	if ( is_nav_menu( $menu ) ) {
		$menu_items = trm_get_nav_menu_items( $menu->term_id, array('post_status' => 'any') );
		...
		$walker_class_name = apply_filters( 'trm_edit_nav_menu_walker', 'Walker_Nav_Menu_Edit', $menu_id );

		if ( class_exists( $walker_class_name ) )
			$walker = new $walker_class_name;
		else
			return new TRM_Error( 'menu_walker_not_exist', sprintf( __('The Walker class named <strong>%s</strong> does not exist.'), $walker_class_name ) );
		...
		$result .= walk_nav_menu_tree( array_map('trm_setup_nav_menu_item', $menu_items), 0, (object) array('walker' => $walker ) );
		$result .= ' </ul> ';
		return $result;
	} elseif ( is_trm_error( $menu ) ) {
		return $menu;
	}
 ...
 * @return string|TRM_Error $output The menu formatted to edit or error object on failure.
 */
function trm_nav_menu_manage_columns() {
	return array(
 ...
 *
 */
function _trm_delete_orphaned_draft_menu_items() {
	global $trmdb;
	$delete_timestamp = time() - (60*60*24*EMPTY_TRASH_DAYS);

	// delete orphaned draft menu items
	$menu_items_to_delete = $trmdb->get_col($trmdb->prepare("SELECT ID FROM $trmdb->posts AS p LEFT JOIN $trmdb->postmeta AS m ON p.ID = m.post_id WHERE post_type = 'nav_menu_item' AND post_status = 'draft' AND meta_key = '_menu_item_orphaned' AND meta_value < '%d'", $delete_timestamp ) );

	foreach( (array) $menu_items_to_delete as $menu_item_id )
		trm_delete_post( $menu_item_id, true );
}

add_action('admin_head-nav-menus.php', '_trm_delete_orphaned_draft_menu_items');
