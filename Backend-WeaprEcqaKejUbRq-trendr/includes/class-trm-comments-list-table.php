 ...
 * @access private
 */
class TRM_Comments_List_Table extends TRM_List_Table {
	...
	function __construct() {
		...
		if ( get_option('show_portraits') )
			add_filter( 'comment_author', 'floated_admin_portrait' );
	...
	function get_views() {
		...
		$status_links = array();
		$num_comments = ( $post_id ) ? trm_count_comments( $post_id ) : trm_count_comments();
...
<?php
		...
		if ( ( 'spam' == $comment_status || 'trash' == $comment_status ) && current_user_can( 'moderate_comments' ) ) {
			trm_nonce_field( 'bulk-destroy', '_destroy_nonce' );
...
<?php
	...
	function single_row( $a_comment ) {
		...
		$comment = $a_comment;
		$the_comment_status = trm_get_comment_status( $comment->comment_ID );
	...
	function column_comment( $comment ) {
		...
		if ( $user_can ) {
			$del_nonce = esc_html( '_key=' . trm_create_nonce( "delete-comment_$comment->comment_ID" ) );
			$approve_nonce = esc_html( '_key=' . trm_create_nonce( "approve-comment_$comment->comment_ID" ) );
		...
		if ( $user_can ) {
			...
			foreach ( $actions as $action => $link ) {
				...
				elseif ( ( $action == 'untrash' && $the_comment_status == 'trash' ) || ( $action == 'unspam' && $the_comment_status == 'spam' ) ) {
					if ( '1' == get_comment_meta( $comment->comment_ID, '_trm_trash_meta_status', true ) )
						$action .= ' approve';
					else
						$action .= ' unapprove';
				}
	...
	function column_response( $comment ) {
		...
		echo '</div>';
		if ( 'attachment' == $post->post_type && ( $thumb = trm_get_attachment_image( $post->ID, array( 80, 60 ), true ) ) )
			echo $thumb;
	}
 ...
 * @access private
 *
 * @see TRM_Comments_Table
 */
class TRM_Post_Comments_List_Table extends TRM_Comments_List_Table {
