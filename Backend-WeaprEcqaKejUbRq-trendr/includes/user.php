 ...
 * has the ability to alter the user's role. Trnder core seems to
 ...
 * @return null|TRM_Error|int Null when adding user, TRM_Error or User ID integer when no parameters.
 */
function add_user() {
	if ( func_num_args() ) { // The hackiest hack that ever did hack
		...
		if ( isset( $_POST['role'] ) ) {
			...
			if ( $user_id != get_current_user_id() || $trm_roles->role_objects[$new_role]->has_cap( 'edit_users' ) ) {
				// If the new role isn't editable by the logged-in user die with error
				$editable_roles = get_editable_roles();
				if ( empty( $editable_roles[$new_role] ) )
					trm_die(__('You can&#8217;t give users that role.'));
...
function edit_user( $user_id = 0 ) {
	global $trm_roles, $trmdb;
	$user = new stdClass;
	if ( $user_id ) {
		$update = true;
		$user->ID = (int) $user_id;
		$userdata = get_userdata( $user_id );
		$user->user_login = $trmdb->escape( $userdata->user_login );
	} else {
		$update = false;
	}
	...
	if ( isset( $_POST['role'] ) && current_user_can( 'edit_users' ) ) {
		$new_role = sanitize_text_field( $_POST['role'] );
		$potential_role = isset($trm_roles->role_objects[$new_role]) ? $trm_roles->role_objects[$new_role] : false;
		// Don't let anyone with 'edit_users' (admins) edit their own role to something without it.
	...
	if ( $update ) {
		if ( empty($pass1) && !empty($pass2) )
			$errors->add( 'pass', __( ' You entered your new password only once.' ), array( 'form-field' => 'pass1' ) );
		elseif ( !empty($pass1) && empty($pass2) )
			$errors->add( 'pass', __( ' You entered your new password only once.' ), array( 'form-field' => 'pass2' ) );
	} else {
		if ( empty($pass1) )
	...
	if ( $update ) {
		$user_id = trm_update_user( get_object_vars( $user ) );
	} else {
		$user_id = trm_insert_user( get_object_vars( $user ) );
		trm_new_user_notification( $user_id, isset($_POST['send_password']) ? $pass1 : '' );
	}
	return $user_id;
}
 ...
 * list of roles in the $trm_roles object so that plugins can remove
 ...
 * @return object TRM_User object with user data.
 */
function get_user_to_edit( $user_id ) {
	$user = new TRM_User( $user_id );

	$user_contactmethods = _trm_get_user_contactmethods( $user );
...
function get_users_drafts( $user_id ) {
	global $trmdb;
	$query = $trmdb->prepare("SELECT ID, post_title FROM $trmdb->posts WHERE post_type = 'post' AND post_status = 'draft' AND post_author = %d ORDER BY post_modified DESC", $user_id);
	$query = apply_filters('get_users_drafts', $query);
	return $trmdb->get_results( $query );
}
 ...
 */
function trm_delete_user( $id, $reassign = 'novalue' ) {
	global $trmdb;
	...
	if ( 'novalue' === $reassign || null === $reassign ) {
		...
		$trmdb->update( $trmdb->posts, array('post_author' => $reassign), array('post_author' => $id) );
		$trmdb->update( $trmdb->links, array('link_owner' => $reassign), array('link_owner' => $id) );
	}
	...
	if ( !is_multisite() ) {
		...
		$trmdb->query("DELETE FROM $trmdb->usermeta WHERE user_id = $id AND meta_key = '{$level_key}'");
	}
