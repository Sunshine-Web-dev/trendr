...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && !isset($_POST['html-upload']) )
		trm_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );

	return $file;
}
add_filter( 'trm_handle_upload_prefilter', 'check_upload_size' );
 ...
 * @return void
 */
function trmmu_delete_blog( $blog_id, $drop = false ) {
	global $trmdb;
	...
	if ( $drop ) {
		...
		$drop_tables = $trmdb->get_results( "SHOW TABLES LIKE '{$blog_prefix}%'", ARRAY_A );
		$drop_tables = apply_filters( 'trmmu_drop_tables', $drop_tables );

		reset( $drop_tables );
		foreach ( (array) $drop_tables as $drop_table) {
			$trmdb->query( "DROP TABLE IF EXISTS ". current( $drop_table ) ."" );
		}
		$trmdb->query( $trmdb->prepare( "DELETE FROM $trmdb->blogs WHERE blog_id = %d", $blog_id ) );
		$dir = apply_filters( 'trmmu_delete_blog_upload_dir', TRM_CONTENT_DIR . "/blogs.dir/{$blog_id}/files/", $blog_id );
	...
	$trmdb->query( "DELETE FROM {$trmdb->usermeta} WHERE meta_key = '{$blog_prefix}autosave_draft_ids'" );
...
// @todo Merge with trm_delete_user() ?
function trmmu_delete_user( $id ) {
	global $trmdb;

	$id = (int) $id;

	do_action( 'trmmu_delete_user', $id );
	...
	if ( ! empty( $blogs ) ) {
		foreach ( $blogs as $blog ) {
			...
			$post_ids = $trmdb->get_col( $trmdb->prepare( "SELECT ID FROM $trmdb->posts WHERE post_author = %d", $id ) );
			foreach ( (array) $post_ids as $post_id ) {
				trm_delete_post( $post_id );
			}

			// Clean links
			$link_ids = $trmdb->get_col( $trmdb->prepare( "SELECT link_id FROM $trmdb->links WHERE link_owner = %d", $id ) );
	...
	$trmdb->query( $trmdb->prepare( "DELETE FROM $trmdb->users WHERE ID = %d", $id ) );
	$trmdb->query( $trmdb->prepare( "DELETE FROM $trmdb->usermeta WHERE user_id = %d", $id ) );
...
function trmmu_get_blog_allowedthemes( $blog_id = 0 ) {
	$themes = get_themes();
...
###SITEURL### "), $new_admin_email );
	...
	trm_mail( $value, sprintf( __( '[%s] New Admin Email Address' ), get_option( 'blogname' ) ), $content );
}
...
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		if ( !is_email( $_POST['email'] ) ) {
			$errors->add( 'user_email', __( " The e-mail address isn't correct." ), array( 'form-field' => 'email' ) );
			return;
		}

		if ( $trmdb->get_var( $trmdb->prepare( "SELECT user_email FROM {$trmdb->users} WHERE user_email=%s", $_POST['email'] ) ) ) {
			$errors->add( 'user_email', __( " The e-mail address is already used." ), array( 'form-field' => 'email' ) );
...
###SITEURL###" ), $new_user_email );
		...
		trm_mail( $_POST['email'], sprintf( __( '[%s] New Email Address' ), get_option( 'blogname' ) ), $content );
...
add_action( 'trmmueditblogaction', 'upload_space_setting' );
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	$trmdb->update( $trmdb->users, array( $pref => $value ), array( 'ID' => $id ) );
...
function _access_denied_splash() {
	...
	if ( trm_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) )
		return;

	$blog_name = get_bloginfo( 'name' );

	if ( empty( $blogs ) )
		trm_die( sprintf( __( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ) );
...
/* Warn the admin if SECRET SALT information is missing from trm-setup.php */
function secret_salt_warning() {
	...
	foreach( $secret_keys as $key ) {
		if ( ! defined( $key ) )
			$out .= "define( '$key', '" . esc_html( trm_generate_password( 64, true, true ) ) . "' );<br />";
	}
	if ( $out != '' ) {
		$msg  = __( 'Warning! Trnder encrypts user cookies, but you must add the following lines to <strong>trm-setup.php</strong> for it to be more secure.' );
...
function site_admin_notice() {
	global $trm_db_version;
	if ( !is_super_admin() )
		return false;
	if ( get_site_option( 'trmmu_upgrade_site' ) != $trm_db_version )
...
add_filter( 'trm_insert_post_data', 'avoid_blog_page_permalink_collision', 10, 2 );
	...
	<?php if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ) ) ) : ?>
		<tr>
			<th scope="row" colspan="2" class="th-full">
				<a href="<?php echo apply_filters( 'trm_signup_location', network_home_url( 'trm-signup.php' ) ); ?>"><?php _e( 'Create a New Site' ); ?></a>
			</th>
		</tr>
	<?php endif; ?>
	</table>
...
function ms_deprecated_blogs_file() {
	if ( ! is_super_admin() )
		return;
	if ( ! file_exists( TRM_CONTENT_DIR . '/blogs.php' ) )
		return;
	echo '<div class="update-nag">' . sprintf( __( 'The <code>%1$s</code> file is deprecated. Please remove it and update your server rewrite rules to use <code>%2$s</code> instead.' ), 'trm-src/blogs.php', 'Source-zACHAvU6As28quwr-trendr/ms-files.php' ) . '</div>';
}
...
//<![CDATA[
var tb_pathToImage = "../../Source-zACHAvU6As28quwr-trendr/js/thickbox/loadingAnimation.gif";
var tb_closeImage = "../../Source-zACHAvU6As28quwr-trendr/js/thickbox/tb-close.png";
//]]>
</script>
<?php
}
