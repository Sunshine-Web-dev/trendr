...
function trm_image_editor($post_id, $msg = false) {
	$nonce = trm_create_nonce("image_editor-$post_id");
	...
	$backup_sizes = get_post_meta( $post_id, '_trm_attachment_backup_sizes', true );
...
<?php } ?>
	...
	<?php if ( $thumb && $sub_sizes ) {
		$thumb_img = trm_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
	?>
...
function load_image_to_edit($post_id, $mime_type, $size = 'full') {
	...
	if ( $filepath && file_exists($filepath) ) {
		...
		$filepath = apply_filters('load_image_to_edit_attachmenturl', trm_get_attachment_url($post_id) , $post_id, $size);
	}
...
function trm_stream_image($image, $mime_type, $post_id) {
...
function trm_save_image_file($filename, $image, $mime_type, $post_id) {
	$image = apply_filters('image_save_pre', $image, $post_id);
	$saved = apply_filters('trm_save_image_file', null, $filename, $image, $mime_type, $post_id);
	if ( null !== $saved )
		return $saved;
...
function _flip_image_resource($img, $horz, $vert) {
	$w = imagesx($img);
	$h = imagesy($img);
	$dst = trm_imagecreatetruecolor($w, $h);
	if ( is_resource($dst) ) {
		$sx = $vert ? ($w - 1) : 0;
		$sy = $horz ? ($h - 1) : 0;
		$sw = $vert ? -$w : $w;
...
function stream_preview_image($post_id) {
	...
	$preview = trm_imagecreatetruecolor($w2, $h2);
	imagecopyresampled( $preview, $img, 0, 0, 0, 0, $w2, $h2, $w, $h );
	trm_stream_image($preview, $post->post_mime_type, $post_id);
...
function trm_restore_image($post_id) {
	$meta = trm_get_attachment_metadata($post_id);
	...
	if ( isset($backup_sizes['full-orig']) && is_array($backup_sizes['full-orig']) ) {
		...
		if ( $parts['basename'] != $data['file'] ) {
			if ( defined('IMAGE_EDIT_OVERWRITE') && IMAGE_EDIT_OVERWRITE ) {
				...
				if ( preg_match('/-e[0-9]{13}\./', $parts['basename']) ) {
					$delpath = apply_filters('trm_delete_file', $file);
					@unlink($delpath);
				}
			} else {
		...
		$meta['file'] = _trm_relative_upload_path( $restored_file );
		$meta['width'] = $data['width'];
		$meta['height'] = $data['height'];
		list ( $uwidth, $uheight ) = trm_constrain_dimensions($meta['width'], $meta['height'], 128, 96);
			...
			if ( isset($meta['sizes'][$default_size]) && $meta['sizes'][$default_size]['file'] != $data['file'] ) {
				if ( defined('IMAGE_EDIT_OVERWRITE') && IMAGE_EDIT_OVERWRITE ) {
					...
					if ( preg_match('/-e[0-9]{13}-/', $meta['sizes'][$default_size]['file']) ) {
						$delpath = apply_filters( 'trm_delete_file', path_join($parts['dirname'], $meta['sizes'][$default_size]['file']) );
						@unlink($delpath);
					}
				} else {
	...
	if ( !trm_update_attachment_metadata($post_id, $meta) || !update_post_meta( $post_id, '_trm_attachment_backup_sizes', $backup_sizes) ) {
...
function trm_save_image($post_id) {
	$return = new stdClass;
	...
	if ( $scale && $fwidth > 0 && $fheight > 0 ) {
		...
		if ( -0.1 < $diff && $diff < 0.1 ) {
			...
			$dst = trm_imagecreatetruecolor($fwidth, $fheight);
	...
	if ( !trm_save_image_file($new_path, $img, $post->post_mime_type, $post_id) ) {
	...
	if ( 'nothumb' == $target || 'all' == $target || 'full' == $target || $scaled ) {
		...
		$meta['file'] = _trm_relative_upload_path($new_path);
	...
	if ( $success ) {
		trm_update_attachment_metadata($post_id, $meta);
		update_post_meta( $post_id, '_trm_attachment_backup_sizes', $backup_sizes);
	...
	if ( $delete ) {
		$delpath = apply_filters('trm_delete_file', $new_path);
		@unlink($delpath);
	}
