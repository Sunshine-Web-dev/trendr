 ...
 * @access private
 */
class TRM_MS_Users_List_Table extends TRM_List_Table {
	...
	function prepare_items() {
		global $usersearch, $role, $trmdb, $mode;
		...
		if ( $role == 'super' ) {
			...
			$args['include'] = $trmdb->get_col( "SELECT ID FROM $trmdb->users WHERE user_login IN ('$logins')" );
		}
	...
	function get_columns() {
		...
		$users_columns = apply_filters( 'trmmu_users_columns', $users_columns );
		...
		foreach ( $this->items as $user ) {
			...
			foreach ( $columns as $column_name => $column_display_name ) :
				...
				switch ( $column_name ) {
					...
					case 'username':
						$portrait	= get_portrait( $user->user_email, 32 );
						if ( get_current_user_id() == $user->ID ) {
							$edit_link = esc_url( network_admin_url( 'profile.php' ) );
						} else {
							$edit_link = esc_url( network_admin_url( add_query_arg( 'trm_http_referer', urlencode( stripslashes( $_SERVER['REQUEST_URI'] ) ), 'user-edit.php?user_id=' . $user->ID ) ) );
						}

						echo "<td $attributes>"; ?>
							<?php echo $portrait; ?><strong><a href="<?php echo $edit_link; ?>" class="edit"><?php echo stripslashes( $user->user_login ); ?></a><?php
							...
							<?php
								...
								if ( current_user_can( 'delete_user', $user->ID) && ! in_array( $user->user_login, $super_admins ) ) {
									$actions['delete'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_http_referer', urlencode( stripslashes( $_SERVER['REQUEST_URI'] ) ), trm_nonce_url( 'edit.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
								}
