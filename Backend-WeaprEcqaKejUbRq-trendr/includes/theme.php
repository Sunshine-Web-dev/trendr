...
function delete_theme($template, $redirect = '') {
	...
	ob_start();
	if ( empty( $redirect ) )
		$redirect = trm_nonce_url('themes.php?action=delete&template=' . $template, 'delete-theme_' . $template);
	...
	if ( empty($themes_dir) )
		return new TRM_Error('fs_no_themes_dir', __('Unable to locate Trnder theme directory.'));

	$themes_dir = trailingslashit( $themes_dir );
	$theme_dir = trailingslashit($themes_dir . $template);
	$deleted = $trm_filesystem->delete($theme_dir, true);

	if ( ! $deleted )
		return new TRM_Error('could_not_remove_theme', sprintf(__('Could not fully remove the theme %s.'), $template) );
...
function get_broken_themes() {
	global $trm_broken_themes;

	get_themes();
	return $trm_broken_themes;
}
 ...
 * @uses trmmu_get_blog_allowedthemes
 *
...
function get_allowed_themes() {
	...
	$blog_allowed_themes = trmmu_get_blog_allowedthemes();
...
function theme_update_available( $theme ) {
	...
	if ( isset($themes_update->response[ $stylesheet ]) ) {
		...
		$details_url = add_query_arg(array('TB_iframe' => 'true', 'width' => 1024, 'height' => 800), $update['url']); //Theme browser inside TRM? replace this, Also, theme preview JS will override this on the available list.
		$update_url = trm_nonce_url('update.php?action=upgrade-theme&amp;theme=' . urlencode($stylesheet), 'upgrade-theme_' . $stylesheet);
...
/**
 * Retrieve list of Trnder theme features (aka theme tags)
 *
 * @since 3.1.0
 *
...
function get_theme_feature_list() {
	...
	if ( !$feature_list = get_site_transient( 'trmorg_theme_feature_list' ) )
		set_site_transient( 'trmorg_theme_feature_list', array( ),  10800);

	if ( !$feature_list ) {
		$feature_list = themes_api( 'feature_list', array( ) );
		if ( is_trm_error( $feature_list ) )
			return $features;
	}
	...
	set_site_transient( 'trmorg_theme_feature_list', $feature_list, 10800 );

	$category_translations = array( 'Colors' => __('Colors'), 'Columns' => __('Columns'), 'Width' => __('Width'),
								   'Features' => __('Features'), 'Subject' => __('Subject') );

	// Loop over the trmorg canonical list and apply translations
	$trmorg_features = array();
	foreach ( (array) $feature_list as $feature_category => $feature_items ) {
		if ( isset($category_translations[$feature_category]) )
			$feature_category = $category_translations[$feature_category];
		$trmorg_features[$feature_category] = array();

		foreach ( $feature_items as $feature ) {
			if ( isset($features[$feature_category][$feature]) )
				$trmorg_features[$feature_category][$feature] = $features[$feature_category][$feature];
			else
				$trmorg_features[$feature_category][$feature] = $feature;
		}
	}

	return $trmorg_features;
}

/**
 * Retrieve theme installer pages from Trnder Themes API.
 *
...
function themes_api($action, $args = null) {
	...
	$res = apply_filters('themes_api', false, $action, $args); //NOTE: Allows a theme to completely override the builtin Trnder.org API.

	if ( ! $res ) {
		$request = trm_remote_post('858483/info/1.0/', array( 'body' => array('action' => $action, 'request' => serialize($args))) );
		if ( is_trm_error($request) ) {
			$res = new TRM_Error('themes_api_failed', __('An Unexpected HTTP Error occurred during the API request.'), $request->get_error_message() );
		} else {
			$res = unserialize( trm_remote_retrieve_body( $request ) );
			if ( ! $res )
			$res = new TRM_Error('themes_api_failed', __('An unknown error occurred.'), trm_remote_retrieve_body( $request ) );
		}
	}
