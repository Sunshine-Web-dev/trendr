...
case 'add-tag':
	...
	check_admin_referer( 'add-tag', '_key_add-tag' );

	if ( !current_user_can( $tax->cap->edit_terms ) )
		trm_die( __( 'Cheatin&#8217; uh?' ) );

	$ret = trm_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	$location = 'edit-tags.php?taxonomy=' . $taxonomy;
	if ( 'post' != $post_type )
		$location .= '&post_type=' . $post_type;

	if ( $referer = trm_get_original_referer() ) {
		if ( false !== strpos( $referer, 'edit-tags.php' ) )
			$location = $referer;
	}

	if ( $ret && !is_trm_error( $ret ) )
...
case 'delete':
	...
	trm_delete_term( $tag_ID, $taxonomy );
...
case 'editedtag':
	...
	$ret = trm_update_term( $tag_ID, $taxonomy, $_POST );
...
if ( ! empty($_REQUEST['_http_referer']) ) {
	$location = remove_query_arg( array('_http_referer', '_key'), stripslashes($_SERVER['REQUEST_URI']) );
...
trm_enqueue_script('admin-tags');
if ( current_user_can($tax->cap->edit_terms) )
	trm_enqueue_script('inline-edit-tax');
...
if ( 'category' == $taxonomy || 'link_category' == $taxonomy || 'post_tag' == $taxonomy  ) {
	...
	if ( 'category' == $taxonomy )
		$help .= '<p>' . __( '<a href="http://codex.trendr.org/Posts_Categories_Screen" target="_blank">Documentation on Categories</a>' ) . '</p>';
	elseif ( 'link_category' == $taxonomy )
		$help .= '<p>' . __( '<a href="http://codex.trendr.org/Links_Link_Categories_Screen" target="_blank">Documentation on Link Categories</a>' ) . '</p>';
	else
		$help .= '<p>' . __( '<a href="http://codex.trendr.org/Posts_Post_Tags_Screen" target="_blank">Documentation on Post Tags</a>' ) . '</p>';
...
<?php $trm_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>
...
if ( !is_null( $tax->labels->popular_items ) ) {
	if ( current_user_can( $tax->cap->edit_terms ) )
		$tag_cloud = trm_tag_cloud( array( 'taxonomy' => $taxonomy, 'echo' => false, 'link' => 'edit' ) );
	else
		$tag_cloud = trm_tag_cloud( array( 'taxonomy' => $taxonomy, 'echo' => false ) );
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />
<?php trm_nonce_field('add-tag', '_key_add-tag'); ?>
...
<div class="form-field">
	...
	<?php trm_dropdown_categories(array('hide_empty' => 0, 'hide_if_empty' => false, 'taxonomy' => $taxonomy, 'name' => 'parent', 'orderby' => 'name', 'hierarchical' => true, 'show_option_none' => __('None'))); ?>
...
<?php $trm_list_table->inline_edit(); ?>
