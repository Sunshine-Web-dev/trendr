 ...
 * plugin installation. This file is not included by Trnder and it is
 ...
 *
 * <code>
 * check_column('trm_links', 'link_description', 'mediumtext');
 * if (check_column($trmdb->comments, 'comment_author', 'tinytext'))
 *     echo "ok\n";
 *
 * $error_count = 0;
 * $tablename = $trmdb->links;
 * // check the column
 * if (!check_column($trmdb->links, 'link_description', 'varchar(255)')) {
 *     $ddl = "ALTER TABLE $trmdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *     $q = $trmdb->query($ddl);
 * }
 *
 * if (check_column($trmdb->links, 'link_description', 'varchar(255)')) {
...
function maybe_create_table($table_name, $create_ddl) {
	global $trmdb;
	foreach ($trmdb->get_col("SHOW TABLES",0) as $table ) {
		if ($table == $table_name) {
			return true;
		}
	}
	//didn't find it try to create it.
	$trmdb->query($create_ddl);
...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
	global $trmdb, $debug;
	$diffs = 0;
	$results = $trmdb->get_results("DESC $table_name");
