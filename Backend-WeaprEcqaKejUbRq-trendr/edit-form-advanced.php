...
trm_enqueue_script('post');

if ( post_type_supports($post_type, 'editor') || post_type_supports($post_type, 'thumbnail') ) {
	add_thickbox();
	trm_enqueue_script('media-upload');
}
...
$messages['post'] = array(
	...
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __('Post restored to revision from %s'), trm_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page'] = array(
	 ...
	 3 => __('Custom field deleted.'),
	 4 => __('Page updated.'),
	 5 => isset($_GET['revision']) ? sprintf( __('Page restored to revision from %s'), trm_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' == $post->post_status ) {
	...
	$autosave = trm_get_post_autosave( $post_ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _trm_post_revision_fields() as $autosave_field => $_autosave_field ) {
...
if ( post_type_supports($post_type, 'revisions') && 0 < $post_ID && trm_get_post_revisions( $post_ID ) )
...
if ( 'post' == $post_type ) {
	...
	'<p>' . __('<strong>Send Trackbacks</strong> - Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other Trnder sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.') . '</p>' .
	...
	'<p>' . __('<a href="http://codex.trendr.org/Posts_Add_New_Screen" target="_blank">Documentation on Writing and Editing Posts</a>') . '</p>' .
	...
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Pages_Add_New_Screen" target="_blank">Documentation on Adding New Pages</a>') . '</p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Pages_Screen#Editing_Individual_Pages" target="_blank">Documentation on Editing Pages</a>') . '</p>' .
	'<p>' . __('<a href="http://trendr.org/support/" target="_blank">Support Forums</a>') . '</p>'
	);
}
...
<?php trm_nonce_field($nonce_action); ?>
...
<input type="hidden" id="referredby" name="referredby" value="<?php echo esc_url(stripslashes(trm_get_referer())); ?>" />
<?php
if ( 'draft' != $post->post_status )
	trm_original_referer_field(true, 'previous');

echo $form_extra;

trm_nonce_field( 'autosave', 'autosavenonce', false );
...
$shortlink = trm_get_shortlink($post->ID, 'post');
if ( !empty($shortlink) )
...
<?php
trm_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
?>
</div>
<?php } ?>
...
<table id="post-status-info" cellspacing="0"><tbody><tr>
	<td id="trm-word-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></td>
