 ...
 * Private use only!
 */
var PubSub, fullscreen, trmtitlehint;
...
(function($){
	...
	api.save = function() {
		var hidden = $('#hiddenaction'), old = hidden.val(), spinner = $('#trm-fullscreen-save img'),
			message = $('#trm-fullscreen-save span');

		spinner.show();
		api.savecontent();

		hidden.val('trm-fullscreen-save-post');
		...
		$.post( ajaxurl, $('form#post').serialize(), function(r){
			...
			if ( r.last_edited )
				$('#trm-fullscreen-save input').attr( 'title',  r.last_edited );
	...
	api.savecontent = function() {
		...
		$('#' + s.title_id).val( $('#trm-fullscreen-title').val() );

		if ( s.mode === 'tinymce' && (ed = tinyMCE.get('trm_mce_fullscreen')) ) {
			content = ed.save();
		} else {
			content = $('#trm_mce_fullscreen').val();
		}

		$('#' + s.editor_id).val( content );
		$(document).triggerHandler('trmcountwords', [ content ]);
	}
	...
	api.dfw_width = function(n) {
		var el = $('#trm-fullscreen-wrap'), w = el.width();

		if ( !n ) { // reset to theme width
			el.width( $('#trm-fullscreen-central-toolbar').width() );
	...
	ps.subscribe( 'showToolbar', function() {
		...
		$('#trm-fullscreen-body').addClass('trm-fullscreen-focus');
	...
	ps.subscribe( 'hideToolbar', function() {
		...
		$('#trm-fullscreen-body').removeClass('trm-fullscreen-focus');
	});
	...
	ps.subscribe( 'show', function() { // This event occurs before the overlay blocks the UI.
		var title = $('#trm-fullscreen-title').val( $('#' + s.title_id).val() );

		set_title_hint( title );
		$('#trm-fullscreen-save input').attr( 'title',  $('#last-edit').text() );

		s.textarea_obj.value = edCanvas.value;

		if ( s.has_tinymce && s.mode === 'tinymce' )
			tinyMCE.execCommand('trmFullScreenInit');
	...
	ps.subscribe( 'shown', function() { // This event occurs after the DFW overlay is shown
		...
		if ( s.has_tinymce && ! s.is_mce_on ) {
			...
			htmled.value = switchEditors.trmautop( old_val );
	...
	ps.subscribe( 'hide', function() { // This event occurs before the overlay blocks DFW.
		...
		if ( s.has_tinymce && s.mode === 'tinymce' )
			tinyMCE.execCommand('trmFullScreenSave');
	...
	ps.subscribe( 'hidden', function() { // This event occurs after DFW is removed.
		s.visible = false;
		$('#trm_mce_fullscreen').removeAttr('style');

		if ( s.has_tinymce && s.is_mce_on )
			tinyMCE.execCommand('trmFullScreenClose');
	...
	ps.subscribe( 'switchMode', function( from, to ) {
		...
		ed = tinyMCE.get('trm_mce_fullscreen');

		if ( from === 'html' && to === 'tinymce' ) {
			s.textarea_obj.value = switchEditors.trmautop( s.textarea_obj.value );
	...
	api.refresh_buttons = function( fade ) {
		...
		if ( s.mode === 'html' ) {
			$('#trm-fullscreen-mode-bar').removeClass('trm-tmce-mode').addClass('trm-html-mode');

			if ( fade )
				$('#trm-fullscreen-button-bar').fadeOut( 150, function(){
					$(this).addClass('trm-html-mode').fadeIn( 150 );
				});
			else
				$('#trm-fullscreen-button-bar').addClass('trm-html-mode');

		} else if ( s.mode === 'tinymce' ) {
			$('#trm-fullscreen-mode-bar').removeClass('trm-html-mode').addClass('trm-tmce-mode');

			if ( fade )
				$('#trm-fullscreen-button-bar').fadeOut( 150, function(){
					$(this).removeClass('trm-html-mode').fadeIn( 150 );
				});
			else
				$('#trm-fullscreen-button-bar').removeClass('trm-html-mode');
		}
	}
	...
	api.ui = {
		init: function() {
			var topbar = $('#fullscreen-topbar'), txtarea = $('#trm_mce_fullscreen'), last = 0;
			s.toolbars = topbar.add( $('#trm-fullscreen-status') );
			s.element = $('#fullscreen-fader');
			s.textarea_obj = txtarea[0];
			s.has_tinymce = typeof(tinyMCE) != 'undefined';

			if ( !s.has_tinymce )
				$('#trm-fullscreen-mode-bar').hide();

			if ( trmtitlehint )
				trmtitlehint('trm-fullscreen-title');
			...
			$(document).keyup(function(e){
				...
				if ( 27 == c ) { // Esc
					data = {
						...
						cb: fullscreen.off,
						condition: function(){
							if ( $('#TB_window').is(':visible') || $('.trm-dialog').is(':visible') )
								return false;
							return true;
						}
					};

					if ( ! jQuery(document).triggerHandler( 'trm_CloseOnEscape', [data] ) )
						fullscreen.off();
				}
			...
			if ( typeof(trmWordCount) != 'undefined' ) {
				...
				txtarea.keyup( function(e) {
					...
					if ( 13 == k || 8 == last || 46 == last )
						$(document).triggerHandler('trmcountwords', [ txtarea.val() ]);
