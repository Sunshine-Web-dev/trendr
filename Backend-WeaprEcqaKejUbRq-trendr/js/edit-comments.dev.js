...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		...
		if ( action ) {
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_key=' + settings.data._ajax_nonce);
			a.attr('class', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1 vim-z vim-destructive');
			$('.portrait', el).clone().prependTo('#undo-' + id + ' .' + action + '-undo-inside');

			a.click(function(){
				list.trmList.del(this);
	...
	delAfter = function( r, settings ) {
		...
		theList.get(0).trmList.add( theExtraList.children(':eq(0)').remove().clone() );
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see TRM_Comments_List_Table::prepare_items() @ class-trm-comments-list-table.php
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).trmList.add( response.rows );
			}
		});
	};

	theExtraList = $('#the-extra-comment-list').trmList( { alt: '', delColor: 'none', addColor: 'none' } );
	theList = $('#the-comment-list').trmList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('trmListDelEnd', function(e, s){
...
commentReply = {
	...
	show : function(xml) {
		...
		r = trmAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': trmAjax.broken});
			return false;
		}
