jQuery(document).ready(function($) {
		...
		if ( r ) {
			...
			$.post(ajaxurl, data, function(r){
				if ( '1' == r ) {
					...
					$('#ajax-response').empty().append('<div class="error"><p>' + tagslan.noPerm + '</p></div>');
					tr.children().css('backgroundColor', '');
				} else {
					$('#ajax-response').empty().append('<div class="error"><p>' + tagslan.broken + '</p></div>');
					tr.children().css('backgroundColor', '');
				}
			});
	...
	$('#submit').click(function(){
		...
		$.post(ajaxurl, $('#addtag').serialize(), function(r){
		   $('#ajax-response').empty();
			var res = trmAjax.parseAjaxResponse(r, 'ajax-response');
			if ( ! res )
				return;
