...
function send_to_editor(h) {
	...
	if ( typeof tinyMCE != 'undefined' && ( ed = tinyMCE.activeEditor ) && !ed.isHidden() ) {
		...
		if ( h.indexOf('[caption') === 0 ) {
			if ( ed.plugins.trmeditimage )
				h = ed.plugins.trmeditimage._do_shcode(h);
		} else if ( h.indexOf('[gallery') === 0 ) {
			if ( ed.plugins.trmgallery )
				h = ed.plugins.trmgallery._do_gallery(h);
		} else if ( h.indexOf('[embed') === 0 ) {
			if ( ed.plugins.trendr )
				h = ed.plugins.trendr._setEmbed(h);
		}
