...
trmWidgets = {
	...
	init : function() {
		...
		sidebars.not('#trm_inactive_widgets').each(function(){
		...
		$('input.widget-control-save').live('click', function(){
			trmWidgets.save( $(this).closest('div.widget'), 0, 1, 0 );
			return false;
		});

		$('a.widget-control-remove').live('click', function(){
			trmWidgets.save( $(this).closest('div.widget'), 1, 1, 0 );
			return false;
		});

		$('a.widget-control-close').live('click', function(){
			trmWidgets.close( $(this).closest('div.widget') );
			return false;
		});

		sidebars.children('.widget').each(function() {
			trmWidgets.appendTitle(this);
		...
		sidebars.sortable({
			...
			stop: function(e,ui) {
				...
				if ( ui.item.hasClass('deleting') ) {
					trmWidgets.save( ui.item, 1, 0, 1 ); // delete widget
					ui.item.remove();
					return;
				}
				...
				if ( add ) {
					...
					trmWidgets.save( ui.item, 0, 0, 1 );
					ui.item.find('input.add_new').val('');
					ui.item.find('a.widget-action').click();
					return;
				}
				trmWidgets.saveOrder(sb);
			},
	...
	resize : function() {
		$('div.widgets-sortables').not('#trm_inactive_widgets').each(function(){
			var h = 50, H = $(this).children('.widget').length;
			h = h + parseInt(H * 48, 10);
			$(this).css( 'minHeight', h + 'px' );
		});
...
$(document).ready(function($){ trmWidgets.init(); });
