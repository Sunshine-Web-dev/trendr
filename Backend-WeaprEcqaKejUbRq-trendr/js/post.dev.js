var tagBox, commentsBox, editPermalink, makeSlugeditClickable, TRMSetThumbnailHTML, TRMSetThumbnailID, TRMRemoveThumbnail, trmtitlehint;
	...
	get : function(total, num) {
		...
		$.post(ajaxurl, data,
			function(r) {
				...
				$('#the-comment-list').append('<tr><td colspan="2">'+trmAjax.broken+'</td></tr>');
			}
		);
...
TRMSetThumbnailHTML = function(html){
	$('.inside', '#postimagediv').html(html);
};

TRMSetThumbnailID = function(id){
...
TRMRemoveThumbnail = function(nonce){
	$.post(ajaxurl, {
...
jQuery(document).ready( function($) {
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see trm-lists.dev.js
		...
		$('#' + taxonomy + '-add-toggle').click( function() {
			$('#' + taxonomy + '-adder').toggleClass( 'trm-hidden-children' );
	...
	if ( $('#postcustom').length ) {
		$('#the-list').trmList( { addAfter: function( xml, s ) {
	...
	if ( typeof(trmWordCount) != 'undefined' ) {
		$(document).triggerHandler('trmcountwords', [ co.val() ]);
