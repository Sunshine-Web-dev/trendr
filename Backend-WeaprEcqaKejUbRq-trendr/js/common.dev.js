...
adminMenu = {
	init : function() {
		...
		$('.trm-menu-toggle', menu).each( function() {
			var t = $(this), sub = t.siblings('.trm-submenu');
			if ( sub.length )
	...
	toggle : function(el) {
		el.slideToggle(150, function() {
			var id = el.css('display','').parent().toggleClass( 'trm-menu-open' ).attr('id');
			if ( id ) {
				$('li.trm-has-submenu', '#adminmenu').each(function(i, e) {
					if ( id == e.id ) {
						var v = $(e).hasClass('trm-menu-open') ? 'o' : 'c';
	...
	fold : function(off) {
		if (off) {
			...
			$('#adminmenu li.trm-has-submenu').unbind();
		} else {
			$('body').addClass('folded');
			$('#adminmenu li.trm-has-submenu').hoverIntent({
				over: function(e){
					var m, b, h, o, f;
					m = $(this).find('.trm-submenu');
					b = $(this).offset().top + m.height() + 1; // Bottom offset of the menu
					h = $('#trmwrap').height(); // Height of the entire page
					o = 60 + b - h;
				...
				out: function(){
					$(this).find('.trm-submenu').removeClass('sub-open');
				},
				timeout: 220,
				sensitivity: 8,
				interval: 100
...
$(document).ready( function() {
	...
	// tab in textareas
	$('#newcontent').bind('keydown.trmevent_InsertTab', function(e) {
		if ( e.keyCode != 9 )
			return true;
	...
	$('#newcontent').bind('blur.trmevent_InsertTab', function(e) {
...
// internal use
$(document).bind( 'trm_CloseOnEscape', function( e, data ) {
