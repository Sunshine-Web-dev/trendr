...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction && !empty( $_REQUEST['pagegen_timestamp'] ) ) {
		$comment_status = $trmdb->escape( $_REQUEST['comment_status'] );
		$delete_time = $trmdb->escape( $_REQUEST['pagegen_timestamp'] );
		$comment_ids = $trmdb->get_col( "SELECT comment_ID FROM $trmdb->comments WHERE comment_approved = '$comment_status' AND '$delete_time' > comment_date_gmt" );
		$doaction = 'delete';
		...
		trm_redirect( trm_get_referer() );
		exit;
	}

	$approved = $unapproved = $spammed = $unspammed = $trashed = $untrashed = $deleted = 0;

	$redirect_to = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'spammed', 'unspammed', 'approved', 'unapproved', 'ids' ), trm_get_referer() );
	...
	foreach ( $comment_ids as $comment_id ) { // Check the permissions on each
		...
		switch ( $doaction ) {
			case 'approve' :
				trm_set_comment_status( $comment_id, 'approve' );
				$approved++;
				break;
			case 'unapprove' :
				trm_set_comment_status( $comment_id, 'hold' );
				$unapproved++;
				break;
			case 'spam' :
				trm_spam_comment( $comment_id );
				$spammed++;
				break;
			case 'unspam' :
				trm_unspam_comment( $comment_id );
				$unspammed++;
				break;
			case 'trash' :
				trm_trash_comment( $comment_id );
				$trashed++;
				break;
			case 'untrash' :
				trm_untrash_comment( $comment_id );
				$untrashed++;
				break;
			case 'delete' :
...
if ( $post_id )
	$title = sprintf(__('Comments on &#8220;%s&#8221;'), trm_html_excerpt(_draft_or_post_title($post_id), 50));
else
	$title = __('Comments');
...
add_contextual_help( $current_screen, '<p>' . __( 'You can manage comments made on your site similar to the way you manage Posts and other content. This screen is customizable in the same ways as other management screens, and you can act on comments using the on-hover action links or the Bulk Actions.' ) . '</p>' .
	...
	'<p>' . __( '<a href="http://codex.trendr.org/Administration_Screens#Comments" target="_blank">Documentation on Comments</a>' ) . '</p>' .
	'<p>' . __( '<a href="http://codex.trendr.org/Comment_Spam" target="_blank">Documentation on Comment Spam</a>' ) . '</p>' .
	'<p>' . __( '<a href="http://codex.trendr.org/Keyboard_Shortcuts" target="_blank">Documentation on Keyboard Shortcuts</a>' ) . '</p>' .
...
if ( $post_id )
	echo sprintf(__('Comments on &#8220;%s&#8221;'),
		sprintf('<a href="%s">%s</a>',
			...
			trm_html_excerpt(_draft_or_post_title($post_id), 50)
		)
	);
else
	echo __('Comments');
...
if ( isset($_REQUEST['s']) && $_REQUEST['s'] )
	printf( '<span class="subtitle">' . sprintf( __( 'Search results for &#8220;%s&#8221;' ), trm_html_excerpt( esc_html( stripslashes( $_REQUEST['s'] ) ), 50 ) ) . '</span>' ); ?>
</h2>
...
if ( isset($_REQUEST['approved']) || isset($_REQUEST['deleted']) || isset($_REQUEST['trashed']) || isset($_REQUEST['untrashed']) || isset($_REQUEST['spammed']) || isset($_REQUEST['unspammed']) || isset($_REQUEST['same']) ) {
	...
	if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
		...
		if ( $spammed > 0 ) {
			...
			$messages[] = sprintf( _n( '%s comment marked as spam.', '%s comments marked as spam.', $spammed ), $spammed ) . ' <a href="' . esc_url( trm_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", "bulk-comments" ) ) . '">' . __('Undo') . '</a><br />';
		}
		...
		if ( $trashed > 0 ) {
			$ids = isset($_REQUEST['ids']) ? $_REQUEST['ids'] : 0;
			$messages[] = sprintf( _n( '%s comment moved to the Trash.', '%s comments moved to the Trash.', $trashed ), $trashed ) . ' <a href="' . esc_url( trm_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", "bulk-comments" ) ) . '">' . __('Undo') . '</a><br />';
		}
...
<?php $trm_list_table->search_box( __( 'Search Comments' ), 'comment' ); ?>
...
<input type="hidden" name="_total" value="<?php echo esc_attr( $trm_list_table->get_pagination_arg('total_items') ); ?>" />
<input type="hidden" name="_per_page" value="<?php echo esc_attr( $trm_list_table->get_pagination_arg('per_page') ); ?>" />
<input type="hidden" name="_page" value="<?php echo esc_attr( $trm_list_table->get_pagination_arg('page') ); ?>" />
...
<?php
trm_comment_reply('-1', true, 'detail');
