...
add_contextual_help($current_screen,
	...
	'<p>' . __('The Trnder URL and the Site URL can be the same (example.com) or different; for example, having the Trnder core files (example.com/trendr) in a subdirectory instead of the root directory.') . '</p>' .
	...
	'<p>' . __('<a href="http://codex.trendr.org/Settings_General_Screen" target="_blank">Documentation on General Settings</a>') . '</p>' .
...
<?php if ( !is_multisite() ) { ?>
<tr valign="top">
<th scope="row"><label for="siteurl"><?php _e('Trnder address (URL)') ?></label></th>
<td><input name="siteurl" type="text" id="siteurl" value="<?php form_option('siteurl'); ?>"<?php disabled( defined( 'TRM_SITEURL' ) ); ?> class="regular-text code<?php if ( defined( 'TRM_SITEURL' ) ) echo ' disabled' ?>" /></td>
</tr>
<tr valign="top">
<th scope="row"><label for="home"><?php _e('Site address (URL)') ?></label></th>
<td><input name="home" type="text" id="home" value="<?php form_option('home'); ?>"<?php disabled( defined( 'TRM_HOME' ) ); ?> class="regular-text code<?php if ( defined( 'TRM_HOME' ) ) echo ' disabled' ?>" />
<span class="description"><?php _e('Enter the address here if you want your site homepage <a href="http://codex.trendr.org/Giving_Trnder_Its_Own_Directory">to be different from the directory</a> you installed Trnder.'); ?></span></td>
</tr>
<tr valign="top">
...
<td>
<select name="default_role" id="default_role"><?php trm_dropdown_roles( get_option('default_role') ); ?></select>
</td>
</tr>
<?php } else { ?>
<tr valign="top">
...
<?php echo trm_timezone_choice($tzstring); ?>
</select>
...
<?php
	...
	checked( $custom );
	echo '/> ' . __('Custom:') . ' </label><input type="text" name="date_format_custom" value="' . esc_attr( get_option('date_format') ) . '" class="small-text" /> <span class="example"> ' . date_i18n( get_option('date_format') ) . "</span> <img class='ajax-loading' src='" . esc_url( admin_url( 'images/trmspin_light.gif' ) ) . "' />\n";

	echo "\t<p>" . __('<a href="http://codex.trendr.org/Formatting_Date_and_Time">Documentation on date and time formatting</a>.') . "</p>\n";
?>
	</fieldset>
</td>
</tr>
...
<?php
	...
	checked( $custom );
	echo '/> ' . __('Custom:') . ' </label><input type="text" name="time_format_custom" value="' . esc_attr( get_option('time_format') ) . '" class="small-text" /> <span class="example"> ' . date_i18n( get_option('time_format') ) . "</span> <img class='ajax-loading' src='" . esc_url( admin_url( 'images/trmspin_light.gif' ) ) . "' />\n";
	;
?>
	</fieldset>
</td>
...
for ($day_index = 0; $day_index <= 6; $day_index++) :
	...
	echo "\n\t<option value='" . esc_attr($day_index) . "' $selected>" . $trm_locale->get_weekday($day_index) . '</option>';
endfor;
?>
</select></td>
</tr>
...
?>
	<tr valign="top">
		...
		<td>
			<select name="TRMLANG" id="TRMLANG">
				<?php mu_dropdown_languages( $languages, get_option('TRMLANG') ); ?>
			</select>
		</td>
	</tr>
<?php
