...
function press_it() {
	...
	$post_ID = trm_insert_post($quick, true);
...
if ( !empty($_REQUEST['ajax']) ) {
	...
	case 'photo_images':
		...
		$url = trm_kses(urldecode($url), null);
...
<?php
	add_thickbox();
	trm_enqueue_style( 'press-this' );
	trm_enqueue_style( 'press-this-ie');
	...
	function show(tab_name) {
		...
		switch(tab_name) {
			case 'video' :
				jQuery('#extra-fields').load('<?php echo esc_url($_SERVER['PHP_SELF']); ?>', { ajax: 'video', s: '<?php echo esc_attr($selection); ?>'}, function() {
					...
					if ( preg_match("/youtube\.com\/watch/i", $url) ) {
						...
						$video_id = esc_attr($video_id);
						$content = '<object width="400" height="225"><param name="allowfullscreen" value="true" /><param name="allowscriptaccess" value="always" /><param name="movie" value="http://www.vimeo.com/moogaloop.swf?clip_id=' . $video_id . '&amp;server=www.vimeo.com&amp;show_title=1&amp;show_byline=1&amp;show_portrait=0&amp;color=amp;fullscreen=1" />	<embed src="http://www.vimeo.com/moogaloop.swf?clip_id=' . $video_id . '&amp;server=www.vimeo.com&amp;show_title=1&amp;show_byline=1&amp;show_portrait=0&amp;color=amp;fullscreen=1" type="application/x-shockwave-flash" allowfullscreen="true" allowscriptaccess="always" width="400" height="225"></embed></object>';
			...
			case 'photo' :
				...
				jQuery('#extra-fields').before('<div id="waiting"><img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" alt="" /> <?php echo esc_js( __( 'Loading...' ) ); ?></div>');
...
<body class="press-this Backend-WeaprEcqaKejUbRq-trendr">
<?php
if ( user_can_richedit() ) {
	trm_tiny_mce( true, array( 'height' => '370' ) );
}
?>
...
<div id="poststuff" class="metabox-holder">
	<div id="side-info-column">
		<div class="sleeve">
			<?php trm_nonce_field('press-this') ?>
			...
			<div id="submitdiv" class="postbox">
				...
				<div class="inside">
					...
					<?php
						...
						<img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" alt="" id="saving" style="display:none;" />
					</p>
				...
				<div id="taxonomy-category" class="categorydiv">
					...
					<div id="category-pop" class="tabs-panel" style="display: none;">
						<ul id="categorychecklist-pop" class="categorychecklist form-no-clear" >
							<?php $popular_ids = trm_popular_terms_checklist( 'category' ); ?>
						</ul>
					</div>

					<div id="category-all" class="tabs-panel">
						<ul id="categorychecklist" class="list:category categorychecklist form-no-clear">
							<?php trm_terms_checklist($post_ID, array( 'taxonomy' => 'category', 'popular_cats' => $popular_ids ) ) ?>
						</ul>
					</div>
					...
					<?php if ( current_user_can($tax->cap->edit_terms) ) : ?>
						<div id="category-adder" class="trm-hidden-children">
							...
							<p id="category-add" class="category-add trm-hidden-child">
								...
								<?php trm_dropdown_categories( array( 'taxonomy' => 'category', 'hide_empty' => 0, 'name' => 'newcategory_parent', 'orderby' => 'name', 'hierarchical' => 1, 'show_option_none' => '&mdash; ' . $tax->labels->parent_item . ' &mdash;', 'tab_index' => 3 ) ); ?>
								...
								<?php trm_nonce_field( 'add-category', '_ajax_nonce-add-category', false ); ?>
...
<img alt="<?php _e('Insert an Image'); ?>" src="<?php echo esc_url( admin_url( 'images/media-button-image.gif?ver=20100531' ) ); ?>"/></a><?php
			...
			<div class="editor-container">
				<textarea name="content" id="content" style="width:100%;" class="theEditor" rows="15"><?php
					if ( $selection )
						echo trm_richedit_pre($selection);
					if ( $url ) {
						echo '<p>';
						if ( $selection )
							_e('via ');
