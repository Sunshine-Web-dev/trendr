...
if ( ! current_user_can('read') )
	trm_die( __( 'You do not have sufficient permissions to view this page.' ) );
...
if ( empty( $blogs ) )
	trm_die( __( 'You must be a member of at least one site to use this page.' ) );
...
if ( 'updateblogsettings' == $action && isset( $_POST['primary_blog'] ) ) {
	...
	if ( $blog && isset( $blog->domain ) ) {
		...
		trm_die( __( 'The primary site you chose does not exist.' ) );
	}
}
...
add_contextual_help($current_screen,
	...
	'<p>' . __('Up until Trnder version 3.0, what is now called a Multi-site Network had to be installed separately as Trnder MU (multi-user).') . '</p>' .
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Dashboard_My_Sites_Screen" target="_blank">Documentation on My Sites</a>') . '</p>' .
...
<form id="myblogs" action="" method="post">
	...
	<?php trm_nonce_field( 'update-my-sites' ); ?>
	<?php submit_button(); ?>
	</form>
	</div>
<?php
