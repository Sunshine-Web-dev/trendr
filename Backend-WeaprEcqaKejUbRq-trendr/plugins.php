...
if ( !current_user_can('activate_plugins') )
	trm_die( __( 'You do not have sufficient permissions to manage plugins for this site.' ) );

$trm_list_table = _get_list_table('TRM_Plugins_List_Table');
$pagenum = $trm_list_table->get_pagenum();

$action = $trm_list_table->current_action();
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can('activate_plugins') )
				trm_die(__('You do not have sufficient permissions to activate plugins for this site.'));
			...
			if ( is_trm_error( $result ) ) {
				if ( 'unexpected_output' == $result->get_error_code() ) {
					...
					trm_redirect(add_query_arg('_error_nonce', trm_create_nonce('plugin-activation-error_' . $plugin), $redirect));
					exit;
				} else {
					trm_die($result);
				}
			...
			if ( isset($_GET['from']) && 'import' == $_GET['from'] ) {
				trm_redirect( self_admin_url("import.php?import=" . str_replace('-importer', '', dirname($plugin))) ); // overrides the ?error=true one above and redirects to the Imports page, stripping the -importer suffix
			} else {
				trm_redirect( self_admin_url("plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s") ); // overrides the ?error=true one above
			}
			exit;
			break;
		...
		case 'network-activate-selected':
			...
			if ( empty($plugins) ) {
				trm_redirect( self_admin_url("plugins.php?plugin_status=$status&paged=$page&s=$s") );
				exit;
			}
			...
			trm_redirect( self_admin_url("plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
		case 'update-selected' :
			...
			echo '</div>';
			require_once(ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/admin-footer.php');
			exit;
			break;
		case 'error_scrape':
			if ( ! current_user_can('activate_plugins') )
		...
		case 'deactivate':
			if ( ! current_user_can('activate_plugins') )
				trm_die(__('You do not have sufficient permissions to deactivate plugins for this site.'));
			...
			if ( headers_sent() )
				...
				trm_redirect( self_admin_url("plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
		case 'deactivate-selected':
			...
			trm_redirect( self_admin_url("plugins.php?deactivate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
		case 'delete-selected':
			if ( ! current_user_can('delete_plugins') )
				trm_die(__('You do not have sufficient permissions to delete plugins for this site.'));
			...
			if ( empty( $plugins ) ) {
				trm_redirect( self_admin_url( "plugins.php?error=true&main=true&plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}

			include(ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/update.php');
			...
			<div class="wrap">
				<?php
					...
					foreach ( (array) $plugins as $plugin ) {
						if ( '.' == dirname($plugin) ) {
							$files_to_delete[] = TRM_PLUGIN_DIR . '/' . $plugin;
							if( $data = get_plugin_data(TRM_PLUGIN_DIR . '/' . $plugin) ) {
							...
							$files = list_files( TRM_PLUGIN_DIR . '/' . dirname($plugin) );
							if ( $files ) {
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array)$files_to_delete as $file )
							echo '<li>' . esc_html(str_replace(TRM_PLUGIN_DIR, '', $file)) . '</li>';
					?>
					</ul>
				</div>
			</div>
			...
			trm_redirect( self_admin_url("plugins.php?deleted=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
...
add_contextual_help($current_screen,
	'<p>' . __('Plugins extend and expand the functionality of Trnder. Once a plugin is installed, you may activate it or deactivate it here.') . '</p>' .
	'<p>' . sprintf(__('You can find additional plugins for your site by using the <a href="%1$s">Plugin Browser/Installer</a> functionality or by browsing the <a href="%2$s" target="_blank">Trnder Plugin Directory</a> directly and installing new plugins manually. To manually install a plugin you generally just need to upload the plugin file into your <code>/trm-src/module</code> directory. Once a plugin has been installed, you can activate it here.'), 'plugin-install.php', 'http://trendr.org/extend/module/') . '</p>' .
	'<p>' . __('Most of the time, plugins play nicely with the core of Trnder and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.') . '</p>' .
	'<p>' . sprintf( __('If something goes wrong with a plugin and you can&#8217;t use Trnder, delete or rename that file in the <code>%s</code> directory and it will be automatically deactivated.'), TRM_PLUGIN_DIR) . '</p>' .
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Managing_Plugins#Plugin_Management" target="_blank">Documentation on Managing Plugins</a>') . '</p>' .
...
<?php if ( isset($_GET['error']) ) :
	...
	<?php
		if ( !isset( $_GET['main'] ) && !isset($_GET['charsout']) && trm_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $plugin) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php echo 'plugins.php?action=error_scrape&amp;plugin=' . esc_attr($plugin) . '&amp;_key=' . esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php
		}
	?>
	</div>
...
<?php $trm_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
