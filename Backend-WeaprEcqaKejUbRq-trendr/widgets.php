...
/** trendr Administration Bootstrap */
require_once( './admin.php' );

/** trendr Administration Widgets API */
require_once(ABSPATH . 'Backend-WeaprEcqaKejUbRq-trendr/includes/widgets.php');

if ( ! current_user_can('edit_theme_options') )
	trm_die( __( 'Cheatin&#8217; uh?' ));

trm_admin_css( 'widgets' );
...
if ( 'on' == $widgets_access )
	...
	trm_enqueue_script('admin-widgets');
...
$help .= '<p>' . __('<a href="Appearance_Widgets_Screen" target="_blank">Documentation on Widgets</a>') . '</p>';
$help .= '<p>' . __('<a href="support/" target="_blank">Support Forums</a>') . '</p>';
add_contextual_help($current_screen, $help);

// register the inactive_widgets area as sidebar
register_sidebar(array(
	'name' => __('Inactive Widgets'),
	'id' => 'trm_inactive_widgets',
	'description' => '',
	'before_widget' => '',
	'after_widget' => '',
	'before_title' => '',
...
if ( empty( $sidebars_widgets ) )
	$sidebars_widgets = trm_get_widget_defaults();

// look for "lost" widgets, this has to run at least on each theme change
function retrieve_widgets() {
	global $trm_registered_widget_updates, $trm_registered_sidebars, $sidebars_widgets, $trm_registered_widgets;
	...
	foreach ( $_sidebars_widgets as $sidebar => $widgets ) {
		...
		foreach ( $widgets as $widget ) {
			if ( isset($trm_registered_widgets[$widget]) )
				$_widgets[] = $widget;
		}
	...
	$lost_widgets = array();
	foreach ( $trm_registered_widgets as $key => $val ) {
	...
	$sidebars_widgets['trm_inactive_widgets'] = array_merge($lost_widgets, (array) $sidebars_widgets['trm_inactive_widgets']);
	trm_set_sidebars_widgets($sidebars_widgets);
}
retrieve_widgets();

if ( count($trm_registered_sidebars) == 1 ) {
	// If only "trm_inactive_widgets" is defined the theme has no sidebars, die.
...
?>
	...
	<h2><?php echo esc_html( $title ); ?></h2>
		...
		<p><?php _e( 'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your theme widget-aware, please <a href="Widgetizing_Themes">follow these instructions</a>.' ); ?></p>
	</div>
...
if ( isset($_POST['savewidget']) || isset($_POST['removewidget']) ) {
	...
	if ( isset($_POST['removewidget']) && $_POST['removewidget'] ) {
		...
		if ( !in_array($widget_id, $sidebar, true) ) {
			trm_redirect( admin_url('widgets.php?error=0') );
			exit;
		}
	...
	trm_set_sidebars_widgets($sidebars_widgets);
	trm_redirect( admin_url('widgets.php?message=0') );
	exit;
}
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
	...
	if ( isset($_GET['addnew']) ) {
		...
		$sidebar = array_shift( $keys = array_keys($trm_registered_sidebars) );

		if ( isset($_GET['base']) && isset($_GET['num']) ) { // multi-widget
			// Copy minimal info from an existing instance of this widget to a new instance
			foreach ( $trm_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					$control_callback = $control['callback'];
					$multi_number = (int) $_GET['num'];
					$control['params'][0]['number'] = -1;
					$widget_id = $control['id'] = $control['id_base'] . '-' . $multi_number;
					$trm_registered_widget_controls[$control['id']] = $control;
					break;
				}
			}
		}
	...
	if ( isset($trm_registered_widget_controls[$widget_id]) && !isset($control) ) {
		$control = $trm_registered_widget_controls[$widget_id];
		$control_callback = $control['callback'];
	} elseif ( !isset($trm_registered_widget_controls[$widget_id]) && isset($trm_registered_widgets[$widget_id]) ) {
		$name = esc_html( strip_tags($trm_registered_widgets[$widget_id]['name']) );
	}
	...
	if ( !isset($sidebar) )
		$sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'trm_inactive_widgets';
...
<?php
	foreach ( $trm_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr($sbname) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'trm_inactive_widgets' == $sbname ) {
			echo '&nbsp;';
		} else {
...
<?php	trm_nonce_field("save-delete-widget-$widget_id"); ?>
	<br class="clear" />
	</div>
	</form>
	</div>
...
<div id="widgets-left">
	...
	<div class="widgets-holder-wrap">
		...
		<span><img src="<?php echo esc_url( admin_url( 'images/trmspin_light.gif' ) ); ?>" class="ajax-feedback" title="" alt="" /></span></h3></div>
		<div class="widget-holder inactive">
		<p class="description"><?php _e('Drag widgets here to remove them from the sidebar but keep their settings.'); ?></p>
		<?php trm_list_widget_controls('trm_inactive_widgets'); ?>
		<br class="clear" />
		</div>
	</div>
</div>
...
<?php
$i = 0;
foreach ( $trm_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( 'trm_inactive_widgets' == $sidebar )
		continue;
	$closed = $i ? ' closed' : ''; ?>
	...
	<span><img src="<?php echo esc_url( admin_url( 'images/trmspin_dark.gif' ) ); ?>" class="ajax-feedback" title="" alt="" /></span></h3></div>
	<?php trm_list_widget_controls( $sidebar ); // Show the control forms for each of the widgets in this sidebar ?>
	</div>
<?php
	$i++;
} ?>
...
<?php trm_nonce_field( 'save-sidebar-widgets', '_key_widgets', false ); ?>
</form>
<br class="clear" />
</div>
