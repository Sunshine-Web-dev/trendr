...
if ( is_multisite() && ! is_network_admin() ) {
	trm_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit();
}

if ( !current_user_can('edit_plugins') )
	trm_die( __('You do not have sufficient permissions to edit plugins for this site.') );

$title = __("Edit Plugins");
$parent_file = 'plugins.php';

trm_reset_vars(array('action', 'redirect', 'profile', 'error', 'warning', 'a', 'file', 'plugin'));

trm_admin_css( 'theme-editor' );

$plugins = get_plugins();

if ( empty($plugins) )
	trm_die( __('There are no plugins installed on this site.') );
...
case 'update':
	...
	if ( is_writeable($real_file) ) {
		...
		if ( is_plugin_active($file) || isset($_POST['phperror']) ) {
			...
			trm_redirect(add_query_arg('_key', trm_create_nonce('edit-plugin-test_' . $file), "plugin-editor.php?file=$file&liveupdate=1&scrollto=$scrollto&networkwide=" . $network_wide));
			exit;
		}
		trm_redirect( self_admin_url("plugin-editor.php?file=$file&a=te&scrollto=$scrollto") );
	} else {
		trm_redirect( self_admin_url("plugin-editor.php?file=$file&scrollto=$scrollto") );
	}
	exit;
...
default:
	...
	if ( ! is_file($real_file) ) {
		trm_die(sprintf('<p>%s</p>', __('No such file exists! Double check the name and try again.')));
	} else {
		...
		if ( preg_match('/\.([^.]+)$/', $real_file, $matches) ) {
			...
			if ( !in_array( $ext, $editable_extensions) )
				trm_die(sprintf('<p>%s</p>', __('Files of this type are not editable.')));
		}
	}
	...
	add_contextual_help($current_screen,
		...
		'<p>' . __('<a href="http://codex.trendr.org/Plugins_Editor_Screen" target="_blank">Documentation on Editing Plugins</a>') . '</p>' .
		'<p>' . __('<a href="http://codex.trendr.org/Writing_a_Plugin" target="_blank">Documentation on Writing Plugins</a>') . '</p>' .
	...
	update_recently_edited(TRM_PLUGIN_DIR . '/' . $file);

	$content = file_get_contents( $real_file );

	if ( '.php' == substr( $real_file, strrpos( $real_file, '.' ) ) ) {
		$functions = trm_doc_link_parse( $content );
...
<?php elseif (isset($_GET['phperror'])) : ?>
 <div id="message" class="updated"><p><?php _e('This plugin has been deactivated because your changes resulted in a <strong>fatal error</strong>.') ?></p>
	<?php
		if ( trm_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $file) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php bloginfo('trmurl'); ?>/Backend-WeaprEcqaKejUbRq-trendr/module.php?action=error_scrape&amp;plugin=<?php echo esc_attr($file); ?>&amp;_key=<?php echo esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php } ?>
</div>
<?php endif; ?>
<div class="wrap">
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php trm_nonce_field('edit-plugin_' . $file) ?>
		...
		<div id="documentation" class="hide-if-no-js"><label for="docs-list"><?php _e('Documentation:') ?></label> <?php echo $docs_select ?> <input type="button" class="button" value="<?php esc_attr_e( 'Lookup' ) ?> " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_locale() ) ?>&amp;version=<?php echo urlencode( $trm_version ) ?>&amp;redirect=true'); }" /></div>
		<?php endif; ?>
...
<?php else : ?>
	<p><em><?php _e('You need to make this file writable before you can save your changes. See <a href="http://codex.trendr.org/Changing_File_Permissions">the Codex</a> for more information.'); ?></em></p>
<?php endif; ?>
</form>
<br class="clear" />
</div>
