...
trm_enqueue_script('list-revisions');

trm_reset_vars(array('revision', 'left', 'right', 'action'));
...
case 'restore' :
	...
	if ( ( ! TRM_POST_REVISIONS || !post_type_supports($post->post_type, 'revisions') ) && !trm_is_post_autosave( $revision ) ) {
	...
	trm_restore_post_revision( $revision->ID );
...
case 'diff' :
	...
	if ( ! TRM_POST_REVISIONS || !post_type_supports($post->post_type, 'revisions') ) { // Revisions disabled
		if (
			// we're not looking at an autosave
			( !trm_is_post_autosave( $left_revision ) && !trm_is_post_autosave( $right_revision ) )
		||
	...
	||
		...
		( !trm_get_post_revision( $left_revision->ID ) && !trm_get_post_revision( $right_revision->ID ) )
	)
		break;
...
default :
	...
	$post_title = '<a href="' . get_edit_post_link() . '">' . get_the_title() . '</a>';
	$revision_title = trm_post_revision_title( $revision, false );
...
<tr id="revision">
	...
	<th scope="col" class="th-full">
		<span class="alignleft"><?php printf( __('Older: %s'), trm_post_revision_title( $left_revision ) ); ?></span>
		<span class="alignright"><?php printf( __('Newer: %s'), trm_post_revision_title( $right_revision ) ); ?></span>
	</th>
</tr>
<?php endif;
...
$identical = true;
foreach ( _trm_post_revision_fields() as $field => $field_title ) :
	if ( 'diff' == $action ) {
		$left_content = apply_filters( "_trm_post_revision_field_$field", $left_revision->$field, $field );
		$right_content = apply_filters( "_trm_post_revision_field_$field", $right_revision->$field, $field );
		if ( !$content = trm_text_diff( $left_content, $right_content ) )
			continue; // There is no difference between left and right
		$identical = false;
	} else {
		add_filter( "_trm_post_revision_field_$field", 'htmlspecialchars' );
		$content = apply_filters( "_trm_post_revision_field_$field", $revision->$field, $field );
	}
	?>
...
if ( ! TRM_POST_REVISIONS || !post_type_supports($post->post_type, 'revisions') )
	$args['type'] = 'autosave';

trm_list_post_revisions( $post, $args );
