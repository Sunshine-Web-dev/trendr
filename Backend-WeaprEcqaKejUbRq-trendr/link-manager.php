...
$trm_list_table = _get_list_table('TRM_Links_List_Table');
...
if ( $doaction && isset( $_REQUEST['linkcheck'] ) ) {
	...
	if ( 'delete' == $doaction ) {
		...
		trm_redirect( add_query_arg('deleted', count( $bulklinks ), admin_url( 'link-manager.php' ) ) );
		exit;
	}
...
add_contextual_help( $current_screen,
	'<p>' . sprintf(__('You can add links here to be displayed on your site, usually using <a href="%s">Widgets</a>. By default, links to several sites in the Trnder community are included as examples.'), 'widgets.php') . '</p>' .
	...
	'<p>' . __('<a href="http://codex.trendr.org/Links_Screen" target="_blank">Documentation on Managing Links</a>') . '</p>' .
...
<?php $trm_list_table->search_box( __( 'Search Links' ), 'link' ); ?>
