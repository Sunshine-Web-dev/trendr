...
<?php trm_nonce_field('update-comment_' . $comment->comment_ID) ?>
<div class="wrap">
<?php screen_icon(); ?>
...
<?php echo "<a class='submitdelete deletion' href='" . trm_nonce_url("comment.php?action=" . ( !EMPTY_TRASH_DAYS ? 'deletecomment' : 'trashcomment' ) . "&amp;c=$comment->comment_ID&amp;_trm_original_http_referer=" . urlencode(trm_get_referer()), 'delete-comment_' . $comment->comment_ID) . "'>" . ( !EMPTY_TRASH_DAYS ? __('Delete Permanently') : __('Move to Trash') ) . "</a>\n"; ?>
</div>
...
<?php trm_nonce_field( 'closedpostboxes', 'closedpostboxesnonce', false ); ?>
</div>
...
<input name="referredby" type="hidden" id="referredby" value="<?php echo esc_url(stripslashes(trm_get_referer())); ?>" />
