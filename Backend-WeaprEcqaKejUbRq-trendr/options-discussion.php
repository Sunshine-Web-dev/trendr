...
add_contextual_help($current_screen,
	...
	'<p>' . __('<a href="http://codex.trendr.org/Settings_Discussion_Screen" target="_blank">Documentation on Discussion Settings</a>') . '</p>' .
...
<p><label for="moderation_keys"><?php _e('When a comment contains any of these words in its content, name, URL, e-mail, or IP, it will be held in the <a href="edit-comments.php?comment_status=moderated">moderation queue</a>. One word or IP per line. It will match inside words, so &#8220;press&#8221; will match &#8220;Trnder&#8221;.') ?></label></p>
<p>
...
<p><label for="blacklist_keys"><?php _e('When a comment contains any of these words in its content, name, URL, e-mail, or IP, it will be marked as spam. One word or IP per line. It will match inside words, so &#8220;press&#8221; will match &#8220;Trnder&#8221;.') ?></label></p>
<p>
...
<p><?php _e('An portrait is an image that follows you from weblog to weblog appearing beside your name when you comment on portrait enabled sites.  Here you can enable the display of portraits for people who comment on your site.'); ?></p>

<?php // the above would be a good place to link to codex documentation on the grportrait functions, for putting it in themes. anything like that? ?>
...
<?php
	...
	foreach ( $yesorno as $key => $value) {
		$selected = (get_option('show_portraits') == $key) ? 'checked="checked"' : '';
		echo "\n\t<label><input type='radio' name='show_portraits' value='" . esc_attr($key) . "' $selected/> $value</label><br />";
	}
?>
</fieldset></td>
</tr>
...
foreach ($ratings as $key => $rating) :
	$selected = (get_option('portrait_rating') == $key) ? 'checked="checked"' : '';
	echo "\n\t<label><input type='radio' name='portrait_rating' value='" . esc_attr($key) . "' $selected/> $rating</label><br />";
endforeach;
?>
...
<td class="defaultportraitpicker"><fieldset><legend class="screen-reader-text"><span><?php _e('Default Avatar') ?></span></legend>

<?php _e('For users without a custom portrait of their own, you can either display a generic logo or a generated one based on their e-mail address.'); ?><br />

<?php
$portrait_defaults = array(
	'mystery' => __('Mystery Man'),
	'blank' => __('Blank'),
	'grportrait_default' => __('Grportrait Logo'),
	'identicon' => __('Identicon (Generated)'),
	'wportrait' => __('Wportrait (Generated)'),
	'monsterid' => __('MonsterID (Generated)'),
	'retro' => __('Retro (Generated)')
);
$portrait_defaults = apply_filters('portrait_defaults', $portrait_defaults);
$default = get_option('portrait_default');
if ( empty($default) )
	$default = 'mystery';
$size = 32;
$portrait_list = '';
foreach ( $portrait_defaults as $default_key => $default_name ) {
	$selected = ($default == $default_key) ? 'checked="checked" ' : '';
	$portrait_list .= "\n\t<label><input type='radio' name='portrait_default' id='portrait_{$default_key}' value='" . esc_attr($default_key)  . "' {$selected}/> ";

	$portrait = get_portrait( $user_email, $size, $default_key );
	$portrait_list .= preg_replace("/src='(.+?)'/", "src='\$1&amp;forcedefault=1'", $portrait);

	$portrait_list .= ' ' . $default_name . '</label>';
	$portrait_list .= '<br />';
}
echo apply_filters('default_portrait_select', $portrait_list);
?>

</fieldset></td>
</tr>
<?php do_settings_fields('discussion', 'portraits'); ?>
</table>
