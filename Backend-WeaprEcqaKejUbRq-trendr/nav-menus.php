<?php
/**
 * Trnder Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) )
	trm_die( __( 'Your theme does not support navigation menus or widgets.' ) );

// Permissions Check
if ( ! current_user_can('edit_theme_options') )
	trm_die( __( 'Cheatin&#8217; uh?' ) );

// Nav Menu CSS
trm_admin_css( 'nav-menu' );
...
// Nav Menu functions
trm_enqueue_script( 'nav-menu' );

// Metaboxes
trm_enqueue_script( 'common' );
trm_enqueue_script( 'trm-lists' );
trm_enqueue_script( 'postbox' );
...
switch ( $action ) {
	case 'add-menu-item':
		...
		elseif ( isset( $_REQUEST['menu-item'] ) )
			trm_save_nav_menu_items( $nav_menu_selected_id, $_REQUEST['menu-item'] );
		break;
	case 'move-down-menu-item' :
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			$menus = isset( $_REQUEST['menu'] ) ? array( (int) $_REQUEST['menu'] ) : trm_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
			if ( ! is_trm_error( $menus ) && ! empty( $menus[0] ) ) {
				$menu_id = (int) $menus[0];
				$ordered_menu_items = trm_get_nav_menu_items( $menu_id );
				$menu_item_data = (array) trm_setup_nav_menu_item( get_post( $menu_item_id ) );
				...
				if (
					...
					$next_item_data = (array) trm_setup_nav_menu_item( get_post( $next_item_id ) );
					...
					if (
						...
						$parent_object = trm_setup_nav_menu_item( get_post( $parent_db_id ) );

						if ( ! is_trm_error( $parent_object ) ) {
					...
					// make menu item a child of its next sibling
						...
						trm_update_post($menu_item_data);
						trm_update_post($next_item_data);
					}
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && trm_delete_post( $menu_item_id, true ) )
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deleted_nav_menu = trm_get_nav_menu_object( $nav_menu_selected_id );
			$delete_nav_menu = trm_delete_nav_menu( $nav_menu_selected_id );

			if ( is_trm_error($delete_nav_menu) ) {
				...
				$_nav_menus = trm_get_nav_menus( array('orderby' => 'name') );
	...
	case 'update':
		...
		if ( 0 == $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = trm_update_nav_menu_object( 0, array('menu-name' => $new_menu_title) );

				if ( is_trm_error( $_nav_menu_selected_id ) ) {
					$messages[] = '<div id="message" class="error"><p>' . $_nav_menu_selected_id->get_error_message() . '</p></div>';
				} else {
					$_menu_object = trm_get_nav_menu_object( $_nav_menu_selected_id );
		...
		// update existing menu
			...
			$_menu_object = trm_get_nav_menu_object( $nav_menu_selected_id );
			...
			if ( ! is_trm_error( $_menu_object ) ) {
				$_nav_menu_selected_id = trm_update_nav_menu_object( $nav_menu_selected_id, array( 'menu-name' => $menu_title ) );
				if ( is_trm_error( $_nav_menu_selected_id ) ) {
			...
			if ( ! is_trm_error( $_menu_object ) ) {
				$unsorted_menu_items = trm_get_nav_menu_items( $nav_menu_selected_id, array('orderby' => 'ID', 'output' => ARRAY_A, 'output_key' => 'ID', 'post_status' => 'draft,publish') );
				$menu_items = array();
				// Index menu items by db ID
				foreach( $unsorted_menu_items as $_item )
					$menu_items[$_item->db_id] = $_item;

				$post_fields = array( 'menu-item-db-id', 'menu-item-object-id', 'menu-item-object', 'menu-item-parent-id', 'menu-item-position', 'menu-item-type', 'menu-item-title', 'menu-item-url', 'menu-item-description', 'menu-item-attrm-title', 'menu-item-target', 'menu-item-classes', 'menu-item-xfn' );
				trm_defer_term_counting(true);
				...
				if ( ! empty( $_POST['menu-item-db-id'] ) ) {
					foreach( (array) $_POST['menu-item-db-id'] as $_key => $k ) {
						...
						$menu_item_db_id = trm_update_nav_menu_item( $nav_menu_selected_id, ( $_POST['menu-item-db-id'][$_key] != $_key ? 0 : $_key ), $args );

						if ( is_trm_error( $menu_item_db_id ) )
				...
				$nav_menu_option['auto_add'] = array_intersect( $nav_menu_option['auto_add'], trm_get_nav_menus( array( 'fields' => 'ids' ) ) );
				update_option( 'nav_menu_options', $nav_menu_option );

				trm_defer_term_counting(false);

				do_action( 'trm_update_nav_menu', $nav_menu_selected_id );
...
// Get all nav menus
$nav_menus = trm_get_nav_menus( array('orderby' => 'name') );
...
if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) {
	$_menu_object = trm_get_nav_menu_object( $nav_menu_selected_id );
	$nav_menu_selected_title = ! is_trm_error( $_menu_object ) ? $_menu_object->name : '';
}

// Generate truncated menu names
foreach( (array) $nav_menus as $key => $_nav_menu ) {
	$_nav_menu->truncated_name = trim( trm_html_excerpt( $_nav_menu->name, 40 ) );
...
$_trm_nav_menu_max_depth = 0;

// Calling trm_get_nav_menu_to_edit generates $_trm_nav_menu_max_depth
if ( is_nav_menu( $nav_menu_selected_id ) )
	$edit_markup = trm_get_nav_menu_to_edit( $nav_menu_selected_id  );

function trm_nav_menu_max_depth() {
	global $_trm_nav_menu_max_depth;
	return "menu-max-depth-$_trm_nav_menu_max_depth";
}

add_action('admin_body_class','trm_nav_menu_max_depth');

trm_nav_menu_setup();
trm_initial_nav_menu_meta_boxes();

if ( ! current_theme_supports( 'menus' ) && ! trm_get_nav_menus() )
...
$help .= '<p>' . __('<a href="http://codex.trendr.org/Appearance_Menus_Screen" target="_blank">Documentation on Menus</a>') . '</p>';
...
<div class="wrap">
	...
	<div id="menu-settings-column" class="metabox-holder<?php if ( !$nav_menu_selected_id ) { echo ' metabox-holder-disabled'; } ?>">
		...
		<form id="nav-menu-meta" action="<?php echo admin_url( 'nav-menus.php' ); ?>" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php trm_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
						...
						<div id="submitpost" class="submitbox">
							<div class="major-publishing-actions">
								...
								<div class="delete-action">
									<a class="submitdelete deletion menu-delete" href="<?php echo esc_url( trm_nonce_url( admin_url('nav-menus.php?action=delete&amp;menu=' . $nav_menu_selected_id), 'delete-nav_menu-' . $nav_menu_selected_id ) ); ?>"><?php _e('Delete Menu'); ?></a>
								</div><!-- END .delete-action -->
								<?php endif; ?>
							</div><!-- END .major-publishing-actions -->
						</div><!-- END #submitpost .submitbox -->
						<?php
						trm_nonce_field( 'closedpostboxes', 'closedpostboxesnonce', false );
						trm_nonce_field( 'meta-box-order', 'meta-box-order-nonce', false );
						trm_nonce_field( 'update-nav_menu', 'update-nav-menu-nonce' );
						?>
