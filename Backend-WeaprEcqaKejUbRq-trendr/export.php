<?php
/**
 * Trnder Export Administration Screen
 *
 * @package Trnder
...
if ( !current_user_can('export') )
	trm_die(__('You do not have sufficient permissions to export the content of this site.'));

/** Load Trnder export API */
...
add_contextual_help( $current_screen,
	...
	'<p>' . __('Once generated, your WXR file can be imported by another Trnder site or by another blogging platform able to access this format.') . '</p>' .
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Tools_Export_Screen" target="_blank">Documentation on Export</a>') . '</p>' .
...
function export_date_options() {
	...
	foreach ( $months as $date ) {
		...
		echo '<option value="' . $date->year . '-' . $month . '">' . $trm_locale->get_month( $month ) . ' ' . $date->year . '</option>';
	}
}
?>
...
<p><?php _e('When you click the button below Trnder will create an XML file for you to save to your computer.'); ?></p>
<p><?php _e('This format, which we call Trnder eXtended RSS or WXR, will contain your posts, pages, comments, custom fields, categories, and tags.'); ?></p>
<p><?php _e('Once you&#8217;ve saved the download file, you can use the Import function in another Trnder installation to import this site.'); ?></p>
...
<ul id="post-filters" class="export-filters">
	<li>
		...
		<?php trm_dropdown_categories( array( 'show_option_all' => __('All') ) ); ?>
	</li>
	<li>
		<label><?php _e( 'Authors:' ); ?></label>
<?php
		$authors = $trmdb->get_col( "SELECT DISTINCT post_author FROM {$trmdb->posts} WHERE post_type = 'post'" );
		trm_dropdown_users( array( 'include' => $authors, 'name' => 'post_author', 'multi' => true, 'show_option_all' => __('All') ) );
?>
	</li>
	<li>
...
<ul id="page-filters" class="export-filters">
	<li>
		<label><?php _e( 'Authors:' ); ?></label>
<?php
		$authors = $trmdb->get_col( "SELECT DISTINCT post_author FROM {$trmdb->posts} WHERE post_type = 'page'" );
		trm_dropdown_users( array( 'include' => $authors, 'name' => 'page_author', 'multi' => true, 'show_option_all' => __('All') ) );
?>
	</li>
	<li>
		<label><?php _e( 'Date range:' ); ?></label>
