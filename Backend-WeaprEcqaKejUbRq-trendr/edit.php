...
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		trm_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array('trashed', 'untrashed', 'deleted', 'ids'), trm_get_referer() );
	...
	if ( 'delete_all' == $doaction ) {
		...
		if ( get_post_status_object($post_status) ) // Check the post status exists first
			$post_ids = $trmdb->get_col( $trmdb->prepare( "SELECT ID FROM $trmdb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		...
		case 'untrash':
			...
			foreach( (array) $post_ids as $post_id ) {
				if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
					trm_die( __('You are not allowed to restore this item from the Trash.') );
		...
		case 'delete':
			...
			foreach( (array) $post_ids as $post_id ) {
				...
				if ( $post_del->post_type == 'attachment' ) {
					if ( ! trm_delete_attachment($post_id) )
						trm_die( __('Error in deleting...') );
				} else {
					if ( !trm_delete_post($post_id) )
						trm_die( __('Error in deleting...') );
				}
				$deleted++;
			}
	...
	trm_redirect($sendback);
	exit();
} elseif ( ! empty($_REQUEST['_http_referer']) ) {
...
trm_enqueue_script('inline-edit-post');
...
if ( 'post' == $post_type ) {
	...
	'<p>' . __('<a href="http://codex.trendr.org/Posts_Screen" target="_blank">Documentation on Managing Posts</a>') . '</p>' .
	...
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.trendr.org/Pages_Screen" target="_blank">Documentation on Managing Pages</a>') . '</p>' .
	'<p>' . __('<a href="http://trendr.org/support/" target="_blank">Support Forums</a>') . '</p>'
	);
}
...
if ( isset($_REQUEST['trashed']) && (int) $_REQUEST['trashed'] ) {
	...
	echo ' <a href="' . esc_url( trm_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", "bulk-posts" ) ) . '">' . __('Undo') . '</a><br />';
...
<?php $trm_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
