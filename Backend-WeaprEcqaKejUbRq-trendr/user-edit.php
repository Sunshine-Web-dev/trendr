...
trm_reset_vars(array('action', 'redirect', 'profile', 'user_id', 'trm_http_referer'));
...
elseif ( ! $user_id && ! IS_PROFILE_PAGE )
	trm_die(__( 'Invalid user ID.' ) );
elseif ( ! get_userdata( $user_id ) )
	trm_die( __('Invalid user ID.') );

trm_enqueue_script('user-profile');
...
add_contextual_help($current_screen,
    '<p>' . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using Trnder.') . '</p>' .
    '<p>' . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your Trnder administration screens, and turn off the WYSIWYG (Visual) editor, among other things.') . '</p>' .
    '<p>' . __('Your username cannot be changed, but you can use other fields to enter your real name or a nickname, and change which name to display on your posts.') . '</p>' .
    '<p>' . __('Required fields are indicated; the rest are optional. Profile information will only be displayed if your theme is set up to do so.') . '</p>' .
    '<p>' . __('Remember to click the Update Profile button when you are finished.') . '</p>' .
    '<p><strong>' . __('For more information:') . '</strong></p>' .
    '<p>' . __('<a href="http://codex.trendr.org/Users_Your_Profile_Screen" target="_blank">Documentation on User Profiles</a>') . '</p>' .
    '<p>' . __('<a href="http://trendr.org/support/" target="_blank">Support Forums</a>') . '</p>'
);


$trm_http_referer = remove_query_arg(array('update', 'delete_count'), stripslashes($trm_http_referer));
...
if ( is_multisite() && ! current_user_can( 'manage_network_users' ) && $user_id != $current_user->ID && ! apply_filters( 'enable_edit_any_user_configuration', true ) )
	trm_die( __( 'You do not have permission to edit this user.' ) );
...
if ( is_multisite() && IS_PROFILE_PAGE && isset( $_GET[ 'newuseremail' ] ) && $current_user->ID ) {
	...
	if ( $new_email[ 'hash' ] == $_GET[ 'newuseremail' ] ) {
		...
		if ( $trmdb->get_var( $trmdb->prepare( "SELECT user_login FROM {$trmdb->signups} WHERE user_login = %s", $current_user->user_login ) ) )
			$trmdb->query( $trmdb->prepare( "UPDATE {$trmdb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		trm_update_user( get_object_vars( $user ) );
		delete_option( $current_user->ID . '_new_email' );
		trm_redirect( add_query_arg( array('updated' => 'true'), self_admin_url( 'profile.php' ) ) );
		die();
	}
...
if ( !is_multisite() ) {
	...
	if ( $user->user_login && isset( $_POST[ 'email' ] ) && is_email( $_POST[ 'email' ] ) && $trmdb->get_var( $trmdb->prepare( "SELECT user_login FROM {$trmdb->signups} WHERE user_login = %s", $user->user_login ) ) )
		$trmdb->query( $trmdb->prepare( "UPDATE {$trmdb->signups} SET user_email = %s WHERE user_login = %s", $_POST[ 'email' ], $user_login ) );

	// TRMMU must delete the user from the current blog if TRM added him after editing.
	$delete_role = false;
	$blog_prefix = $trmdb->get_blog_prefix();
	if ( $user_id != $current_user->ID ) {
		$cap = $trmdb->get_var( "SELECT meta_value FROM {$trmdb->usermeta} WHERE user_id = '{$user_id}' AND meta_key = '{$blog_prefix}capabilities' AND meta_value = 'a:0:{}'" );
...
if ( !is_trm_error( $errors ) ) {
	...
	if ( $trm_http_referer )
		$redirect = add_query_arg('trm_http_referer', urlencode($trm_http_referer), $redirect);
...
<div id="message" class="updated">
	...
	<?php if ( $trm_http_referer && !IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( $trm_http_referer ); ?>"><?php _e('&larr; Back to Users'); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
<?php if ( isset( $errors ) && is_trm_error( $errors ) ) : ?>
...
<?php trm_nonce_field('update-user_' . $user_id) ?>
<?php if ( $trm_http_referer ) : ?>
	<input type="hidden" name="trm_http_referer" value="<?php echo esc_url($trm_http_referer); ?>" />
<?php endif; ?>
<p>
...
<?php endif; ?>
<?php if ( count($_trm_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>
<tr>
<th scope="row"><?php _e('Admin Color Scheme')?></th>
<td><?php do_action( 'admin_color_scheme_picker' ); ?></td>
</tr>
<?php
endif; // $_trm_admin_css_colors
if ( !( IS_PROFILE_PAGE && !$user_can_edit ) ) : ?>
<tr>
<th scope="row"><?php _e( 'Keyboard Shortcuts' ); ?></th>
<td><label for="comment_shortcuts"><input type="checkbox" name="comment_shortcuts" id="comment_shortcuts" value="true" <?php if ( !empty($profileuser->comment_shortcuts) ) checked('true', $profileuser->comment_shortcuts); ?> /> <?php _e('Enable keyboard shortcuts for comment moderation.'); ?></label> <?php _e('<a href="http://codex.trendr.org/Keyboard_Shortcuts" target="_blank">More information</a>'); ?></td>
</tr>
<?php endif; ?>
...
// TODO: create a function that does this: trm_get_user_role()
$user_roles = $profileuser->roles;
$user_role = array_shift($user_roles);

// print the full list of roles with the primary one selected.
trm_dropdown_roles($user_role);
...
<?php
	foreach (_trm_get_user_contactmethods( $profileuser ) as $name => $desc) {
?>
<tr>
